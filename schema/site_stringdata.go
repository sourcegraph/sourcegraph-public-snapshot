  ...
  "definitions": {
    "GitHubConnection": {
      ...
      "properties": {
        ...
        "repositoryQuery": {
          "description":
            "An array of strings specifying which GitHub or GitHub Enterprise repositories to mirror on Sourcegraph Server. The valid values are:\n\n(1) the special string \"affiliated\", which refers to all repositories that the configured token's user is affiliated with: private repositories with read access, public repositories owned by the user or their orgs, and public repositories with write access)\n\n(2) the special string \"none\", which means no repositories will be mirrored (except those specified in the \"repos\" configuration property or added manually)\n\nIf you need to narrow the set of mirrored repositories further (and don't want to enumerate the set in the \"repos\" configuration property), create a new bot/machine user on GitHub or GitHub Enterprise that is only affiliated with the desired repositories.",
          "type": "array",
          "items": {
            "type": "string",
            "enum": ["affiliated"]
