...
//go:generate $PWD/.bin/go-jsonschema-compiler -o schema.go -pkg schema aws_codecommit.schema.json bitbucket_cloud.schema.json bitbucket_server.schema.json critical.schema.json site.schema.json settings.schema.json general_protocol.schema.json github.schema.json gitlab.schema.json gitolite.schema.json other_external_service.schema.json phabricator.schema.json
...
//go:generate env GO111MODULE=on go run stringdata.go -i general_protocol.schema.json -name GeneralProtocolSchemaJSON -pkg schema -o general_protocol_stringdata.go
