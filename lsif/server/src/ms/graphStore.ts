            ...
            switch (property.compressionKind) {
                ...
                case CompressionKind.any:
                    ...
                    if (convertedAny === undefined) {
                        throw new Error(
                            `Comression kind any can't be handled for property ${
                                property.name
...
class VertexRetriever extends Retriever<VertexResult> {
    private static statement: string = ['Select v.id, v.label, v.value from vertices v', 'Where v.id in'].join(' ')
...
export class GraphStore extends Database {
    ...
    private findRange(uri: string, position: lsp.Position): RangeResult | undefined {
        let result: RangeResult[] = this.findRangeStmt.all({
            uri: uri,
            line: position.line,
    ...
    private createLocation(data: LocationResult): lsp.Location {
        return lsp.Location.create(
            ...
            lsp.Range.create(data.startLine, data.startCharacter, data.endLine, data.endCharacter)
        )
    }
