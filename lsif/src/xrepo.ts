...
export class XrepoDatabase {
  ...
  public async getReferences({
    ...
    const keepFlags = await Promise.all(
      results.map(result => testFilter(result.filter, value))
    );

    // Zip reference model results and the result of the bloom filter test together
    const zip: { result: ReferenceModel; keep: boolean }[] = results.map(
      (result, i) => ({
        result,
        keep: keepFlags[i]
      })
   ...
   */
  private async withTransactionalEntityManager<T>(
