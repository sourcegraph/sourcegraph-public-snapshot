 ...
 */
export function createLsifRouter(backend: Backend, queue: Queue, logger: Logger, tracer?: Tracer): express.Router {
    ...
    router.post(
        ...
        wrap(
            async (req: express.Request, res: express.Response): Promise<void> => {
                ...
                // Add upload record
                const upload = await queue.enqueue({ repository, commit, root, filename }, tracer, ctx.span)

                if (blocking) {
                    logger.debug('Blocking on upload conversion', { repository, commit, root })

                    if (await queue.waitForUploadToConvert(upload.id, maxWait)) {
