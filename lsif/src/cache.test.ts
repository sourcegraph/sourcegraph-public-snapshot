...
describe('GenericCache', () => {
  it('should evict items based by reverse recency', async () => {
    ...
    for (const value of values) {
      const returnValue = await cache.withValue(
        value,
        async () => { factoryArgs.push(value); return value },
        async v => v,
      )
  ...
  it('should asynchronously resolve cache values', async () => {
    ...
    let outerCalls = 0
    const p1 = cache.withValue("foo", () => { outerCalls++; return innerPromise }, async v => v)
    const p2 = cache.withValue("foo", () => { outerCalls++; return innerPromise }, async v => v)
    const p3 = cache.withValue("foo", () => { outerCalls++; return innerPromise }, async v => v)
  ...
  it('should call dispose function on eviction', async () => {
    ...
    const cache = new GenericCache<string, string>(2, () => 1, v => { disposeArgs.push(v) })
    ...
    setTimeout(() => { expect(disposeArgs).toEqual(["foo", "bar"]) }, 10)
  ...
  it('should not evict referenced cache entries', async () => {
    const disposeArgs: string[] = []
    const cache = new GenericCache<string, string>(5, () => 1, v => {
      disposeArgs.push(v)
    })

    await cache.withValue("foo", async () => "foo", async () => {
      await cache.withValue("bar", async () => "bar", async () => {
        await cache.withValue("baz", async () => "baz", async () => {
          await cache.withValue("bonk", async () => "bonk", async () => {
            await cache.withValue("quux", async () => "quux", async () => {
              // Sixth entry, but nothing to evict (all held)
              await cache.withValue("honk", async () => "honk", async () => {
              ...
              await cache.withValue("ronk", async () => "ronk", async () => {
                // allow disposal to run asynchronously
                setTimeout(() => { expect(disposeArgs).toEqual(["honk"]) }, 10)
              })
            })
          })
        })
    ...
    await cache.withValue("honk", async () => "honk", async () => {
      // allow disposal to run asynchronously
      setTimeout(() => { expect(disposeArgs).toEqual(["honk", "foos"]) }, 10)
    })
  })
})
