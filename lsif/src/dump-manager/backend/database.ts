...
import { Location, OrderedLocationSet } from './location'
...
export class Database {
     ...
     *
     * @param connectionCache A shared connection cache.
     * @param documentCache A shared docment cache.
     * @param resultChunkCache A shared result chunk cache.
     ...
     * @param ctx The tracing context.
     */
    public async definitions(path: string, position: lsp.Position, ctx: TracingContext = {}): Promise<Location[]> {
        return this.logAndTraceCall(ctx, 'Fetching definitions', async ctx => {
            ...
            for (const range of ranges) {
                ...
                if (!definitionResults.some(v => v.documentPath.includes('node_modules'))) {
                    return this.convertRangesToLocations(path, document, definitionResults)
                }
            }
            ...
            for (const range of ranges) {
                if (range.referenceResultId) {
                    ...
                    if (referenceResults.length > 0) {
                        for (const location of await this.convertRangesToLocations(path, document, referenceResults)) {
    ...
    public monikerResults(
        ...
        return this.logAndTraceCall(ctx, 'Fetching moniker results', async ctx => {
            ...
            return {
                locations: results.map(result => ({ path: result.documentPath, range: createRange(result) })),
                count,
            }
        })
    }
     ...
     * a set of LSP lcoation objects. Each pair holds the range identifier as well as the
     * document path. For document paths matching the loaded document, find the range data
     * locally. For all other paths, find the document in this database and find the range
     * in that document.
     *
     ...
     */
    private async convertRangesToLocations(
        path: string,
...
export function mapRangesToLocations(
    ...
    const locations = []
    for (const id of ids) {
        locations.push({ path, range: createRange(mustGet(ranges, id, 'range')) })
    }
