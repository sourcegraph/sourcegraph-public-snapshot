...
func main() {
	...
	logger := log.Scoped(raw_indexer, "indexer testing").With(log.String("directory", directory, privacy.Unknown))
...
func testDirectory(ctx context.Context, logger log.Logger, indexer []string, directory string) error {
	...
	for res := range resultChan {
		...
		if res.err != nil {
			...
			logger.Warn("Failed to run test", log.String("name", res.name, privacy.Unknown))
			fmt.Println(res.err)
			continue
		}
...
func testProject(ctx context.Context, logger log.Logger, indexer []string, project, name string) (projectResult, error) {
	...
	logger.Debug("... \t Resource usage", log.String("usage", string(usageData), privacy.Unknown))
...
func validateTestCases(logger log.Logger, projectRoot string, bundle *precise.GroupedBundleDataMaps) (testSuiteResult, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			logger.Warn("No lsif test directory exists here", log.String("directory", projectRoot, privacy.Unknown))
	...
	for _, file := range testFiles {
		...
		if err != nil {
			logger.Fatal("Had an error while we do the test file", log.String("file", testFileName, privacy.Unknown), log.Error(err))
		}
...
func runOneDefinitionRequest(logger log.Logger, projectRoot string, bundle *precise.GroupedBundleDataMaps, testCase DefinitionTest, fileResult *testFileResult) error {
	...
	definitionsField := log.String("definitions", string(definitionsData), privacy.Unknown)
