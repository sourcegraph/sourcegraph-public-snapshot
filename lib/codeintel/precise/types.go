...
// DocumentationSearchResult describes a search result from the lsif_data_documentation_search_* tables.
type DocumentationSearchResult struct {
	// Lang is the lowercase language name (go, java, etc.) OR, if unknown, the LSIF indexer name.
	Lang string

	// RepoID is the repo ID, from the main app DB repo table.
	RepoID int32

	// RepoName is the name of the repository containing this search key.
	RepoName string

	// DumpID is the identifier of the associated dump in the lsif_uploads table (state=completed).
	DumpID int
	...
	// PathID is the fully qualified documentation page path ID, e.g. including "#section". See
	// GraphQL codeintel.schema:documentationPage for what this is.
	PathID string

	// Label markdown string, e.g. a one-line function signature. See protocol/documentation.go:Documentation
	Label string

	// Detail markdown string (e.g. the full function signature and its docs). See protocol/documentation.go:Documentation
	Detail string
