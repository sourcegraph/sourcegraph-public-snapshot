...
func TestValidateEventFeatureAction(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			feature: "Invalid",
			action:  "valid",
			wantErr: autogold.Expect("'feature' must start with a lowercase letter and contain only letters, numbers, dashes, and dots"),
		},
		{
			name:    "action starts with uppercase",
			feature: "valid",
			action:  "Invalid",
			wantErr: autogold.Expect("'action' must start with a lowercase letter and contain only letters, numbers, dashes, and dots"),
		},
		{
