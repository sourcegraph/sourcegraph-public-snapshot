...
func TestServer_handleUncachedGithubDotComRepoLookup(t *testing.T) {
	s := &Server{
		InternalAPI: &internalAPIFake{},
	}
	...
	repoLookup := func(t *testing.T, repo api.RepoName) (resp *protocol.RepoLookupResult, statusCode int) {
		...
		req := httptest.NewRequest("GET", "/uncached-github-dot-com-repo-lookup", bytes.NewReader(body))
		h.ServeHTTP(rr, req)
		if rr.Code == http.StatusOK {
			if err := json.NewDecoder(rr.Body).Decode(&resp); err != nil {
				t.Fatal(err)
...
func TestUncachedGithubDotComRepoLookup(t *testing.T) {
	s := Server{
		Store:       new(repos.FakeStore),
		InternalAPI: &internalAPIFake{},
	}
	...
	t.Run("no args", func(t *testing.T) {
		if _, err := s.uncachedGithubDotComRepoLookup(context.Background(), protocol.RepoLookupArgs{}); err == nil {
			t.Error()
		}
	})
	...
	t.Run("not authoritative", func(t *testing.T) {
		...
		result, err := s.uncachedGithubDotComRepoLookup(context.Background(), protocol.RepoLookupArgs{Repo: "example.com/a/b"})
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("found", func(t *testing.T) {
		...
		result, err := s.uncachedGithubDotComRepoLookup(context.Background(), protocol.RepoLookupArgs{Repo: "github.com/c/d"})
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("not found", func(t *testing.T) {
		...
		result, err := s.uncachedGithubDotComRepoLookup(context.Background(), protocol.RepoLookupArgs{Repo: "github.com/a/b"})
		if err != nil {
			t.Fatal(err)
		}
		if want := (&protocol.RepoLookupResult{ErrorNotFound: true}); !reflect.DeepEqual(result, want) {
