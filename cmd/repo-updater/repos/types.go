...
// func (r *types.Repo) CloneURLs() []string {
...
// // repo belongs to.
// func (r *types.Repo) ExternalServiceIDs() []int64 {
...
// func (r *types.Repo) IsDeleted() bool { return !r.DeletedAt.IsZero() }

// // Update updates Repo r with the fields from the given newer Repo n,
// // returning true if modified.
// func (r *types.Repo) Update(n *types.Repo) (modified bool) {
// 	if r.Name != n.Name {
...
// 		cp := *x
// 		cp.ViewerPermission = ""
// 		n = n.With(func(clone *types.Repo) {
...
// func (r *types.Repo) Clone() *types.Repo {
// 	if r == nil {
// 		return nil
// 	}
// 	clone := *r
// 	if r.Sources != nil {
// 		clone.Sources = make(map[string]*types.SourceInfo, len(r.Sources))
...
// func (r *types.Repo) Apply(opts ...func(*types.Repo)) {
// 	if r == nil {
// 		return
// 	}
...
// func (r *types.Repo) With(opts ...func(*types.Repo)) *types.Repo {
// 	clone := r.Clone()
// 	clone.Apply(opts...)
// 	return clone
// }
...
// func (r *types.Repo) Less(s *types.Repo) bool {
// 	if r.ID != s.ID {
// 		return r.ID < s.ID
// 	}
// 	if r.Name != s.Name {
...
// func (r *types.Repo) String() string {
...
// func sourcesKeys(m map[string]*types.SourceInfo) []string {
...
// type Repos []*types.Repo

// // IDs returns the list of ids from all Repos.
// func (rs types.Repos) IDs() []api.RepoID {
...
// func (rs types.Repos) Names() []string {
...
// func (rs types.Repos) NamesSummary() string {
// 	if len(rs) > 20 {
...
// func (rs types.Repos) Kinds() (kinds []string) {
// 	set := map[string]bool{}
// 	for _, r := range rs {
...
// func (rs types.Repos) ExternalRepos() []api.ExternalRepoSpec {
...
// func (rs types.Repos) Sources() map[api.RepoID][]types.SourceInfo {
// 	sources := make(map[api.RepoID][]types.SourceInfo)
// 	for i := range rs {
...
// func (rs types.Repos) Len() int {
// 	return len(rs)
// }

// func (rs types.Repos) Swap(i, j int) {
// 	rs[i], rs[j] = rs[j], rs[i]
// }

// func (rs types.Repos) Less(i, j int) bool {
// 	return rs[i].Less(rs[j])
// }

// // Concat adds the given Repos to the end of rs.
// func (rs *types.Repos) Concat(others ...Repos) {
...
// func (rs types.Repos) Clone() types.Repos {
// 	o := make(types.Repos, 0, len(rs))
// 	for _, r := range rs {
// 		o = append(o, r.Clone())
// 	}
// 	return o
...
// // Apply applies the given functional options to the Repo.
// func (rs types.Repos) Apply(opts ...func(*types.Repo)) {
// 	for _, r := range rs {
// 		r.Apply(opts...)
// 	}
// }
...
// func (rs types.Repos) With(opts ...func(*types.Repo)) types.Repos {
// 	clone := rs.Clone()
// 	clone.Apply(opts...)
// 	return clone
// }
...
// func (rs types.Repos) Filter(pred func(*types.Repo) bool) (fs types.Repos) {
// 	for _, r := range rs {
// 		if pred(r) {
// 			fs = append(fs, r)
// 		}
