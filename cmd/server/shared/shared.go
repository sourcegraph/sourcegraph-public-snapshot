...
var defaultEnv = map[string]string{
	...
	"LSIF_API_SERVER_URL":     "http://127.0.0.1:3186",
...
func Main() {
	...
	procfile := []string{
		...
		`symbols: symbols`,
		`pci-api-server: node /precise-code-intel/out/api-server/api.js`,
		`pci-bundle-manager: node /precise-code-intel/out/bundle-manager/manager.js`,
		`pci-worker: node /precise-code-intel/out/worker/worker.js`,
		`searcher: searcher`,
		`replacer: replacer`,
