...
func (s *Server) handleExists(w http.ResponseWriter, r *http.Request) {
	s.dbQuery(w, r, func(db database.IDatabase) (interface{}, error) {
...
func (s *Server) dbQuery(w http.ResponseWriter, r *http.Request, handler func(db database.IDatabase) (interface{}, error)) {
	filename := paths.DBFilename(s.bundleDir, idFromRequest(r))

	openDatabase := func() (database.IDatabase, error) {
		return database.OpenDatabase(filename, s.documentDataCache, s.resultChunkDataCache)
	}

	cacheHandler := func(db database.IDatabase) error {
