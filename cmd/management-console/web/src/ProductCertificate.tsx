    ...
    <>
        <div className={`product-certificate card ${className || ''}`}>
            <div className="card-body d-flex align-items-center">
                <img className="product-certificate__logo mr-1 p-2" src={logo} />
                <div>
                    <h2 className="font-weight-normal mb-1 mt-0">{title}</h2>
                    {subtitle && <h3 className="text-muted font-weight-normal">{subtitle}</h3>}
                    {detail && <p className="text-muted my-0">{detail}</p>}
                </div>
            </div>
