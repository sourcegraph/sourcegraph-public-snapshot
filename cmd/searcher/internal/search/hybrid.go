...
func (s *Service) hybrid(ctx context.Context, rootLogger log.Logger, p *protocol.Request, sender matchSender) (unsearched []string, ok bool, err error) {
	...
	for try := 0; try < 5; try++ {
		...
		retryReason, err := zoektSearchIgnorePaths(logger, ctx, client, p, sender, indexed, indexedIgnore)
		if err != nil {
...
func zoektSearchIgnorePaths(logger log.Logger, ctx context.Context, client zoekt.Streamer, p *protocol.Request, sender matchSender, indexed api.CommitID, ignoredPaths []string) (retryReason string, err error) {
