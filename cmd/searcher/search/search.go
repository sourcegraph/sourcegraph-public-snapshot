...
func (s *Service) search(ctx context.Context, p *protocol.Request) (matches []protocol.FileMatch, limitHit, deadlineHit bool, err error) {
	...
	if p.IsRegExp {
		...
		matches, limitHit, err = combyFind(ctx, zipPath, p.FileMatchLimit, p.OnlyFiles)
	} else {
		err = errors.New("Invariant broken in searcher: either IsRegExp or IsStructuralPat must be true to perform search")
	}
