...
func makeProcfile(numAPIs, numBundleManagers, numWorkers int64) string {
	...
	if numAPIs > 0 {
		addProcess("lsif-api", "node /lsif/out/cmd/api/api.js")
	}

	if numBundleManagers > 0 {
		addProcess("lsif-bundle-manager", "node /lsif/out/cmd/bundle-manager/manager.js")
	}

	for i := 0; i < int(numWorkers); i++ {
		addProcess(
			fmt.Sprintf("lsif-worker-%d", i),
			fmt.Sprintf("env METRICS_PORT=%d node /lsif/out/cmd/worker/worker.js", workerPort+i),
		)
	}
