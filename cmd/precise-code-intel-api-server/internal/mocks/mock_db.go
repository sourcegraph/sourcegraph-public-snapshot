...
// 2020-04-20T09:55:11-05:00
// using the command
// $ /Users/efritz/dev/sourcegraph/sourcegraph/cmd/precise-code-intel-api-server/.bin/go-mockgen -f github.com/sourcegraph/sourcegraph/cmd/precise-code-intel-api-server/internal/db -i DB -o mock_db.go
...
// the parameter and result values of this invocation.
func (m *MockDB) ResetStalled(v0 context.Context) ([]int, error) {
	r0, r1 := m.ResetStalledFunc.nextHook()(v0)
	m.ResetStalledFunc.appendCall(DBResetStalledFuncCall{v0, r0, r1})
	return r0, r1
}

// SetDefaultHook sets function that is called when the ResetStalled method
// of the parent MockDB instance is invoked and the hook queue is empty.
func (f *DBResetStalledFunc) SetDefaultHook(hook func(context.Context) ([]int, error)) {
	f.defaultHook = hook
}
...
// front of the queue and discards it. After the queue is empty, the default
// hook function is invoked for any future action.
func (f *DBResetStalledFunc) PushHook(hook func(context.Context) ([]int, error)) {
	f.mutex.Lock()
	f.hooks = append(f.hooks, hook)
	f.mutex.Unlock()
}
...
func (f *DBResetStalledFunc) nextHook() func(context.Context) ([]int, error) {
	f.mutex.Lock()
	defer f.mutex.Unlock()
