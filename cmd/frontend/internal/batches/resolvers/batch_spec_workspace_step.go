...
func (r *batchSpecWorkspaceStepV1Resolver) Diff(ctx context.Context) (graphqlbackend.PreviewRepositoryComparisonResolver, error) {
	if r.CachedResultFound() {
		return graphqlbackend.NewPreviewRepositoryComparisonResolver(ctx, r.store.DatabaseDB(), gitserver.NewClient("graphql.batches.step"), r.repo, r.baseRev, r.cachedResult.Diff)
	}
	if r.stepInfo.DiffFound {
		return graphqlbackend.NewPreviewRepositoryComparisonResolver(ctx, r.store.DatabaseDB(), gitserver.NewClient("graphql.batches.step"), r.repo, r.baseRev, r.stepInfo.Diff)
	}
	return nil, nil
}
...
func (r *batchSpecWorkspaceStepV2Resolver) Diff(ctx context.Context) (graphqlbackend.PreviewRepositoryComparisonResolver, error) {
	...
	if r.cachedResult != nil {
		return graphqlbackend.NewPreviewRepositoryComparisonResolver(ctx, r.store.DatabaseDB(), gitserver.NewClient("graphql.batches.diffpreview"), r.repo, r.baseRev, r.cachedResult.Diff)
	}
	if r.stepInfo != nil && r.stepInfo.DiffFound {
		return graphqlbackend.NewPreviewRepositoryComparisonResolver(ctx, r.store.DatabaseDB(), gitserver.NewClient("graphql.batches.diffpreview"), r.repo, r.baseRev, r.stepInfo.Diff)
	}
	return nil, nil
}
