...
type Tree implements TreeEntry {
    ...
    repository: Repository!
    # Whether this tree's contents were fetched recursively.
    #
    # TODO(sqs): Other fields that return a GitTree have a recursive parameter. That parameter should be removed
    # from those fields and moved to GitTree's own fields that depend on its value, because "recursive" is a
    # property of those fields' values, not the Git tree per se.
    isRecursive: Boolean!
...
type User implements Node, ConfigurationSubject {
    ...
    # The organizations that this user is a member of.
    orgs: [Org!]! @deprecated(reason: "use organizations instead")
