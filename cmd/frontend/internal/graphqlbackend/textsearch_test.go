...
func TestSearchRepos(t *testing.T) {
	mockSearchRepo = func(ctx context.Context, repo *api.Repo, rev string, info *patternInfo) (matches []*fileMatch, limitHit bool, err error) {
		repoName := repo.URI
		switch repoName {
		case "foo/one":
			return []*fileMatch{
...
func makeRepositoryRevisions(repos ...string) []*repositoryRevisions {
	...
	for i, uri := range repos {
		r[i] = &repositoryRevisions{repo: &api.Repo{URI: uri}}
	}
	return r
}
