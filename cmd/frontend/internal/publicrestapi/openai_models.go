	...
	case []interface{}:
		...
		for _, item := range v {
			if s, ok := item.(string); ok {
				...
				return fmt.Errorf("invalid type for stop item. Expected string, got %T", item)
			}
		}
		c.Stop = stops
	default:
		return fmt.Errorf("invalid type for stop. Expected string or []interface{}, got %T", v)
	}
...
func (c *ChatCompletionRequestContentProperty) UnmarshalJSON(data []byte) error {
	...
	case []interface{}:
		...
		c.Parts = parts
	default:
		return fmt.Errorf("invalid type for ChatCompletionRequestContent. Expected string or []interface{}, got %T", v)
	}
...
func (r *ResponseFormatType) UnmarshalJSON(data []byte) error {
	...
	case ResponseFormatTypeText, ResponseFormatTypeJSONObject:
		*r = ResponseFormatType(s)
		return nil
	default:
		return fmt.Errorf("invalid ResponseFormatType: %s", s)
	}
}
