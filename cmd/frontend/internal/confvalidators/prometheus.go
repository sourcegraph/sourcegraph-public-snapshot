...
func validatePrometheusConnection(c conftypes.SiteConfigQuerier) (problems conf.Problems) {
	return newPrometheusValidator(c, srcprometheus.PrometheusURL)
}

func newPrometheusValidator(c conftypes.SiteConfigQuerier, promURL string) (problems conf.Problems) {
	prom, promErr := srcprometheus.NewClient(promURL)
