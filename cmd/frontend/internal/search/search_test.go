...
func TestDisplayLimit(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(fmt.Sprintf("q=%s;displayLimit=%d", c.queryString, c.displayLimit), func(t *testing.T) {
			...
			mock := client.NewMockSearchClient()
			mock.PlanFunc.SetDefaultHook(func(_ context.Context, _ string, _ *string, queryString string, _ int, _ search.Protocol, _ *schema.Settings, _ bool) (*search.Inputs, error) {
