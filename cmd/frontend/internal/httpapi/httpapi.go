...
func NewInternalHandler(m *mux.Router) http.Handler {
	...
	m.Get(apirouter.Telemetry).Handler(trace.TraceRoute(telemetryHandler))
	m.Get(apirouter.GraphQL).Handler(trace.TraceRoute(handler(serveGraphQL)))
	m.Get(apirouter.ConfigurationBasicRawJSON).Handler(trace.TraceRoute(handler(serveConfigurationBasicRawJson)))
	m.Get(apirouter.ConfigurationCoreRawJSON).Handler(trace.TraceRoute(handler(serveConfigurationCoreRawJson)))
