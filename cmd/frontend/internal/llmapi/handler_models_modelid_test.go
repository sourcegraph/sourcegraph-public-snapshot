...
func TestModelsModelIDHandler(t *testing.T) {
	...
	handler := &modelsModelIDHandler{
		logger: nil,
		GetModelConfig: func() (*types.ModelConfiguration, error) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			req, err := http.NewRequest("GET", "/api/v1/models/"+tc.modelID, nil)
			require.NoError(t, err)

			rr := httptest.NewRecorder()

			router := mux.NewRouter()
			router.Handle("/api/v1/models/{modelId}", handler)
