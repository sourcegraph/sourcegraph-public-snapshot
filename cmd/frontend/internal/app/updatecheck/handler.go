...
var (
	...
	latestReleaseDockerServerImageBuild = newBuild("3.41.1")

	// latestReleaseKubernetesBuild is only used by sourcegraph.com to tell existing Sourcegraph
	// cluster deployments what the latest version is. The version here _must_ be available in
	// a tag at https://github.com/sourcegraph/deploy-sourcegraph before landing in master.
	latestReleaseKubernetesBuild = newBuild("3.41.1")
	...
	latestReleaseDockerComposeOrPureDocker = newBuild("3.41.1")
)
...
type pingRequest struct {
	...
	AdminAnalyticsUsage           json.RawMessage `json:"adminAnalyticsUsage"`
...
type pingPayload struct {
	...
	AdminAnalyticsUsage           json.RawMessage `json:"admin_analytics_usage"`
...
func marshalPing(pr *pingRequest, hasUpdate bool, clientAddr string, now time.Time) ([]byte, error) {
	...
	return json.Marshal(&pingPayload{
		...
		AdminAnalyticsUsage:           pr.AdminAnalyticsUsage,
