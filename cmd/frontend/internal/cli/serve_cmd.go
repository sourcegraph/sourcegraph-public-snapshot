...
var (
	...
	appURL                  = conf.GetTODO().Core.AppURL
	disableBrowserExtension = conf.GetTODO().Basic.DisableBrowserExtension
...
func Main() error {
	...
	config := conf.GetTODO()
	useTLS := httpsAddr != "" && (tlsCertAndKey || (globals.AppURL.Scheme == "https" && config.Core.TlsLetsencrypt != "off"))
	...
	if useTLS {
		...
		if err != nil {
			...
			if tlsCertAndKey || config.Core.TlsLetsencrypt == "on" {
