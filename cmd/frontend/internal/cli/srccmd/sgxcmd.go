...
// must always use that path instead for executing the command.
...
func init() {
	...
	// Enforce Unix-style path, because that variable may be used in Makefiles
	...
	// It is for that reason that `src` must be on the system path during
	// testing.
