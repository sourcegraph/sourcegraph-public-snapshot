...
func (o dbEventsListOptions) sqlConditions() []*sqlf.Query {
	...
	if o.Objects.Campaign != 0 {
		conds = append(conds, sqlf.Sprintf("campaign_id=%d OR thread_id IN (SELECT thread_id FROM exp_campaigns_threads WHERE campaign_id=%d) OR comment_id IN (SELECT id FROM comments WHERE parent_comment_id IN (SELECT id FROM comments WHERE thread_id IN (SELECT thread_id FROM exp_campaigns_threads WHERE campaign_id=%d)))", o.Objects.Campaign, o.Objects.Campaign, o.Objects.Campaign))
	}
