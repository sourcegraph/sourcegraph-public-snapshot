...
const selectColumns = `id, type, actor_user_id, external_actor_username, external_actor_url, created_at, data, thread_id, thread_diagnostic_edge_id, campaign_id, rule_id, repository_id, user_id, organization_id, registry_extension_id, external_service_id`
...
func (o dbEventsListOptions) sqlConditions() []*sqlf.Query {
	...
	if o.Objects.Campaign != 0 {
		conds = append(conds, sqlf.Sprintf("campaign_id=%d OR thread_id IN (SELECT thread_id FROM exp_campaigns_threads WHERE campaign_id=%d)", o.Objects.Campaign, o.Objects.Campaign))
	}
