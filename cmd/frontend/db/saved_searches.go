...
func (s *savedSearches) ListAll(ctx context.Context) (_ []api.SavedQuerySpecAndConfig, err error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&sq.Config.Key, &sq.Config.Description, &sq.Config.Query, &sq.Config.Notify, &sq.Config.NotifySlack, &sq.Config.UserOrOrg, &sq.Config.UserID, &sq.Config.OrgID); err != nil {
			return nil, err
		}
		sq.Spec.Key = sq.Config.Key
		if sq.Config.UserOrOrg == "User" {
			sq.Spec.Subject.User = sq.Config.UserID
		} else if sq.Config.UserOrOrg == "Org" {
...
func (s *savedSearches) Update(ctx context.Context, id string, description string, query string, notify bool, notifySlack bool, userOrOrg string, userID *int32, orgID *int32) (err error) {
	tr, ctx := trace.New(ctx, "db.SavedSearches.ListAll", "")
	defer func() {
		tr.SetError(err)
		tr.Finish()
	...
	fieldUpdates := []*sqlf.Query{
		...
		sqlf.Sprintf("owner_kind=%s", strings.ToLower(userOrOrg)),
