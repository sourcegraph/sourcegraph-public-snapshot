...
func TestSearchResolver_DynamicFilters(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.descr, func(t *testing.T) {
			actualDynamicFilters := (&SearchResultsResolver{db: databasemocks.NewMockDB(), Matches: test.searchResults}).DynamicFilters(context.Background())
	...
	for _, tt := range tts {
		t.Run(tt.name, func(t *testing.T) {
			...
			db.EventLogsFunc.SetDefaultHook(func() database.EventLogStore {
				return databasemocks.NewMockEventLogStore()
			})
