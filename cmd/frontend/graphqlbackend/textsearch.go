...
// `unindexed`.
func zoektIndexedRepos(ctx context.Context, z *searchbackend.Zoekt, revs []*search.RepositoryRevisions, hasSymbols bool) (indexed, unindexed []*search.RepositoryRevisions, err error) {
	count := 0
	for _, r := range revs {
...
func searchFilesInRepos(ctx context.Context, args *search.Args) (res []*fileMatchResolver, common *searchResultsCommon, err error) {
	...
	if args.Zoekt.Enabled() {
		zoektRepos, searcherRepos, err = zoektIndexedRepos(ctx, args.Zoekt, args.Repos, false)
		if err != nil {
