...
func (r *schemaResolver) UpdatePassword(ctx context.Context, args *struct {
	...
	if conf.CanSendEmail() {
		if err := backend.UserEmails.SendUserEmailOnFieldUpdate(ctx, r.logger.Scoped("UserEmails", ""), r.db, user.ID, "updated the password"); err != nil {
...
func (r *schemaResolver) CreatePassword(ctx context.Context, args *struct {
	...
	if conf.CanSendEmail() {
		if err := backend.UserEmails.SendUserEmailOnFieldUpdate(ctx, r.logger.Scoped("UserEmails", ""), r.db, user.ID, "created a password"); err != nil {
