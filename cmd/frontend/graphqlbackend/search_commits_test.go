...
func TestSearchCommitsInRepo(t *testing.T) {
	...
	for i, want := range wantResults {
		...
		if got.commit.oid != want.commit.oid {
			t.Errorf("wrong commit oid for result %d\ngot %v\nwant %v\n", i, got.commit.oid, want.commit.oid)
		}
		if !reflect.DeepEqual(got.commit.repo, want.commit.repo) {
			t.Errorf("result[%d] wrong repo for commit\ndiff: %v", i, pretty.Compare(results, want))
		}

		if got.icon != want.icon {
			t.Errorf("wrong icon for result %d\ngot %v\nwant %v\n", i, got.icon, want.icon)
		}
	}
