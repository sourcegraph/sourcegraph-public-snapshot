...
func (r *gitCommitResolver) Tree(ctx context.Context, args *struct {
	...
	stat, err := git.Stat(ctx, backend.CachedGitRepo(ctx, r.repo.repo), api.CommitID(r.oid), args.Path)
	if err != nil {
		return nil, err
	}
	if !stat.Mode().IsDir() {
...
func (r *gitCommitResolver) BehindAhead(ctx context.Context, args *struct {
	...
	counts, err := git.GetBehindAhead(ctx, backend.CachedGitRepo(ctx, r.repo.repo), args.Revspec, string(r.oid))
	if err != nil {
		return nil, err
	}
