...
func TestSetUserEmailVerified(t *testing.T) {
	t.Run("only allowed by site admins", func(t *testing.T) {
		...
		ffs := databasemocks.NewMockFeatureFlagStore()
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			userExternalAccounts := databasemocks.NewMockUserExternalAccountsStore()
...
func TestPrimaryEmail(t *testing.T) {
	...
	for name, testCase := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			fs := fakedb.New()
			db := databasemocks.NewMockDB()
			emails := databasemocks.NewMockUserEmailsStore()
