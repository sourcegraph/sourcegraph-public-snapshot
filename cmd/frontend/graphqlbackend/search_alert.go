...
func alertForError(err error, inputs *SearchInputs) *searchAlert {
	...
	if errors.As(err, &mErr) {
		...
		alert = &searchAlert{
			...
			description:    fmt.Sprintf(`%s search can currently only handle searching across %d repositories at a time. Try using the "repo:" filter to narrow down which repositories to search. Tracking issue: https://github.com/sourcegraph/sourcegraph/issues/6826`, strings.Title(tErr.ResultType), tErr.Max),
		}
		alert.priority = 1
	}
	return alert
