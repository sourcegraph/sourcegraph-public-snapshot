...
func (r *searchResolver) alertForNoResolvedRepos(ctx context.Context) (*searchAlert, error) {
	...
	case len(repoGroupFilters) == 1 && len(repoFilters) > 1:
		...
		repos1, _, err := resolveRepositories(ctx, resolveRepoOp{repoFilters: repoFilters, minusRepoFilters: minusRepoFilters, onlyForks: onlyForks, noForks: noForks})
		if err != nil {
			return nil, err
		}
		if len(repos1) > 0 {
		...
		repos2, _, err := resolveRepositories(ctx, resolveRepoOp{repoFilters: []string{unionRepoFilter}, minusRepoFilters: minusRepoFilters, repoGroupFilters: repoGroupFilters, onlyForks: onlyForks, noForks: noForks})
		if err != nil {
			return nil, err
		}
		if len(repos2) > 0 {
	...
	case len(repoGroupFilters) == 1 && len(repoFilters) == 1:
		...
		repos1, _, err := resolveRepositories(ctx, resolveRepoOp{repoFilters: repoFilters, minusRepoFilters: minusRepoFilters, noForks: noForks, onlyForks: onlyForks})
		if err != nil {
			return nil, err
		}
		if len(repos1) > 0 {
	...
	case len(repoGroupFilters) == 0 && len(repoFilters) > 1:
		...
		unionRepoFilter := unionRegExps(repoFilters)
		repos2, _, err := resolveRepositories(ctx, resolveRepoOp{repoFilters: []string{unionRepoFilter}, minusRepoFilters: minusRepoFilters, repoGroupFilters: repoGroupFilters, noForks: noForks, onlyForks: onlyForks})
		if err != nil {
			return nil, err
		}
		if len(repos2) > 0 {
...
outer:
	for i, repoParent := range pathParentsByFrequency(paths) {
		...
		defer cancel()
		_, _, err := r.resolveRepositories(ctx, repoFieldValues)
		if ctx.Err() != nil {
			continue
		} else if err != nil {
			return nil, err
