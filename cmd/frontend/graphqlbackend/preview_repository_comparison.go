...
func applyPatch(fileContent string, fileDiff *diff.FileDiff) (string, error) {
	...
	contentReader := byteutils.NewLineReader([]byte(fileContent))
	...
	for i, hunk := range fileDiff.Hunks {
		// Iterate over the lines of the original file to fill up the lines in between the hunks.
		for contentReader.Scan() && currentLine < hunk.OrigStartLine {
			newContent.Write(contentReader.Line())
		...
		hunkHasFinalNewline := bytes.HasSuffix(hunk.Body, []byte("\n"))
		...
		for lr.Scan() {
			...
			case bytes.HasPrefix(line, []byte(" ")):
				// If we're not at the end of the file, output the current line from the contentReader
				if contentReader.Scan() {
					newContent.Write(contentReader.Line())
					newContent.Write([]byte("\n"))
					currentLine++
				}
			case bytes.HasPrefix(line, []byte("-")):
				// Consume the current line from the contentReader and discard it
				contentReader.Scan()
				currentLine++
			case bytes.HasPrefix(line, []byte("+")):
				// Append the line, stripping off the diff signifier at the beginning.
				newContent.Write(line[1:])
				newContent.Write([]byte("\n"))
			default:
	...
	// If we're not at the end of the file, append remaining lines from the original content.
	for contentReader.Scan() {
		newContent.Write(contentReader.Line())
		newContent.Write([]byte("\n"))
	}
	...
	} else if origHasFinalNewline {
		// Ensure that the new content ends with a newline if the original content did.
...
func applyPatchOld(fileContent string, fileDiff *diff.FileDiff) (string, error) {
	if diffPathOrNull(fileDiff.NewName) == nil {
		// the file was deleted, no need to do costly computation.
		return "", nil
	}
