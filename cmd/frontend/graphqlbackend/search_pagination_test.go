...
func TestSearchPagination_sliceSearchResults(t *testing.T) {
	...
	sharedCommon := &streaming.SearchResultsCommon{
...
func TestSearchPagination_repoPaginationPlan(t *testing.T) {
	...
	resultsExecutor := func(batch []*search.RepositoryRevisions) (results []SearchResultResolver, common *streaming.SearchResultsCommon, err error) {
		searchedBatches = append(searchedBatches, batch)
		common = &streaming.SearchResultsCommon{Repos: reposMap()}
	...
	noResultsExecutor := func(batch []*search.RepositoryRevisions) (results []SearchResultResolver, common *streaming.SearchResultsCommon, err error) {
		return nil, &streaming.SearchResultsCommon{}, nil
	}
...
func TestSearchPagination_issue_6287(t *testing.T) {
	...
	executor := func(batch []*search.RepositoryRevisions) (results []SearchResultResolver, common *streaming.SearchResultsCommon, err error) {
		common = &streaming.SearchResultsCommon{Repos: reposMap()}
		for _, repoRev := range batch {
