...
func TestSearchPagination_sliceSearchResults(t *testing.T) {
	...
	sharedCommon := &progress.SearchResultsCommon{
...
func TestSearchPagination_repoPaginationPlan(t *testing.T) {
	...
	resultsExecutor := func(batch []*search.RepositoryRevisions) (results []SearchResultResolver, common *progress.SearchResultsCommon, err error) {
		searchedBatches = append(searchedBatches, batch)
		common = &progress.SearchResultsCommon{Repos: reposMap()}
	...
	noResultsExecutor := func(batch []*search.RepositoryRevisions) (results []SearchResultResolver, common *progress.SearchResultsCommon, err error) {
	...
	tests := []struct {
		...
		wantCommon          *progress.SearchResultsCommon
		wantErr             error
	}{
		{
			name: "first request",
...
func TestSearchPagination_issue_6287(t *testing.T) {
	...
	executor := func(batch []*search.RepositoryRevisions) (results []SearchResultResolver, common *progress.SearchResultsCommon, err error) {
		common = &progress.SearchResultsCommon{Repos: reposMap()}
		for _, repoRev := range batch {
