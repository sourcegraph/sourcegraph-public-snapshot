...
func TestSyncExternalService_ContextTimeout(t *testing.T) {
	...
	err := backend.NewExternalServices(logtest.Scoped(t), databasemocks.NewMockDB(), repoupdater.NewClient(s.URL)).SyncExternalService(ctx, svc, 0*time.Millisecond)
...
func TestCancelExternalServiceSync(t *testing.T) {
	...
	newExternalServices := func() *databasemocks.MockExternalServiceStore {
		externalServices := databasemocks.NewMockExternalServiceStore()
		externalServices.GetByIDFunc.SetDefaultHook(func(_ context.Context, id int64) (*types.ExternalService, error) {
			return &types.ExternalService{
