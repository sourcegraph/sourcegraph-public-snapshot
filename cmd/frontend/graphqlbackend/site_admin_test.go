...
func TestDeleteUser(t *testing.T) {
	t.Run("authenticated as non-admin", func(t *testing.T) {
		...
		result, err := newSchemaResolver(db, gitserver.NewTestClient(t), nil).DeleteUser(ctx, &struct {
			User graphql.ID
			Hard *bool
		}{
	...
	t.Run("delete current user", func(t *testing.T) {
		...
		ctx := actor.WithActor(context.Background(), &actor.Actor{UID: 1})
		_, err := newSchemaResolver(db, gitserver.NewTestClient(t), nil).DeleteUser(ctx, &struct {
			User graphql.ID
			Hard *bool
		}{
			User: MarshalUserID(1),
...
func TestSetIsSiteAdmin(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			s := newSchemaResolver(db, gitserver.NewTestClient(t), nil)
