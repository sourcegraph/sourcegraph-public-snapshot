...
func TestDeleteUser(t *testing.T) {
	...
	externalAccounts := databasemocks.NewMockUserExternalAccountsStore()
	...
	const aliceUID = 6
	authzStore := databasemocks.NewMockAuthzStore()
...
func TestDeleteOrganization_OnPremise(t *testing.T) {
	...
	orgs := databasemocks.NewMockOrgStore()
...
func TestSetIsSiteAdmin(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			securityLogEvents := databasemocks.NewMockSecurityEventLogsStore()
			...
			s := newSchemaResolver(db, gitserver.NewClient(db))
