...
type LocalExternalServiceResolver interface {
	...
	Path() string
	Autogenerated() bool
	RepoCount(ctx context.Context) (int32, error)
}
...
func (r localExternalServiceResolver) RepoCount(ctx context.Context) (int32, error) {
	id, err := UnmarshalExternalServiceID(r.id)
	if err != nil {
		return 0, err
	}
	return r.db.ExternalServices().RepoCount(ctx, id)
}
