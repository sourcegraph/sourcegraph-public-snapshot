...
type GitBlobLSIFData implements TreeEntryLSIFData {
    ...
    # indicated by the given start and end lines (zero-based, inclusive). The associated
    # data for each range is "local", in that the locations and hover must also be defined
    # in the same index as the source range. To get cross-repository and cross-bundle
    # results, you must query the definitions, references, and hovers of that range
    # explicitly.
    window(startLine: Int!, endLine: Int!): AggregateCodeIntelligenceConnection
