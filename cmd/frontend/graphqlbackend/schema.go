...
type Query {
    ...
    # - Fetch results 100-200: `search(query:"some query", cursor:Search.cursor, limit:100)`
    #
    # And so on, until Search.finished is true.
    #
    ...
    search(
        ...
        # `Search.cursor` that is returned.
        limit: Int
    ): Search
...
type Search {
    ...
    # The cursor that can be passed into a future search request in order to
    # fetch more results starting where this search left off. See the `search`
    # resolver for more details.
    #
    # Only valid when the original request was a paginated one.
    cursor: ID!
    # Whether or not pagination has consumed all results that are available.
    #
    # Will always be false for non-paginated requests.
    finished: Boolean!
}
