...
type PatchSet implements Node {
    ...
    # The hidden patches for which the user doesn't have read permissions.
    hiddenPatches(first: Int): HiddenPatchConnection!
...
type Campaign implements Node {
    ...
    # Only includes changesets for which the user has read permissions.
    changesets(
        first: Int
    ...
    # All the changesets in this campaign for which the user doesn't have read
    # permissions.
    hiddenChangesets(
        first: Int
    ): HiddenExternalChangesetConnection!

    # The changeset counts over time, in 1-day intervals backwards from the point in time given in
    # the "to" parameter.
    # The counts include hidden changesets.
    ...
    # The hidden patches that will be turned into changesets on the code host
    # when publishing the campaign. Mirroring the behavior of patches &
    # changesets, the number of nodes in hiddenChangesets will increase with
    # each decrease in hiddenPatches.
    hiddenPatches(first: Int): HiddenPatchConnection!

    # The diff stat for all the patches and changesets in the campaign.
    # Excluding the stats of hidden changesets and hidden patches, for which
...
# A changeset on a code host (e.g., a pull request on GitHub) for which the
# viewer doesn't have read permissions.
type HiddenExternalChangeset implements Node {
    # The unique ID for the changeset.
    id: ID!
...
# A list of hidden changesets.
type HiddenExternalChangesetConnection {
    # A list of changesets.
    nodes: [HiddenExternalChangeset!]!
...
# A hidden patch is patch for which the user doesn't have read permissions.
type HiddenPatch implements Node {
    # The id of the patch.
    id: ID!
...
# A list of hidden patches.
type HiddenPatchConnection {
    # A list of hidden patches.
    nodes: [HiddenPatch!]!

    # The total number of hidden patches in the connection.
    totalCount: Int!
