...
func (r *externalServiceConnectionResolver) Nodes(ctx context.Context) ([]*externalServiceResolver, error) {
	...
	for _, externalService := range externalServices {
		resolvers = append(resolvers, &externalServiceResolver{logger: log.Scoped("Nodes", ""), db: r.db, externalService: externalService})
	}
...
func (r *computedExternalServiceConnectionResolver) Nodes(ctx context.Context) []*externalServiceResolver {
	...
	for _, svc := range svcs {
		resolvers = append(resolvers, &externalServiceResolver{logger: log.Scoped("Nodes", ""), db: r.db, externalService: svc})
	}
	return resolvers
}
