...
func TestUpdateUser(t *testing.T) {
	...
	t.Run("not site admin nor the same user", func(t *testing.T) {
		...
		result, err := newSchemaResolver(db, logtest.Scoped(t)).UpdateUser(context.Background(),
			&updateUserArgs{
				User: "VXNlcjox",
			},
		)
	...
	t.Run("disallow suspicious names", func(t *testing.T) {
		...
		_, err := newSchemaResolver(db, logtest.Scoped(t)).UpdateUser(ctx,
			&updateUserArgs{
	...
	t.Run("non site admin cannot change username when not enabled", func(t *testing.T) {
		...
		ctx := actor.WithActor(context.Background(), &actor.Actor{UID: 1})
		result, err := newSchemaResolver(db, logtest.Scoped(t)).UpdateUser(ctx,
			&updateUserArgs{
				User:     "VXNlcjox",
	...
	t.Run("only allowed by authenticated user on Sourcegraph.com", func(t *testing.T) {
		...
		for _, test := range tests {
			t.Run(test.name, func(t *testing.T) {
				...
				_, err := newSchemaResolver(db, logtest.Scoped(t)).UpdateUser(
					test.ctx,
					&updateUserArgs{
						User: MarshalUserID(1),
					},
