...
func searchRepositories(ctx context.Context, db dbutil.DB, args *search.TextParameters, limit int32, stream Streamer) error {
...
func repoRevsToSearchResultResolver(ctx context.Context, db dbutil.DB, repos []*search.RepositoryRevisions) []SearchResultResolver {
	...
	for _, r := range repos {
		...
		for _, rev := range revs {
			results = append(results, &RepositoryResolver{db: db, innerRepo: r.Repo.ToRepo(), icon: repoIcon, rev: rev})
		}
	}
	return results
}
