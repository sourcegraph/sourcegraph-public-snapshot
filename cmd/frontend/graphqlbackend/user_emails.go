...
func (r *schemaResolver) AddUserEmail(ctx context.Context, args *addUserEmailArgs) (*EmptyResponse, error) {
	...
	if err := backend.UserEmails.Add(ctx, r.logger.Scoped("UserEmails", ""), r.db, userID, args.Email); err != nil {
		return nil, err
	}

	if conf.CanSendEmail() {
		if err := backend.UserEmails.SendUserEmailOnFieldUpdate(ctx, r.logger.Scoped("UserEmails", ""), r.db, userID, "added an email"); err != nil {
...
func (r *schemaResolver) RemoveUserEmail(ctx context.Context, args *removeUserEmailArgs) (*EmptyResponse, error) {
	...
	if conf.CanSendEmail() {
		if err := backend.UserEmails.SendUserEmailOnFieldUpdate(ctx, r.logger.Scoped("UserEmails", ""), r.db, userID, "removed an email"); err != nil {
...
func (r *schemaResolver) SetUserEmailPrimary(ctx context.Context, args *setUserEmailPrimaryArgs) (*EmptyResponse, error) {
	...
	if conf.CanSendEmail() {
		if err := backend.UserEmails.SendUserEmailOnFieldUpdate(ctx, r.logger.Scoped("UserEmails", ""), r.db, userID, "changed primary email"); err != nil {
