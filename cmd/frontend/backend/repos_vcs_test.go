...
func TestRepos_ResolveRev_noRevSpecified_getsDefaultBranch(t *testing.T) {
	...
	commitID, err := Repos.ResolveRev(ctx, types.NewRepoWithIDs(0, "a", nil), "")
	if err != nil {
		t.Fatal(err)
	}
	if calledRepoLookup {
...
func TestRepos_ResolveRev_noCommitIDSpecified_resolvesRev(t *testing.T) {
	...
	commitID, err := Repos.ResolveRev(ctx, types.NewRepoWithIDs(0, "a", nil), "b")
	if err != nil {
		t.Fatal(err)
	}
	if calledRepoLookup {
...
func TestRepos_ResolveRev_commitIDSpecified_resolvesCommitID(t *testing.T) {
	...
	commitID, err := Repos.ResolveRev(ctx, types.NewRepoWithIDs(0, "a", nil), strings.Repeat("a", 40))
	if err != nil {
		t.Fatal(err)
	}
	if calledRepoLookup {
...
func TestRepos_ResolveRev_commitIDSpecified_failsToResolve(t *testing.T) {
	...
	_, err := Repos.ResolveRev(ctx, types.NewRepoWithIDs(0, "a", nil), strings.Repeat("a", 40))
...
func TestRepos_GetCommit_repoupdaterError(t *testing.T) {
	...
	commit, err := Repos.GetCommit(ctx, types.NewRepoWithIDs(0, "a", nil), want)
	if err != nil {
		t.Fatal(err)
	}
	if calledRepoLookup {
