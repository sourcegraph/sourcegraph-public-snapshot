	...
	if v, ok := v.(*common.Variable); ok {
		for _, op := range c.ops {
			if v2 := op.Vars.Get(v.Name); v2 != nil {
				...
				if err == nil && !typeCanBeUsedAsLazy(t2, t) {
					c.addErrMultiLoc([]errors.Location{v2.Loc, v.Loc}, "VariablesInAllowedPosition", "Variable %q of type %q used in position expecting type %q.", "$"+v.Name, t2, t)
				} else if err == nil && !typeCanBeUsedAs(t2, t) {
					c.addErrMultiLoc([]errors.Location{v2.Loc, v.Loc}, "VariablesInAllowedPositionNullable", "Variable %q of type %q used in position expecting type %q.", "$"+v.Name, t2, t)
				}
			}
		}
		return true, ""
...
// Hack until our client queries are up to date.
func typeCanBeUsedAsLazy(t, as common.Type) bool {
	nnT, okT := t.(*common.NonNull)
	if okT {
		t = nnT.OfType
	}
	...
	if lT, ok := t.(*common.List); ok {
		if lAs, ok := as.(*common.List); ok {
			return typeCanBeUsedAsLazy(lT.OfType, lAs.OfType)
		}
	}
	return false
}
