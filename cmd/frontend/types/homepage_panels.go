...
func GetHomepagePanels(ctx context.Context) (*types.GetHomepagePanels, error) {
	const q = `
	WITH sub AS (SELECT name, user_id FROM event_logs WHERE name LIKE '%Panel%' AND DATE_TRUNC('week', timestamp) = DATE_TRUNC('week', current_date) - interval '3 weeks')
		SELECT
  ...
  COUNT(*) FILTER (WHERE name = 'RecentSearchesPanelLoaded') as totalShownPercentage,
