...
go test -bench ^BenchmarkQuery -count 1 github.com/sourcegraph/sourcegraph/cmd/fileskip/bench | tee -a "$query_output_path"
...
echo "Running benchmark" >> "$query_output_path"
go test -bench ^BenchmarkQuery  -count 1 github.com/sourcegraph/sourcegraph/cmd/fileskip/bench | tee -a "$query_output_path_case_sensitive"
