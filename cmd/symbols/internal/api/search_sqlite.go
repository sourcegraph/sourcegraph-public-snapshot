...
func MakeSqliteSearchFunc(operations *sharedobservability.Operations, cachedDatabaseWriter writer.CachedDatabaseWriter, gitserverClient gitserver.GitserverClient) types.SearchFunc {
	return func(ctx context.Context, args search.SymbolsParameters) (results []result.Symbol, err error) {
		...
		if args.Timeout > 0 && time.Duration(args.Timeout)*time.Second < timeout {
			fmt.Println("Setting lower timeout of", args.Timeout, "seconds")
		...
		defer func() {
			...
			if err2 != nil {
				err = errors.New("Processing symbols is taking a while. If this repository is ~1GB+, try enabling [Rockskip](https://docs.sourcegraph.com/code_intelligence/explanations/rockskip).")
				return
			}

			help := ""
			if size > 1_000_000_000 {
				help = "Try enabling [Rockskip](https://docs.sourcegraph.com/code_intelligence/explanations/rockskip)."
			} else if size > 100_000_000 {
				help = "Consider enabling [Rockskip](https://docs.sourcegraph.com/code_intelligence/explanations/rockskip)."
			} else {
				help = "If this persists, make sure the symbols service has an SSD, a few GHz of CPU, and a few GB of RAM."
			}

			err = errors.Newf("Processing symbols is taking a while on this %s repository. %s", humanize.Bytes(uint64(size)), help)
			return
		}()
