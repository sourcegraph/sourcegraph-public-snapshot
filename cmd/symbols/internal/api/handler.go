...
func (s *grpcService) Search(ctx context.Context, r *proto.SearchRequest) (*proto.SearchResponse, error) {
	...
	if err != nil {
		...
		var oobErr *symbols.OutOfBoundsErr
		if errors.As(err, &oobErr) {
			// We use code.OutOfRange to "encode" our custom error. Per documentation of
			// grpc.Code, codes.OutOfRange will never be generated by the grpc package.
			st := status.New(codes.OutOfRange, "limit out of bounds")
			ds, err1 := st.WithDetails(&errdetails.BadRequest_FieldViolation{
				Field:       "First",
				Description: oobErr.Description,
			})
			if err1 != nil {
				return nil, st.Err()
			}
