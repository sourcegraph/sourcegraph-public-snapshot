...
// github.com/sourcegraph/sourcegraph/cmd/symbols/internal/parser) used for
// unit testing.
...
func NewMockGitserverClient() *MockGitserverClient {
	return &MockGitserverClient{
		...
		GitDiffFunc: &GitserverClientGitDiffFunc{
			defaultHook: func(context.Context, api.RepoName, api.CommitID, api.CommitID) (*Changes, error) {
				return nil, nil
			},
		},
	}
...
type GitserverClientGitDiffFunc struct {
	defaultHook func(context.Context, api.RepoName, api.CommitID, api.CommitID) (*Changes, error)
	hooks       []func(context.Context, api.RepoName, api.CommitID, api.CommitID) (*Changes, error)
...
// parameter and result values of this invocation.
func (m *MockGitserverClient) GitDiff(v0 context.Context, v1 api.RepoName, v2 api.CommitID, v3 api.CommitID) (*Changes, error) {
...
// the parent MockGitserverClient instance is invoked and the hook queue is
// empty.
func (f *GitserverClientGitDiffFunc) SetDefaultHook(hook func(context.Context, api.RepoName, api.CommitID, api.CommitID) (*Changes, error)) {
	f.defaultHook = hook
}
...
// hook at the front of the queue and discards it. After the queue is empty,
// the default hook function is invoked for any future action.
func (f *GitserverClientGitDiffFunc) PushHook(hook func(context.Context, api.RepoName, api.CommitID, api.CommitID) (*Changes, error)) {
	f.mutex.Lock()
	f.hooks = append(f.hooks, hook)
	f.mutex.Unlock()
}
...
// SetDefaultReturn calls SetDefaultDefaultHook with a function that returns
// the given values.
func (f *GitserverClientGitDiffFunc) SetDefaultReturn(r0 *Changes, r1 error) {
	f.SetDefaultHook(func(context.Context, api.RepoName, api.CommitID, api.CommitID) (*Changes, error) {
		return r0, r1
	})
}
...
// PushReturn calls PushDefaultHook with a function that returns the given
// values.
func (f *GitserverClientGitDiffFunc) PushReturn(r0 *Changes, r1 error) {
	f.PushHook(func(context.Context, api.RepoName, api.CommitID, api.CommitID) (*Changes, error) {
		return r0, r1
	})
}

func (f *GitserverClientGitDiffFunc) nextHook() func(context.Context, api.RepoName, api.CommitID, api.CommitID) (*Changes, error) {
	f.mutex.Lock()
	defer f.mutex.Unlock()
