...
func NewAnthropicHandler(
	...
	return makeUpstreamHandler(
		...
		upstreamHandlerMethods[anthropicRequest]{
			validateRequest: func(ctx context.Context, logger log.Logger, _ codygateway.Feature, ar anthropicRequest) (int, error, bool) {
				if ar.MaxTokensToSample > int32(maxTokensToSample) {
					return http.StatusBadRequest, errors.Errorf("max_tokens_to_sample exceeds maximum allowed value of %d: %d", maxTokensToSample, ar.MaxTokensToSample), false
				}
