...
var (
	...
	syncRepoStateUpdatePerSecond   = env.MustGetInt("SRC_REPOS_SYNC_STATE_UPSERT_PER_SEC", 500, "The number of updated rows allowed per second across all p4server instances")
...
func Main(ctx context.Context, observationCtx *observation.Context, ready service.ReadyFunc, config *Config, enterpriseInit EnterpriseInit) error {
	...
	p4server.RegisterMetrics(observationCtx, db)

	if tmpDir, err := p4server.SetupAndClearTmp(); err != nil {
	...
	go p4server.Janitor(actor.WithInternalActor(ctx), janitorInterval)
	go p4server.SyncRepoState(syncRepoStateInterval, syncRepoStateBatchSize, syncRepoStateUpdatePerSecond)

	p4server.StartClonePipeline(ctx)
...
func getDB(observationCtx *observation.Context) (*sql.DB, error) {
	...
	return connections.EnsureNewFrontendDB(observationCtx, dsn, "p4server")
}
...
func syncRateLimiters(ctx context.Context, logger log.Logger, store database.ExternalServiceStore, perSecond int) {
	...
	// perSecond should be spread across all p4server instances and we want to wait
	...
	for {
		...
		logger.Warn("found zero p4server instance, trying again after backoff", log.Duration("backoff", backoff))
	}
