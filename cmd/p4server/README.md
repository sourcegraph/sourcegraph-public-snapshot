...
Mirrors depots from their code host. All other Sourcegraph services talk to p4server when they need data from Perforce. Requests for fetch operations, however, go through repo-updater.

p4server exposes an "exec" API over HTTP for running p4 commands against
clones of repositories. p4server also exposes APIs for the management of
clones.

The management of clones comprises most of the complexity in p4server since:
...
p4server. Nearly every operation Sourcegraph does runs one or more git
...
p4server's memory usage consists of short lived git subprocesses.
