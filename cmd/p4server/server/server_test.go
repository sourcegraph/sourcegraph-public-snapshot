...
func TestCloneRepo(t *testing.T) {
	...
	// Verify the p4server repo entry exists.
...
func testHandleRepoDelete(t *testing.T, deletedInDB bool) {
	...
	// Check status in p4server_repos
	want = &types.GitserverRepo{
		RepoID:        dbRepo.ID,
		ShardID:       "",
...
func TestHostnameMatch(t *testing.T) {
	testCases := []struct {
		...
		{
			hostname:    "p4server-1",
			addr:        "p4server-1.p4server:3178",
			shouldMatch: true,
		},
		{
			hostname:    "p4server-1",
			addr:        "p4server-10.p4server:3178",
			shouldMatch: false,
		},
		{
			hostname:    "p4server-1",
		...
		{
			hostname:    "p4server-10",
			addr:        "",
			shouldMatch: false,
		},
		{
			hostname:    "p4server-10",
			addr:        "p4server-10:3178",
			shouldMatch: true,
		},
		{
			hostname:    "p4server-10",
		...
		{
			hostname:    "p4server-0.prod",
			addr:        "p4server-0.prod.default.namespace",
			shouldMatch: true,
		},
	}
...
func TestSyncRepoState(t *testing.T) {
	...
	err = s.syncRepoState(p4server.P4ServerAddresses{Addresses: []string{hostname}}, 10, 10, true)
	if err != nil {
		t.Fatal(err)
	}
