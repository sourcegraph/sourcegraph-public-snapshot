// Package server implements the p4server service.
package server
...
import (
	...
	"github.com/sourcegraph/sourcegraph/cmd/p4server/server/internal/accesslog"
	...
	"github.com/sourcegraph/sourcegraph/internal/p4server"
	"github.com/sourcegraph/sourcegraph/internal/p4server/adapters"
	...
	"github.com/sourcegraph/sourcegraph/internal/p4server/search"
...
type Server struct {
	...
	// ReposDir is the path to the base directory for p4server storage.
	ReposDir string
	...
	// Hostname is how we identify this instance of p4server. Generally it is the
	...
	// set by RegisterMetrics when compiled as part of the p4server binary. The server
...
func headerXRequestedWithMiddleware(next http.Handler) http.HandlerFunc {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		l := log.Scoped("p4server", "headerXRequestedWithMiddleware")
		...
		// at all.
		//
		// 2. /git may be used to run "git fetch" from another p4server instance over
...
func (s *Server) Handler() http.Handler {
	...
	// can happen at once on a single p4server.
	// Used to prevent throttle limits from a code host. Defaults to 5.
	//
	// The new repo-updater scheduler enforces the rate limit across all p4server,
	...
	// This endpoint allows us to expose p4server itself as a "git service"
	// (ETOOMANYGITS!) that allows other services to run commands like "git fetch"
	// directly against a p4server replica and treat it as a git remote.
	//
	// Example use case for this is a repo migration from one replica to another during
	// scaling events and the new destination p4server replica can directly clone from
	// the p4server replica which hosts the repository currently.
	...
	getObjectService := p4domain.GetObjectService{
		RevParse:      gitAdapter.RevParse,
		GetObjectType: gitAdapter.GetObjectType,
	}
	getObjectFunc := p4domain.GetObjectFunc(func(ctx context.Context, repo api.RepoName, objectName string) (*p4domain.P4Object, error) {
...
func (s *Server) Janitor(ctx context.Context, interval time.Duration) {
	for {
		p4serverAddrs := currentGitserverAddresses()
		s.cleanupRepos(actor.WithInternalActor(ctx), p4serverAddrs)
		time.Sleep(interval)
	}
}
...
// p4server addresses has changed since the last run. Otherwise, we only sync
...
func (s *Server) addrForRepo(ctx context.Context, repoName api.RepoName, gitServerAddrs p4server.P4ServerAddresses) (string, error) {
	return p4server.AddrForRepo(ctx, filepath.Base(os.Args[0]), s.DB, repoName, gitServerAddrs)
}

func currentGitserverAddresses() p4server.P4ServerAddresses {
	cfg := conf.Get()
	gitServerAddrs := p4server.P4ServerAddresses{
...
var (
	...
	wrongShardReposTotal = promauto.NewGauge(prometheus.GaugeOpts{
		Name: "src_p4server_repo_wrong_shard",
	...
	wrongShardReposSizeTotalBytes = promauto.NewGauge(prometheus.GaugeOpts{
		Name: "src_p4server_repo_wrong_shard_bytes",
	...
	wrongShardReposDeletedCounter = promauto.NewCounter(prometheus.CounterOpts{
		Name: "src_p4server_repo_wrong_shard_deleted",
		Help: "The number of repos on the wrong shard that we deleted",
	})
)

func (s *Server) syncRepoState(gitServerAddrs p4server.P4ServerAddresses, batchSize, perSecond int, fullSync bool) error {
	...
	if !found {
		return errors.Errorf("p4server hostname, %q, not found in list", s.Hostname)
	}
...
func (s *Server) getRemoteURL(ctx context.Context, name api.RepoName) (*vcs.URL, error) {
	if s.GetRemoteURLFunc == nil {
		return nil, errors.New("p4server GetRemoteURLFunc is unset")
	}
...
//
// This directory is cleaned up by p4server and will be ignored by repository
// listing operations.
...
func (s *Server) handleRepoUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if !repoCloned(dir) && !s.skipCloneForTests {
		...
		// the repo from (upstream URL of the external service or the p4server instance) lies with
...
// Asynchronous errors will have to be checked in the p4server_repos table under last_error.
...
func (s *Server) handleArchive(w http.ResponseWriter, r *http.Request) {
	...
	if err := checkSpecArgSafety(treeish); err != nil {
		...
		s.Logger.Error("p4server.archive.CheckSpecArgSafety", log.Error(err))
		return
	}

	if repo == "" || format == "" {
		w.WriteHeader(http.StatusBadRequest)
		logger.Error("p4server.archive", log.String("error", "empty repo or format"))
		return
	}
	...
	if format == string(p4server.ArchiveFormatZip) {
...
func (s *Server) handleSearch(w http.ResponseWriter, r *http.Request) {
	...
	if honey.Enabled() || traceLogs {
		...
		ev := honey.NewEvent("p4server-search")
		...
		if traceLogs {
			logger.Debug("TRACE p4server search", log.Object("ev.Fields", mapToLoggerField(ev.Fields())...))
		}
	}
}
...
func (s *Server) search(ctx context.Context, args *protocol.SearchRequest, matchesBuf *streamhttp.JSONArrayBuf) (limitHit bool, err error) {
	...
	if !repoCloned(dir) {
		if conf.Get().DisableAutoGitUpdates {
			...
			return false, &p4domain.RepoNotExistError{
				Repo: args.Repo,
			}
		}
...
// via RegisterMetrics (when constructed as part of the p4server binary), or
...
var blockedCommandExecutedCounter = promauto.NewCounter(prometheus.CounterOpts{
	Name: "src_p4server_exec_blocked_command_received",
	Help: "Incremented each time a command not in the allowlist for p4server is executed",
})
...
func (s *Server) exec(w http.ResponseWriter, r *http.Request, req *protocol.ExecRequest) {
	...
	// See https://github.com/sourcegraph/security-issues/issues/213.
	if !p4domain.IsAllowedP4Cmd(logger, req.Args) {
	...
	// Instrumentation
	{
		...
		defer func() {
			...
			if honey.Enabled() || traceLogs || isSlow || isSlowFetch {
				act := actor.FromContext(ctx)
				ev := honey.NewEvent("p4server-exec")
				...
				if traceLogs {
					logger.Debug("TRACE p4server exec", log.Object("ev.Fields", mapToLoggerField(ev.Fields())...))
				}
				if isSlow {
	...
	// to a persistent failure mode where every exec takes > 10s, which is disastrous for p4server performance.
	...
	// Instrumentation
	{
		...
		defer func() {
			...
			if honey.Enabled() || traceLogs || isSlow {
				act := actor.FromContext(ctx)
				ev := honey.NewEvent("p4server-p4exec")
				ev.SetSampleRate(honeySampleRate(cmd, act))
				...
				if traceLogs {
					logger.Debug("TRACE p4server p4exec", log.Object("ev.Fields", mapToLoggerField(ev.Fields())...))
				}
				if isSlow {
...
// setLastErrorNonFatal will set the last_error column for the repo in the p4server table.
...
func setGitAttributes(dir GitDir) error {
	...
	_, err := fileutil.UpdateFileIfDifferent(
		...
		[]byte(`# Managed by Sourcegraph p4server.
...
type cloneOptions struct {
	...
	// CloneFromShard is the hostname of the p4server instance which is the current owner of the
	// repository. If this is a non-zero string, then p4server will attempt to clone the repo from
	// that p4server instance instead of the upstream repo URL of the external service.
...
func (s *Server) cloneRepo(ctx context.Context, repo api.RepoName, opts *cloneOptions) (cloneProgress string, err error) {
	...
	if opts != nil && opts.CloneFromShard != "" {
		// are we cloning from the same p4server instance?
		if s.hostnameMatch(strings.TrimPrefix(opts.CloneFromShard, "http://")) {
			return "", errors.Errorf("cannot clone from the same p4server instance")
		}
...
var (
	execRunning = promauto.NewGaugeVec(prometheus.GaugeOpts{
		Name: "src_p4server_exec_running",
		Help: "number of p4server.GitCommand running concurrently.",
	}, []string{"cmd"})
	execDuration = promauto.NewHistogramVec(prometheus.HistogramOpts{
		Name:    "src_p4server_exec_duration_seconds",
		Help:    "p4server.GitCommand latencies in seconds.",
		Buckets: trace.UserLatencyBuckets,
	}, []string{"cmd", "status"})

	searchRunning = promauto.NewGauge(prometheus.GaugeOpts{
		Name: "src_p4server_search_running",
		Help: "number of p4server.Search running concurrently.",
	})
	searchDuration = promauto.NewHistogramVec(prometheus.HistogramOpts{
		Name:    "src_p4server_search_duration_seconds",
		Help:    "p4server.Search duration in seconds.",
		Buckets: []float64{0.01, 0.05, 0.1, 0.2, 0.5, 1, 2, 5, 10, 30},
	}, []string{"error"})
	searchLatency = promauto.NewHistogram(prometheus.HistogramOpts{
		Name:    "src_p4server_search_latency_seconds",
		Help:    "p4server.Search latency (time until first result is sent) in seconds.",
		Buckets: []float64{0.01, 0.05, 0.1, 0.2, 0.5, 1, 2, 5, 10, 30},
	})

	pendingClones = promauto.NewGauge(prometheus.GaugeOpts{
		Name: "src_p4server_clone_queue",
		Help: "number of repos waiting to be cloned.",
	})
	lsRemoteQueue = promauto.NewGauge(prometheus.GaugeOpts{
		Name: "src_p4server_lsremote_queue",
...
// https://ui.honeycomb.io/sourcegraph/datasets/p4server-exec/result/67e4bLvUddg
