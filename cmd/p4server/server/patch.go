...
func (s *Server) createCommitFromPatch(ctx context.Context, req protocol.CreateCommitFromPatchRequest) (int, protocol.CreateCommitFromPatchResponse) {
	...
	if _, err := os.Stat(repoGitDir); os.IsNotExist(err) {
		...
		if _, err := os.Stat(repoGitDir); os.IsNotExist(err) {
			resp.SetError(repo, "", "", errors.Wrap(err, "p4server: repo does not exist"))
	...
	if err != nil {
		resp.SetError(repo, "", "", errors.Wrap(err, "p4server: make tmp repo"))
		return http.StatusInternalServerError, resp
	}
	...
	run := func(cmd *exec.Cmd, reason string) ([]byte, error) {
		if !p4domain.IsAllowedP4Cmd(logger, cmd.Args[1:]) {
		...
		if err != nil {
			resp.SetError(repo, argsToString(cmd.Args), string(out), errors.Wrap(err, "p4server: "+reason))
	...
	out, err := cmd.Output()
	if err != nil {
		resp.SetError(repo, argsToString(cmd.Args), string(out), errors.Wrap(err, "p4server: retrieving new commit id"))
		return http.StatusInternalServerError, resp
	}
	...
	if req.Push != nil {
		...
		if remoteURL.IsSSH() && req.Push.PrivateKey != "" && req.Push.Passphrase != "" {
			...
			if err != nil {
				resp.SetError(repo, "", "", errors.Wrap(err, "p4server: error creating ssh-agent"))
				return http.StatusInternalServerError, resp
			}
			go agent.Listen()
