...
UPDATE p4server_repos SET shard_id = 1;
UPDATE p4server_repos SET repo_size_bytes = 5 where repo_id = 3;
`); err != nil {
		t.Fatalf("unexpected error while inserting test data: %s", err)
	}

	s.cleanupRepos(actor.WithInternalActor(context.Background()), p4server.P4ServerAddresses{Addresses: []string{"p4server-0"}})
...
func TestCleanupInactive(t *testing.T) {
	...
	s.testSetup(t)
	s.cleanupRepos(context.Background(), p4server.P4ServerAddresses{Addresses: []string{"p4server-0"}})
...
func TestCleanupWrongShard(t *testing.T) {
	t.Run("wrongShardName", func(t *testing.T) {
		root := t.TempDir()
		// should be allocated to shard p4server-1
		...
		s.cleanupRepos(context.Background(), p4server.P4ServerAddresses{Addresses: []string{"p4server-0", "p4server-1"}})
	...
	t.Run("substringShardName", func(t *testing.T) {
		...
		s.testSetup(t)
		s.Hostname = "p4server-0"
		s.cleanupRepos(context.Background(), p4server.P4ServerAddresses{Addresses: []string{"p4server-0.cluster.local:3178", "p4server-1.cluster.local:3178"}})
...
func TestGitGCAuto(t *testing.T) {
	...
	// Now create a bare repo like p4server expects
	root := t.TempDir()
	wdRepo := repo
...
func TestCleanup_RemoveNonExistentRepos(t *testing.T) {
	...
	mockGitServerRepos.GetByNameFunc.SetDefaultHook(func(_ context.Context, name api.RepoName) (*types.GitserverRepo, error) {
		if strings.Contains(string(name), "repo-exists") {
			...
			return nil, errors.Newf("p4server repo not found")
		}
	})
...
func TestRemoveRepoDirectory_Empty(t *testing.T) {
	...
	if len(gr.SetCloneStatusFunc.History()) == 0 {
		t.Fatal("expected p4serverRepos.SetLastError to be called, but wasn't")
	}
...
func TestFreeUpSpace(t *testing.T) {
	...
	t.Run("oldest repo gets removed to free up space", func(t *testing.T) {
		...
		if len(gr.SetCloneStatusFunc.History()) == 0 {
			t.Fatal("expected p4serverRepos.SetCloneStatus to be called, but wasn't")
		}
		require.Equal(t, gr.SetCloneStatusFunc.History()[0].Arg2, types.CloneStatusNotCloned)
	})
}
...
update p4server_repos set shard_id = 1;
update p4server_repos set repo_size_bytes = 228 where repo_id = 1;
`); err != nil {
		t.Fatalf("unexpected error while inserting test data: %s", err)
	}
