...
// sshAgent speaks the ssh-agent protocol and can be used by p4server
...
func newSSHAgent(logger log.Logger, raw, passphrase []byte) (*sshAgent, error) {
	...
	a := &sshAgent{
		logger:  logger.Scoped("sshAgent", "speaks the ssh-agent protocol and can be used by p4server"),
		l:       l,
		sock:    socketName,
		keyring: keyring,
