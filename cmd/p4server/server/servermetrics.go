...
func (s *Server) RegisterMetrics(observationCtx *observation.Context, db dbutil.DB) {
	...
	echoDuration := prometheus.NewGauge(prometheus.GaugeOpts{
		Name: "src_p4server_echo_duration_seconds",
	...
	if s.ReposDir == "" {
		s.Logger.Error("ReposDir is not set, cannot export disk_space_available and p4server_mount_info metric.")
		return
	}

	opts := mountinfo.CollectorOpts{Namespace: "p4server"}
	...
	c := prometheus.NewGaugeFunc(prometheus.GaugeOpts{
		Name: "src_p4server_disk_space_available",
	...
	c = prometheus.NewGaugeFunc(prometheus.GaugeOpts{
		Name: "src_p4server_disk_space_total",
