...
// Controls if p4server cleanup tries to remove repos from disk which are not defined in the DB. Defaults to false.
var removeNonExistingRepos, _ = strconv.ParseBool(env.Get("SRC_REMOVE_NON_EXISTING_REPOS", "false", "controls if p4server cleanup tries to remove repos from disk which are not defined in the DB"))

var (
	reposRemoved = promauto.NewCounterVec(prometheus.CounterOpts{
		Name: "src_p4server_repos_removed",
		Help: "number of repos removed during cleanup",
	}, []string{"reason"})
	reposRecloned = promauto.NewCounter(prometheus.CounterOpts{
		Name: "src_p4server_repos_recloned",
		Help: "number of repos removed and re-cloned due to age",
	})
	reposRemovedDiskPressure = promauto.NewCounter(prometheus.CounterOpts{
		Name: "src_p4server_repos_removed_disk_pressure",
		Help: "number of repos removed due to not enough disk space",
	})
	janitorRunning = promauto.NewGauge(prometheus.GaugeOpts{
		Name: "src_p4server_janitor_running",
		Help: "set to 1 when the p4server janitor background job is running",
	})
	jobTimer = promauto.NewHistogramVec(prometheus.HistogramOpts{
		Name: "src_p4server_janitor_job_duration_seconds",
		Help: "Duration of the individual jobs within the p4server janitor background job",
	}, []string{"success", "job_name"})
	maintenanceStatus = promauto.NewCounterVec(prometheus.CounterOpts{
		Name: "src_p4server_maintenance_status",
	...
	pruneStatus = promauto.NewCounterVec(prometheus.CounterOpts{
		Name: "src_p4server_prune_status",
	...
	janitorTimer = promauto.NewHistogram(prometheus.HistogramOpts{
		Name:    "src_p4server_janitor_duration_seconds",
		Help:    "Duration of p4server janitor background job",
		Buckets: []float64{0.1, 1, 10, 60, 300, 3600, 7200},
	})
	nonExistingReposRemoved = promauto.NewCounter(prometheus.CounterOpts{
		Name: "src_p4server_non_existing_repos_removed",
...
func (s *Server) cleanupRepos(ctx context.Context, gitServerAddrs p4server.P4ServerAddresses) {
	...
	if !knownGitServerShard {
		s.Logger.Warn("current shard is not included in the list of known p4server shards, will not delete repos", log.String("current-hostname", s.Hostname), log.Strings("all-shards", gitServerAddrs.Addresses))
	}
...
func (s *Server) setRepoSizes(ctx context.Context, repoToSize map[api.RepoName]int64) error {
	...
	// of every single row of `p4server_repos` table. This will lead to eventual consistency of
...
func getRecloneTime(dir GitDir) (time.Time, error) {
	...
	// different ways a clone can appear in p4server.
...
const (
	...
	sgmLogHeader = `DO NOT EDIT: generated by p4server.
