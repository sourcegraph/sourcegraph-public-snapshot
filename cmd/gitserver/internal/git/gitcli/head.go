...
func (g *gitCLIBackend) SymbolicRefHead(ctx context.Context, short bool) (refName string, err error) {
	...
	if short {
		args = append(args, FlagArgument{"--short"})
	}

	r, err := g.NewCommand(ctx, "symbolic-ref", WithArguments(args...))
	if err != nil {
		return "", err
	}
	defer r.Close()
...
func (g *gitCLIBackend) RevParseHead(ctx context.Context) (sha api.CommitID, err error) {
	...
	// If our optimized version didn't work, fall back to asking git directly.
	args := []Argument{FlagArgument{"HEAD"}}

	r, err := g.NewCommand(ctx, "rev-parse", WithArguments(args...))
	if err != nil {
		return "", err
	}
	defer r.Close()
