...
func InitOOBMigrationRunner() (*oobmigration.Runner, error) {
	conn, err := initDatabaseMemo.Init()
	if err != nil {
		return nil, err
	}

	return conn.(*oobmigration.Runner), nil
}

var initOOBMigrationRunnerMemo = NewMemoizedConstructor(func() (interface{}, error) {
	...
	// work remaining.
	return newOutOfBandMigrationRunner(context.Background(), db), nil
})
