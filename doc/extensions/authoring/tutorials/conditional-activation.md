# Conditional activation and button display tutorial
...
In this tutorial, you'll build an extension that activates and displays a button, only if a Python file is present.
...
This content would be visible in a hover only when it occurred while viewing a Python file.

More details can be found in the [activation documentation](../activation.md), such as deactivation and troubleshooting.
...
The `when` field uses what is called an expression. If an expression returns true or a truthy value, the properties of the `action` it references are used to display an element in the specified menu location, e.g. `editor/title`.
