...
- `new_path_recognizer` creates a `Recognizer` from a config object containing `patterns` and `generate` fields. See the [example](#example) above for basic usage.
  - Type:
    ...
    ({
      ...
      "hints": (registration_api, paths: array[string]) -> array[index_job_hint]
    }) -> recognizer
    ```
    where `index_job` and `index_job_hint` are tables with specific keys:
    ```
    index_job = {
    ...
    index_job_hint = {
      "root": string,    -- working directory for the indexer
      "indexer": string, -- Docker image for the indexer
      "confidence": ("LANGUAGE_SUPPORTED" | "PROJECT_STRUCTURE_SUPPORTED")
    }
    ```
