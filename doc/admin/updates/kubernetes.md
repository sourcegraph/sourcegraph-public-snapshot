...
**If your deployment uses custom statefulSet names:** A [change](https://github.com/sourcegraph/sourcegraph/pull/23921) to our service discovery logic in this version will prevent the frontend from finding your statefulSet pods. Continue using the old service discovery by setting the environment variable `INDEXED_SEARCH_SERVERS=k8s+rpc://indexed-search:6070` in your frontend pod's `sourcegraph-frontend.Deployment.yaml`. In v3.33 a [revision](https://github.com/sourcegraph/sourcegraph/pull/25146) corrects url construction to allow use of new service discovery with custom statefulSet names. The `INDEXED_SEARCH_SERVERS` environment variable must still be set Ex: `INDEXED_SEARCH_SERVERS=k8s+rpc://<indexed-search-staging>:6070?kind=sts` *note that deploy-sourcegraph uses `indexed-search` as its statefulSet name*
