...
**If your deployment uses custom statefulSet names:** A [change](https://github.com/sourcegraph/sourcegraph/pull/23921) to our service discovery logic in this version will prevent the frontend from finding your statefulSet pods. You can continue using the old service discovery by setting the environment variable `INDEXED_SEARCH_SERVERS=k8s+rpc://indexed-search:6070` in your frontend pod's `sourcegraph-frontend.Deployment.yaml`. In v3.33 a [revision](https://github.com/sourcegraph/sourcegraph/pull/25146) to our endpoint construction will allow use of our new service discovery with divergent statefulSet names. This env var will correctly construct pod rul's from statefulSet.serviceName rather than Service.name which may diverge from statefulSet.name. Ex: `INDEXED_SEARCH_SERVERS=k8s+rpc://<indexed-search-staging>:6070?kind=sts` *note that deploy-sourcegraph uses `indexed-search` as its statefulSet name*
