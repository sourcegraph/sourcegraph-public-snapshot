...
Auto scaling uses the cloud providers managed auto scaling resources (AutoScalingGroups on AWS and Instance Groups on GCP). The Sourcegraph `worker` service will publish a scaling metric (that is, the number of jobs in queue) to the cloud providers. Then, based on that reported value, the auto scalers add and remove compute resources to match the required amount of compute.
