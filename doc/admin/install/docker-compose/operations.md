...
- Create a `release` branch to track all of your customizations to Sourcegraph. This branch will be used to [upgrade Sourcegraph](#upgrade) and [install your Sourcegraph instance](./index.md#installation).
  ...
  export SOURCEGRAPH_VERSION="v3.38.0"
...
> _Note: After adding a new environment variable to your `docker-compose.yaml` you'll need to redeploy from the new spec by running `docker-compose down --remove-orphans` then `docker-compose up -d`, running `docker-compose restart` will not be sufficiant to add the new environment variable._
