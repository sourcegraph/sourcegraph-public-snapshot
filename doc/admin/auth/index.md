...
The `allowOrgs` field restricts logins to members of the specified GitHub organizations while `allowTeams` restricts logins to members of the listed GitHub teams. If used together - let's say you have  a list of allowed orgs and also a list of allowed teams -, members of all teams and subteams that belong to an specified org will be able to login.

Existing user sessions are **not invalidated**. Only new logins after this setting is changed are affected.
