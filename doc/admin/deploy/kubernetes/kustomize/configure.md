...
Before you can configure your Sourcegraph deployment, you will need to create an overlay using the provided instructions below:

1\. Ensure you have met [all the prerequisites](../kustomize/index.md#prerequisites)

2\. Read the [introduction to Kustomize for Sourcegraph](intro.md#overview) to understand how the reference repository is setup

3\. `cd` into your private local copy of the reference repository

4\. Create a new directory, `$OVERLAY_NAME`, inside the `overlays` subdirectory
   
```bash
$ mkdir overlays/$OVERLAY_NAME
```

5\. Copy the overlay file, `kustomization.template.yaml`, from the [overlays/template directory](intro.md#template) to $OVERLAY_NAME, and rename the copied file from `kustomization.template.yaml` to `kustomization.yaml`

```bash
$ cp overlays/template/kustomization.template.yaml overlays/$OVERLAY_NAME/kustomization.yaml
```

Once you have created your overlay, you can then proceed to enable or disable specific components by commenting or uncommenting them in the overlay file. This allows you to customize your deployment to suit your specific needs.

>WARNING: All configurations should take place within the `$OVERLAY_NAME` overlay directory created in step 3. Please avoid making changes to files in directories outside of the [overlays directory](intro.md#overlays) to prevent potential merge conflicts during future updates.
