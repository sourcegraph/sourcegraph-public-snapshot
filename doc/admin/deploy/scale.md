...
|                                                |                                                                                                                                                                                                                                                                                                   |
| :--------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [frontend](scale#frontend)                     | Serves the web application, extensions, and graphQL services. Almost every service has a link back to the frontend, from which it gathers configuration updates.                                                                                                                                  |
| [github-proxy](scale#github-proxy)             | Proxies all requests to github.com to keep track of rate limits and prevent triggering abuse mechanisms                                                                                                                                                                                           |
| [gitserver](scale#gitserver)                   | Mirrors repositories from their code host. All other Sourcegraph services talk to gitserver when they need data from git                                                                                                                                                                          |
| [precise-code-intel](scale#precise-code-intel) | Converts LSIF upload file into Postgres data. The entire index must be read into memory to be correlated                                                                                                                                                                                          |
| [repo-updater](scale#repo-updater)             | Repo-updater tracks the state of repositories, and is responsible for automatically scheduling updates using gitserver. Other apps which desire updates or fetches should be telling repo-updater, rather than using gitserver directly, so repo-updater can take their changes into account.     |
| [searcher](scale#searcher)                     | Provides on-demand unindexed search for repositories. It fetches archives from gitserver and searches them with regexp                                                                                                                                                                            |
| [symbols](scale#symbols)                       | Indexes symbols in repositories using Ctags. By default, the symbols service saves SQLite DBs as files on disk, and copies an old one to a new file when a user visits a new commit. If Rockskip is enabled, the symbols are stored in the codeintel-db instead while the cache is stored on disk |
| [syntect-server](scale#syntect-server)         | An HTTP server that exposes the Rust Syntect syntax highlighting library for use by other services                                                                                                                                                                                                |
| [worker](scale#worker)                         | Runs a collection of background jobs (for both Code-Intel and Code-Insight) periodically or in response to an external event. It is currently janitorial and commit based.                                                                                                                        |
| [zoekt-indexserver](scale#zoekt-indexserver)   | Indexes all enabled repositories on Sourcegraph, as well as keeping the indexes up to date                                                                                                                                                                                                        |
| [zoekt-webserver](scale#zoekt-webserver)       | Runs searches from in-memory indexes, but persists these indexes to disk to avoid re-indexing everything on startup                                                                                                                                                                               |
...
| `Guideline` | If the Postgres instance is OOMing, then it's misconfigured - reduce the amount of memory each worker can utilize.  |
|             | Increase the memory assigned to each worker proportionally (see configuration guide) when database queries are slow |
...
| CPU         |                                                                                       |
| :---------- | :------------------------------------------------------------------------------------ |
| `Overview`  | A thread is dispatched per HTTP request. It is also used to serve Code Insight series |
| `Factors`   | Number of active users                                                                |
...
| `Overview`  | Linear to the concurrent number of API requests proxied                          |
| `Factors`   | Number of API requests to GitHub and GitHub Enterprise                           |
| `Guideline` | Depends on the size of the git commands -- larger the git commands = more memory |
...
| Memory      |                                                                                                        |
| :---------- | :----------------------------------------------------------------------------------------------------- |
| `Overview`  | Searcher fetches archives from gitserver and stores them on disk. It also populates an in-memory cache |
...
| `Factors`   | Number of repositories                                                         |
|             | Number of monorepos                                                            |
| `Guideline` | The disk space should scale with the number of repositories                    |
|             | More repositories = more disk space                                            |
|             | More space results in diminishing returns                                      |
...
| CPU         |                                                |
| :---------- | :--------------------------------------------- |
| `Overview`  | Rate limited / concurrency limited worker pool |
| `Factors`   | Number of repositories                         |
|             | Size of all repositories                       |
|             | Size of the largest repository                 |
|             | Number of insight series defined               |
| `Guideline` | One goroutine per worker                       |
...
| `Factors`   | Number of repositories                                  |
|             | Size of all repositories                                |
| `Guideline` | Replicas number must be parallel with zoekt-indexserver |

| CPU         |                                         |
| :---------- | :-------------------------------------- |
| `Overview`  | A thread is dispatched per search query |
| `Factors`   | Number of repositories                  |
...
| `Factors`   | Number of repositories                  |
|             | Size of all repositories                |
| `Guideline` | More repositories = more memory         |
