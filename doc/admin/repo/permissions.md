...
If the Sourcegraph is configured to sync repositories from multuple code hosts (regardless of if they have the same type, e.g. GitHub + GitHub or GitHub + GitLab), setting up permissions for each of them will make the repoisotry permissions work simultaneously. Setting up a unified SSO for code hosts and Sourcegraph is also possible, read more about how to [Set up Sourcegraph with two GitLab and Keycloak using SAML](https://unknwon.io/posts/200915_setup-sourcegraph-gitlab-keycloak/).
