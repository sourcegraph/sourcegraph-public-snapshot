...
:bulb: Please not that this is only applicable in PR, as for the `main` branch we need to ensure we same the same build steps for everyone until Bazel is fully rolled out. 
...
- When committing, add the `[force-bazel]` message flag in the description of your commit (not on the commit title, but in the description, it's nicer this way).
  - If you commit again, remember to add that message flag again, only the last commit is checked for to determine if we want Bazel on that PR. 
...
:warning: It's highly probable that your build files updates will include changes that you were not responsible for. This is because not everyone is updating buildfiles. Just commit them anyway and move on. This will be get better over time. 
