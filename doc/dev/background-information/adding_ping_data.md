...
1. Open a PR to change [the schema](https://github.com/sourcegraph/analytics/blob/master/BigQuery%20Schemas/sourcegraph_analytics.update_checks_schema.json) as well as this sample pubsub message [pubsub_message.json](https://github.com/sourcegraph/analytics/blob/master/BigQuery%20Schemas/pubsub_message.json) with sourcegraph/bizops as approvers. **Note: we have a 3 business day SLA to test and merge the production schema to properly test before branch cuts.** Keep in mind:
