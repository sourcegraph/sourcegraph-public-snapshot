# Getting started

Have a look around, our code is on [GitHub](https://sourcegraph.com/github.com/sourcegraph/sourcegraph).

## Quickstart

- [Step 1: Install dependencies](quickstart_1_install_dependencies.md)
- [Step 2: Get the code](quickstart_4_clone_repository.md)
- [Step 3: Start Docker](quickstart_2_start_docker.md)
- [Step 4: Initialize your database](quickstart_3_initialize_database.md)
- [Step 5: Configure HTTPS reverse proxy](quickstart_5_configure_https_reverse_proxy.md)
- [Step 6: Start the server](quickstart_6_start_server.md)
- [Step 7: Additional resources](quickstart_7_additional_resources.md)
