...
The LSIF-server process has been extracted into its own diagram.

<object data="/dev/architecture/lsif.svg" type="image/svg+xml" style="width:100%; height: 100%">
</object>

To re-generate the architecture diagram from the `architecture.dot` file with Graphviz, run: `dot -Tsvg -o architecture.svg architecture.dot` (and similar for `lsif.dot`).
