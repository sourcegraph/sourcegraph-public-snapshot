...
- **Webhooks with configurable scope**, which are used by and highly recommended for usage with [campaigns](../batch_changes/index.md)
...
The motivation behind this added functionality is to more efficiently react to updates to Bitbucket Server pull requests when using [campaigns](../batch_changes/index.md) by requiring only a single webhook to receive events for hundreds or thousands of pull requests across projects and repositories.
