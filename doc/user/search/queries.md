...
| **-content:"pattern"** | Exclude results from files whose content matches the pattern. Note: `-content` is currently only supported for literal and regexp patterns on indexed repositories. | [`file:Dockerfile content:alpine: -content:alpine:latest`](https://sourcegraph.com/search?q=file:Dockerfile+content:alpine:+-content:alpine:latest&patternType=literal) |
