...
Changing the state of an upload to or from the `COMPLETED` state can be computationally expensive. When such a change is made, the [repository commit graph](#repository-commit-graph) needs to be refreshed.
