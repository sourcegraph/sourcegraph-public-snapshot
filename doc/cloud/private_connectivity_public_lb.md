# Code hosts or private registries in customer data center exposed via public load balancer
...
> Important:
`private dependency` is customer code host or artifact registry.
...
> For private dependecies in AWS or GCP, please refer to [other deployment methods](./index.md#private-code-host-and-artifact-registry-support)

## How it works

Sourcegraph Cloud is a managed service hosted on GCP. Customer will expose private dependency via load balancer with IP allowlist for 2 static IPs provided by Sourcegraph. Sourcegraph will then be able to access the private dependency over HTTPS through the load balancer from the GCP project hosting Sourcegraph Cloud. Sourcegraph recommends to setup pass-through TCP load balancer to avoid adding private dependecy domain certificate to exposed load balancer.
...
- The public DNS name of the load balancer exposing private code host, e.g. `github.public.company.net` or private artifact registry, e.g. `artifactory.public.company.net`.
...
With pass-through TCP load balancer, the load balancer acts as a simple TCP proxy to forward traffic to the backend private dependency without terminating TLS. This avoids the need to install private dependency server certificate on the load balancer, reducing certificate management overhead.
