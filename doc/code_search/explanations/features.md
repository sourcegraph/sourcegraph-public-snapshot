...
Searching for symbols makes it easier to find specific functions, variables, and more. Use the `type:symbol` filter to search for symbol results. Symbol results also appear in typeahead suggestions, so you can jump directly to symbols by name. When searching an [indexed](../../admin/search.md#indexed-search) commit, Sourcegraph uses Zoekt. Otherwise, it uses the [symbols service](../../code_navigation/explanations/features.md#symbol-search).
...
Note that if the original query finds results (which depends on the code it runs on), Smart Search has no effect. Smart Search does not intervene or interfere with search queries if those queries return results and Sourcegraph behaves as usual.
