...
| **repo:has.topic(...)** | Search only in repos repositories if they have the given GitHub tag. See [built-in predicates](language.md#built-in-repo-predicate) for more. | [`repo:has.topic(go) file:Dockerfile`](https://sourcegraph.com/search?q=context:global+repo:has.topic%28go%29+file:Dockerfile&patternType=standard&sm=1) |
