...
| **select:result-type** | Filters or converts query results to the given type. If results for the query cannot be converted to the given type, they are not returned. After conversion, results are then deduplicated. See [language definition](language#select) for possible values. | [`fmt.Errorf select:repo`](https://sourcegraph.com/search?q=fmt.Errorf+select:repo&patternType=literal) |
