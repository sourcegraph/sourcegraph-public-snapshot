...
- For customers that want to self-host, we recommend one of the single-node [deployment options](admin/deploy/index.md#deployment-options).
- For enterprise customers that require a multi-node, self-hosted deployment, we offer a [Kubernetes](/kubernetes/index.md) option. We strongly encourage you to get in touch by [email](mailto:sales@sourcegraph.com) if you pursue this option.
