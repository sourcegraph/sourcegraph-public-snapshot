...
func Generate(input http.FileSystem, opt Options) error {
	...
	var toc toc
	err = findAndWriteFiles(buf, input, &toc, opt.ModTime)
	if err != nil {
		return err
	}
...
func findAndWriteFiles(buf *bytes.Buffer, fs http.FileSystem, toc *toc, modTime time.Time) error {
