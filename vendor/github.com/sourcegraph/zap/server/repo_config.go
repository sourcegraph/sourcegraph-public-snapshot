...
func (s *Server) ApplyRepoConfiguration(ctx context.Context, logger log.Logger, repo repodb.OwnedRepo, oldConfig, newConfig zap.RepoConfiguration) error {
	oldRemote := oldConfig.Remote
	newRemote := newConfig.Remote

	// Disconnect from removed remote endpoints and connect/reconfigure added or changed remotes.
	if oldRemote.Endpoint != newRemote.Endpoint {
		...
		level.Info(logger).Log("rm-remote", oldRemote)
		if err := s.remotes.closeAndRemoveClient(oldRemote.Endpoint); err != nil {
			return err
		}
	}

	if oldConfig.Remote.EquivalentTo(newRemote) {
		return nil
	}
	logger = log.With(logger, "add-or-update-remote", newRemote)
	level.Debug(logger).Log("new", newRemote, "old", oldRemote)
