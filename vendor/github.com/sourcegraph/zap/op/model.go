...
func createWorkspaceModel(logger log.Logger, ops Ops) (*workspaceModel, error) {
	...
	for i, op := range ops {
		...
		case FileRename:
			...
			if (!ok || oldDst.notExist) && src.edits.Noop() && src.renamedFrom == nil && src.created && len(w.filesCopiedOrRenamedFrom(src)) == 0 {
				src.name = op.Dst
		...
		case Editor:
			f := w.file(op.File)
			if f.notExist {
				return nil, &os.PathError{Op: "editor", Path: op.File, Err: os.ErrNotExist}
			}
			f.exist = true
			if f.editor == nil {
				f.editor = map[string]Editor{}
			}
			f.editor[op.Client] = op
		}
...
func opsForWorkspaceModel(w workspaceModel) Ops {
	...
	emitOpsForFile = func(f *fileModel) {
		...
		for _, editor := range f.editorSorted() {
			ops = append(ops, editor)
		}
...
func (w *workspaceModel) delete(f *fileModel, deleted bool) {
	...
	if nextF := w.nextFile(f); nextF != nil {
		if nextF.exist && nextF.created && !f.created {
			nextF.created = false
			truncated = true
			deleted = false
		}
...
type fileModel struct {
	...
	editor map[string]Editor // client -> Editor
}
...
func (f *fileModel) checkValid() error {
	if f.exist && f.notExist {
		return errors.New("exist && notExist")
	}
	if f.copiedFrom == f {
		panic(f.name + ": copiedFrom self")
	}
	if f.renamedFrom == f {
		panic(f.name + ": renamedFrom self")
	}
	if f.renamedTo == f {
		panic(f.name + ": renamedTo self")
	}
...
func (f *fileModel) editorSorted() []Editor {
	if f.editor == nil {
		return nil
	}
	clients := make([]string, 0, len(f.editor))
	for c := range f.editor {
		clients = append(clients, c)
	}
	sort.Strings(clients)
	editors := make([]Editor, len(clients))
	for i, c := range clients {
		editors[i] = f.editor[c]
	}
	return editors
}
