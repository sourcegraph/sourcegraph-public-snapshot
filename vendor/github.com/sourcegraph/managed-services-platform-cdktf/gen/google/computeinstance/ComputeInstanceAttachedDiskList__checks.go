...
func (j *jsiiProxy_ComputeInstanceAttachedDiskList) validateSetInternalValueParameters(val interface{}) error {
	...
	case cdktf.IResolvable:
		// ok
	case *[]*ComputeInstanceAttachedDisk:
		val := val.(*[]*ComputeInstanceAttachedDisk)
	...
	case []*ComputeInstanceAttachedDisk:
		val_ := val.([]*ComputeInstanceAttachedDisk)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*ComputeInstanceAttachedDisk; received %#v (a %T)", val, val)
		}
	}
