...
var ctagsArgs = []string{
	...
	`--regex-groovy=/^[ \t]*((abstract|final|public|private|protected|static)[ \t]*)*(def|void|byte|int|short|long|float|double|boolean|char|[A-Z][a-zA-Z0-9_]*(<(\w+(, \w+)*)?>)?)[ \t]+([a-zA-Z0-9_]+)\(/\7/m,method/`,
	`--regex-groovy=/^[ \t]*((final|public|private|protected|static|synchronized)[ \t]*)*(def|byte|int|short|long|float|double|boolean|char|[A-Z][A-Za-z0-9_]*(<(\w+(, \w+)*)?>)?)[ \t]+([a-zA-Z0-9_]+)([ \t=]|$)/\7/v,field/`,
	`--langdef=Jsonnet`,
