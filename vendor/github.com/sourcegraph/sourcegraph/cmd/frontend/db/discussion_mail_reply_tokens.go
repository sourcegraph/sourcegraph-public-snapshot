...
func (*discussionMailReplyTokens) Generate(ctx context.Context, userID int32, threadID int64) (string, error) {
	...
	var token string
	err := globalDB.QueryRowContext(ctx, "SELECT token FROM discussion_mail_reply_tokens WHERE user_id=$1 AND thread_id=$2 AND deleted_at IS NULL", userID, threadID).Scan(&token)
	...
	_, err = globalDB.ExecContext(ctx, "INSERT INTO discussion_mail_reply_tokens(token, user_id, thread_id) VALUES($1, $2, $3)", token, userID, threadID)
	if err != nil {
		return "", err
	}
	return token, nil
...
func (*discussionMailReplyTokens) Get(ctx context.Context, token string) (userID int32, threadID int64, err error) {
	...
	err = globalDB.QueryRowContext(ctx, "SELECT user_id, thread_id FROM discussion_mail_reply_tokens WHERE token=$1 AND deleted_at IS NULL", token).Scan(
		&userID,
		&threadID,
	)
	if err != nil {
