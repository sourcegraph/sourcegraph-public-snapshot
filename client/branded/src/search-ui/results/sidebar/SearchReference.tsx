 ...
 */
function augmentFilterInfo(searchReference: FilterInfo, enableOwnershipSearch: boolean): void {
    const filter = filters(enableOwnershipSearch)[searchReference.field]
    if (filter?.alias) {
        searchReference.alias = filter.alias
    }
}

const filterInfos = (enableOwnershipSearch: boolean): FilterInfo[] => {
    const filterInfos = [
        {
...
See [language definition](https://docs.sourcegraph.com/code_search/reference/language#select) for more information on possible values.`,
    ...
    for (const info of filterInfos) {
        augmentFilterInfo(info, enableOwnershipSearch)
    }
    return filterInfos
}

const commonFilters = (enableOwnershipSearch: boolean) =>
    filterInfos(enableOwnershipSearch)
 ...
 * the query.
 */
function shouldShowSuggestions(searchReference: FilterInfo, enableOwnershipSearch: boolean): boolean {
    return Boolean(
        searchReference.showSuggestions !== false &&
            filters(enableOwnershipSearch)[searchReference.field].discreteValues
    )
}
...
const SearchReferenceExample: React.FunctionComponent<React.PropsWithChildren<SearchReferenceExampleProps>> = ({
    ...
    const scanResult = scanSearchQuery(example, false, SearchPatternType.standard, enableOwnershipSearch)
...
const FilterInfoList = ({
    ...
    onExampleClick,
    enableOwnershipSearch,
}: FilterInfoListProps<FilterInfo>): ReactElement => (
...
const SearchReference = React.memo((props: SearchReferenceProps): ReactElement => {
    ...
    const selectedFilters = useMemo(() => {
        if (!hasFilter) {
            return filterInfos(props.enableOwnershipSearch)
        }
        const searchTerms = parseSearchInput(filter)
        return filterInfos(props.enableOwnershipSearch).filter(info => matches(searchTerms, info))
    }, [filter, hasFilter])
        ...
        (searchReference: FilterInfo, negate: boolean) => {
            setQueryState(({ query }) => {
                const updatedQuery = updateQueryWithFilterAndExample(
                    ...
                    searchReference,
                    {
                        singular: Boolean(filters(props.enableOwnershipSearch)[searchReference.field].singular),
                        negate: negate && isNegatableFilter(searchReference.field),
                        emptyValue: shouldShowSuggestions(searchReference, props.enableOwnershipSearch),
                    },
                    props.enableOwnershipSearch
                )
                return {
                    ...
                    hint:
                        (shouldShowSuggestions(searchReference, props.enableOwnershipSearch)
                            ? EditorHint.ShowSuggestions
                            : 0) | EditorHint.Focus,
                }
            })
        },
    ...
    const filterList = (
        <FilterInfoList
            ...
            enableOwnershipSearch={props.enableOwnershipSearch}
        />
    )
            ...
            {hasFilter ? (
                ...
                <Tabs className={styles.tabs} defaultIndex={persistedTabIndex} onChange={setPersistedTabIndex}>
                    ...
                    <TabPanels>
                        <TabPanel>
                            <FilterInfoList
                                filters={commonFilters(props.enableOwnershipSearch)}
                                onClick={updateQuery}
                                onExampleClick={updateQueryWithExample}
                                enableOwnershipSearch={props.enableOwnershipSearch}
                            />
