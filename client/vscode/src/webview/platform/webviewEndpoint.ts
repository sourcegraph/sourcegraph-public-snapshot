...
import { generateUUID, isNestedConnection, NestedConnectionData, RelationshipType } from './proxyTransferHandler'

const panelId = self.document ? self.document.documentElement.dataset.panelId! : 'web-worker'

// GOAL: be able to use the same proxyTransferHandler for bi-directional communication channels
// with both webview <-> node and webview <-> web worker.
...
// TODO could share implementation for createEndpointsForWebToWeb and createEndpointsForWebToNode, just parameterize.
...
// TODO `createEndpointsForWebToNode` vs `createEndpointsForWebToWeb` (for webview main thread <-> web worker)
// We'll have one proxyTransferHandler which chooses the endpointFactory based on the relationshipType (carried in the message).
// They turn an Endpoint into an EndpointPair.
