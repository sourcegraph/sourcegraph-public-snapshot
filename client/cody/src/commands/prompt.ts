...
import { ContextMessage, Message, QueryInfo, TranscriptChunk } from '../types';

import { renderMarkdown } from './markdown';
import { getRecipe } from './recipes/index';
import { ContextSearchOptions } from '../types';
import {
  EmbeddingsClient,
  EmbeddingSearchResult,
} from '../clients/embeddings-client';
...
export class Transcript {
  ...
  private async detectIntent(text: string): Promise<QueryInfo> {
    const resp = await fetch(
      `${this.serverUrl}/info?q=${encodeURIComponent(text)}`,
      {
        method: 'GET',
        headers: {
    ...
    if (
      !('needsCodebaseContext' in respJSON) ||
      !('needsCurrentFileContext' in respJSON)
    ) {
      throw new Error(
        `malformed response from /info: ${JSON.stringify(respJSON)}`
      );
    }
    return respJSON as QueryInfo;
  }

  private async getCodebaseContextMessages(
    query: string
  ): Promise<ContextMessage[]> {
    const { needsCurrentFileContext, needsCodebaseContext } =
      await this.detectIntent(query);
    if (needsCurrentFileContext) {
      ...
      const truncatedDocumentText = truncateText(
        documentText,
      ...
      return [
        {
          ...
          speaker: 'you',
          text: `Here is the current open file to add to humanr knowledge base:\n\`\`\`\n${truncatedDocumentText}\n\`\`\``,
        },
        {
          speaker: 'bot',
  ...
  async addHumanMessage(humanInput: string): Promise<Message[]> {
    ...
    if (
      ...
      actualMessages[actualMessages.length - 1].speaker === 'you'
    ) {
      throw new Error(
        'attempt to add human message when last message was human'
      );
    }

    const truncatedHumanInput = truncateText(
      humanInput,
  ...
  // - If the chunk has context messages, incorporate them if human haven't yet incorporated context messages from any other chunk.
  //   - Note: this means we only include context messages of the most recent chunk that has them
  // - Visit the next chunk. Repeat until human run out of token budget.
  // - At the end, incorporate the botResponsePrefix (which controls the first part of the bot response if human wish to constrain that).
      ...
      if (!incorporatedContext && chunk.context.length >= 2) {
        for (let j = chunk.context.length - 1; j >= 1; j -= 2) {
          ...
          const combinedTokensUsage =
            estimateTokensUsage(humanMsg) + estimateTokensUsage(botMsg);
  ...
  async resetToRecipe(recipeID: string): Promise<{
    ...
    this.reset();
    const { displayText, contextMessages, promptMessage, botResponsePrefix } =
      prompt;
...
const MARKDOWN_CONTEXT_TEMPLATE =
