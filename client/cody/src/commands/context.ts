...
export class LocalKeywordFetcher {
  ...
  async fetchFileStats(
    ...
    const proc = spawn(
      this.rgPath,
      [...fileExtRipgrepParams, '--json', regexQuery, './'],
      {
        cwd: rootPath,
          ...
          if (terms.length !== 2) {
            ...
            if (matches && matches.length === 2) {
              try {
                ...
                console.error(
                  `failed to parse number of files matched from string: ${matches[1]}`
                );
              }
            }
            continue;
  ...
  async fetchKeywordFiles(
    ...
    const filteredTerms = Array.from(
      new Set(removeStopwords(stemmedTerms).filter((term) => term.length >= 3))
    ) as string[];
...
function tfidf(
  ...
  if (terms.length !== tf.length) {
    throw new Error(
      `terms.length !== tf.length ${terms.length} !== ${tf.length}`
    );
  }
