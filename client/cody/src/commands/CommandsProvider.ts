...
export const CommandsProvider = async (context: vscode.ExtensionContext) => {
  ...
  disposables.push(
    vscode.commands.registerCommand(
      'sourcegraph.cody.toggleEnabled',
      async () => {
    ...
    vscode.commands.registerCommand('cody.recipe.explain-code', async () =>
      executeRecipe('explainCode')
    ),
    vscode.commands.registerCommand(
      'cody.recipe.explain-code-high-level',
      async () => await executeRecipe('explainCodeHighLevel')
    ),
    vscode.commands.registerCommand(
      'cody.recipe.generate-unit-test',
      async () => await executeRecipe('generateUnitTest')
    ),
    vscode.commands.registerCommand(
      'cody.recipe.generate-docstring',
      async () => await executeRecipe('generateDocstring')
    ),
    vscode.commands.registerCommand(
      'cody.recipe.translate-to-language',
      async () => await executeRecipe('translateToLanguage')
    ),
    vscode.commands.registerCommand(
      'cody.recipe.git-history',
      async () => await executeRecipe('gitHistory')
    )
  );
  // TODO
