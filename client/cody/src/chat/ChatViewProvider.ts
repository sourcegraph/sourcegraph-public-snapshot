...
export class ChatViewProvider implements vscode.WebviewViewProvider {
    ...
    private sendPrompt(promptMessages: Message[], responsePrefix = ''): void {
        ...
        this.multiplexer.sub(BotResponseMultiplexer.DEFAULT_TOPIC, {
            onResponse: (content: string) => {
                text += content
                this.transcript.addAssistantResponse(reformatBotMessage(content, responsePrefix))
                this.sendTranscript()
        ...
        this.cancelCompletionCallback = this.chat.chat(promptMessages, {
            onChange: text => {
                // TODO: The multiplexer can handle incremental text. Change chat to provide incremental text.
    ...
    public async executeRecipe(recipeId: string, humanChatInput: string = ''): Promise<void> {
        ...
        // TODO: Elaborate the prompt from the multiplexer subscriptions.
