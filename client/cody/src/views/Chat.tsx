...
export const Chat: React.FunctionComponent<
  React.PropsWithChildren<ChatboxProps>
> = ({
  setInConversation,
  setUserInput,
  userInput,
  ...
  const onChatSubmit = useCallback(async () => {
    ...
    setFormInput('');
  }, [formInput, transcripts, transcript, codyIsTyping]);

  const bubbleClassName = (speaker: string) => {
    return speaker === 'you' ? 'human' : 'bot';
  };
      ...
      >
        ...
        {transcripts && transcripts[0] && (
          <div className="container-getting-started">
            {transcripts.map((msg) => (
              <div
                className={`bubble-row ${bubbleClassName(
                  msg.speaker
                )}-bubble-row`}
              >
                <div
                  className={`bubble ${bubbleClassName(msg.speaker)}-bubble`}
                >
                  <div
                    className={`bubble-content ${bubbleClassName(
                      msg.speaker
                    )}-bubble-content`}
                  >
                    {msg.contextFiles && msg.contextFiles[0] && (
                  ...
                  <div
                    className={`bubble-footer ${bubbleClassName(
                      msg.speaker
                    )}-bubble-footer`}
                  >
                    <span>{`${msg.speaker === 'bot' ? 'Cody' : 'Me'} · ${
                      msg.timestamp
                    }`}</span>
                  </div>
                </div>
