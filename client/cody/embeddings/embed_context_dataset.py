...
NO_ADDITIONAL_CONTEXT = [
    ...
    """convert this TypeScript code to a functional style:
```
let historyStrings = [];
for (let i = 0; i < history.length; i++) {
    ...
    if (item.symbols.length > 0) {
        const symbolNames = [];
        for (let j = 0; j < item.symbols.length; j++) {
            symbolNames.push(item.symbols[j].name.trim());
        }
        historyStrings.push(`// ${item.uri}\n${symbolNames.join("\n")}`);
    }
}
```
    """,
...
if __name__ == "__main__":
    ...
    need_additional_context_messages_embeddings = get_embeddings(
        NEED_ADDITIONAL_CONTEXT, engine=EMBEDDING_ENGINE
    )
    np.save(
        os.path.join(
            args.output_dir, "need_additional_context_messages_embeddings.npy"
        ),
        np.array(need_additional_context_messages_embeddings),
    )

    no_additional_context_messages_embeddings = get_embeddings(
        NO_ADDITIONAL_CONTEXT, engine=EMBEDDING_ENGINE
    )
    np.save(
        os.path.join(args.output_dir, "no_additional_context_messages_embeddings.npy"),
        np.array(no_additional_context_messages_embeddings),
    )
