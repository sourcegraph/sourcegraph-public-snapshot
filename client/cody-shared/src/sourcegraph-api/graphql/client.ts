...
import {
    ...
    CODY_APP_ADD_CODE_HOST,
    CODY_APP_REPO_EMBEDDING_JOBS_LIST_QUERY,
    CODY_APP_GET_LOCAL_CODE_HOSTS,
} from './queries'
...
interface CodyAppAddRemoteCodeHostResponse {
    addExternalService: AddExternalServiceResult
}

type AddExternalServiceResult = CodeHost
...
interface CodyAppGetLocalCodeHostsResponse {
    __typename?: 'Query'
    localExternalServices: {
        __typename?: 'LocalExternalService'
        id: string
        path: string
...
export interface RepoEmbeddingJobsListResult {
    __typename?: 'Query'
    repoEmbeddingJobs: {
        __typename?: 'RepoEmbeddingJobsConnection'
        totalCount: number
        nodes: RepoEmbeddingJobsConnection[]
    }
}

export interface RepoEmbeddingJobsConnection {
    __typename?: 'RepoEmbeddingJob'
    id: string
    ...
    cancel: boolean
    repo: { __typename?: 'Repository'; name: string; url: string } | null
    revision: { __typename?: 'GitCommit'; oid: string; abbreviatedOID: string } | null
    stats: {
        __typename?: 'RepoEmbeddingsStats'
...
export class SourcegraphGraphQLAPIClient {
    ...
    public async codyAppAddRemoteCodeHost(path: string): Promise<CodeHost | Error> {
        if (!path) {
            return new Error(`path must be non-empty string: ${path}`)
        }
        return this.fetchSourcegraphAPI<APIResponse<CodyAppAddRemoteCodeHostResponse>>(CODY_APP_ADD_CODE_HOST, {
            input: {
                displayName: `Local repositories service (${path})`,
                config: createDefaultLocalServiceConfigForApp(path),
                kind: 'OTHER',
            },
        }).then(response => extractDataOrError(response, data => data.addExternalService))
    }

    public async codyAppGetLocalCodeHosts(): Promise<string[] | Error> {
        return this.fetchSourcegraphAPI<APIResponse<CodyAppGetLocalCodeHostsResponse>>(CODY_APP_GET_LOCAL_CODE_HOSTS, {
            fetchPolicy: 'network-only',
        }).then(response => extractDataOrError(response, data => getLocalServicePaths(data)))
    }

    public async codyAppGetEmbeddingJobConnection(query?: string): Promise<RepoEmbeddingJobsConnection[] | Error> {
        return this.fetchSourcegraphAPI<APIResponse<RepoEmbeddingJobsListResult>>(
            CODY_APP_REPO_EMBEDDING_JOBS_LIST_QUERY,
            {
                after: null,
                first: 10,
        ...
        ).then(response => extractDataOrError(response, data => data.repoEmbeddingJobs?.nodes))
    }
}
 ...
 */
function createDefaultLocalServiceConfigForApp(path: string): string {
    const srcServeGitUrl = 'http://127.0.0.1:3434'
    return `{ "url":"${srcServeGitUrl}", "root": "${path}", "repos": ["src-serve-local"] }`
}
 ...
 */
export function getLocalServicePaths(data?: CodyAppGetLocalCodeHostsResponse): string[] {
 ...
 */
export function getLocalServices(data?: CodyAppGetLocalCodeHostsResponse, isAutogenerated?: boolean): LocalCodeHost[] {
    if (!data) {
        return []
    }
