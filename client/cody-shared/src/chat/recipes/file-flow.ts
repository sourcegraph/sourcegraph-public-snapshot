...
export class FileFlow implements Recipe {
    ...
    public async getInteraction(humanChatInput: string, context: RecipeContext): Promise<Interaction | null> {
        ...
        const prompt = FileFlow.newFilePrompt
    ...
    private async getContextMessages(
        ...
        contextMessages.push(...(await FileFlow.getEditorDirContext(currentDir)))
        if (contextMessages.length < 12) {
            // Create context messages from open tabs
            contextMessages.push(...FileFlow.getEditorOpenTabsContext(currentDir))
        }

        return contextMessages.slice(0, 12)
    }
    ...
    public static async getEditorDirContext(currentDir: string): Promise<ContextMessage[]> {
        ...
        // Get the list of files in the current directory then filter out directories then get the first 5 files
        const filesInDir = (await vscode.workspace.fs.readDirectory(currentDirUri)).filter(file => !file[1]).slice(0, 5)
    ...
    private getHumanDisplayText(humanChatInput: string, fileName: string): string {
        return humanChatInput + FileFlow.displayPrompt + fileName
    }
