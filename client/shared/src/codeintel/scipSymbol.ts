...
export type SCIPSymbol = LocalSymbol | NonLocalSymbol
...
export function parseSymbol(s: string): SCIPSymbol {
    return symbol.tryParse(s)
}

export function formatSymbol(s: SCIPSymbol): string {
    if (s.kind === 'local') {
...
const simpleIdentifier = P.regexp(/[\w$+-]+/).desc('simple identifier')
