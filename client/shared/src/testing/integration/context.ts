...
>({
    ...
    return {
        ...
        dispose: async () => {
            ...
            await pTimeout(recordCoverage(driver.browser), DISPOSE_ACTION_TIMEOUT, 'Recording coverage timed out')
            await pTimeout(driver.page.close(), DISPOSE_ACTION_TIMEOUT, 'Closing Puppeteer page timed out')
            await pTimeout(polly.stop(), DISPOSE_ACTION_TIMEOUT, 'Stopping Polly timed out')
        },
    }
}
