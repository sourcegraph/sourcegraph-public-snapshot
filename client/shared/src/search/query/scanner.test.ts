...
describe('scanBalancedPattern()', () => {
    ...
    test('single unbalanced paren', () => {
        expect(scanSearchQuery(')', false, SearchPatternType.regexp, true)).toMatchInlineSnapshot(
...
describe('scanSearchQuery() for literal search', () => {
    test('empty', () =>
        expect(scanSearchQuery('', false, SearchPatternType.literal, true)).toMatchInlineSnapshot(
            '{"type":"success","term":[]}'
        ))
    ...
    test('literal', () =>
        expect(scanSearchQuery('a', false, SearchPatternType.literal, true)).toMatchInlineSnapshot(
    ...
    test('filter', () =>
        expect(scanSearchQuery('f:b', false, SearchPatternType.literal, true)).toMatchInlineSnapshot(
    ...
    test('filter with a value ending with a colon', () => {
        expect(scanSearchQuery('f:a:', false, SearchPatternType.literal, true)).toMatchInlineSnapshot(
    ...
    test('quoted, double quotes, literal', () =>
        expect(scanSearchQuery('"a:b"', false, SearchPatternType.literal, true)).toMatchInlineSnapshot(
            '{"type":"success","term":[{"type":"pattern","range":{"start":0,"end":5},"kind":1,"value":"\\"a:b\\"","delimited":false}]}'
        ))

    test('quoted, double quotes, regexp', () =>
        expect(scanSearchQuery('"a:b"', false, SearchPatternType.regexp, true)).toMatchInlineSnapshot(
    ...
    test('parenthesized parameters', () => {
        expect(scanSearchQuery('repo:a (file:b and c)', false, SearchPatternType.literal, true)).toMatchInlineSnapshot(
    ...
    test('nested parenthesized parameters', () => {
        expect(scanSearchQuery('(a and (b or c) and d)', false, SearchPatternType.literal, true)).toMatchInlineSnapshot(
    ...
    test('do not treat links as filters', () => {
        expect(
            scanSearchQuery('http://example.com repo:a', false, SearchPatternType.literal, true)
        ).toMatchInlineSnapshot(
...
describe('scanSearchQuery() for regexp', () => {
    test('interpret regexp pattern with match groups', () => {
        expect(
            scanSearchQuery('((sauce|graph)(\\s)?)is best(g*r*a*p*h*)', false, SearchPatternType.regexp, true)
        ).toMatchInlineSnapshot(
    ...
    test('interpret regexp pattern with match groups between keywords', () => {
        expect(
            scanSearchQuery('(((sauce|graph)\\s?) or (best)) and (gr|aph)', false, SearchPatternType.regexp, true)
        ).toMatchInlineSnapshot(
    ...
    test('interpret regexp slash quotes', () => {
        expect(
            scanSearchQuery('r:a /a regexp \\ pattern/', false, SearchPatternType.regexp, true)
        ).toMatchInlineSnapshot(
...
describe('scanSearchQuery() for standard', () => {
    test('scan literal and regexp patterns', () => {
        expect(scanSearchQuery('pfalz /mosel/', false, SearchPatternType.standard, true)).toMatchInlineSnapshot(
...
// search for thing
thing`
        expect(scanSearchQuery(query, true, SearchPatternType.literal, true)).toMatchInlineSnapshot(
    ...
    test('do not interpret C-style comments', () => {
        expect(scanSearchQuery('// thing', false, SearchPatternType.literal, true)).toMatchInlineSnapshot(
...
describe('scanSearchQuery() with predicate', () => {
    test('recognize predicate', () => {
        expect(
            scanSearchQuery('repo:contains.file(path:README.md)', false, SearchPatternType.literal, true)
        ).toMatchInlineSnapshot(
    ...
    test('recognize multiple predicates over whitespace', () => {
        expect(
            scanSearchQuery(
                'repo:contains.file(path:README.md)\n\nrepo:contains.path(foo)',
                false,
                SearchPatternType.literal,
                true
            )
    ...
    test('recognize parenthesized predicate', () => {
        expect(
            scanSearchQuery('(repo:contains.file(path:bar))', false, SearchPatternType.literal, true)
        ).toMatchInlineSnapshot(
    ...
    test('recognize `not` and other keywords in literal search where unquoted terms might appear to be quoted', () => {
        expect(
            scanSearchQuery("'email is not allowed'", false, SearchPatternType.standard, true)
        ).toMatchInlineSnapshot(
    ...
    test('detect patterntype inside query', () => {
        expect(
            scanSearchQuery('patterntype:standard /test/', false, SearchPatternType.literal, true)
        ).toMatchInlineSnapshot(
