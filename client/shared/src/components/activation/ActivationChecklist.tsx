...
> = ({ className = '', ...props }: ActivationChecklistItemProps) => (
    <div className={classNames('d-flex justify-content-between', styles.activationChecklistItem, className)}>
        ...
        <div>
            {props.done ? (
                <Icon role="img" className="text-success" as={CheckCircleIcon} aria-hidden={true} />
            ) : (
                <Icon role="img" className="text-muted" as={CheckboxBlankCircleOutlineIcon} aria-hidden={true} />
            )}
        </div>
    </div>
)
