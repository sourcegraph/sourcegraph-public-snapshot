...
public class PerforceUtil {
  ...
  public static String getRemoteRepoUrl(@NotNull Project project, @NotNull VirtualFile file)
    ...
    Collection<Pair<P4Connection, Collection<VirtualFile>>> rootsByConnections =
        vcs.getRootsByConnections();
    P4Connection connection =
        rootsByConnections.stream()
            .filter(
                (Pair<P4Connection, Collection<VirtualFile>> pair) ->
                    pair.getSecond().stream()
                        .anyMatch((VirtualFile root) -> file.getPath().startsWith(root.getPath())))
            .map(x -> Pair.getFirst(x))
            .findFirst()
            .orElse(null);
  ...
  @Nullable
  private static String getDepotName(
      @NotNull Project project, @NotNull P4Connection connection, @NotNull VirtualFile file)
      throws VcsException {
    PerforceVcs vcs = PerforceVcs.getInstance(project);
    Collection<Pair<P4Connection, Collection<VirtualFile>>> rootsByConnections =
        vcs.getRootsByConnections();
    Pair<P4Connection, Collection<VirtualFile>> pair =
        rootsByConnections.stream()
            .filter((Pair<P4Connection, Collection<VirtualFile>> x) -> x.getFirst() == connection)
            .findFirst()
            .orElse(null);
    if (pair == null) {
      return null;
    ...
    VirtualFile root =
        pair.getSecond().stream()
            .filter((VirtualFile x) -> file.getPath().startsWith(x.getPath()))
            .findFirst()
            .orElse(null);
    if (root == null) {
      return null;
