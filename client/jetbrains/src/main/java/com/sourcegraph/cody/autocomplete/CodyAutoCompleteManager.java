...
/**
 * Responsible for triggering and clearing inline code completions (the
...
public class CodyAutoCompleteManager {
    ...
    // TODO: figure out how to avoid the ugly nested `Future<CompletableFuture<T>>`
    // type.
    private final AtomicReference<Optional<Future<CompletableFuture<Void>>>> currentJob = new AtomicReference<>(
            Optional.empty());
    ...
    public void triggerAutoComplete(@NotNull Editor editor, int offset) {
        ...
        SourcegraphNodeCompletionsClient client = new SourcegraphNodeCompletionsClient(autoCompleteService(editor),
                token);
        ...
        return provider
                ...
                .thenAccept(
                        result -> {
                            ...
                            Optional<InlineAutoCompleteItem> maybeItem = result.items.stream()
                                        ...
                                        .invokeLater(
                                                () -> {
                                                    ...
                                                    Optional.ofNullable(editor.getProject())
                                                            .ifPresent(
                                                                    p -> GraphQlLogger.logCodyEvent(p, "completion",
                                                                            "suggested"));

                                                    /* display autocomplete */
                                                    AutoCompleteText autoCompleteText = item.toAutoCompleteText(
                                                            autoCompleteDocumentContext.getSameLineSuffix().trim());
                                                    autoCompleteText
                                                            .getInlineRenderer(editor)
                                                            .ifPresent(
                                                                    inlineRenderer -> inlayModel.addInlineElement(
                                                                            offset, true, inlineRenderer));
                                                    autoCompleteText
                                                            .getAfterLineEndRenderer(editor)
                                                            .ifPresent(
                                                                    afterLineEndRenderer -> inlayModel
                                                    ...
                                                    autoCompleteText
                                                            ...
                                                            .ifPresent(
                                                                    blockRenderer -> inlayModel.addBlockElement(
                                                                            offset, true, false, Integer.MAX_VALUE,
                                                                            blockRenderer));
                                                });
                            } catch (Exception e) {
    ...
    public static @NotNull InlineAutoCompleteItem normalizeIndentation(
        ...
        if (item.insertText.matches("^[\t ]*.+")) {
            ...
            String indentation = item.insertText.substring(
            ...
            Range newRange = item.range.withEnd(item.range.end.withCharacter(item.range.end.character - rangeDiff));
    ...
    private boolean isEditorSupported(@NotNull Editor editor) {
        ...
        boolean isSupported = isEditorInstanceSupported(editor)
    ...
    private void cancelCurrentJob() {
        // TODO: change this implementation when we avoid nested
                ...
                .ifPresent(
                        job -> {
                            if (job.isDone()) {
                                ...
                                // `CompletableFuture<>`.
                                // Feel free to reimplement this entire method if it's causing problems because
                                // this
