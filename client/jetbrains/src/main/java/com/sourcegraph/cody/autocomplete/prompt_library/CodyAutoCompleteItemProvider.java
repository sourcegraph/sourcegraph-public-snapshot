...
/**
 * Manually translated logic from
 * <code>cody/vscode/src/completions/index.ts</code> in the VS Code
 * extension. Some code in this class is not used since we haven't translated
 * all the logic yet.
 * Let's keep the unused code to make it easier to see the similarity between
 * the two versions.
 */
...
public class CodyAutoCompleteItemProvider extends InlineAutoCompleteItemProvider {
    ...
    private CompletableFuture<InlineAutoCompleteList> provideInlineAutoCompleteItemsInner(
       ...
       List<ContextMessage> embeddings = Collections.emptyList();
        ...
        // TODO: implement debouncing with a non-blocking way instead of
        // `Thread.sleep()`
        // try {
        // Thread.sleep(waitMs);
        ...
        CompletableFuture<List<InlineAutoCompleteItem>> all = Promises.all(promises)
                .thenApply(
    ...
    private AutoCompleteProvider endOfLineProvider(
            ...
            int defaultN,
            TextDocument document) {
        Function<Optional<String>, AutoCompleteProvider> fallbackDefaultProvider = (maybeErrorToLog) -> {
