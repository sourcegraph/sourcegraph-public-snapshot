...
public class URLBuilder {
  ...
  public static String buildEditorFileUrl(
    ...
    return ConfigUtil.getServerPath(project).getUrl()
        ...
        + (start != null
            ? ("&start_row="
                + URLEncoder.encode(Integer.toString(start.line), StandardCharsets.UTF_8)
                + "&start_col="
                + URLEncoder.encode(Integer.toString(start.column), StandardCharsets.UTF_8))
            : "")
        + (end != null
            ? ("&end_row="
                + URLEncoder.encode(Integer.toString(end.line), StandardCharsets.UTF_8)
                + "&end_col="
                + URLEncoder.encode(Integer.toString(end.column), StandardCharsets.UTF_8))
            : "")
        + "&"
        + buildVersionParams();
  }
  ...
  public static String buildDirectSearchUrl(
    ...
    return ConfigUtil.getServerPath(project).getUrl()
        ...
        + "&q="
        + URLEncoder.encode(
            (repoFilter != null ? repoFilter + " " : "") + search, StandardCharsets.UTF_8);
  }

  @NotNull
  public static String buildCommitUrl(
      @NotNull String sourcegraphBase,
      @NotNull String revisionNumber,
      @NotNull String remoteUrl,
    ...
    // this is pretty hacky but to try to build the repo string we will just try to naively parse
    // the git remote uri. Worst case scenario this 404s
    ...
    return sourcegraphBase
        ...
        + String.format(
            "&version=v%s",
            URLEncoder.encode(ConfigUtil.getPluginVersion(), StandardCharsets.UTF_8))
        + String.format(
            "&utm_product_name=%s", URLEncoder.encode(productName, StandardCharsets.UTF_8))
        + String.format(
            "&utm_product_version=%s", URLEncoder.encode(productVersion, StandardCharsets.UTF_8));
  }
  ...
  public static String buildSourcegraphBlobUrl(
    ...
    return ConfigUtil.getServerPath(project).getUrl()
        ...
        + (start != null
            ? ("L"
                + URLEncoder.encode(Integer.toString(start.line + 1), StandardCharsets.UTF_8)
                + ":"
                + URLEncoder.encode(Integer.toString(start.column + 1), StandardCharsets.UTF_8))
            : "")
        + (end != null
            ? ("-"
                + URLEncoder.encode(Integer.toString(end.line + 1), StandardCharsets.UTF_8)
                + ":"
  ...
  private static String buildVersionParams() {
    return "editor="
        + URLEncoder.encode("JetBrains", StandardCharsets.UTF_8)
        + "&version=v"
        + URLEncoder.encode(ConfigUtil.getPluginVersion(), StandardCharsets.UTF_8);
  }
}
