...
class SelectOptionPopupUI(
    ...
    private val myExecutorService = SequentialTaskExecutor.createSequentialApplicationPoolExecutor(
        ...
        if (clickCount > 1 && clickCount % 2 == 0) {
            ...
            if (i != -1) {
                IdeFocusManager.getGlobalInstance()
                    .doWhenFocusSettlesDown { IdeFocusManager.getGlobalInstance().requestFocus(field, true) }
    ...
    private fun rebuildList() {
        ...
        ReadAction.nonBlocking<CollectionListModel<Any>> {
            CollectionListModel(
                myOptions.stream()
                    .filter { it: String -> it.lowercase(Locale.getDefault()).startsWith(searchPattern.lowercase(Locale.getDefault())) }
    ...
    override fun createTopLeftPanel(): JPanel {
        ...
        val foregroundColor = if (StartupUiUtil.isUnderDarcula()) {
        ...
        if (SystemInfo.isMac) {
            myTextFieldTitle.font = myTextFieldTitle
                .font
                .deriveFont(Font.BOLD, myTextFieldTitle.font.size - 1f)
        } else {
