...
/** This table shows languages in whitelist format specifically for the UI.
 * We actually blacklist language entries under the hood.
 */
class AutocompleteLanguageTable : ListTableWithButtons<LanguageEntry>() {
    ...
    fun setBlacklistedLanguageIds(blacklistedIds: List<String>) {
        elements.stream()
            .forEach { le -> if (blacklistedIds.contains(le.language.id)) le.isBlacklisted = true }
    }
}
