...
> = props => {
    ...
    return (
        <div className="test-batches-settings-page">
            ...
            {batchChangesRolloutWindows && batchChangesRolloutWindows.length > 0 && (
                <RolloutWindowsConfiguration rolloutWindows={batchChangesRolloutWindows} />
            )}
            <UserCodeHostConnections
                headerLine={
                    <Text>Add access tokens to enable Batch Changes changeset creation on your code hosts.</Text>
                }
...
export const RolloutWindowsConfiguration: React.FunctionComponent<
    React.PropsWithChildren<RolloutWindowsConfigurationProps>
> = ({ rolloutWindows }) => (
    <Container className="mb-3">
        <H3>Rollout Windows</H3>
        <Text>
            Specifies specific windows, which can have associated rate limits, to be used when reconciling published
            changesets.
        </Text>
            ...
            {rolloutWindows.map((rolloutWindow, index) => (
                <li key={index} className={styles.rolloutWindowListItem}>
                    <Text className={styles.rolloutWindowListItemFrequency}>
                        ...
                        />
                        {typeof rolloutWindow.rate === 'string'
                            ? upperFirst(rolloutWindow.rate.replace('/', ' changesets per '))
                            : `${rolloutWindow.rate} changesets per minute`}
                    </Text>
                    <small>
                        <strong>Days</strong>:{' '}
                        {rolloutWindow.days && rolloutWindow.days.length > 0
                            ? rolloutWindow.days.join(', ')
                            : 'every other day'}
                    </small>
                    <br />
