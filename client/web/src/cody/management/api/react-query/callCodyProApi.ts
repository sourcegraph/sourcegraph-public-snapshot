...
const buildRequest = (url: string, { headers = {}, ...init }: RequestInit = {}): Promise<Response> =>
    fetch(url, {
...
const signOutAndRedirectToSignIn = async (): Promise<void> => {
    const response = await buildRequest('/-/sign-out', { method: 'GET' })
    if (response.ok) {
...
export const callCodyProApi = async <Data>(call: Call<Data>): Promise<Data | undefined> => {
    const response = await buildRequest(`/.api/ssc/proxy${call.urlSuffix}`, {
        method: call.method,
