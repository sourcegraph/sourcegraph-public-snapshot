...
export function createPlatformContext(): PlatformContext {
    ...
    const viewerSettingsObservable = fromObservableQueryPromise(settingsQueryWatcherPromise).pipe(
        map(({ data, errors }) => {
    ...
    const context: PlatformContext = {
        settings: viewerSettingsObservable.pipe(map(gqlToCascade), publishReplay(1), refCount()),
        updateSettings: async (subject, edit) => {
            ...
            // The error will be emitted to consumers from the `viewerSettingsObservable`.
