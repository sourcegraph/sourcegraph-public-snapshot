...
export const UserSettingsManageRepositoriesPage: React.FunctionComponent<Props> = (
    ...
    // initial state vars
    const emptyRepos: AffiliatedRepositoriesResult['affiliatedRepositories']['nodes']= []
    ...
    const filteredRepos: AffiliatedRepositoriesResult['affiliatedRepositories']['nodes']= []
    ...
    for (let page = 1; page <= Math.ceil(filteredRepos.length/perPage); page++) {
        pages.push(<a className="btn" onClick={event => setPage(page)}><p className={currentPage === page && 'text-primary' || 'text-muted'}>{page}</p></a>)
    }
    ...
    const submit = (event: FormEvent<HTMLFormElement>): void => {
        ...
        for (const host of codeHosts.hosts) {
            ...
            console.log(selectionState.radio)
            setCodeHostRepos({
                    id: host.id,
                        ...
                        {selectionState.radio === 'selected' && (
                            <div className="filtered-connection">
                                ...
                                <table className="filtered-connection test-filtered-connection filtered-connection--noncompact table mt-3">
                                    {selectionState.radio === 'selected' && repoState.loading && !repoState.loaded && (
                                        <tbody>
                                            ...
                                            <tr className="w-100 repository-node d-flex align-items-center justify-content-between">
                                                <td className="w-100 d-flex justify-content-between align-items-baseline">
                                                    <div className="d-flex align-items-center">
                                                        <input className="mr-2" type="checkbox" checked={selectionState.repos.size === filteredRepos.length} onChange={event => {
                                                            ...
                                                            setSelectionState({repos: newMap, loaded: selectionState.loaded, radio: selectionState.radio})
                                                        }}/>
                                                        <span className={(selectionState.repos.size !== 0 && 'font-weight-bold ' || '')+'repositories-header'}>{selectionState.repos.size > 0 && selectionState.repos.size || 'No'} repositories selected.</span>
                                                    </div>
                                                </td>
                                            </tr>
                                        </tr>
                                            ...
                                            return (
                                                <tr className="align-items-baseline d-flex" key={repo.name}>
                                                    <RepositoryNode
                                                        ...
                                                        prefixComponent={(<input className="mr-2" type="checkbox" checked={selectionState.repos.has(repo.name)} onChange={event => {
                                                            ...
                                                            setSelectionState({repos: newMap, radio: selectionState.radio, loaded: selectionState.loaded})
                                                        }}/>)}
                                                    />
                                                </tr>
                                            )
