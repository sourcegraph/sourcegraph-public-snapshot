...
export function PieChart<Datum extends object>(props: PieChartProps<Datum>): ReactElement | null {
    ...
    const {
        ...
        dataKey,
        nameKey,
        linkURLKey = '' as keyof Datum,
        fillKey = '' as keyof Datum,
    } = content;

    const sortedData = useMemo(
        () => distributePieArcs(data, dataKey),
        [data, dataKey]
    )
    ...
    const getLink = useCallback(
        (arc: PieArcDatum<Datum>): string => arc.data[linkURLKey] as unknown as string,
        [linkURLKey]
    );
...
function PieArc<Datum>(props: PieArcProps<Datum>): ReactElement {
    ...
    const handleGroupClick = useCallback(
        (event: MouseEvent) => onClick({ originEvent: event, link}),
        [link, onClick]
    );
