...
import { FuzzyFilesProps, QueryProps } from "./FuzzyFiles";
...
class Bucket {
  ...
  public matches(query: string, hashParts: number[]): BucketResult {
    ...
    for (var i = 0; i < this.files.length; i++) {
      ...
      const positions = fuzzyMatches(queryParts, file);
...
export class FuzzySearch implements FuzzyFilesProps {
  buckets: Bucket[];
  BUCKET_SIZE = 500;
  constructor(readonly files: string[]) {
    this.buckets = [];
  ...
  public search(query: QueryProps): HighlightedTextProps[] {
    ...
    this.updateSearchResults(finalQuery, hashParts, result, isVisited);
    if (isExact && result.length < query.maxResults) {
      const nonExactParts = allQueryHashParts(finalQuery, false);
      this.updateSearchResults(finalQuery, nonExactParts, result, isVisited);
    }
    result.sort((a, b) => {
