    ...
    const getManifest = useCallback(
        (name: string, webhookURL?: string): string =>
            JSON.stringify({
                ...
                redirect_url: new URL('/.api/githubapp/redirect', originURL).href,
                setup_url: new URL('/.api/githubapp/setup', originURL).href,
    ...
    const createState = useCallback(async () => {
        ...
        try {
            const response = await fetch(
                `/.api/githubapp/new-app-state?appName=${name}&webhookURN=${url}&domain=${appDomain}&baseURL=${url}`
            )
            if (!response.ok) {
