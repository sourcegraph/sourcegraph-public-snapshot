...
func TestSearch(t *testing.T) {
	...
	localstore.Mocks.Repos.GetByURI = func(ctx context.Context, repo string) (*sourcegraph.Repo, error) {
		return &sourcegraph.Repo{URI: repo, ID: repoID}, nil
	}

	calledDefsSearch := false
	localstore.Mocks.Defs.Search = func(ctx context.Context, op localstore.DefSearchOp) (*sourcegraph.SearchResultsList, error) {
