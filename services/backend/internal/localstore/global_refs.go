...
func (g *globalRefs) Update(ctx context.Context, op store.RefreshIndexOp) error {
	...
	// We exclude the beta here, because we wouldn't want all repositories to
	// have their data generated by universe (which would affect ALL users, not
	// just beta ones).
	if feature.Features.NoSrclib && universe.EnabledExcludingBeta(repo) {
		start = time.Now()
		...
		if universe.Shadow(repo) {
			go func() {
				shadowStart := time.Now()
				shadowRefs, shadowErr := lpAllRefs(ctx, repo, commitID)
				observe("langp", shadowStart)
				if shadowErr != nil {
					log15.Debug("universe globalRefs.Update failed", "repo", repo, "commitID", op.CommitID, "err", shadowErr)
					return
				}
				log15.Debug("universe globalRefs.Update success", "repo", repo, "commitID", op.CommitID, "count", len(shadowRefs))
			}()
		}
