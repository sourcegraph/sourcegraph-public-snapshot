...
func NewHandler(m *mux.Router) http.Handler {
	...
	m.Get(apirouter.DefAuthors).Handler(httptrace.TraceRoute(handler(serveDefAuthors)))
	m.Get(apirouter.DefRefs).Handler(httptrace.TraceRoute(handler(serveDefRefs)))
	m.Get(apirouter.DefRefLocations).Handler(httptrace.TraceRoute(handler(serveDefRefLocations)))
	...
	m.Get(apirouter.RepoTreeSearch).Handler(httptrace.TraceRoute(handler(serveRepoTreeSearch)))
	m.Get(apirouter.RepoBuild).Handler(httptrace.TraceRoute(handler(serveRepoBuild)))
	m.Get(apirouter.RepoBuilds).Handler(httptrace.TraceRoute(handler(serveRepoBuilds)))
	m.Get(apirouter.RepoBuildTasks).Handler(httptrace.TraceRoute(handler(serveBuildTasks)))
	m.Get(apirouter.RepoBuildsCreate).Handler(httptrace.TraceRoute(handler(serveRepoBuildsCreate)))
