...
PRIVATE_HASH := c2ce51aa3af9391a87fa8e4b0f4e10223e29c43d
...
serve-dev: serve-dep
	...
	DEBUG=t SG_USE_WEBPACK_DEV_SERVER=$(SG_USE_WEBPACK_DEV_SERVER) $(GODEP) rego -tags="$(GOTAGS)" -p sourcegraph.com/sourcegraph/srclib sourcegraph.com/sourcegraph/sourcegraph/cmd/src $(SRCFLAGS) serve --reload $(SERVEFLAGS) # -n 0
...
serve-test-ui: serve-dep
	...
	@echo
	@# specify any Godeps-vendored pkg in -p to work around the issue where stale vendored pkgs are not rebuilt (see https://github.com/tools/godep/issues/45#issuecomment-73411554)
	DEBUG=t SG_USE_WEBPACK_DEV_SERVER=$(SG_USE_WEBPACK_DEV_SERVER) $(GODEP) rego -p sourcegraph.com/sourcegraph/srclib sourcegraph.com/sourcegraph/sourcegraph/cmd/src serve --reload $(SERVEFLAGS) --test-ui -n 0
...
serve-dep:
	...
	@[ $(SG_USE_WEBPACK_DEV_SERVER) = t ] && curl -Ss -o /dev/null http://localhost:8080 || (echo "The webpack devserver needs to be running. In another terminal run:\n\n  make dep\n  cd app\n  npm start\n" 1>&2; exit 1)
...
check: generate-dep
	cd app && ./node_modules/.bin/eslint ./*.js script
	cd app && ./node_modules/.bin/lintspaces -t -n -d tabs ./style/*.scss ./style/**/*.scss ./templates/*.html ./templates/**/*.html
	GOBIN=Godeps/_workspace/bin $(GODEP) go install sourcegraph.com/sourcegraph/go-template-lint && Godeps/_workspace/bin/go-template-lint -f app/tmpl_funcs.go,app/internal/godocsupport/template.go -t app/internal/tmpl/tmpl.go -td app/templates
