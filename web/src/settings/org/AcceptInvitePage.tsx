...
export const AcceptInvitePage = reactive<Props>(props => {
    ...
    const tokenPayload: Observable<TokenPayload> = inviteToken.map(token =>
        JSON.parse(Base64.decode(token.split('.')[1]))
    )

    return (
        Observable.merge<Update>(
            // Any update to these should cause a rerender
            tokenPayload.map(payload => (state: State): State => ({
                ...state,
            ...
            submitEvents
                ...
                .mergeMap(([, inviteToken, tokenPayload]) =>
                    // Show loader
                    Observable.of<Update>(state => ({ ...state, loading: true, email: tokenPayload.email })).concat(
                        acceptUserInvite({ inviteToken })
                            ...
                            .catch(error => {
                                console.error(error)
                                return [
                                    (state: State): State => ({ ...state, hasSubmitted: true, loading: false, error }),
                                ]
                            })
                    )
                )
