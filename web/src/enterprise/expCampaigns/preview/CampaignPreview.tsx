        ...
        <div className={`campaign-preview ${className}`}>
            <h2 className="d-flex align-items-center">
                ...
                {status.messages &&
                    status.messages.map((m, i) => (
                        <span key={i} className="text-muted mx-2">
                            {m}
                        </span>
                    ))}
            </h2>
            {status.errors &&
                status.errors.map((e, i) => (
                    <div key={i} className="alert alert-danger mb-3">
                        {e}
                    </div>
                ))}
                    ...
                    <div style={isLoading ? { opacity: 0.5, cursor: 'wait' } : undefined}>
                        ...
                        campaignPreview.diagnostics.nodes.length === 0 ? (
                            ...
                            <>
                                ...
                                {campaignPreview.sideEffects.totalCount > 0 && (
                                    <>
                                        <a id="side-effects" />
                                        <SideEffectList
                                            {...props}
                                            sideEffects={campaignPreview.sideEffects}
                                            className="mb-4"
                                        />
                                    </>
                                )}
