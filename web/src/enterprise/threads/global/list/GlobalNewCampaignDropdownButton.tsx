...
export const GlobalNewThreadDropdownButton: React.FunctionComponent<Props> = ({ className = '' }) => {
    ...
    return (
        <ButtonDropdown isOpen={isOpen} toggle={toggleIsOpen} className={className}>
            <DropdownToggle color="" className="btn btn-primary" caret={true}>
                <PlusBoxIcon className="icon-inline mr-2" /> New thread
            </DropdownToggle>
            <DropdownMenu>
                <DropdownItem header={true}>New thread in...</DropdownItem>
                ...
                {namespaces === undefined ? (
                    ...
                    namespaces.map(({ __typename, id, namespaceName, url }) => (
                        <Link key={id} className="dropdown-item" to={`${url}/threads/new`}>
