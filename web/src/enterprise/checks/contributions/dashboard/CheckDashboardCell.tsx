...
import {
  Changeset,
  computeChangesets,
  getChangesetExternalStatus
} from "../../__OLD/threads/detail/backend";
...
export const CheckDashboardCell: React.FunctionComponent<Props> = ({
  ...
  const [changesetsOrError, setChangesetsOrError] = useState<
    typeof LOADING | Changeset[] | ErrorLike
  >(LOADING);
  ...
  return changesetsOrError === LOADING ? (
    ...
    <div className={paddingClassName}>
      <AlertIcon
        className="icon-inline p-2 text-danger"
        data-tooltip={changesetsOrError.message}
      />
    </div>
  ) : (
          ...
          return (
            <Link
              ...
              className={classNames(
                ...
                {
                  "bg-success":
                    changeset.fileDiffs.length === 0 && status === "merged",
                  "bg-warning":
                    changeset.fileDiffs.length > 0 && status === "merged",
                  "bg-danger":
                    changeset.fileDiffs.length > 0 && status !== "merged"
              ...
              data-tooltip={`${
                status === "merged" ? "1 PR merged so far, " : ""
              }${changeset.fileDiffs.length} ${pluralize(
                "outstanding problem",
