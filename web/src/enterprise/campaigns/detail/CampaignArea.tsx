...
export interface MinimalChangeset extends GQL.IChangeset {}
...
export const CampaignArea: React.FunctionComponent<Props> = ({
    ...
    const campaign = useObservable(
        useMemo(
            () =>
                merge(of(undefined), _noSubject ? new Observable<void>() : campaignUpdates).pipe(
                    switchMap(() => _fetchCampaignById(campaignID)),
                    distinctUntilChanged((a, b) => isEqual(a, b))
                ),
            [_fetchCampaignById, _noSubject, campaignID, campaignUpdates]
        )
    )
                ...
                <Route path={`${match.url}/edit`}>
                    <div className="container">
                        <UpdateCampaignPage
                            {...props}
                            campaign={campaign}
                            campaignSpecID={new URLSearchParams(location.search).get('spec')}
                        />
                    </div>
                </Route>
                {/* TODO(sqs): have /edit and /apply pages */}
                <Route
                    path={match.url}
