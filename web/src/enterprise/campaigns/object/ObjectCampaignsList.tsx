 ...
 */
export const ObjectCampaignsList: React.FunctionComponent<Props> = ({ object }) => {
    ...
    return campaigns === LOADING ? (
        ...
        <ul className="list-unstyled">
            {campaigns.nodes.map(campaign => (
                <li key={campaign.id} className="text-truncate">
                    <Link to={campaign.url} className="d-flex align-items-center">
                        <CampaignsIcon className="icon-inline small mr-1" /> {campaign.name}
                    </Link>
                </li>
            ))}
        </ul>
