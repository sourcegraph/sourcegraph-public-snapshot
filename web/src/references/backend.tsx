    ...
    (ctx: AbsoluteRepoFilePosition): Observable<GQL.IDependencyReferences | null> => {
        ...
        return queryGraphQL(
            ...
            if (
                ...
                !result.data.root.repository.commit.commit.file.dependencyReferences.dependencyReferenceData.references
                    .length
            ) {
                return null
            }
...
export const fetchExternalReferences = (ctx: AbsoluteRepoFilePosition): Observable<Location[]> =>
    ...
    Observable.fromPromise(fetchXdefinition(ctx)).mergeMap(defInfo => {
        if (!defInfo) {
            return []
        }
            ...
            .mergeMap(dependents => {
                ...
                return Observable.from(dependents)
                    ...
                    .concatMap(batch => {
                        ...
                        return Observable.from(batch).mergeMap(dependent => {
