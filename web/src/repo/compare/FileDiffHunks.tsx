...
const DiffBoundary: React.SFC<{
    ...
    <tr className="diff-boundary">
        <td className={`diff-boundary__num ${props.lineNumberClassName}`} colSpan={2} />
...
const DiffHunk: React.SFC<{ hunk: GQL.IFileDiffHunk }> = ({ hunk }) => {
    let oldLine = hunk.oldRange.startLine
    let newLine = hunk.newRange.startLine
    return (
        <>
            <DiffBoundary {...hunk} lineNumberClassName="diff-hunk__num--both" contentClassName="diff-hunk__content" />
            {hunk.body
                .split('\n')
                .slice(0, -1)
                .map((line, i) => (
        ...
        return (
            <div className={`file-diff-hunks ${this.props.className}`}>
                <table>
                    ...
                    <tbody>{this.props.hunks.map((hunk, i) => <DiffHunk key={i} hunk={hunk} />)}</tbody>
                </table>
            </div>
        )
    }
