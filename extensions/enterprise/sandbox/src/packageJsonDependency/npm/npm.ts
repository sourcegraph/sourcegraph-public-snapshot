...
export const npmPackageManager: PackageJsonPackageManager = {
    ...
    editForDependencyUpgrade: async (pkg, dep) => {
        ...
        const result = await npmExecClient({
            commands: [['npm', 'upgrade', '--', `${dep.name}@${dep.version}`]],
            context: {
...
function getPackageLockDependencyPlain(packageLock: string, packageName: string): { version: string } | null {
    const o: { dependencies: { [packageName: string]: { version?: string } } } = JSON.parse(packageLock)
    if (o && o.dependencies && o.dependencies[packageName] && typeof o.dependencies[packageName].version === 'string') {
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        return { version: o.dependencies[packageName].version! }
    }
    return null
}
