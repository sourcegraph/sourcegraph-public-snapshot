...
var HistoryService = (function (_super) {
    ...
    HistoryService.prototype.onEditorClosed = function (event) {
        ...
        if (event.pinned) {
            var editor_2 = this.restoreInput(event.editor); // closed editors are always disposed so we need to restore
            if (editor_2) {
                // Remove all inputs matching and add as last recently closed
                this.removeFromRecentlyClosed(editor_2);
                this.recentlyClosed.push({ editor: editor_2, index: event.index });
                // Bounding
                if (this.recentlyClosed.length > HistoryService.MAX_RECENTLY_CLOSED_EDITORS) {
                    this.recentlyClosed.shift();
                }
                // Restore on dispose
                var onceDispose = event_1.once(editor_2.onDispose);
                onceDispose(function () {
                    _this.restoreInRecentlyClosed(editor_2);
                });
            }
        }
    };
    HistoryService.prototype.popLastClosedEditor = function () {
        this.ensureLoaded();
    ...
    HistoryService.prototype.navigate = function () {
        ...
        this.editorService.openEditor(state.input, options).done(function () {
    ...
    HistoryService.prototype.handleEditorEventInHistory = function (editor) {
        ...
        onceDispose(function () {
            _this.restoreInHistory(input);
        });
    };
    HistoryService.prototype.restoreInHistory = function (input) {
        var index = this.indexOf(input);
        if (index < 0) {
            return;
        }
        // Using the factory we try to recreate the input
        var restoredInput = this.restoreInput(input);
        if (restoredInput) {
    ...
    HistoryService.prototype.remove = function (input) {
        ...
        setTimeout(function () { return _this.removeFromRecentlyClosed(input); }); // race condition with editor close and dispose
    };
    ...
    HistoryService.prototype.restoreInStack = function (input) {
        var _this = this;
        var restoredInput;
        var restored = false;
        this.stack.forEach(function (e, i) {
            if (e.input.matches(input)) {
                if (!restored) {
                    restoredInput = _this.restoreInput(input);
                    restored = true;
                }
                if (restoredInput) {
                    _this.stack[i].input = restoredInput;
                }
                else {
    ...
    HistoryService.prototype.restoreInRecentlyClosed = function (input) {
        var _this = this;
        var restoredInput;
        var restored = false;
        this.recentlyClosed.forEach(function (e, i) {
            if (e.editor.matches(input)) {
                ...
                if (restoredInput) {
                    _this.recentlyClosed[i].editor = restoredInput;
                }
                else {
                    _this.stack.splice(i, 1);
                }
    ...
    HistoryService.prototype.restoreInput = function (input) {
        if (input instanceof editor_1.EditorInput) {
            var factory = this.registry.getEditorInputFactory(input.getTypeId());
            if (factory) {
                var inputRaw = factory.serialize(input);
                if (inputRaw) {
    ...
    HistoryService.prototype.removeFromRecentlyClosed = function (input) {
        var _this = this;
        this.recentlyClosed.forEach(function (e, i) {
            if (e.editor.matches(input)) {
                _this.recentlyClosed.splice(i, 1);
            }
        });
    };
    ...
    HistoryService.prototype.save = function () {
        ...
        var entries = this.history.map(function (input) {
            var factory = _this.registry.getEditorInputFactory(input.getTypeId());
            if (factory) {
    ...
    HistoryService.prototype.load = function () {
        ...
        this.history = entries.map(function (entry) {
            var factory = _this.registry.getEditorInputFactory(entry.id);
            if (factory && typeof entry.value === 'string') {
                return factory.deserialize(_this.instantiationService, entry.value);
            }
            return void 0;
