...
var sideBySideEditorControl_1 = require('vs/workbench/browser/parts/editor/sideBySideEditorControl');
...
var EditorPart = (function (_super) {
    ...
    function EditorPart(id, restoreFromStorage, messageService, telemetryService, storageService, partService, configurationService, instantiationService) {
        ...
        this.visibleEditors = [];
        this.editorOpenToken = arrays.fill(editor_2.POSITIONS.length, function () { return 0; });
        this.instantiatedEditors = arrays.fill(editor_2.POSITIONS.length, function () { return []; });
        this.mapEditorInstantiationPromiseToEditor = arrays.fill(editor_2.POSITIONS.length, function () { return Object.create(null); });
        this.pendingEditorInputsToClose = [];
        this.pendingEditorInputCloseTimeout = null;
        this.stacks = this.instantiationService.createInstance(editorStacksModel_1.EditorStacksModel, restoreFromStorage);
        var editorConfig = configurationService.getConfiguration().workbench.editor;
    ...
    EditorPart.prototype.onConfigurationUpdated = function (configuration) {
        var _this = this;
        var newPreviewEditors = configuration.workbench.editor.enablePreview;
    ...
    EditorPart.prototype.openEditor = function (input, options, arg3, widthRatios) {
        ...
        var position = this.findPosition(input, options, arg3, widthRatios);
        ...
        var event = new editor_1.EditorInputEvent(input);
    ...
    EditorPart.prototype.doOpenEditor = function (position, descriptor, input, options, widthRatios) {
        var _this = this;
        ...
        var monitor = new ProgressMonitor(editorOpenToken, winjs_base_1.TPromise.timeout(this.partService.isCreated() ? 800 : 3200 /* less ugly initial startup */).then(function () {
            ...
            if (editorOpenToken === _this.editorOpenToken[position]) {
                _this.sideBySideControl.updateProgress(position, sideBySideEditorControl_1.ProgressState.INFINITE);
            }
        }));
        // Show editor
        return this.doShowEditor(group, descriptor, input, options, widthRatios, monitor).then(function (editor) {
            if (!editor) {
    ...
    EditorPart.prototype.doShowEditor = function (group, descriptor, input, options, widthRatios, monitor) {
        var _this = this;
        ...
        return this.doCreateEditor(group, descriptor, monitor).then(function (editor) {
            ...
            _this.sideBySideControl.show(editor, position, options && options.preserveFocus, widthRatios);
    ...
    EditorPart.prototype.doInstantiateEditor = function (group, descriptor) {
        ...
        var progressService = this.instantiationService.createInstance(progressService_1.WorkbenchProgressService, this.sideBySideControl.getProgressBar(position), descriptor.getId(), true);
        var editorInstantiationService = this.sideBySideControl.getInstantiationService(position).createChild(new serviceCollection_1.ServiceCollection([progress_1.IProgressService, progressService]));
        var loaded = false;
    ...
    EditorPart.prototype.doSetInput = function (group, editor, input, options, monitor) {
        ...
        return editor.setInput(input, options).then(function () {
            ...
            // Progress Done
            _this.sideBySideControl.updateProgress(position, sideBySideEditorControl_1.ProgressState.DONE);
    ...
    EditorPart.prototype.doHandleSetInputError = function (e, group, editor, input, options, monitor) {
        ...
        this.sideBySideControl.updateProgress(position, sideBySideEditorControl_1.ProgressState.DONE);
        // Event
    ...
    EditorPart.prototype.doHideEditor = function (position, layoutAndRochade) {
        ...
        var rochade = this.sideBySideControl.hide(editor, position, layoutAndRochade);
        // Clear any running Progress
        this.sideBySideControl.updateProgress(position, sideBySideEditorControl_1.ProgressState.STOP);
        // Indicate to Editor
        editor.clearInput();
        ...
        if (rochade !== sideBySideEditorControl_1.Rochade.NONE) {
    ...
    EditorPart.prototype.countEditors = function (editor, includeDiffEditors) {
        var _this = this;
        var editors = [editor];
        if (includeDiffEditors && editor instanceof diffEditorInput_1.DiffEditorInput) {
    ...
    EditorPart.prototype.doMoveEditorAcrossGroups = function (input, fromGroup, to, index) {
        ...
        if (activeEditor instanceof textEditor_1.BaseTextEditor && activeEditor.position === this.stacks.positionOfGroup(fromGroup) && input.matches(activeEditor.input)) {
    ...
    EditorPart.prototype.createContentArea = function (parent) {
        ...
        this.toUnbind.push(this.sideBySideControl.onGroupFocusChanged(function () { return _this.onGroupFocusChanged(); }));
        // get settings
    ...
    EditorPart.prototype.doOpenEditors = function (editors, activePosition, widthRatios) {
        var _this = this;
        var leftEditors = editors.filter(function (e) { return e.position === editor_2.Position.LEFT; });
        var centerEditors = editors.filter(function (e) { return e.position === editor_2.Position.CENTER; });
        var rightEditors = editors.filter(function (e) { return e.position === editor_2.Position.RIGHT; });
        var leftGroup = this.stacks.groupAt(editor_2.Position.LEFT);
        var centerGroup = this.stacks.groupAt(editor_2.Position.CENTER);
        var rightGroup = this.stacks.groupAt(editor_2.Position.RIGHT);
        // Compute the imaginary count if we const all editors open as the way requested
        var leftCount = leftEditors.length + (leftGroup ? leftGroup.count : 0);
        var centerCount = centerEditors.length + (centerGroup ? centerGroup.count : 0);
        var rightCount = rightEditors.length + (rightGroup ? rightGroup.count : 0);
        ...
        if ((!leftCount && (centerCount || rightCount) || (!centerCount && rightCount))) {
            leftEditors.push.apply(leftEditors, centerEditors);
            leftEditors.push.apply(leftEditors, rightEditors);
            centerEditors.splice(0, centerEditors.length);
            rightEditors.splice(0, rightEditors.length);
        }
        // Validate active input
        if (typeof activePosition !== 'number') {
            activePosition = editor_2.Position.LEFT;
        }
        // Validate width ratios
        var positions = rightEditors.length ? 3 : centerEditors.length ? 2 : 1;
        ...
        var promises = [];
        [leftEditors.shift(), centerEditors.shift(), rightEditors.shift()].forEach(function (editor, position) {
            if (!editor) {
            ...
            promises.push(_this.openEditor(input, options, position, widthRatios));
        });
        return winjs_base_1.TPromise.join(promises).then(function (editors) {
            ...
            [leftEditors, centerEditors, rightEditors].forEach(function (editors, index) {
    ...
    EditorPart.prototype.shutdown = function () {
        // Persist UI State
        var editorState = { widthRatio: this.sideBySideControl.getWidthRatios() };
    ...
    EditorPart.prototype.findPosition = function (input, options, arg1, widthRatios) {
    ...
    EditorPart.prototype.startDelayedCloseEditorsFromInputDispose = function () {
        ...
        if (this.pendingEditorInputCloseTimeout === null) {
            this.pendingEditorInputCloseTimeout = setTimeout(function () {
                ...
                visibleEditors
                    .sort(function (a1, a2) { return _this.stacks.positionOfGroup(a2.group) - _this.stacks.positionOfGroup(a1.group); }) // reduce layout work by starting right first
    ...
    EditorPart.prototype.renameGroups = function () {
        ...
        if (groups.length > 0) {
            // LEFT | CENTER | RIGHT
            if (groups.length > 2) {
                this.stacks.renameGroup(this.stacks.groupAt(editor_2.Position.LEFT), EditorPart.GROUP_LEFT_LABEL);
                this.stacks.renameGroup(this.stacks.groupAt(editor_2.Position.CENTER), EditorPart.GROUP_CENTER_LABEL);
                this.stacks.renameGroup(this.stacks.groupAt(editor_2.Position.RIGHT), EditorPart.GROUP_RIGHT_LABEL);
            }
            else if (groups.length > 1) {
                this.stacks.renameGroup(this.stacks.groupAt(editor_2.Position.LEFT), EditorPart.GROUP_LEFT_LABEL);
                this.stacks.renameGroup(this.stacks.groupAt(editor_2.Position.CENTER), EditorPart.GROUP_RIGHT_LABEL);
            }
            else {
                this.stacks.renameGroup(this.stacks.groupAt(editor_2.Position.LEFT), EditorPart.GROUP_LEFT_LABEL);
            }
    ...
    EditorPart.GROUP_LEFT_LABEL = nls.localize('leftGroup', "Left");
    EditorPart.GROUP_CENTER_LABEL = nls.localize('centerGroup', "Center");
    EditorPart.GROUP_RIGHT_LABEL = nls.localize('rightGroup', "Right");
