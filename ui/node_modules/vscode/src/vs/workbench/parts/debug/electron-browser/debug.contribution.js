...
var OpenDebugPanelAction = (function (_super) {
    ...
    function OpenDebugPanelAction(id, label, panelService, partService) {
        _super.call(this, id, label, debug.REPL_ID, panelService, partService);
    }
...
registry.registerWorkbenchAction(new actions_1.SyncActionDescriptor(debugActions_1.StepBackAction, debugActions_1.StepBackAction.ID, debugActions_1.StepBackAction.LABEL, { primary: 16384 /* Shift */ | 68 /* F10 */ }, debug.CONTEXT_IN_DEBUG_MODE), 'Debug: Step Back', debugCategory);
...
keybindingsRegistry_1.KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    handler: function (accessor, configuration) {
        var debugService = accessor.get(debug.IDebugService);
        if (typeof configuration === 'string') {
            var configurationManager_1 = debugService.getConfigurationManager();
            return configurationManager_1.setConfiguration(configuration)
                .then(function () {
                return configurationManager_1.configuration ? debugService.createSession(false)
                    : winjs_base_1.TPromise.wrapError(new Error(nls.localize('launchConfigDoesNotExist', "Launch configuration '{0}' does not exist.", configuration)));
            });
        }
        var noDebug = configuration && !!configuration.noDebug;
