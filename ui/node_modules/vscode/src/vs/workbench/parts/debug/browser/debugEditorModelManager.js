...
var DebugEditorModelManager = (function () {
    ...
    DebugEditorModelManager.prototype.createCallStackDecorations = function (modelUrlStr) {
        ...
        focusedStackFrame.thread.getCallStack().filter(function (sf) { return sf.source.uri.toString() === modelUrlStr; }).forEach(function (sf) {
    ...
    DebugEditorModelManager.prototype.onModelDecorationsChanged = function (modelUrlStr, e) {
        var _this = this;
        var modelData = this.modelData[modelUrlStr];
        var myDecorationsCount = Object.keys(modelData.breakpointDecorationsAsMap).length;
        if (myDecorationsCount === 0) {
            // I have no decorations
            return;
        }
        if (!e.changedDecorations.some(function (decorationId) { return modelData.breakpointDecorationsAsMap.hasOwnProperty(decorationId); })) {
            // nothing to do, my decorations did not change.
            return;
        }
        var data = [];
        var lineToBreakpointDataMap = {};
        ...
        modelData.dirty = this.debugService.state !== debug_1.State.Inactive && this.debugService.state !== debug_1.State.Disabled;
    ...
    DebugEditorModelManager.prototype.getBreakpointDecorationOptions = function (breakpoint) {
        ...
        var modelData = this.modelData[breakpoint.uri.toString()];
        ...
        var mode = modelData ? modelData.model.getMode() : null;
        var modeId = mode ? mode.getId() : '';
        var condition;
        ...
        var glyphMarginHoverMessage = "```" + modeId + "\n" + condition + "```";
        return {
