    ...
    Thread.prototype.getCallStackImpl = function (debugService, startFrame) {
        ...
        return session.stackTrace({ threadId: this.threadId, startFrame: startFrame, levels: 20 }).then(function (response) {
            ...
            return response.body.stackFrames.map(function (rsf, level) {
                if (!rsf) {
                    return new StackFrame(_this.threadId, 0, new debugSource_1.Source({ name: UNKNOWN_SOURCE_LABEL }, false), nls.localize('unknownStack', "Unknown stack location"), undefined, undefined);
                }
                return new StackFrame(_this.threadId, rsf.id, rsf.source ? new debugSource_1.Source(rsf.source) : new debugSource_1.Source({ name: UNKNOWN_SOURCE_LABEL }, false), rsf.name, rsf.line, rsf.column);
            });
        }, function (err) {
...
var OutputElement = (function () {
    function OutputElement(id) {
        if (id === void 0) { id = OutputElement.ID_COUNTER++; }
        this.id = id;
        // noop
    }
...
var ExpressionContainer = (function () {
    function ExpressionContainer(reference, id, cacheChildren, namedVariables, indexedVariables, startOfVariables) {
    ...
    ExpressionContainer.prototype.getChildren = function (debugService) {
        ...
        if (!this.cacheChildren || !this.children) {
            var session_1 = debugService.getActiveSession();
            // only variables with reference > 0 have children.
            if (!session_1 || this.reference <= 0) {
                this.children = winjs_base_1.TPromise.as([]);
            }
            else {
                // Check if object has named variables, fetch them independent from indexed variables #9670
                this.children = (!!this.namedVariables ? this.fetchVariables(session_1, undefined, undefined, 'named') : winjs_base_1.TPromise.as([])).then(function (childrenArray) {
                    ...
                    if (_this.indexedVariables > chunkSize) {
                        ...
                        for (var i = 0; i < numberOfChunks; i++) {
                            var start_1 = _this.startOfVariables + i * chunkSize;
                            var count_1 = Math.min(chunkSize, _this.indexedVariables - i * chunkSize);
                            childrenArray.push(new Variable(_this, _this.reference, "[" + start_1 + ".." + (start_1 + count_1 - 1) + "]", '', null, count_1, null, true, start_1));
                        }
                        return childrenArray;
                    }
                    var start = _this.getChildrenInChunks ? _this.startOfVariables : undefined;
                    var count = _this.getChildrenInChunks ? _this.indexedVariables : undefined;
                    return _this.fetchVariables(session_1, start, count, 'indexed')
                        .then(function (variables) { return arrays.distinct(childrenArray.concat(variables), function (child) { return child.name; }); });
                });
            }
        }
        return this.children;
    ...
    ExpressionContainer.prototype.fetchVariables = function (session, start, count, filter) {
        var _this = this;
        return session.variables({
            ...
            return response && response.body && response.body.variables ? arrays.distinct(response.body.variables.filter(function (v) { return !!v; }), function (v) { return v.name; }).map(function (v) { return new Variable(_this, v.variablesReference, v.name, v.value, v.namedVariables, v.indexedVariables, v.type); }) : [];
        }, function (e) { return [new Variable(_this, 0, null, e.message, 0, 0, null, false)]; });
    };
...
var Expression = (function (_super) {
    ...
    function Expression(name, cacheChildren, id) {
        ...
        _super.call(this, 0, id, cacheChildren, 0, 0);
        this.name = name;
...
var Variable = (function (_super) {
    ...
    function Variable(parent, reference, name, value, namedVariables, indexedVariables, type, available, startOfVariables) {
        if (type === void 0) { type = null; }
        if (available === void 0) { available = true; }
        if (startOfVariables === void 0) { startOfVariables = 0; }
        _super.call(this, reference, "variable:" + parent.getId() + ":" + name + ":" + reference, true, namedVariables, indexedVariables, startOfVariables);
        this.parent = parent;
        this.name = name;
        this.type = type;
...
var Scope = (function (_super) {
    ...
    function Scope(threadId, name, reference, expensive, namedVariables, indexedVariables) {
        _super.call(this, reference, "scope:" + threadId + ":" + name + ":" + reference, true, namedVariables, indexedVariables);
        this.name = name;
        ...
        if (!this.scopes) {
            this.scopes = debugService.getActiveSession().scopes({ frameId: this.frameId }).then(function (response) {
                return response && response.body && response.body.scopes ?
                    response.body.scopes.map(function (rs) { return new Scope(_this.threadId, rs.name, rs.variablesReference, rs.expensive, rs.namedVariables, rs.indexedVariables); }) : [];
            }, function (err) { return []; });
        }
        return this.scopes;
    };
...
var Breakpoint = (function () {
    function Breakpoint(source, desiredLineNumber, enabled, condition, hitCondition) {
        this.source = source;
...
var Model = (function () {
    ...
    Model.prototype.addBreakpoints = function (rawData) {
        ...
        this.breakpoints = this.breakpoints.concat(rawData.map(function (rawBp) {
            return new Breakpoint(new debugSource_1.Source(debugSource_1.Source.toRawSource(rawBp.uri, _this)), rawBp.lineNumber, rawBp.enabled, rawBp.condition, rawBp.hitCondition);
        }));
        ...
        if (data.stoppedDetails) {
            ...
            if (data.allThreadsStopped) {
                Object.keys(this.threads).forEach(function (ref) {
                    ...
                    _this.threads[ref].stoppedDetails = objects.clone(data.stoppedDetails);
