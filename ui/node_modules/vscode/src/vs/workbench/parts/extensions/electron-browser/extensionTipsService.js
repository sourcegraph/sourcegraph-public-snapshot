...
var ExtensionTipsService = (function () {
    function ExtensionTipsService(_galleryService, _modelService, storageService, choiceService, extensionsService, instantiationService, configurationService) {
    ...
    ExtensionTipsService.prototype.getWorkspaceRecommendations = function () {
        var configuration = this.configurationService.getConfiguration(extensions_1.ConfigurationKey);
        if (configuration.recommendations) {
            var regEx_1 = new RegExp(extensionManagement_1.EXTENSION_IDENTIFIER_PATTERN);
            return configuration.recommendations.filter(function (element, position) {
                return configuration.recommendations.indexOf(element) === position && regEx_1.test(element);
            });
        }
        ...
        setImmediate(function () {
            ...
            _this.extensionsService.getInstalled(extensionManagement_1.LocalExtensionType.User).done(function (local) {
                Object.keys(_this.importantRecommendations)
                    ...
                    .forEach(function (id) {
                    var pattern = _this.importantRecommendations[id];
                    if (!glob_1.match(pattern, uri.fsPath)) {
                        return;
                    }
                    var message = nls_1.localize('reallyRecommended', "It is recommended to install the '{0}' extension.", id);
    ...
    ExtensionTipsService.prototype._suggestWorkspaceRecommendations = function () {
        ...
        var allRecommendations = this.getWorkspaceRecommendations();
        if (!allRecommendations.length) {
            return;
        }
        this.extensionsService.getInstalled(extensionManagement_1.LocalExtensionType.User).done(function (local) {
