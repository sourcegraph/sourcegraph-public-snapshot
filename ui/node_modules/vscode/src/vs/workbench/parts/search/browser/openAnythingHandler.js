...
var openFileHandler_1 = require('vs/workbench/parts/search/browser/openFileHandler');
/* tslint:disable:no-unused-variable */
var openSymbolHandler = require('vs/workbench/parts/search/browser/openSymbolHandler');
...
var OpenAnythingHandler = (function (_super) {
    ...
    function OpenAnythingHandler(messageService, instantiationService, configurationService, telemetryService) {
        ...
        this.searchDelayer = new async_1.ThrottledDelayer(OpenAnythingHandler.FILE_SEARCH_DELAY);
        ...
        this.updateHandlers(this.configurationService.getConfiguration());
        this.registerListeners();
    }
    OpenAnythingHandler.prototype.registerListeners = function () {
        var _this = this;
        this.configurationService.onDidUpdateConfiguration(function (e) { return _this.updateHandlers(e.config); });
    };
    ...
    OpenAnythingHandler.prototype.getResults = function (searchValue) {
        ...
        var promiseFactory = function () {
            var resultPromises = [];
            // File Results
            resultPromises.push(_this.openFileHandler.getResults(searchValue, OpenAnythingHandler.MAX_DISPLAYED_RESULTS));
            // Symbol Results (unless disabled or a range or absolute path is specified)
            if (_this.includeSymbols && !searchWithRange && !paths.isAbsolute(searchValue)) {
            ...
            _this.pendingSearch = winjs_base_1.TPromise.join(resultPromises).then(function (results) {
                ...
                var mergedResults = results[0].entries.concat(results[1].entries);
                // Sort
                ...
                var fileSearchStats;
                if (results[0] instanceof openFileHandler_1.FileQuickOpenModel) {
                    fileSearchStats = results[0].stats;
                }
                else if (results[1] instanceof openFileHandler_1.FileQuickOpenModel) {
                    fileSearchStats = results[1].stats;
                }
