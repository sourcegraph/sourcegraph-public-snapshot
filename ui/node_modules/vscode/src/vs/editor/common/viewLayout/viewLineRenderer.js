...
var RenderLineInput = (function () {
    function RenderLineInput(lineContent, tabSize, spaceWidth, stopRenderingLineAfter, renderWhitespace, renderControlCharacters, lineParts) {
...
var RenderLineOutput = (function () {
    function RenderLineOutput(charOffsetInPart, lastRenderedPartIndex, output, isWhitespaceOnly) {
...
function renderLine(input) {
    ...
    var actualLineParts = input.lineParts.parts;
...
function isControlCharacter(characterCode) {
    return characterCode < 32;
}
var _controlCharacterSequenceConversionStart = 9216;
function controlCharacterToPrintable(characterCode) {
    return String.fromCharCode(_controlCharacterSequenceConversionStart + characterCode);
}
function renderLineActual(lineText, lineTextLength, tabSize, spaceWidth, actualLineParts, renderWhitespace, renderControlCharacters, charBreakIndex) {
    ...
    var tabsCharDelta = 0;
    var isWhitespaceOnly = /^\s*$/.test(lineText);
    out += '<span>';
    for (var partIndex = 0, partIndexLen = actualLineParts.length; partIndex < partIndexLen; partIndex++) {
        ...
        var parsRendersWhitespace = (renderWhitespace !== 'none' && isWhitespace(part.type));
        ...
        if (parsRendersWhitespace) {
            ...
            for (; charIndex < toCharIndex; charIndex++) {
                ...
                if (charIndex >= charBreakIndex) {
                    ...
                    charOffsetInPartArr[charIndex] = charOffsetInPart;
                    return new RenderLineOutput(charOffsetInPartArr, partIndex, out, isWhitespaceOnly);
                }
            }
            out += '<span class="token ' + part.type + '" style="width:' + (spaceWidth * partContentCnt) + 'px">';
            out += partContent;
    ...
    return new RenderLineOutput(charOffsetInPartArr, actualLineParts.length - 1, out, isWhitespaceOnly);
}
