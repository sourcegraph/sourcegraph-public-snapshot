...
var ShowSnippetsActions = (function (_super) {
    ...
    ShowSnippetsActions.prototype.run = function (accessor, editor) {
        ...
        var picks = [];
        platform_1.Registry.as(snippetsRegistry_1.Extensions.Snippets).visitSnippets(editor.getModel().getModeId(), function (snippet) {
            picks.push({
        ...
        return quickOpenService.pick(picks).then(function (pick) {
            if (pick) {
                snippetController_1.SnippetController.get(editor).run(snippet_1.CodeSnippet.fromTextmate(pick.snippet.codeSnippet), 0, 0);
            }
        });
    };
