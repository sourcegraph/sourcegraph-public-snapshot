...
import { registerEditorCallbacks, registerQuickopenListeners, setEditorDiffState, syncEditorWithRouterProps, toggleQuickopen as quickopen, updateWorkspace } from "sourcegraph/editor/config";
...
export class WorkbenchShell extends React.Component<Props, State> {
	...
	domRef(parent: HTMLDivElement): void {
		...
		updateWorkspace(this.props).then(() => {
			...
			syncEditorWithRouterProps(this.props).then(() => {
				this.layout();
			});
		});
