...
export class Editor implements IDisposable {
	...
	private _toggleAuthorshipWidget(authorWidget: IContentWidget, AuthorshipWidgetID: string, blameInfo: GQL.IHunk): void {
		const widget = (this._editor as any).contentWidgets[AuthorshipWidgetID];
		if (widget && widget.widget.blame === blameInfo) {
			this._removeWidgetForID(AuthorshipWidgetID);
		} else {
			this._removeWidgetForID(AuthorshipWidgetID);
