...
function fillInActions(menu, context, target, isPrimaryGroup) {
    if (isPrimaryGroup === void 0) { isPrimaryGroup = function (group) { return group === 'navigation'; }; }
    var groups = menu.getActions(context);
    if (groups.length === 0) {
        return;
    }
...
var MenuItemActionItem = (function (_super) {
    ...
    MenuItemActionItem.prototype.onClick = function (event) {
        ...
        this._command.run().done(undefined, function (err) { return _this._messageService.show(severity_1.default.Error, err); });
    };
    ...
    MenuItemActionItem.prototype._updateTooltip = function () {
        ...
        var keybinding = this._keybindingService.lookupKeybinding(this._command.id);
        var keybindingLabel = keybinding && keybinding.getLabel();
        element.title = keybindingLabel
            ? nls_1.localize('titleAndKb', "{0} ({1})", this._command.label, keybindingLabel)
            : this._command.label;
    };
