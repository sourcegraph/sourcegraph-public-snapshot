...
(function (schema) {
    ...
    function parseMenuId(value) {
        switch (value) {
            ...
            case 'scm/resource/context': return actions_1.MenuId.SCMResourceContext;
        }
        return void 0;
    }
    ...
    function isValidMenuItems(menu, collector) {
        if (!Array.isArray(menu)) {
            collector.error(nls_1.localize('requirearry', "menu items must be an arry"));
            return false;
        }
    ...
    function isValidCommand(command, collector) {
        ...
        if (strings_1.isFalsyOrWhitespace(command.title)) {
        ...
        if (command.category && typeof command.category !== 'string') {
            collector.error(nls_1.localize('optstring', "property `{0}` can be omitted or must be of type `string`", 'category'));
            return false;
        }
...
extensionsRegistry_1.ExtensionsRegistry.registerExtensionPoint('commands', [], schema.commandsContribution).setHandler(function (extensions) {
    var ids = new idGenerator_1.IdGenerator('contrib-cmd-icon-');
    function handleCommand(userFriendlyCommand, extension) {
        ...
        if (icon) {
            ...
            if (typeof icon === 'string') {
                var path = paths_1.join(extension.description.extensionFolderPath, icon);
                dom_1.createCSSRule(".icon." + iconClass, "background-image: url(\"" + path + "\")");
            }
            else {
                var light = paths_1.join(extension.description.extensionFolderPath, icon.light);
                var dark = paths_1.join(extension.description.extensionFolderPath, icon.dark);
                dom_1.createCSSRule(".icon." + iconClass, "background-image: url(\"" + light + "\")");
                dom_1.createCSSRule(".vs-dark .icon." + iconClass + ", hc-black .icon." + iconClass, "background-image: url(\"" + dark + "\")");
            }
        }
        if (actions_1.MenuRegistry.addCommand({ id: command, title: title, category: category, iconClass: iconClass })) {
    ...
    var _loop_1 = function (extension) {
        ...
        collections_1.forEach(value, function (entry) {
            ...
            for (var _i = 0, _a = entry.value; _i < _a.length; _i++) {
                ...
                if (!command) {
                    collector.warn(nls_1.localize('missing.command', "Menu item references a command `{0}` which is not defined in the 'commands' section.", item.command));
                }
                if (item.alt && !alt) {
                ...
                if (item.alt && menu !== actions_1.MenuId.EditorTitle && item.group !== 'navigation') {
                    collector.info(nls_1.localize('nosupport.altCommand', "Sorry, but currently only the 'navigation' group of the 'editor/title' menu supports alt-commands"));
                }
                var group = void 0;
                var order = void 0;
                if (item.group) {
