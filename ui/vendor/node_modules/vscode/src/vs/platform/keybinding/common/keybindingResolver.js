...
var NormalizedKeybindingItem = (function () {
    ...
    NormalizedKeybindingItem.fromKeybindingItem = function (source, isDefault) {
        ...
        if (source.keybinding !== 0) {
            keybinding = keyCodes_1.createKeybinding(source.keybinding);
        }
...
var KeybindingResolver = (function () {
    function KeybindingResolver(defaultKeybindings, overrides, shouldWarnOnConflict) {
        ...
        for (var i = 0, len = allKeybindings.length; i < len; i++) {
            ...
            if (k.keybinding.isChord()) {
                // This is a chord
                var keybindingFirstPart = k.keybinding.extractFirstPart().value;
                var keybindingChordPart = k.keybinding.extractChordPart().value;
        ...
        for (var i = conflicts.length - 1; i >= 0; i--) {
            ...
            if (KeybindingResolver.whenIsEntirelyIncluded(true, conflict.when, item.when)) {
                ...
                if (this._shouldWarnOnConflict && item.isDefault) {
                    console.warn('Conflict detected, command `' + conflict.commandId + '` cannot be triggered by ' + keybindingLabels_1.KeybindingLabels.toUserSettingsLabel(conflict.keybinding) + ' due to ' + item.command);
                }
    ...
    KeybindingResolver.prototype.lookupKeybinding = function (commandId) {
        ...
        return result.map(function (trigger) {
            return keyCodes_1.createKeybinding(trigger);
    ...
    KeybindingResolver.prototype.resolve = function (context, currentChord, keypress) {
        ...
        if (currentChord !== null) {
            var chords = this._chords[currentChord.value];
            if (!chords) {
                return null;
            }
...
var IOSupport = (function () {
    ...
    IOSupport.readKeybindingItem = function (input, index) {
        ...
        if (typeof input.when === 'string') {
            when = contextkey_1.ContextKeyExpr.deserialize(input.when);
        }
        var command = null;
    ...
    IOSupport.writeKeybinding = function (keybinding, Platform) {
        if (Platform === void 0) { Platform = platform; }
        return keybindingLabels_1.KeybindingLabels.toUserSettingsLabel(keybinding, Platform);
    };
    IOSupport.readKeybinding = function (input, Platform) {
        if (Platform === void 0) { Platform = platform; }
        return keybindingLabels_1.KeybindingLabels.fromUserSettingsLabel(input, Platform);
    };
    return IOSupport;
}());
