...
var EditorWorkerServiceImpl = (function () {
    function EditorWorkerServiceImpl(modelService, configurationService) {
        ...
        var completionProvider = modes.SuggestRegistry.register('*', new WordBasedCompletionItemProvider(this._workerManager, configurationService));
...
var WordBasedCompletionItemProvider = (function () {
    function WordBasedCompletionItemProvider(workerManager, configurationService) {
        this._workerManager = workerManager;
        this._configurationService = configurationService;
    }
    WordBasedCompletionItemProvider.prototype.provideCompletionItems = function (model, position) {
        var wordBasedSuggestions = this._configurationService.getConfiguration('editor').wordBasedSuggestions;
        if (wordBasedSuggestions === false) {
            ...
            var tokens = model.getLineTokens(position.lineNumber);
            var tokenType = tokens.findTokenAtOffset(position.column - 1).tokenType;
            var shoudSuggestHere = (tokenType === 1 /* Comment */ && wordBasedSuggestions.comments)
                || (tokenType === 2 /* String */ && wordBasedSuggestions.strings)
                || (tokenType === 0 /* Other */ && wordBasedSuggestions.default);
            if (shoudSuggestHere) {
                return this._workerManager.withWorker().then(function (client) { return client.textualSuggest(model.uri, position); });
            }
            else {
