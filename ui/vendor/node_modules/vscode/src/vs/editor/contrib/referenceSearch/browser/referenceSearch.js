...
var ReferenceAction = (function (_super) {
    ...
    function ReferenceAction() {
        return _super.call(this, {
            ...
            precondition: contextkey_1.ContextKeyExpr.and(ModeContextKeys.hasReferenceProvider, peekViewWidget_1.PeekContext.notInPeekEditor, ModeContextKeys.isInEmbeddedEditor.toNegated()),
            kbOpts: {
    ...
    ReferenceAction.prototype.run = function (accessor, editor) {
        ...
        var provideReferencesPromise;
        var references = new winjs_base_1.PPromise(function (complete, error, progress) {
            provideReferencesPromise = referenceSearch_1.provideReferences(model, range.getStartPosition(), progress).then(function (references) { return complete(new referencesModel_1.ReferencesModel(references)); }, error);
        }, function () {
...
var findReferencesCommand = function (accessor, resource, position) {
    ...
    return accessor.get(editor_1.IEditorService).openEditor({ resource: resource }).then(function (editor) {
        ...
        var provideReferencesPromise;
        var references = new winjs_base_1.PPromise(function (complete, error, progress) {
            provideReferencesPromise = referenceSearch_1.provideReferences(control.getModel(), position_1.Position.lift(position), progress).then(function (references) { return complete(new referencesModel_1.ReferencesModel(references)); }, error);
        }, function () {
            provideReferencesPromise.cancel();
        });
