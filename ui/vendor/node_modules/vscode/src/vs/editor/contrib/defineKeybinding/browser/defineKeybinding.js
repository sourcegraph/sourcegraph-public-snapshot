...
var DefineKeybindingController = DefineKeybindingController_1 = (function () {
    ...
    DefineKeybindingController.prototype._updateDecorationsNow = function () {
        var _this = this;
        var model = this._editor.getModel();
        var regex = keybindingLabels_1.KeybindingLabels.getUserSettingsKeybindingRegex();
        var m = model.findMatches(regex, false, true, false, false, false).map(function (m) { return m.range; });
        var data = m.map(function (range) {
            ...
            var keybinding = keyCodes_1.createKeybinding(numKeybinding);
            var resolvedKeybinding = _this._keybindingService.resolveKeybinding(keybinding);
            return {
                strKeybinding: strKeybinding,
                keybinding: keybinding,
                usLabel: keybindingLabels_1.KeybindingLabels._toUSLabel(keybinding),
...
var DefineKeybindingLauncherWidget = (function () {
    function DefineKeybindingLauncherWidget(editor, keybindingService, onLaunch) {
        ...
        var keybinding = keybindingService.lookupKeybinding(DefineKeybindingAction.ID);
        var extra = '';
        if (keybinding) {
...
var DefineKeybindingWidget = (function () {
    function DefineKeybindingWidget(editor, keybindingService, onAccepted) {
        ...
        this._toDispose.push(dom.addDisposableListener(this._inputNode, 'keydown', function (e) {
            ...
            switch (kb.value) {
                case 3 /* Enter */:
                    if (_this._lastKeybinding) {
                        _this._onAccepted(_this._lastKeybinding.getUserSettingsLabel());
                    }
                    _this._stop();
                    return;
                case 9 /* Escape */:
            ...
            _this._lastKeybinding = _this._keybindingService.resolveKeybinding(kb);
            _this._inputNode.value = _this._lastKeybinding.getUserSettingsLabel().toLowerCase();
            _this._inputNode.title = 'keyCode: ' + keyEvent.browserEvent.keyCode;
            dom.clearNode(_this._outputNode);
            var htmlkb = _this._lastKeybinding.getHTMLLabel();
