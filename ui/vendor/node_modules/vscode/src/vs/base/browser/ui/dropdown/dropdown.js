...
var BaseDropdown = (function (_super) {
    ...
    function BaseDropdown(container, options) {
        ...
        if (options.tick || options.action) {
            _this.$label.addClass('tick');
        }
        var labelRenderer = options.labelRenderer;
        if (!labelRenderer && options.action) {
            _this.$action = builder_1.$('.dropdown-action').appendTo(_this.$el);
            var item_1 = new actionbar_1.ActionItem(null, options.action, {
                icon: true,
                label: true
            });
            item_1.actionRunner = _this;
            item_1.render(_this.$action.getHTMLElement());
        ...
        _this.$label.on(['mousedown', touch_1.EventType.Tap], function (e) {
            e.preventDefault();
            e.stopPropagation();
            _this.show();
        }).appendTo(_this.$el);
        ...
        if (cleanupFn) {
            _this.toDispose.push(cleanupFn);
        }
        _this.toDispose.push(new touch_1.Gesture(_this.$label.getHTMLElement()));
        return _this;
    }
...
var DropdownGroup = (function (_super) {
    ...
    function DropdownGroup(container) {
        ...
        _this.el.className = 'dropdown-group';
