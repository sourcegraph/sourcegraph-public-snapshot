...
var toggleActivityBarVisibility_1 = require("vs/workbench/browser/actions/toggleActivityBarVisibility");
var ActivitybarPart = (function (_super) {
    __extends(ActivitybarPart, _super);
    function ActivitybarPart(id, viewletService, extensionService, storageService, contextMenuService, instantiationService, partService) {
        ...
        _this.viewletIdToActivity = Object.create(null);
        _this.memento = _this.getMemento(_this.storageService, memento_1.Scope.GLOBAL);
        _this.pinnedViewlets = _this.memento[ActivitybarPart.PINNED_VIEWLETS] || _this.viewletService.getViewlets().map(function (v) { return v.id; });
    ...
    ActivitybarPart.prototype.showActivity = function (viewletId, badge, clazz) {
        ...
        // Keep for future use
        if (badge) {
            this.viewletIdToActivity[viewletId] = { badge: badge, clazz: clazz };
        }
        else {
    ...
    ActivitybarPart.prototype.createContentArea = function (parent) {
        ...
        builder_1.$(parent).on(DOM.EventType.DROP, function (e) {
            var draggedViewlet = activitybarActions_1.ActivityActionItem.getDraggedViewlet();
            if (draggedViewlet) {
                DOM.EventHelper.stop(e, true);
                activitybarActions_1.ActivityActionItem.clearDraggedViewlet();
    ...
    ActivitybarPart.prototype.updateViewletSwitcher = function () {
        ...
        var overflows = false;
        if (this.dimension) {
            var maxVisible = Math.floor(this.dimension.height / ActivitybarPart.ACTIVITY_ACTION_HEIGHT);
        ...
        if (newViewletsToShow.length) {
            ...
            Object.keys(this.viewletIdToActions).forEach(function (viewletId) {
                var activity = _this.viewletIdToActivity[viewletId];
                if (activity) {
                    _this.showActivity(viewletId, activity.badge, activity.clazz);
                }
                else {
                    _this.showActivity(viewletId);
                }
            });
        }
    ...
    ActivitybarPart.prototype.toAction = function (viewlet) {
        ...
        this.viewletIdToActionItems[action.id] = this.instantiationService.createInstance(activitybarActions_1.ActivityActionItem, action, viewlet);
    ...
    ActivitybarPart.prototype.unpin = function (viewletId) {
        ...
        if (!activeViewlet || activeViewlet.getId() !== viewletId) {
            ...
            unpinPromise = winjs_base_1.TPromise.as(this.partService.setSideBarHidden(true));
        }
        else {
