...
var editorGroupsControl_1 = require("vs/workbench/browser/parts/editor/editorGroupsControl");
...
var EditorPart = (function (_super) {
    ...
    function EditorPart(id, restoreFromStorage, messageService, telemetryService, storageService, partService, configurationService, instantiationService) {
        ...
        if (config && config.workbench && config.workbench.editor) {
            ...
            _this.previewEditors = editorConfig.enablePreview;
    ...
    EditorPart.prototype.doOpenEditor = function (position, descriptor, input, options, ratio) {
        ...
        var pinned = !this.previewEditors || (options && (options.pinned || typeof options.index === 'number')) || input.isDirty();
    ...
    EditorPart.prototype.doShowEditor = function (group, descriptor, input, options, ratio, monitor) {
        ...
        // Create Editor
        var timerEvent = timer.start(timer.Topic.WORKBENCH, strings.format('Creating Editor: {0}', descriptor.getName()));
        return this.doCreateEditor(group, descriptor, monitor).then(function (editor) {
            ...
            timerEvent.stop();
            return editor;
        }, function (e) { return _this.messageService.show(message_1.Severity.Error, types.isString(e) ? new Error(e) : e); });
    };
    ...
    EditorPart.prototype.doInstantiateEditor = function (group, descriptor) {
        ...
        var onInstantiate = function (arg) {
            ...
            return winjs_base_1.TPromise.wrapError(arg);
        };
        var instantiateEditorPromise = editorInstantiationService.createInstance(descriptor).then(onInstantiate, onInstantiate);
        if (!loaded) {
    ...
    EditorPart.prototype.doSetInput = function (group, editor, input, options, monitor) {
        ...
        var previousInput = editor.getInput();
        ...
        return editor.setInput(input, options).then(function () {
            ...
            return editor;
        }, function (e) { return _this.doHandleSetInputError(e, group, editor, input, options, monitor); });
    };
    ...
    EditorPart.prototype.doHideEditor = function (position, layoutAndRochade) {
    ...
    EditorPart.prototype.closeEditors = function (position, except, direction) {
        var _this = this;
        var group = this.stacks.groupAt(position);
        if (!group) {
            return winjs_base_1.TPromise.as(null);
        ...
        if (types.isUndefinedOrNull(direction)) {
            editorsToClose = group.getEditors().filter(function (e) { return !except || !e.matches(except); });
        }
        else {
            editorsToClose = (direction === editor_2.Direction.LEFT) ? group.getEditors().slice(0, group.indexOf(except)) : group.getEditors().slice(group.indexOf(except) + 1);
        }
        return this.handleDirty(editorsToClose.map(function (editor) { return { group: group, editor: editor }; }), true /* ignore if opened in other group */).then(function (veto) {
            ...
            _this.doCloseEditors(group, except, direction);
        });
    };
    EditorPart.prototype.doCloseEditors = function (group, except, direction) {
        var _this = this;
        ...
        if (!except) {
            ...
            this.openEditor(except, null, this.stacks.positionOfGroup(group)).done(function () {
    ...
    EditorPart.prototype.moveEditor = function (input, arg2, arg3, index) {
    ...
    EditorPart.prototype.doMoveEditorInsideGroups = function (input, group, toIndex) {
    ...
    EditorPart.prototype.doMoveEditorAcrossGroups = function (input, fromGroup, to, index) {
        ...
        var options = editor_1.EditorOptions.create({ pinned: true, index: index });
        ...
        if (codeEditor && activeEditor.position === this.stacks.positionOfGroup(fromGroup) && input.matches(activeEditor.input)) {
            options = editor_1.TextEditorOptions.create({ pinned: true, index: index });
    ...
    EditorPart.prototype.replaceEditors = function (editors) {
        var _this = this;
    ...
    EditorPart.prototype.findPosition = function (input, options, arg1, ratio) {
        ...
        if (options && options.revealIfVisible) {
            var editorsToCheck_1 = [];
            if (activeEditor) {
                editorsToCheck_1.push(activeEditor);
            }
            visibleEditors.forEach(function (e) { if (e !== activeEditor) {
                editorsToCheck_1.push(e);
            } });
            for (var i = 0; i < editorsToCheck_1.length; i++) {
                var editorToCheck = editorsToCheck_1[i];
