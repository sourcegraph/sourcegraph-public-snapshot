...
var WorkspaceReferenceAdapter = (function () {
    function WorkspaceReferenceAdapter(_provider) {
        this._provider = _provider;
    }
    WorkspaceReferenceAdapter.prototype.provideWorkspaceReferences = function (resource, query, hints, progress) {
        var _this = this;
        var convertReferences = function (references) {
        ...
        var convertedProgress = function (references) {
            var convertedReferences = convertReferences(references);
            if (convertedReferences) {
                progress(convertedReferences);
            }
        };
        return async_1.asWinJsPromise(function (token) { return _this._provider.provideWorkspaceReferences(query, hints, token, convertedProgress); }).then(convertReferences);
    };
    return WorkspaceReferenceAdapter;
}());
...
var ExtHostLanguageFeatures = (function (_super) {
    ...
    ExtHostLanguageFeatures.prototype.registerWorkspaceReferenceProvider = function (selector, provider, workspace) {
        var handle = this._nextHandle();
    ...
    ExtHostLanguageFeatures.prototype.$provideWorkspaceReferences = function (handle, progressHandle, resource, query, hints) {
        var _this = this;
        var progress = function (references) {
            _this._proxy.$notifyProvideWorkspaceReferencesProgress(handle, progressHandle, references);
        };
        return this._withAdapter(handle, WorkspaceReferenceAdapter, function (adapter) { return adapter.provideWorkspaceReferences(resource, query, hints, progress); });
    };
    // --- quick fix
