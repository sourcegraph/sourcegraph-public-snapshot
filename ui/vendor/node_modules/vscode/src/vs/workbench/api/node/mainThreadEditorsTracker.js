...
var MainThreadEditorsTracker = (function () {
    ...
    MainThreadEditorsTracker.prototype._onCodeEditorAdd = function (codeEditor) {
        var _this = this;
        this._editorModelChangeListeners[codeEditor.getId()] = codeEditor.onDidChangeModel(function (_) { return _this._updateMapping.schedule(); });
        this._updateMapping.schedule();
    };
    ...
    MainThreadEditorsTracker.prototype._doUpdateMapping = function () {
        ...
        Object.keys(visibleModels).forEach(function (modelUri) {
            ...
            for (var i = existingTextEditors.length; i < codeEditors.length; i++) {
                var newTextEditor = new MainThreadTextEditor(MainThreadEditorsTracker._Ids.nextId(), model, codeEditors[i], _this._focusTracker, _this._modelService);
        ...
        allModels.forEach(function (model) {
            ...
            if (existingTextEditors.length === 0) {
                var newTextEditor = new MainThreadTextEditor(MainThreadEditorsTracker._Ids.nextId(), model, null, _this._focusTracker, _this._modelService);
                existingTextEditors.push(newTextEditor);
                _this._onTextEditorAdd.fire(newTextEditor);
            }
            else {
    ...
    MainThreadEditorsTracker.prototype._getVisibleModels = function () {
        ...
        allCodeEditors.forEach(function (codeEditor) {
            ...
            if (!model || model.isTooLargeForHavingARichMode() || !_this._modelService.getModel(model.uri)) {
                return;
            }
            var modelUri = model.uri.toString();
...
MainThreadEditorsTracker._Ids = new idGenerator_1.IdGenerator('');
