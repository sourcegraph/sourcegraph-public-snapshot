...
var MainThreadLanguageFeatures = (function (_super) {
    ...
    function MainThreadLanguageFeatures(threadService, heapService, modeService) {
    ...
    // --- code lens
    MainThreadLanguageFeatures.prototype.$registerCodeLensSupport = function (handle, selector, eventHandle, workspace) {
        var _this = this;
        var provider = {
    ...
    MainThreadLanguageFeatures.prototype.$registerWorkspaceReferenceSupport = function (handle, selector, workspace) {
        var _this = this;
        this._registrations[handle] = modes.WorkspaceReferenceProviderRegistry.register(selector, {
            provideWorkspaceReferences: function (workspace, query, hints, token, progress) {
                var progressHandle = _this._callbackRegistrations.registerChild(handle, progress);
                var refs = _this._proxy.$provideWorkspaceReferences(handle, progressHandle, workspace, query, hints);
                refs = async_1.always(refs, function () { return _this._callbackRegistrations.unregisterChild(handle, progressHandle); });
                return async_1.wireCancellationToken(token, refs);
            }
        }, workspace);
    ...
    MainThreadLanguageFeatures.prototype.$notifyProvideWorkspaceReferencesProgress = function (handle, progressHandle, locations) {
        var progressHandler = this._callbackRegistrations.getChild(handle, progressHandle);
        if (progressHandler) {
            progressHandler(locations);
        }
