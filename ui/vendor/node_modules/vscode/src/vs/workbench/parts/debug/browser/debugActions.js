...
var ConfigureAction = (function (_super) {
    ...
    function ConfigureAction(id, label, debugService, keybindingService, contextService) {
...
var StartAction = (function (_super) {
    ...
    function StartAction(id, label, debugService, keybindingService, commandService, contextService, fileService, textFileService) {
    ...
    StartAction.prototype.run = function () {
        ...
        return this.textFileService.saveAll().then(function () {
            if (_this.debugService.getModel().getProcesses().length === 0) {
                _this.debugService.removeReplExpressions();
            }
            var manager = _this.debugService.getConfigurationManager();
            var configName = _this.debugService.getViewModel().selectedConfigurationName;
            var compound = manager.getCompound(configName);
            if (compound) {
                return _this.commandService.executeCommand('_workbench.startDebug', configName);
            }
            var configuration = manager.getConfiguration(configName);
            return manager.getStartSessionCommand(configuration ? configuration.type : undefined).then(function (commandAndType) {
                configuration = _this.massageConfiguartion(configuration);
                if (commandAndType && commandAndType.command) {
                    return _this.commandService.executeCommand(commandAndType.command, configuration || _this.getDefaultConfiguration()).then(function (result) {
                        if (_this.contextService.getWorkspace()) {
                            ...
                            if (result && result.status === 'saveConfiguration') {
                                return _this.fileService.updateContent(manager.configFileUri, result.content).then(function () { return manager.openConfigFile(false); });
                            }
                        }
                        return undefined;
                    });
                ...
                if (configName) {
                    return _this.commandService.executeCommand('_workbench.startDebug', configuration || configName);
                }
                if (_this.contextService.getWorkspace() && commandAndType) {
                    return manager.openConfigFile(false, commandAndType.type);
                }
                return undefined;
            });
    ...
    StartAction.prototype.getDefaultConfiguration = function () {
        return {};
    };
    StartAction.prototype.massageConfiguartion = function (config) {
        return config;
    };
    ...
    StartAction.prototype.isEnabled = function (state) {
        ...
        return state !== debug_1.State.Initializing && processes.every(function (p) { return p.name !== _this.debugService.getViewModel().selectedConfigurationName; }) &&
...
var RunAction = (function (_super) {
    ...
    function RunAction(id, label, debugService, keybindingService, commandService, contextService, fileService, textFileService) {
        return _super.call(this, id, label, debugService, keybindingService, commandService, contextService, fileService, textFileService) || this;
    }
    RunAction.prototype.getDefaultConfiguration = function () {
        return { noDebug: true };
    };
    RunAction.prototype.massageConfiguartion = function (config) {
        if (config) {
            config.noDebug = true;
        }
        return config;
...
var SelectAndStartAction = (function (_super) {
...
var EnableBreakpointAction = (function (_super) {
...
var DisableBreakpointAction = (function (_super) {
...
var FocusProcessAction = (function (_super) {
    ...
    FocusProcessAction.prototype.run = function (processName) {
        ...
        return this.debugService.focusStackFrameAndEvaluate(null, process).then(function () {
            ...
            if (stackFrame) {
                return stackFrame.openInEditor(_this.editorService, true);
            }
            return undefined;
        });
    };
