...
var RepoQuickOpenModel = (function (_super) {
    __extends(RepoQuickOpenModel, _super);
    function RepoQuickOpenModel(entries, stats) {
        var _this = _super.call(this, entries) || this;
        _this.stats = stats;
        return _this;
    }
    return RepoQuickOpenModel;
}(quickOpenModel_1.QuickOpenModel));
exports.RepoQuickOpenModel = RepoQuickOpenModel;
var RepoEntry = (function (_super) {
    __extends(RepoEntry, _super);
    function RepoEntry(resource, name, description, icon, editorService, modeService, modelService, configurationService, contextService) {
    ...
    RepoEntry.prototype.getLabel = function () {
        return this.name;
    };
    RepoEntry.prototype.getLabelOptions = function () {
        return {
            extraClasses: labels_1.getIconClasses(this.modelService, this.modeService, this.resource)
        };
    };
    RepoEntry.prototype.getAriaLabel = function () {
        return nls.localize('entryAriaLabel', "{0}, repo picker", this.getLabel());
    };
    RepoEntry.prototype.getDescription = function () {
        return this.description;
    };
    RepoEntry.prototype.getIcon = function () {
        return this.icon;
    };
    RepoEntry.prototype.getResource = function () {
        return this.resource;
    };
    RepoEntry.prototype.setRange = function (range) {
        this.range = range;
    };
    RepoEntry.prototype.isFile = function () {
        return false;
    };
    RepoEntry.prototype.getInput = function () {
        var input = {
...
exports.RepoEntry = RepoEntry;
var OpenRepoHandler = (function (_super) {
    __extends(OpenRepoHandler, _super);
    function OpenRepoHandler(editorGroupService, instantiationService, themeService, contextService, searchService) {
    ...
    OpenRepoHandler.prototype.setOptions = function (options) {
        this.options = options;
    };
    OpenRepoHandler.prototype.getResults = function (searchValue, maxSortedResults) {
        searchValue = searchValue.trim();
        // Respond directly to empty search
        if (!searchValue) {
            return winjs_base_1.TPromise.as(new RepoQuickOpenModel([]));
        }
        // Do find results
        return this.doFindResults(searchValue, this.cacheState.cacheKey, maxSortedResults);
    };
    OpenRepoHandler.prototype.doFindResults = function (searchValue, cacheKey, maxSortedResults) {
        var _this = this;
        var query = {
        ...
        return this.searchService.searchRepo(this.queryBuilder.file(query)).then(function (complete) {
            var results = [];
            for (var i = 0; i < complete.results.length; i++) {
                ...
                results.push(_this.instantiationService.createInstance(RepoEntry, fileMatch.resource, label, description, iconClass));
            }
            results.forEach(function (result) {
                // results.setRange(searchWithRange ? searchWithRange.range : null);
                var _a = quickOpenModel_1.QuickOpenEntry.highlight(result, searchValue, true /* fuzzy highlight */), labelHighlights = _a.labelHighlights, descriptionHighlights = _a.descriptionHighlights;
                result.setHighlights(labelHighlights, descriptionHighlights);
            });
            return new RepoQuickOpenModel(results, complete.stats);
        });
    };
    OpenRepoHandler.prototype.hasShortResponseTime = function () {
        return this.isCacheLoaded;
    };
    OpenRepoHandler.prototype.onOpen = function () {
        var _this = this;
        this.cacheState = new CacheState(function (cacheKey) { return _this.cacheQuery(cacheKey); }, function (query) { return _this.searchService.searchRepo(query); }, function (cacheKey) { return _this.searchService.clearCache(cacheKey); }, this.cacheState);
        this.cacheState.load();
    };
    OpenRepoHandler.prototype.cacheQuery = function (cacheKey) {
        var options = {
            folderResources: this.contextService.getWorkspace() ? [this.contextService.getWorkspace().resource] : [],
            extraFileResources: editor_1.getOutOfWorkspaceEditorResources(this.editorGroupService, this.contextService),
            filePattern: '',
    ...
    Object.defineProperty(OpenRepoHandler.prototype, "isCacheLoaded", {
        get: function () {
    ...
    OpenRepoHandler.prototype.getGroupLabel = function () {
        return nls.localize('repositorySearchResults', "repository results");
    };
    OpenRepoHandler.prototype.getEmptyLabel = function (searchString) {
    ...
    OpenRepoHandler.prototype.getAutoFocus = function (searchValue) {
        return {
            autoFocusFirstEntry: true
        };
    };
    return OpenRepoHandler;
}(quickopen_1.QuickOpenHandler));
OpenRepoHandler = __decorate([
...
exports.OpenRepoHandler = OpenRepoHandler;
var LoadingPhase;
