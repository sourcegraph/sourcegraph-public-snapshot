...
var SelectColorThemeAction = (function (_super) {
    ...
    function SelectColorThemeAction(id, label, quickOpenService, messageService, themeService, extensionGalleryService, viewletService) {
    ...
    SelectColorThemeAction.prototype.run = function () {
        ...
        return this.themeService.getColorThemes().then(function (themes) {
            var currentThemeId = _this.themeService.getColorTheme();
            var currentTheme = themes.filter(function (theme) { return theme.id === currentThemeId; })[0];
            ...
            var selectTheme = function (theme, broadcast) {
                if (theme === pickInMarketPlace) {
                    theme = currentTheme;
                }
                _this.themeService.setColorTheme(theme.id, broadcast)
                    .done(null, function (err) { return _this.messageService.show(message_1.Severity.Info, nls_1.localize('problemChangingTheme', "Problem loading theme: {0}", err)); });
            };
            var placeHolder = nls_1.localize('themes.selectTheme', "Select Color Theme");
            var autoFocusIndex = arrays_1.firstIndex(picks, function (p) { return p.id === currentThemeId; });
...
var SelectIconThemeAction = (function (_super) {
    ...
    function SelectIconThemeAction(id, label, quickOpenService, messageService, themeService, extensionGalleryService, viewletService) {
        var _this = _super.call(this, id, label) || this;
        _this.quickOpenService = quickOpenService;
        _this.messageService = messageService;
        _this.themeService = themeService;
    ...
    SelectIconThemeAction.prototype.run = function () {
        ...
        return this.themeService.getFileIconThemes().then(function (themes) {
            var currentThemeId = _this.themeService.getFileIconTheme();
            var currentTheme = themes.filter(function (theme) { return theme.id === currentThemeId; })[0];
            ...
            var selectTheme = function (theme, broadcast) {
                ...
                _this.themeService.setFileIconTheme(theme && theme.id, broadcast)
                    .done(null, function (err) { return _this.messageService.show(message_1.Severity.Info, nls_1.localize('problemChangingIconTheme', "Problem loading icon theme: {0}", err.message)); });
            };
...
var colorThemeDescriptor = new actions_2.SyncActionDescriptor(SelectColorThemeAction, SelectColorThemeAction.ID, SelectColorThemeAction.LABEL, { primary: keyCodes_1.KeyChord(32768 /* CtrlCmd */ | 41 /* KEY_K */, 32768 /* CtrlCmd */ | 50 /* KEY_T */) });
