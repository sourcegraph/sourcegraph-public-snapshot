...
var EditorInputCache = (function () {
    function EditorInputCache(replaceService, editorService, modelService, textModelResolverService) {
    ...
    EditorInputCache.prototype.refreshInput = function (fileMatch, reloadFromSource) {
        ...
        if (editorInputPromise) {
            editorInputPromise.done(function () {
                if (reloadFromSource) {
                    _this.textModelResolverService.createModelReference(fileMatch.resource()).done(function (ref) {
                        var model = ref.object;
                        if (model.textEditorModel) {
                            var replaceResource = _this.getReplaceResource(fileMatch.resource());
                            _this.modelService.getModel(replaceResource).setValue(model.textEditorModel.getValue());
    ...
    EditorInputCache.prototype.createInput = function (fileMatch) {
        ...
        return winjs_base_1.TPromise.join([this.createLeftInput(fileMatch),
            ...
            var editorInput = new diffEditorInput_1.DiffEditorInput(nls.localize('fileReplaceChanges', "{0} ↔ {1} (Replace Preview)", fileMatch.name(), fileMatch.name()), undefined, left, right);
    ...
    EditorInputCache.prototype.createRightInput = function (element) {
        var _this = this;
        return this.textModelResolverService.createModelReference(element.resource()).then(function (ref) {
            var model = ref.object;
            var textEditorModel = model.textEditorModel;
            var replaceResource = _this.getReplaceResource(element.resource());
            _this.modelService.createModel(textEditorModel.getValue(), textEditorModel.getMode(), replaceResource);
            ref.dispose();
            return _this.editorService.createInput({ resource: replaceResource });
        });
    };
...
var ReplaceService = (function () {
    function ReplaceService(telemetryService, eventService, editorService, modelService, textModelResolverService) {
    ...
    ReplaceService.prototype.replace = function (arg, progress, resource) {
        ...
        var bulkEdit = bulkEdit_1.createBulkEdit(this.eventService, this.textModelResolverService, null);
