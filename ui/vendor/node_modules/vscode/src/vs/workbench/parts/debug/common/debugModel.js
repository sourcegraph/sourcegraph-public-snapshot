    ...
    Object.defineProperty(Variable.prototype, "evaluateName", {
        get: function () {
            ...
            names.forEach(function (name) {
                ...
                else if (Variable.ARRAY_ELEMENT_SYNTAX.test(name) || (_this.process.configuration.type === 'node' && !Variable.NOT_PROPERTY_SYNTAX.test(name))) {
...
var StackFrame = (function () {
    ...
    StackFrame.prototype.getMostSpecificScopes = function (range) {
        return this.getScopes().then(function (scopes) {
            ...
            return [scopes.filter(function (scope) { return scope.range && range_1.Range.containsRange(scope.range, range); })
                    .sort(function (first, second) { return (first.range.endLineNumber - first.range.startLineNumber) - (second.range.endLineNumber - second.range.startLineNumber); }).shift()];
        });
    };
    ...
    Thread.prototype.getCallStackImpl = function (startFrame) {
        ...
        return this.process.session.stackTrace({ threadId: this.threadId, startFrame: startFrame, levels: 20 }).then(function (response) {
            ...
            return response.body.stackFrames.map(function (rsf, level) {
                if (!rsf) {
                    return new StackFrame(_this, 0, new debugSource_1.Source({ name: UNKNOWN_SOURCE_LABEL }, true), nls.localize('unknownStack', "Unknown stack location"), null, null);
                }
                var source = rsf.source ? new debugSource_1.Source(rsf.source, rsf.source.presentationHint === 'deemphasize') : new debugSource_1.Source({ name: UNKNOWN_SOURCE_LABEL }, true);
                ...
                return new StackFrame(_this, rsf.id, source, rsf.name, rsf.line, rsf.column);
            });
        }, function (err) {
            if (_this.stoppedDetails) {
...
var Process = (function () {
    ...
    Process.prototype.isAttach = function () {
        ...
        if (data.stoppedDetails) {
            ...
            if (data.allThreadsStopped) {
                this.threads.forEach(function (thread) {
                    ...
                    thread.stoppedDetails = objects_1.clone(data.stoppedDetails);
                    thread.stopped = true;
...
var Breakpoint = (function () {
    function Breakpoint(uri, lineNumber, column, enabled, condition, hitCondition, respectColumn // TODO@Isidor remove this in March
    ) {
        this.uri = uri;
        this.lineNumber = lineNumber;
        this.column = column;
        ...
        this.respectColumn = respectColumn; // TODO@Isidor remove this in March
...
var Model = (function () {
    ...
    Model.prototype.addBreakpoints = function (uri, rawData) {
        this.breakpoints = this.breakpoints.concat(rawData.map(function (rawBp) {
            return new Breakpoint(uri, rawBp.lineNumber, rawBp.column, rawBp.enabled, rawBp.condition, rawBp.hitCondition, true);
        }));
    ...
    Model.prototype.deemphasizeSource = function (uri) {
