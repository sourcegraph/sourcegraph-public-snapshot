...
var TerminalInstance = (function () {
    function TerminalInstance(_terminalFocusContextKey, _configHelper, _container, workspace, name, shell, _contextKeyService, _keybindingService, _messageService) {
    ...
    TerminalInstance.prototype.attachToElement = function (container) {
        ...
        this._xterm.element.addEventListener('mouseup', function (event) {
        ...
        this._xterm.element.addEventListener('keyup', function (event) {
    ...
    TerminalInstance.prototype._createProcess = function (workspace, name, shell) {
        ...
        this._process.on('exit', function (exitCode) {
            ...
            if (!_this._isExiting) {
                _this.dispose();
                if (exitCode) {
                    if (_this._isLaunching) {
                        var args = shell.args && shell.args.length ? ' ' + shell.args.map(function (a) { return a.indexOf(' ') !== -1 ? "'" + a + "'" : a; }).join(' ') : '';
                        _this._messageService.show(message_1.Severity.Error, nls.localize('terminal.integrated.launchFailed', 'The terminal process command `{0}{1}` failed to launch (exit code: {2})', shell.executable, args, exitCode));
                    }
                    else {
    ...
    TerminalInstance.prototype.setScrollback = function (lineCount) {
...
TerminalInstance.LAUNCHING_DURATION = 500;
