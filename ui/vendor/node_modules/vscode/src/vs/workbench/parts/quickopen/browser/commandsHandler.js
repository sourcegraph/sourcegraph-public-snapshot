...
var CommandsHandler = (function (_super) {
    ...
    function CommandsHandler(editorService, instantiationService, keybindingService, menuService, contextKeyService) {
        var _this = _super.call(this) || this;
    ...
    CommandsHandler.prototype.getResults = function (searchValue) {
        ...
        // Other Actions
        var menu = editorCommon_1.isCommonCodeEditor(activeEditorControl)
            ? activeEditorControl.invokeWithinContext(function (accessor) { return _this.menuService.createMenu(actions_1.MenuId.CommandPalette, accessor.get(contextkey_1.IContextKeyService)); })
            : this.menuService.createMenu(actions_1.MenuId.CommandPalette, this.contextKeyService);
        ...
        // Concat
        var entries = workbenchEntries.concat(editorEntries, commandEntries);
        // Remove duplicates
    ...
    CommandsHandler.prototype.actionDescriptorsToEntries = function (actionDescriptors, searchValue) {
        ...
        for (var i = 0; i < actionDescriptors.length; i++) {
            ...
            var keybind = this.keybindingService.lookupKeybinding(actionDescriptor.id);
            var keyLabel = keybind ? keybind.getLabel() : '';
            var keyAriaLabel = keybind ? keybind.getAriaLabel() : '';
    ...
    CommandsHandler.prototype.editorActionsToEntries = function (actions, searchValue) {
        ...
        for (var i = 0; i < actions.length; i++) {
            ...
            var keybind = this.keybindingService.lookupKeybinding(action.id);
            var keyLabel = keybind ? keybind.getLabel() : '';
            var keyAriaLabel = keybind ? keybind.getAriaLabel() : '';
    ...
    CommandsHandler.prototype.menuItemActionsToEntries = function (actions, searchValue) {
        ...
        for (var _i = 0, actions_2 = actions; _i < actions_2.length; _i++) {
            ...
            var label = action.item.category
                ? nls.localize('cat.title', "{0}: {1}", action.item.category, action.item.title)
                : action.item.title;
            if (label) {
                var labelHighlights = wordFilter(searchValue, label);
                var keybind = this.keybindingService.lookupKeybinding(action.item.id);
                var keyLabel = keybind ? keybind.getLabel() : '';
                var keyAriaLabel = keybind ? keybind.getAriaLabel() : '';
                var alias = action.item.alias ? action.item.alias : null;
                var aliasHighlights = alias ? wordFilter(searchValue, alias) : null;
                if (labelHighlights || aliasHighlights) {
