...
var TaskSystem = require("vs/workbench/parts/tasks/common/taskSystem");
/**
    ...
    ConfigurationParser.prototype.mergeTaskRunnerConigurations = function (result, osConfig) {
        ...
        if (osConfig.options) {
            ...
            if (osConfig.options.env) {
                var osEnv_1 = osConfig.options.env;
                var env_1 = result.options.env;
                if (env_1) {
                    Object.keys(osEnv_1).forEach(function (key) {
                        env_1[key] = osEnv_1[key];
                    });
                }
                else {
                    result.options.env = osEnv_1;
                }
            }
        }
        if (osConfig.tasks) {
            var taskNames_1 = Object.create(null);
            Object.keys(result.tasks).forEach(function (key) {
                var task = result.tasks[key];
                taskNames_1[task.name] = task.id;
            });
            var osTaskNames_1 = Object.create(null);
            Object.keys(osConfig.tasks).forEach(function (key) {
                var task = osConfig.tasks[key];
                osTaskNames_1[task.name] = task.id;
            });
            Object.keys(osTaskNames_1).forEach(function (taskName) {
                var id = taskNames_1[taskName];
                var osId = osTaskNames_1[taskName];
