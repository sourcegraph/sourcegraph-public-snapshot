...
var debugConfigurationManager_1 = require("vs/workbench/parts/debug/electron-browser/debugConfigurationManager");
...
var DebugService = (function () {
    ...
    DebugService.prototype.registerListeners = function (lifecycleService) {
        ...
        this.toDispose.push(this.configurationService.onDidUpdateConfiguration(function (event) {
            if (event.sourceConfig) {
                var names = _this.configurationManager.getConfigurationNames();
                if (names.every(function (name) { return name !== _this.viewModel.selectedConfigurationName; })) {
                    // Current selected configuration no longer exists - take the first configuration instead.
                    _this.viewModel.setSelectedConfigurationName(names.length ? names[0] : undefined);
                }
            }
        }));
    };
    DebugService.prototype.onBroadcast = function (broadcast) {
        ...
        var process = this.model.getProcesses().filter(function (p) { return p.configuration.type === 'extensionHost'; }).pop();
    ...
    DebugService.prototype.registerSessionListeners = function (process, session) {
            ...
            session.threads().then(function (response) {
                ...
                if (thread) {
                    thread.fetchCallStack().then(function (callStack) {
                        if (callStack.length > 0 && !_this.viewModel.focusedStackFrame) {
                            // focus first stack frame from top that has source location if no other stack frame is focussed
                            var stackFrameToFocus = arrays_1.first(callStack, function (sf) { return sf.source && !sf.source.deemphasize; }, callStack[0]);
                            ...
                            return stackFrameToFocus.openInEditor(_this.editorService);
                        }
                        return undefined;
                    });
                }
        ...
        this.toDisposeOnSessionEnd.get(session.getId()).push(session.onDidContinued(function (event) {
            ...
            _this.setStateAndEmit(session.getId(), debug.State.Running);
        }));
            ...
            if (event.body.category === 'telemetry') {
                ...
                container.getChildren().then(function (children) {
                    children.forEach(function (child) {
                        ...
                        child.name = null;
                        _this.model.appendToRepl(child, outputSeverity);
                    });
                });
            }
        ...
        this.toDisposeOnSessionEnd.get(session.getId()).push(session.onDidExitAdapter(function (event) {
            ...
            if (process && session && process.getId() === session.getId() && process.configuration.type === 'extensionHost' && _this.sessionStates.get(session.getId()) === debug.State.Running &&
                process && _this.contextService.getWorkspace() && process.configuration.noDebug) {
    ...
    DebugService.prototype.loadBreakpoints = function () {
        ...
        try {
            result = JSON.parse(this.storageService.get(DEBUG_BREAKPOINTS_KEY, storage_1.StorageScope.WORKSPACE, '[]')).map(function (breakpoint) {
                return new debugModel_1.Breakpoint(uri_1.default.parse(breakpoint.uri.external || breakpoint.source.uri.external), breakpoint.lineNumber, breakpoint.respectColumn ? breakpoint.column : undefined, breakpoint.enabled, breakpoint.condition, breakpoint.hitCondition, breakpoint.respectColumn);
            });
        }
        catch (e) { }
        return result || [];
    ...
    DebugService.prototype.focusStackFrameAndEvaluate = function (stackFrame, process) {
        if (!process) {
    ...
    DebugService.prototype.createProcess = function (configurationOrName) {
        var _this = this;
        return this.textFileService.saveAll().then(function () { return _this.configurationService.reloadConfiguration(); }) // make sure configuration is up to date
            ...
            var config = typeof configurationOrName === 'string' ? _this.configurationManager.getConfiguration(configurationOrName) : configurationOrName;
            if (!config) {
                return winjs_base_1.TPromise.wrapError(new Error(nls.localize('configMissing', "Configuration '{0}' is missing in 'launch.json'.", configurationOrName)));
            }
            return _this.configurationManager.resloveConfiguration(config).then(function (resolvedConfig) {
                if (!resolvedConfig) {
                ...
                return _this.runPreLaunchTask(resolvedConfig.preLaunchTask).then(function (taskSummary) {
                    ...
                    if (successExitCode || (errorCount === 0 && !failureExitCode)) {
                        return _this.doCreateProcess(resolvedConfig);
                    }
                ...
                if (!_this.contextService.getWorkspace()) {
                    return _this.messageService.show(severity_1.default.Error, nls.localize('noFolderWorkspaceDebugError', "The active file can not be debugged. Make sure it is saved on disk and that you have a debug extension installed for that file type."));
                }
    ...
    DebugService.prototype.doCreateProcess = function (configuration) {
        var _this = this;
        ...
        return this.telemetryService.getTelemetryInfo().then(function (info) {
            ...
            return session.initialize({
                ...
                if (!_this.viewModel.changedWorkbenchViewState && (_this.partService.isVisible(partService_1.Parts.SIDEBAR_PART) || !_this.contextService.getWorkspace())) {
                ...
                return _this.telemetryService.publicLog('debugSessionStart', {
                    ...
                    launchJsonExists: !!_this.configurationService.getConfiguration('launch')
                });
    ...
    DebugService.prototype.rawAttach = function (session, port) {
        ...
        var config = this.configurationManager.getConfiguration(this.viewModel.selectedConfigurationName);
        return this.configurationManager.resloveConfiguration(config).then(function (resolvedConfig) {
            resolvedConfig.request = 'attach';
            resolvedConfig.port = port;
            _this.doCreateProcess(resolvedConfig);
        });
    };
    DebugService.prototype.deemphasizeSource = function (uri) {
    ...
    DebugService.prototype.restartProcess = function (process) {
        ...
        var preserveFocus = process.getId() === this.viewModel.focusedProcess.getId();
        return process.session.disconnect(true).then(function () {
            return new winjs_base_1.TPromise(function (c, e) {
                setTimeout(function () {
                    ...
                    var config = _this.configurationManager.getConfiguration(process.configuration.name);
                    if (config) {
                        // Take the type from the process since the debug extension might overwrite it #21316
                        config.type = process.configuration.type;
                    }
                    _this.createProcess(config || process.configuration).then(function () { return c(null); }, function (err) { return e(err); });
                }, 300);
            });
        }).then(function () {
            if (preserveFocus) {
    ...
    DebugService.prototype.sendBreakpoints = function (modelUri, sourceModified, targetProcess) {
        ...
        var sendBreakpointsToProcess = function (process) {
            ...
            var rawSource = source ? source.raw : { path: paths.normalize(modelUri.fsPath, true) };
