...
var BreakpointWidget = (function (_super) {
    ...
    function BreakpointWidget(editor, lineNumber, contextViewService, debugService) {
        var _this = _super.call(this, editor, { showFrame: true, showArrow: false, frameColor: '#007ACC', frameWidth: 1 }) || this;
    ...
    BreakpointWidget.prototype._fillContainer = function (container) {
        ...
        var breakpoint = this.debugService.getModel().getBreakpoints().filter(function (bp) { return bp.lineNumber === _this.lineNumber && bp.uri.toString() === uri.toString(); }).pop();
        this.hitCountContext = breakpoint && breakpoint.hitCondition && !breakpoint.condition;
        var selected = this.hitCountContext ? 1 : 0;
        var selectBox = new selectBox_1.SelectBox([nls.localize('expression', "Expression"), nls.localize('hitCount', "Hit Count")], selected);
        selectBox.render(dom.append(container, $('.breakpoint-select-container')));
        selectBox.onDidSelect(function (e) {
            _this.hitCountContext = e === 'Hit Count';
            ...
            if (!disposed) {
                ...
                if (success) {
                    ...
                    var oldBreakpoint = _this.debugService.getModel().getBreakpoints()
                        .filter(function (bp) { return bp.lineNumber === _this.lineNumber && bp.uri.toString() === uri.toString(); }).pop();
                    var raw = {
                    ...
                    if (_this.hitCountContext) {
                        raw.hitCondition = _this.inputBox.value;
                        if (_this.conditionInput) {
                            raw.condition = _this.conditionInput;
                        }
                    }
                    else {
                        raw.condition = _this.inputBox.value;
                        if (_this.hitCountInput) {
                            raw.hitCondition = _this.hitCountInput;
                        }
                    }
                    if (oldBreakpoint) {
                        _this.debugService.removeBreakpoints(oldBreakpoint.getId()).done(null, errors.onUnexpectedError);
                    }
                    _this.debugService.addBreakpoints(uri, [raw]).done(null, errors.onUnexpectedError);
                }
                _this.dispose();
            }
        });
