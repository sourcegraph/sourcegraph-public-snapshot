...
var CloneAction = (function (_super) {
    ...
    function CloneAction(id, label, gitService, quickOpenService, messageService, windowsService, telemetryService, workspaceService) {
    ...
    CloneAction.prototype.run = function (event, data) {
        var _this = this;
        return this.quickOpenService.input({
            prompt: nls_1.localize('valid', "Provide a valid git repository URL"),
            placeHolder: nls_1.localize('url', "Repository URL"),
            validateInput: function (input) {
                var parsedUrl = url.parse(input);
                if (!parsedUrl.protocol || !parsedUrl.host) {
                    return winjs_base_1.TPromise.as(nls_1.localize('valid', "Provide a valid git repository URL"));
                }
            ...
            .then(function (url) {
            if (!url) {
                _this.telemetryService.publicLog('gitClone', __assign({}, data, { outcome: 'no_URL' }));
            ...
            if (!result || result.length === 0) {
                _this.telemetryService.publicLog('gitClone', __assign({}, data, { outcome: 'no_directory' }));
                return winjs_base_1.TPromise.as(null);
            }
            ...
            return clone.then(function (path) {
                _this.telemetryService.publicLog('gitClone', __assign({}, data, { outcome: 'success' }));
                var forceNewWindow = _this.workspaceService.hasWorkspace();
                return _this.windowsService.openWindow([path], { forceNewWindow: forceNewWindow, forceReuseWindow: !forceNewWindow });
            }).then(null, function (e) {
                if (/already exists and is not an empty directory/.test(e.stderr || '')) {
                    _this.telemetryService.publicLog('gitClone', __assign({}, data, { outcome: 'directory_not_empty' }));
                ...
                _this.telemetryService.publicLog('gitClone', __assign({}, data, { outcome: errors_1.isPromiseCanceledError(e) ? 'canceled' : 'error' }));
