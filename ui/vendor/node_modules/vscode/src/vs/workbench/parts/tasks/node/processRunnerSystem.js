...
var ProcessRunnerSystem = (function (_super) {
    ...
    function ProcessRunnerSystem(configuration, markerService, modelService, telemetryService, outputService, configurationResolverService, outputChannelId, hasErrors) {
        ...
        _this.errorsShown = !hasErrors;
        return _this;
    }
    ProcessRunnerSystem.prototype.build = function () {
        var buildTaskIdentifier = this.configuration.buildTasks.length > 0 ? this.configuration.buildTasks[0] : undefined;
        if (this.activeTaskIdentifier) {
            var task = this.configuration.tasks[this.activeTaskIdentifier];
            return { kind: taskSystem_1.TaskExecuteKind.Active, active: { same: this.activeTaskIdentifier === buildTaskIdentifier, background: task.isBackground }, promise: this.activeTaskPromise };
        }
        ...
        return this.executeTask(buildTaskIdentifier, taskSystem_1.Triggers.shortcut);
    };
    ...
    ProcessRunnerSystem.prototype.runTest = function () {
        var testTaskIdentifier = this.configuration.testTasks.length > 0 ? this.configuration.testTasks[0] : undefined;
        if (this.activeTaskIdentifier) {
            var task = this.configuration.tasks[this.activeTaskIdentifier];
            return { kind: taskSystem_1.TaskExecuteKind.Active, active: { same: this.activeTaskIdentifier === testTaskIdentifier, background: task.isBackground }, promise: this.activeTaskPromise };
        }
        ...
        return this.executeTask(testTaskIdentifier, taskSystem_1.Triggers.shortcut);
    };
    ProcessRunnerSystem.prototype.run = function (taskIdentifier) {
        if (this.activeTaskIdentifier) {
            var task = this.configuration.tasks[this.activeTaskIdentifier];
            return { kind: taskSystem_1.TaskExecuteKind.Active, active: { same: this.activeTaskIdentifier === taskIdentifier, background: task.isBackground }, promise: this.activeTaskPromise };
        }
    ...
    ProcessRunnerSystem.prototype.doExecuteTask = function (task, telemetryEvent) {
        ...
        this.childProcess = new processes_1.LineProcess(command, args, !!commandConfig.isShellCommand, this.resolveOptions(commandConfig.options));
