...
var TextFileService = (function () {
    function TextFileService(lifecycleService, contextService, configurationService, telemetryService, editorGroupService, fileService, untitledEditorService, instantiationService, backupService, messageService) {
    ...
    TextFileService.prototype.registerListeners = function () {
        ...
        this.toUnbind.push(DOM.addDisposableListener(window, DOM.EventType.BLUR, function () { return _this.onEditorFocusChanged(); }, true));
        ...
        if (dirty.length) {
            ...
            return handleAutoSave.then(function (dirty) {
                ...
                if (dirty.length) {
                    ...
                    if (_this.backupService.isHotExitEnabled) {
                        return _this.backupService.backupBeforeShutdown(dirty, _this.models, reason).then(function (result) {
                            if (result.didBackup) {
    ...
    TextFileService.prototype.noVeto = function (options) {
        ...
        return this.backupService.cleanupBackupsBeforeShutdown().then(function () { return false; }, function () { return false; });
    };
    TextFileService.prototype.onWindowFocusLost = function () {
        if (this.configuredAutoSaveOnWindowChange && this.isDirty()) {
            this.saveAll(void 0, textfiles_1.SaveReason.WINDOW_CHANGE).done(null, errors.onUnexpectedError);
        }
    };
    TextFileService.prototype.onEditorFocusChanged = function () {
        if (this.configuredAutoSaveOnFocusChange && this.isDirty()) {
            this.saveAll(void 0, textfiles_1.SaveReason.FOCUS_CHANGE).done(null, errors.onUnexpectedError);
        }
    };
    ...
    TextFileService.prototype.save = function (resource, options) {
        ...
        if (options && options.force && resource.scheme === 'file' && !this.isDirty(resource)) {
            return this.fileService.touchFile(resource).then(function () { return true; }, function () { return true; } /* gracefully ignore errors if just touching */);
        }
...
TextFileService = __decorate([
    ...
    __param(8, backup_1.IBackupModelService),
