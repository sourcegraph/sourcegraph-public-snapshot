...
var TextFileEditorModel = (function (_super) {
    ...
    function TextFileEditorModel(resource, preferredEncoding, messageService, modeService, modelService, fileService, lifecycleService, instantiationService, telemetryService, textFileService, backupFileService) {
        ...
        _this.dirty = false;
        _this.autoSavePromises = [];
        _this.versionId = 0;
        _this.lastSaveAttemptTime = 0;
        _this.mapPendingSaveToVersionId = {};
    ...
    TextFileEditorModel.prototype.load = function (force /* bypass any caches and really go to disk */) {
        ...
        // Resolve Content
        return this.textFileService.resolveTextContent(this.resource, { acceptTextOnly: true, etag: etag, encoding: this.preferredEncoding }).then(function (content) { return _this.loadWithContent(content); }, function (error) {
            ...
            if (result === files_1.FileOperationResult.FILE_NOT_FOUND) {
                if (!_this.textEditorModel && !_this.createTextEditorModelPromise) {
                    return _this.backupFileService.loadBackupResource(_this.resource).then(function (backup) {
                        // Make sure meanwhile someone else did not suceed or start loading
                        if (_this.createTextEditorModelPromise) {
                            return _this.createTextEditorModelPromise;
                        }
                        else if (_this.textEditorModel) {
                            return winjs_base_1.TPromise.as(_this.textEditorModel);
                        }
                        ...
                        if (!!backup) {
                            ...
                            return _this.loadWithContent(content);
                        }
                        // Otherwise bubble up the error
                        return winjs_base_1.TPromise.wrapError(error);
                    }, function (ignoreError) { return winjs_base_1.TPromise.wrapError(error); });
                }
            }
            // Otherwise bubble up the error
            return winjs_base_1.TPromise.wrapError(error);
    ...
    TextFileEditorModel.prototype.loadWithContent = function (content) {
        var _this = this;
        diag('load() - resolved content', this.resource, new Date());
        // Telemetry
        this.telemetryService.publicLog('fileGet', { mimeType: mime_1.guessMimeTypes(this.resource.fsPath).join(', '), ext: paths.extname(this.resource.fsPath), path: telemetry_1.anonymize(this.resource.fsPath) });
        ...
        if (this.textEditorModel) {
            ...
            this.createTextEditorModelPromise = this.backupFileService.loadBackupResource(this.resource).then(function (backupResource) {
                var resolveBackupPromise;
                // Try get restore content, if there is an issue fallback silently to the original file's content
                if (backupResource) {
                    resolveBackupPromise = _this.textFileService.resolveTextContent(backupResource, backup_1.BACKUP_FILE_RESOLVE_OPTIONS).then(function (backup) {
                        return _this.backupFileService.parseBackupContent(backup);
                    }, function (error) { return content.value; });
                }
                else {
                    resolveBackupPromise = winjs_base_1.TPromise.as(content.value);
                }
                return resolveBackupPromise.then(function (fileContent) {
                    return _this.createTextEditorModel(fileContent, content.resource).then(function () {
