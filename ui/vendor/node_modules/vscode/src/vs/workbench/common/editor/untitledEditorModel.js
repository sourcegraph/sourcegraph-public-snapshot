...
var UntitledEditorModel = (function (_super) {
    ...
    function UntitledEditorModel(modeId, resource, hasAssociatedFilePath, modeService, modelService, backupFileService, textFileService, configurationService) {
        var _this = _super.call(this, '', modeId, resource, modeService, modelService) || this;
        ...
        return this.backupFileService.loadBackupResource(this.resource).then(function (backupResource) {
            if (backupResource) {
                return _this.textFileService.resolveTextContent(backupResource, backup_1.BACKUP_FILE_RESOLVE_OPTIONS).then(function (rawTextContent) {
                    return _this.backupFileService.parseBackupContent(rawTextContent);
                });
            }
            return null;
        }).then(function (backupContent) {
            if (backupContent) {
                _this.setValue(backupContent);
            }
            _this.setDirty(_this.hasAssociatedFilePath || !!backupContent); // untitled associated to file path are dirty right away as well as untitled with content
            return _super.prototype.load.call(_this).then(function (model) {
