...
func (s *graphUplink) PushEvents(ctx context.Context, eventList *sourcegraph.UserEventList) (*pbtypes.Void, error) {
	...
	for _, event := range eventList.Events {
		if event.ClientID != "" && actorID != event.ClientID {
			// The event is not verified to have been generated by the client.
			log15.Warn("GraphUplink.PushEvents got unverified clientID", "clientID", event.ClientID, "actorID", actorID)
		}
		if event.ClientID == "" {
