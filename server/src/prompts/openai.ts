...
import {
	getCharCountLimitedPrefixAtLineBreak,
	tokenCost,
	tokenCountToChars,
} from "./common";
...
export class OpenAIBackend {
	...
	async getCompletions({
		...
		const prompt = this.createPrompt({
			...
			references,
			maxTokens:
				this.contextWindowInfo.totalSize - this.contextWindowInfo.numGenerated,
			tokenCost,
		});
...
export function langKeywordStopStrings(uri: string): string[] | undefined {
	...
	switch (ext) {
		...
		default:
			console.error(
				`no stopwords defined for language ${ext}, defaulting to no stopwords`
			);
			return undefined;
	}
}
