...
export class AnthropicAPIClient {
	...
	async completion(
		...
		callbacks: AnthropicCompletionCallbacks
	): Promise<void> {
		...
		const onResponseLine = (line: string): void => {
			if (
				!line.startsWith(RESPONSE_DATA_PREFIX) ||
				line === RESPONSE_DONE_PREFIX
			) {
				return;
			}

			const completionResponse = parseCompletionResponse(
				line.slice(RESPONSE_DATA_PREFIX.length)
			);
...
function parseCompletionResponse(
	data: string
): AnthropicCompletionResponse | null {
	try {
...
function iterateLines(
	...
	const isPartialLastLine =
		lastLine && lastLine.endsWith(buffer[buffer.length - 1]);
