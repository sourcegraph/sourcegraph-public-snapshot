...
var coverageTemplate = template.Must(template.New("").Parse(`<html>
<head>
<style>
  .success {
...
{{range .Langs}}
<tr><th style="text-align: left;" colspan="6">{{.Lang}}</th></tr>
{{range .RepoCov}}
{{if .SuccessfullyBuilt}}
<tr>
<td><a href="{{$endpoint}}/{{.Repo}}">{{.Repo}}</a></td>
