...
func (c *coverageCmd) Execute(args []string) error {
	...
	for _, repo := range langCov.Repos {
		...
		c.logf("\t\tmin. response time: %v", coverage.MinDuration(repo.DefDurations))
		c.logf("\t\tmax. response time: %v", coverage.MaxDuration(repo.DefDurations))
		...
		c.logf("\t\tmin. response time: %v", coverage.MinDuration(repo.HoverDurations))
		c.logf("\t\tmax. response time: %v", coverage.MaxDuration(repo.HoverDurations))
