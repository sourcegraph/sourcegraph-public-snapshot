...
func (o *orgSettings) CreateIfUpToDate(ctx context.Context, orgID int32, lastKnownSettingsID *int32, authorAuth0ID, contents string) (latestSetting *sourcegraph.OrgSettings, err error) {
	...
	if latestSetting == nil || creatorIsUpToDate {
		err := tx.QueryRow(
			"INSERT INTO org_settings(org_id, author_uid, contents) VALUES($1, $2, $3) RETURNING id, created_at",
...
const getLatestByOrgIDSql = "SELECT id, org_id, author_uid, contents, created_at FROM org_settings WHERE org_id = $1 ORDER BY id DESC LIMIT 1"
