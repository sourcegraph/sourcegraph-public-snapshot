...
func (u *users) GetByAuth0ID(ctx context.Context, id string) (*sourcegraph.User, error) {
	...
	return u.getOneBySQL(ctx, "WHERE uid=$1 AND deleted_at IS NULL LIMIT 1", id)
}
...
func (u *users) GetByCurrentAuthUser(ctx context.Context) (*sourcegraph.User, error) {
	...
	return u.getOneBySQL(ctx, "WHERE uid=$1 AND deleted_at IS NULL LIMIT 1", actor.UID)
}
...
func (*users) getBySQL(ctx context.Context, query string, args ...interface{}) ([]*sourcegraph.User, error) {
	rows, err := globalDB.QueryContext(ctx, "SELECT u.id, u.uid, u.email, u.username, u.display_name, u.provider, u.avatar_url, u.created_at, u.updated_at FROM users u "+query, args...)
	if err != nil {
		return nil, err
	}
