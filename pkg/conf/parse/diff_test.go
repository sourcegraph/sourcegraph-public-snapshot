...
func TestDiff(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "slice_diff",
			before: &schema.SiteConfiguration{AppURL: "a", ReposList: []*schema.Repository{{Path: "a"}}},
			after:  &schema.SiteConfiguration{AppURL: "a", ReposList: []*schema.Repository{{Path: "b"}}},
			want:   []string{"repos.list"},
		},
		{
			name:   "slice_nodiff",
			before: &schema.SiteConfiguration{AppURL: "a", ReposList: []*schema.Repository{{Path: "a"}}},
			after:  &schema.SiteConfiguration{AppURL: "a", ReposList: []*schema.Repository{{Path: "a"}}},
			want:   nil,
		},
		{
			name:   "multi_diff",
			before: &schema.SiteConfiguration{AppURL: "a", ReposList: []*schema.Repository{{Path: "b"}}},
			after:  &schema.SiteConfiguration{AppURL: "b", ReposList: []*schema.Repository{{Path: "a"}}},
