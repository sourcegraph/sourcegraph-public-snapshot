...
func configFromEnv() (base *schema.BasicSiteConfiguration, core *schema.CoreSiteConfiguration, envVarNames []string, err error) {

	var generateEnvConfig = func(configType reflect.Type, configVal reflect.Value) error {
		for i := 0; i < configType.NumField(); i++ {
			...
			if envVal != "" {
				...
				default:
					return fmt.Errorf("unhandled config field type: %s (field: %s)", valField.Kind(), typeField.Name)
				}
			}
	...
	err = generateEnvConfig(reflect.TypeOf(base), reflect.ValueOf(&base))
	if err != nil {
		return nil, nil, nil, errors.Wrap(err, "when generating basic configuration from environment variables")
	}

	err = generateEnvConfig(reflect.TypeOf(core), reflect.ValueOf(&core))
	if err != nil {
		return nil, nil, nil, errors.Wrap(err, "when generating core configuration from environment variables")
	}
	...
	if prd := os.Getenv("PUBLIC_REPO_REDIRECTS"); prd != "" {
		if publicRepoRedirects, err := strconv.ParseBool(prd); err == nil {
			base.DisablePublicRepoRedirects = !publicRepoRedirects
		}
	}

	return base, core, envVarNames, err
}
