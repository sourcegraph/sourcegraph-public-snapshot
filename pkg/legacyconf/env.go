...
import (
	...
	schema "github.com/sourcegraph/sourcegraph/legacyschema"
...
func configFromEnv() (configJSON []byte, envVarNames []string, err error) {
	...
	for i := 0; i < configType.NumField(); i++ {
		...
		if envVal == "" {
			...
			if legacyEnvName := legacyEnvToFieldName[typeField.Name]; legacyEnvName != "" {
				...
				if envVal != "" {
					logDeprecated(legacyEnvName, envVal)
	...
	if prd := os.Getenv("PUBLIC_REPO_REDIRECTS"); prd != "" {
		logDeprecated("PUBLIC_REPO_REDIRECTS", prd)
...
func logDeprecated(name, value string) {
	log15.Warn("detected deprecated env var", "name", name)
	log15.Warn("You should set this value in the site config or management console.")
}
