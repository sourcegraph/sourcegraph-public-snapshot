...
// New returns a new http.Client from the factory with the given opts
// applied to it.
func (f Factory) New(opts ...Opt) (Doer, error) {
	return f(opts...)
}

// NewFactory returns a Factory that applies the given base
// Opts before the ones provided on each invocation of New.
//
// If the given Middleware stack is not nil, the final configured client
// will be wrapped by it before being returned.
func NewFactory(stack Middleware, base ...Opt) Factory {
	return func(runtime ...Opt) (Doer, error) {
		opts := make([]Opt, 0, len(base)+len(runtime))
		opts = append(opts, base...)
		opts = append(opts, runtime...)
		...
		return stack(&cli), err.ErrorOrNil()
	}
}
