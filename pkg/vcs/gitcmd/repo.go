...
var (
	...
	gitCmdWhitelist = map[string][]string{
		"log":    []string{"--name-status", "--full-history", "-M", "--date", "--format", "-i", "-n1", "-m", "--", "-n200", "-n2", "--follow", "--author", "--grep", "--date-order", "--decorate", "--skip", "--max-count", "--numstat", "--pretty", "--parents", "--topo-order", "--raw", "--follow", "--all", "--before"},
		"show":   []string{},
		"remote": []string{"-v"},
	}
)
...
func (r *Repository) String() string {
	return fmt.Sprintf("git repo %s", r.Repo.URI)
}

func Open(repo *sourcegraph.Repo) *Repository {
