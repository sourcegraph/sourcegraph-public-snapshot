...
allSvgsOptimized() {
  for file in ./ui/assets/img/*.svg; do
    ...
    diff -w -q "$file" <(pnpm --silent run optimize-svg-assets -i "$file" -o -) || return 1
  done
}

if ! allSvgsOptimized; then
  echo 'Found SVG optimizations. Please run "pnpm optimize-svg-assets ui/assets/img" and commit the result.'
  exit 1
fi
