...
var (
	...
	ciCommand = &ffcli.Command{
		Name:       "ci",
		ShortUsage: "sg ci [preview|status|trigger]",
...
Note that Sourcegraph's CI pipelines are under our enterprise license: https://github.com/sourcegraph/sourcegraph/blob/main/LICENSE.enterprise`,
		...
		Subcommands: []*ffcli.Command{{
			...
			Exec: func(ctx context.Context, args []string) error {
				...
				if err != nil {
					return fmt.Errorf("failed to trigger build for %q at %q: %w", branch, commit, err)
				}
				out.WriteLine(output.Linef(output.EmojiSuccess, nil, "Created build: %s", *build.WebURL))
				return nil
			},
		}},
	}
