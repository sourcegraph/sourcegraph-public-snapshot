...
Note that Sourcegraph's CI pipelines are under our enterprise license: https://github.com/sourcegraph/sourcegraph/blob/main/LICENSE.enterprise`,
		...
		Subcommands: []*ffcli.Command{{
			...
			LongHelp:  "Manually request a Buildkite build for the currently checked out commit and branch. Optionally provide a special build tag (https://docs.sourcegraph.com/dev/background-information/ci/reference). This is most useful when triggering builds for PRs from forks (such as those from external contributors), which do not trigger Buildkite builds automatically for security reasons (we do not want to run insecure code on our infrastructure by default!)",
			Exec: func(ctx context.Context, args []string) error {
				client, err := bk.NewClient(ctx, stdout.Out)
				if err != nil {
					return err
