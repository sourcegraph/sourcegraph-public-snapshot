...
var allowDocChangesPrefixAllowlist = []string{
	"doc/cloud/",
}
...
func docChangesLint() *linter {
	return runCheck("Stale doc check", func(ctx context.Context, out *std.Output, state *repo.State) error {
		...
		cmd := []string{"bazel", "cquery", `"filter("\.md", deps(//dev/tools:docsite union
			//doc-legacy/admin/observability:doc_files union
        	//doc-legacy/cli/references:doc_files union
        	//doc-legacy/dev/background-information/telemetry:doc_files))"`, "--output=files"}
