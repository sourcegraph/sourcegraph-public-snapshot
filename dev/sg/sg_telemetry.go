...
func addAllowList(ctx context.Context, cmd *cli.Command) (err error) {
	events := cmd.Args().Slice()
	if len(events) == 0 {
		return cli.Exit("no events provided", 1)
	}

	return editAllowlist(cmd, events, false)
}

func removeAllowList(ctx context.Context, cmd *cli.Command) (err error) {
	events := cmd.Args().Slice()
	if len(events) == 0 {
		return cli.Exit("no events provided", 1)
	}

	return editAllowlist(cmd, events, true)
}

func editAllowlist(cmd *cli.Command, events []string, reverse bool) error {
	header := fmt.Sprintf("-- This migration was generated by the command `sg telemetry %s`", cmd.FullName())
