...
func SquashAll(database db.Database, inContainer, runInTimescaleDBContainer, skipTeardown bool, filepath string) error {
	...
	squashedUpMigration, _, err := generateSquashedMigrations(database, leafIDs, inContainer, runInTimescaleDBContainer, skipTeardown)
	if err != nil {
		return err
	}
...
func Squash(database db.Database, commit string, inContainer, runInTimescaleDBContainer, skipTeardown bool) error {
	definitions, err := readDefinitions(database)
	if err != nil {
		return err
	}
	...
	squashedUpMigration, squashedDownMigration, err := generateSquashedMigrations(database, []int{newRoot.ID}, inContainer, runInTimescaleDBContainer, skipTeardown)
	if err != nil {
		return err
	}
...
func generateSquashedMigrations(database db.Database, targetVersions []int, inContainer, runInTimescaleDBContainer, skipTeardown bool) (up, down string, err error) {
