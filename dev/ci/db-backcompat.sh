...
if [ "$LATEST_SCHEMA" != "$CURRENT_DB_SCHEMA" ]; then
    ...
    echo '    You can run `GOCACHE=off go test -v ./cmd/frontend/internal/db/  -run=TestMigrations` to update the test DB schema.'
    exit 1
fi
...
function runTest() {
	...
	EOF
        ...
        GOCACHE=off SKIP_MIGRATION_TEST=true go test -v ./cmd/frontend/internal/db/
        set +ex
