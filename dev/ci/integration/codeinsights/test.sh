...
export NAMESPACE="codeinsights-$BUILDKITE_BUILD_NUMBER-$BUILDKITE_JOB_ID"
...
function test_setup() {
  ...
  for p in $PODS; do
    i=0
    until kubectl logs "$p" -c frontend -n "$NAMESPACE" | grep "Sourcegraph is ready at" || $i <= 5; do
...
test_setup
set +o pipefail
# special exit code to capture qa failures
qa || exit 123
