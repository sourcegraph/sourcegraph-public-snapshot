...
Schema :: {
	...
	on: def.event | [def.event, ...def.event] | {
	...
	jobs: [=~"^[_a-zA-Z][a-zA-Z0-9_-]*$" & !~"^()$"]: {
		...
		needs?: [...def.name] & list.MinItems(1) | def.name
		...
		strategy?: {
			...
			matrix: struct.MinFields(1) &
				{
					[=~"^(in|ex)clude$" & !~"^()$"]: [...{
						[string]: def.configuration
					}] & list.MinItems(1)

					[!~"^(in|ex)clude$" & !~"^()$"]: [...def.configuration] & list.MinItems(1)
				}
...
def: step :: {
	...
	shell?: string & ("{$comment: \"https://help.github.com/en/actions/automating-your-workflow-with-github-actions/workflow-syntax-for-github-actions#custom-shell\"}" | ("bash" | "pwsh" | "python" | "sh" | "cmd" | "powershell"))
