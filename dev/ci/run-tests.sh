...
i=0
cmds=("./dev/gofmt.sh" "(cd ui; yarn run test)" "make check" "./dev/ci/run-checkup.sh" "(cd client/browser-ext; yarn && yarn run test && yarn run build && yarn run test)" "(cd xlang/javascript-typescript/buildserver && yarn && ./node_modules/.bin/tsc && yarn run fmt-check && yarn test)" "go test -v -timeout 5m ./xlang")
for cmd in "${cmds[@]}"; do
