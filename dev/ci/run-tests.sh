...
cmds=(
	...
	"(cd client/browser-ext; yarn && yarn run build)"
	"./xlang/php/test.sh"
	"(cd xlang/javascript-typescript/buildserver && yarn && ./node_modules/.bin/tsc && yarn run fmt-check && yarn test)"
	"go test -v -timeout 5m ./xlang"
)
for cmd in "${cmds[@]}"; do
