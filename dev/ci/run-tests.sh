...
i=0
cmds=("./dev/gofmt.sh" "(cd ui; npm run dep && npm test)" "make check" "./dev/ci/run-checkup.sh" "(cd client/browser-ext; yarn install && yarn run test && yarn run build && yarn run test)" "./xlang/php/test.sh") 
for cmd in "${cmds[@]}"; do
