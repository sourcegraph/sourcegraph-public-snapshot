...
func main() {
	...
	for {
		req, err := http.NewRequest("POST", os.ExpandEnv(fmt.Sprintf("https://api.buildkite.com/v2/organizations/sourcegraph/pipelines/enterprise/builds/%s?access_token=$BUILDKITE_TOKEN", build.Number)), nil)
		if err != nil {
			panic(err)
		}
		resp, err := http.DefaultClient.Do(req)
