...
# apply over old code.
bazel run //dev/ci/go-backcompat:go-backcompat --run_under="cd $PWD &&" -- "${MIGRATION_STAGING}"
...
echo "--- run tests"
# Run configure again, because the migrations changed and they are test targets.
bazel run :gazelle

if ! (bazel query 'kind(go_test, //...)' | xargs bazel test) then
  annotation=$(
    cat <<EOF
