...
func GeneratePipeline(c Config) (*bk.Pipeline, error) {
	...
	default:
		// Executor VM image
		alwaysRebuild := false // c.MessageFlags.SkipHashCompare || c.RunType.Is(runtype.ReleaseBranch, runtype.TaggedRelease, runtype.InternalRelease) || c.Diff.Has(changed.ExecutorVMImage)
		// Slow image builds
		imageBuildOps := operations.NewNamedSet("Image builds")
