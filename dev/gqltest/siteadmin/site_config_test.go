...
func TestSiteConfig(t *testing.T) {
	t.Run("builtin auth provider: allowSignup", func(t *testing.T) {
		...
		testClient1, err := gqltestutil.NewClient(*gqltest.BaseURL)
		require.NoError(t, err)
		require.NoError(t, testClient1.SignUp(testUsername1+"@sourcegraph.com", testUsername1, "mysecurepassword"))
		gqltest.RemoveTestUserAfterTest(t, testClient1.AuthenticatedUserID())
		...
		err = gqltestutil.Retry(5*time.Second, func() error {
			...
			testClient2, err := gqltestutil.NewClient(*gqltest.BaseURL)
			require.NoError(t, err)
			...
			gqltest.RemoveTestUserAfterTest(t, testClient2.AuthenticatedUserID())
