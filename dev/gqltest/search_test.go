	...
	t.Run("And/Or queries", func(t *testing.T) {
		tests := []struct {
			...
			},
			{
				...
				query: `repo:^github\.com/sgtest/go-diff$ (not .svg) stable:yes type:file count:1 patterntype:literal`,
			},
			{
				name:  `Literals, not keyword and implicit and inside group`,
				query: `repo:^github\.com/sgtest/go-diff$ (a/foo not .svg) stable:yes type:file count:1 patterntype:literal`,
			},
			{
				name:  `Literals, not and and keyword inside group`,
				query: `repo:^github\.com/sgtest/go-diff$ (a/foo and not .svg) stable:yes type:file count:1 patterntype:literal`,
			},
			{
