...
func main() {
	...
	oldTargetSpawns := make(map[string][]*proto.SpawnExec)
	newTargetSpawns := make(map[string][]*proto.SpawnExec)
	...
	keys := maps.Keys(oldTargetSpawns)
	slices.Sort(keys)
	fmt.Println(keys)
	...
	keys = maps.Keys(newTargetSpawns)
	slices.Sort(keys)
	fmt.Println(keys)
