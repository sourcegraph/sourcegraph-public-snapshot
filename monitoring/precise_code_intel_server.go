		...
		Groups: []Group{
					...
					{
						{
							...
							// TODO(efritz) - ensure these exclude error durations
							Query:             `histogram_quantile(0.99, sum by (le)(rate(src_code_intel_db_duration_seconds_bucket{job="precise-code-intel-server"}[5m])))`,
							DataMayNotExist:   true,
							DataMayBeNaN:      true,
							Warning:           Alert{GreaterOrEqual: 20},
						...
						},
						{
							...
							Query:             `increase(src_code_intel_db_errors_total{job="precise-code-intel-server"}[5m])`,
							DataMayNotExist:   true,
							Warning:           Alert{GreaterOrEqual: 20},
					...
					{
						{
							...
							Query:             `sum(increase(src_precise_code_intel_server_janitor_errors_total[5m]))`,
							DataMayNotExist:   true,
							Warning:           Alert{GreaterOrEqual: 20},
						...
						},
						{
							...
							Query:             `sum(increase(src_precise_code_intel_server_janitor_upload_records_removed_total[5m]))`,
							DataMayNotExist:   true,
							Warning:           Alert{GreaterOrEqual: 20},
			...
			},
			{
				...
				Rows: []Row{
					{
						{
							...
							Query:             `histogram_quantile(0.99, sum by (le,category)(rate(src_precise_code_intel_bundle_manager_request_duration_seconds_bucket{job="precise-code-intel-server",category!="transfer"}[5m])))`,
							DataMayNotExist:   true,
							DataMayBeNaN:      true,
							Warning:           Alert{GreaterOrEqual: 20},
						...
						},
						{
							...
							Query:             `sum by (category)(increase(src_precise_code_intel_bundle_manager_request_duration_seconds_count{job="precise-code-intel-server",code!~"2.."}[5m]))`,
							DataMayNotExist:   true,
					...
					{
						{
							...
							Query:             `histogram_quantile(0.99, sum by (le,category)(rate(src_gitserver_request_duration_seconds_bucket{job="precise-code-intel-server"}[5m])))`,
							DataMayNotExist:   true,
							DataMayBeNaN:      true,
							Warning:           Alert{GreaterOrEqual: 20},
							PanelOptions:      PanelOptions().LegendFormat("{{category}}").Unit(Seconds),
						...
						},
						{
							...
							Query:             `sum by (category)(increase(src_gitserver_request_duration_seconds_count{job="precise-code-intel-server",code!~"2.."}[5m]))`,
							DataMayNotExist:   true,
							Warning:           Alert{GreaterOrEqual: 5},
							PanelOptions:      PanelOptions().LegendFormat("{{category}}"),
							PossibleSolutions: "none",
					...
					{
						sharedFrontendInternalAPIErrorResponses("precise-code-intel-server"),
					},
				},
			},
			{
				...
				Rows: []Row{
					{
						sharedContainerRestarts("precise-code-intel-server"),
						sharedContainerMemoryUsage("precise-code-intel-server"),
						sharedContainerCPUUsage("precise-code-intel-server"),
					},
				},
			},
		},
