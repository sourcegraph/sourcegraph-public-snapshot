					...
					{
						{
							...
							Query:             `histogram_quantile(0.99, sum by (le,op)(rate(src_apiworker_apiclient_duration_seconds_bucket{job="sourcegraph-code-intel-indexers"}[5m])))`,
							DataMayNotExist:   true,
							Warning:           monitoring.Alert().GreaterOrEqual(20),
						...
						},
						{
							...
							Query:             `sum by (op)(increase(src_apiworker_apiclient_errors_total{job="sourcegraph-code-intel-indexers"}[5m]))`,
							DataMayNotExist:   true,
							Warning:           monitoring.Alert().GreaterOrEqual(20),
					...
					{
						{
							...
							Query:             `histogram_quantile(0.99, sum by (le,op)(rate(src_apiworker_command_duration_seconds_bucket{job="sourcegraph-code-intel-indexers", op=~"setup.*"}[5m])))`,
							DataMayNotExist:   true,
							NoAlert:           true,
							PanelOptions:      monitoring.PanelOptions().LegendFormat("{{op}}").Unit(monitoring.Seconds),
							Owner:             monitoring.ObservableOwnerCodeIntel,
						...
						},
						{
							...
							Query:             `sum by (op)(increase(src_apiworker_command_errors_total{job="sourcegraph-code-intel-indexers", op=~"setup.*"}[5m]))`,
							DataMayNotExist:   true,
							Warning:           monitoring.Alert().GreaterOrEqual(20),
							PanelOptions:      monitoring.PanelOptions().LegendFormat("{{op}}"),
							Owner:             monitoring.ObservableOwnerCodeIntel,
					...
					{
						{
							...
							Query:             `histogram_quantile(0.99, sum by (le,op)(rate(src_apiworker_command_duration_seconds_bucket{job="sourcegraph-code-intel-indexers", op=~"exec.*"}[5m])))`,
							DataMayNotExist:   true,
							NoAlert:           true,
							PanelOptions:      monitoring.PanelOptions().LegendFormat("{{op}}").Unit(monitoring.Seconds),
							Owner:             monitoring.ObservableOwnerCodeIntel,
						...
						},
						{
							...
							Query:             `sum by (op)(increase(src_apiworker_command_errors_total{job="sourcegraph-code-intel-indexers", op=~"exec.*"}[5m]))`,
							DataMayNotExist:   true,
							Warning:           monitoring.Alert().GreaterOrEqual(20),
							PanelOptions:      monitoring.PanelOptions().LegendFormat("{{op}}"),
							Owner:             monitoring.ObservableOwnerCodeIntel,
					...
					{
						{
							...
							Query:             `histogram_quantile(0.99, sum by (le,op)(rate(src_apiworker_teardown_command_duration_seconds_bucket{job="sourcegraph-code-intel-indexers", op=~"teardown.*"}[5m])))`,
							DataMayNotExist:   true,
							NoAlert:           true,
							PanelOptions:      monitoring.PanelOptions().LegendFormat("{{op}}").Unit(monitoring.Seconds),
							Owner:             monitoring.ObservableOwnerCodeIntel,
						...
						},
						{
							...
							Query:             `sum by (op)(increase(src_apiworker_teardown_command_errors_total{job="sourcegraph-code-intel-indexers", op=~"teardown.*"}[5m]))`,
							DataMayNotExist:   true,
							Warning:           monitoring.Alert().GreaterOrEqual(20),
							PanelOptions:      monitoring.PanelOptions().LegendFormat("{{op}}"),
							Owner:             monitoring.ObservableOwnerCodeIntel,
