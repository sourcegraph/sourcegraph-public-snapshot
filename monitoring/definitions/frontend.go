					...
					{
						{
							...
							Description:       "number of dirty repositories",
							Query:             `max(src_dirty_repositories_total)`,
							DataMayNotExist:   true,
							Warning:           monitoring.Alert().GreaterOrEqual(100),
							PanelOptions:      monitoring.PanelOptions().LegendFormat("number of dirty repositories"),
							Owner:             monitoring.ObservableOwnerCodeIntel,
							PossibleSolutions: "none",
						},
						{
							Name:            "dirty_repositories_growth_rate",
							Description:     "dirty repositories growth rate every 5m",
							Query:           `sum(increase(src_dirty_repositories_total[30m])) / sum(increase(src_codeintel_commit_updater_total[30m]))`,
							DataMayNotExist: true,

							Warning:           monitoring.Alert().GreaterOrEqual(5),
							PanelOptions:      monitoring.PanelOptions().LegendFormat("dirty repositories growth rate"),
							Owner:             monitoring.ObservableOwnerCodeIntel,
							PossibleSolutions: "none",
						},
						{
							Name:              "codeintel_commit_updater_99th_percentile_duration",
							Description:       "99th percentile successful commit updater operation duration over 5m",
							Query:             `histogram_quantile(0.99, sum by (le)(rate(src_codeintel_commit_updater_duration_seconds_bucket{job=~"(sourcegraph-)?frontend"}[5m])))`,
							DataMayNotExist:   true,
							Warning:           monitoring.Alert().GreaterOrEqual(20),
						...
						{
							Name:              "codeintel_commit_updater_errors",
							Description:       "commit updater errors every 5m",
							Query:             `sum(increase(src_codeintel_commit_updater_errors_total{job=~"(sourcegraph-)?frontend"}[5m]))`,
							DataMayNotExist:   true,
							Warning:           monitoring.Alert().GreaterOrEqual(20),
							PanelOptions:      monitoring.PanelOptions().LegendFormat("error"),
							Owner:             monitoring.ObservableOwnerCodeIntel,
					...
					{
						{
							...
							DataMayNotExist:   true,
							Warning:           monitoring.Alert().GreaterOrEqual(20),
							PanelOptions:      monitoring.PanelOptions().LegendFormat("upload records removed"),
							Owner:             monitoring.ObservableOwnerCodeIntel,
							PossibleSolutions: "none",
						},
						{
							...
							DataMayNotExist:   true,
							Warning:           monitoring.Alert().GreaterOrEqual(20),
							PanelOptions:      monitoring.PanelOptions().LegendFormat("index records removed"),
							Owner:             monitoring.ObservableOwnerCodeIntel,
							PossibleSolutions: "none",
						},
						{
							Name:              "codeintel_uploads_purged",
							Description:       "uploads purged every 5m",
						...
						},
						{
							...
							Description:       "background process errors every 5m",
