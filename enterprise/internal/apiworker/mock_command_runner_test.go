...
import (
	"context"
	command "github.com/sourcegraph/sourcegraph/enterprise/internal/apiworker/command"
	"sync"
)
...
// parameter and result values of this invocation.
func (m *MockRunner) Setup(v0 context.Context, v1 []string) error {
	r0 := m.SetupFunc.nextHook()(v0, v1)
	m.SetupFunc.appendCall(RunnerSetupFuncCall{v0, v1, r0})
	return r0
}

// SetDefaultHook sets function that is called when the Setup method of the
// parent MockRunner instance is invoked and the hook queue is empty.
func (f *RunnerSetupFunc) SetDefaultHook(hook func(context.Context, []string) error) {
	f.defaultHook = hook
}
...
// front of the queue and discards it. After the queue is empty, the default
// hook function is invoked for any future action.
func (f *RunnerSetupFunc) PushHook(hook func(context.Context, []string) error) {
	f.mutex.Lock()
	f.hooks = append(f.hooks, hook)
	f.mutex.Unlock()
}
...
func (f *RunnerSetupFunc) nextHook() func(context.Context, []string) error {
	f.mutex.Lock()
	defer f.mutex.Unlock()
