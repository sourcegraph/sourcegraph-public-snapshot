...
func NewMockClient() *MockClient {
	return &MockClient{
		...
		WithTokenFunc: &ClientWithTokenFunc{
			defaultHook: func(*auth.OAuthBearerToken) (r0 client) {
				return
			},
		},
	}
...
func NewStrictMockClient() *MockClient {
	return &MockClient{
		...
		WithTokenFunc: &ClientWithTokenFunc{
			defaultHook: func(*auth.OAuthBearerToken) client {
...
type surrogateMockClient interface {
	...
	WithToken(*auth.OAuthBearerToken) client
}
...
type ClientWithTokenFunc struct {
	defaultHook func(*auth.OAuthBearerToken) client
	hooks       []func(*auth.OAuthBearerToken) client
...
func (m *MockClient) WithToken(v0 *auth.OAuthBearerToken) client {
...
func (f *ClientWithTokenFunc) SetDefaultHook(hook func(*auth.OAuthBearerToken) client) {
	f.defaultHook = hook
}
...
// default hook function is invoked for any future action.
func (f *ClientWithTokenFunc) PushHook(hook func(*auth.OAuthBearerToken) client) {
	f.mutex.Lock()
	f.hooks = append(f.hooks, hook)
	f.mutex.Unlock()
}
...
func (f *ClientWithTokenFunc) SetDefaultReturn(r0 client) {
	f.SetDefaultHook(func(*auth.OAuthBearerToken) client {
		return r0
	})
}
...
func (f *ClientWithTokenFunc) PushReturn(r0 client) {
	f.PushHook(func(*auth.OAuthBearerToken) client {
		return r0
	})
}

func (f *ClientWithTokenFunc) nextHook() func(*auth.OAuthBearerToken) client {
	f.mutex.Lock()
	defer f.mutex.Unlock()
