...
type client interface {
	...
	WithToken(token *auth.OAuthBearerToken) client
}
...
func (c *ClientAdapter) WithToken(token *auth.OAuthBearerToken) client {
	return &ClientAdapter{
		V3Client: c.V3Client.WithAuthenticator(token),
	}
}
