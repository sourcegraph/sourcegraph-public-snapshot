...
func newAppProvider(
	...
	return &Provider{
		...
		client: func() (client, error) {
			token, err := repos.GetOrRenewGitHubAppInstallationAccessToken(context.Background(), log.Scoped("newAppProvider", ""), externalServicesStore, svc, appClient, installationID)
			if err != nil {
