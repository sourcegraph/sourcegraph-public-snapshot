...
func ProvidersFromConfig(
	...
	userEmailsStore UserEmailsStore, // Used by Perforce authz providers
) (
	...
	for {
		svcs, err := externalServicesStore.List(ctx, opt)
		if err != nil {
	...
	if len(perforceConns) > 0 {
		pfProviders, pfProblems, pfWarnings := perforce.NewAuthzProviders(userEmailsStore, perforceConns)
