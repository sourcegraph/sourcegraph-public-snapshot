...
func (s *store) ScanDocuments(
	ctx context.Context,
	id int,
	repo, root string,
	populateDefsAndRefs func(ctx context.Context, id int, repo, root, path string, document *scip.Document) error,
) (err error) {
	...
	rows, err := s.db.Query(ctx, sqlf.Sprintf(scanDocumentsQuery, id))
	if err != nil {
		return err
	}
	...
	for rows.Next() {
		...
		err = populateDefsAndRefs(ctx, id, repo, root, path, &document)
		if err != nil {
			return err
		}
	}
