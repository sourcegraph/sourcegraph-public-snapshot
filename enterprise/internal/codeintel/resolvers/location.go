...
func (r *locationConnectionResolver) Nodes(ctx context.Context) ([]graphqlbackend.LocationResolver, error) {
	...
	for _, location := range r.locations {
		// TODO - should come back from lsif-server with dump id
		// so we know if we should be deleting this particular data.

		treeResolver, err := collectionResolver.resolve(ctx, newTreeResolver(nil), location.Repository, location.Commit, location.Path)
		if err != nil {
			return nil, err
		}
