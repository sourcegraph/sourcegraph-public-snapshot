...
func (r *repositoryCollectionResolver) resolve(ctx context.Context, treeResolver *treeResolver, repoName, commit, path string) (*graphqlbackend.GitTreeEntryResolver, error) {
	commitCollectionResolver, err := r.resolveRepository(ctx, treeResolver, repoName)
	if err != nil {
		return nil, err
	}

	pathCollectionResolver, err := commitCollectionResolver.resolveCommit(ctx, treeResolver, commit)
	if err != nil {
		return nil, err
	}

	return pathCollectionResolver.resolvePath(ctx, treeResolver, path)
}

// resolveRepository returns a commitCollectionResolver with the given resolved repository.
func (r *repositoryCollectionResolver) resolveRepository(ctx context.Context, treeResolver *treeResolver, repoName string) (*commitCollectionResolver, error) {
	if payload, ok := r.commitCollectionResolvers[repoName]; ok {
		return payload, nil
	}

	repositoryResolver, err := treeResolver.resolveRepository(ctx, repoName)
	if err != nil {
		return nil, err
	}
...
func (r *commitCollectionResolver) resolveCommit(ctx context.Context, treeResolver *treeResolver, commit string) (*pathCollectionResolver, error) {
	if resolver, ok := r.pathCollectionResolvers[commit]; ok {
		return resolver, nil
	}

	commitResolver, err := treeResolver.resolveCommitFrom(ctx, r.repositoryResolver, commit)
	if err != nil {
		return nil, err
	}
...
func (r *pathCollectionResolver) resolvePath(ctx context.Context, treeResolver *treeResolver, path string) (*graphqlbackend.GitTreeEntryResolver, error) {
	if resolver, ok := r.pathResolvers[path]; ok {
		return resolver, nil
	}

	resolver, err := treeResolver.resolvePathFrom(ctx, r.commitResolver, path)
	if err != nil {
		return nil, err
	}
