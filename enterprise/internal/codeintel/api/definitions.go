...
func (api *codeIntelAPI) definitionsRaw(ctx context.Context, dump store.Dump, pathInBundle string, line, character int) ([]ResolvedLocation, error) {
	locations, err := api.bundleManagerClient.BundleClient(dump.ID).Definitions(ctx, pathInBundle, line, character)
	if err != nil {
	...
	rangeMonikers, err := api.bundleManagerClient.BundleClient(dump.ID).MonikersByPosition(context.Background(), pathInBundle, line, character)
	if err != nil {
		if err == bundles.ErrNotFound {
			log15.Warn("Bundle does not exist")
			return nil, nil
	...
	for _, monikers := range rangeMonikers {
		for _, moniker := range monikers {
			if moniker.Kind == "import" {
				...
				locations, _, err := api.bundleManagerClient.BundleClient(dump.ID).MonikerResults(context.Background(), "definition", moniker.Scheme, moniker.Identifier, 0, DefintionMonikersLimit)
				if err != nil {
					if err == bundles.ErrNotFound {
						log15.Warn("Bundle does not exist")
						return nil, nil
