...
func lookupMoniker(
	...
	pid, err := bundleManagerClient.BundleClient().PackageInformation(context.Background(), dumpID, path, moniker.PackageInformationID)
	if err != nil {
	...
	locations, count, err := bundleManagerClient.BundleClient().MonikerResults(context.Background(), dump.ID, modelType, moniker.Scheme, moniker.Identifier, skip, take)
	if err != nil {
		if err == bundles.ErrNotFound {
			log15.Warn("Bundle does not exist")
			return nil, 0, nil
