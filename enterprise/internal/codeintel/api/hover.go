...
func (api *codeIntelAPI) Hover(ctx context.Context, file string, line, character, uploadID int) (string, bundles.Range, bool, error) {
	...
	text, rn, exists, err := api.bundleManagerClient.BundleClient().Hover(ctx, dump.ID, pathInBundle, line, character)
	if err != nil {
	...
	text, rn, exists, err = api.bundleManagerClient.BundleClient().Hover(ctx, definition.Dump.ID, pathInDefinitionBundle, definition.Range.Start.Line, definition.Range.Start.Character)
	if err != nil {
		if err == bundles.ErrNotFound {
			log15.Warn("Bundle does not exist")
			return "", bundles.Range{}, false, nil
