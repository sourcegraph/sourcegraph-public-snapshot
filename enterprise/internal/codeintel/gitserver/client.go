...
func (c *Client) DirectoryChildren(ctx context.Context, repositoryID int, commit string, dirnames []string) (_ map[string][]string, err error) {
	...
	children, err := c.listDirectoryChildren(ctx, c.db, authz.DefaultSubRepoPermsChecker, repo, api.CommitID(commit), dirnames)
	if err == nil {
		return children, err
	}
...
func (c *Client) listDirectoryChildren(
	...
	args = append(args, c.cleanDirectoriesForLsTree(dirnames)...)
	...
	return c.parseDirectoryChildren(dirnames, paths), nil
}
...
func (c *Client) cleanDirectoriesForLsTree(dirnames []string) []string {
	var args []string
...
func (c *Client) parseDirectoryChildren(dirnames, paths []string) map[string][]string {
