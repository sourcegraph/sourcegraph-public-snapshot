...
type lsifRequest struct {
	...
	// (Optional) used in routing to select the "hot" lsif-api that
	// was used in recent requests for similar data. Requests that
	// are likely to open the same dump should hit the same server
	// so that the SQLite database is already in the cache.
	routingKey string
}
...
// buildURL constructs a URL to the backend LSIF api with the given path and query
// values. If the provided cursor is non-nil, that will be used instead of the given
// path. If path is relative (indicated by a leading slash), then the configured
// LSIF API url is prepended. Otherwise, it is treated as an absolute URL. The given
// query values will override any query string that is present in the given path.
//
