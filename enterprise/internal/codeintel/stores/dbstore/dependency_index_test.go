...
func TestInsertDependencyIndexingQueueingJob(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	db := dbtesting.GetDB(t)
	...
	// No error if upload exists
	if _, err := store.InsertDependencyIndexingQueueingJob(context.Background(), uploadID, "asdf", time.Now()); err != nil {
		t.Fatalf("unexpected error enqueueing dependency index queueing job: %s", err)
	}

	// Error with unknown identifier
	if _, err := store.InsertDependencyIndexingQueueingJob(context.Background(), uploadID+1, "asdf", time.Now()); err == nil {
