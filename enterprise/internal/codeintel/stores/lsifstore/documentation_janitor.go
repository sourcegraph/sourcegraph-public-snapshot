...
func (s *Store) DeleteOldPublicSearchRecords(ctx context.Context, minimumTimeSinceLastCheck time.Duration, limit int) (err error) {
	return s.deleteOldSearchRecords(ctx, minimumTimeSinceLastCheck, limit, "public", timeutil.Now())
}

func (s *Store) DeleteOldPrivateSearchRecords(ctx context.Context, minimumTimeSinceLastCheck time.Duration, limit int) (err error) {
	return s.deleteOldSearchRecords(ctx, minimumTimeSinceLastCheck, limit, "private", timeutil.Now())
}

func (s *Store) deleteOldSearchRecords(ctx context.Context, minimumTimeSinceLastCheck time.Duration, limit int, tableSuffix string, now time.Time) (err error) {
	ctx, traceLog, endObservation := s.operations.deleteOldSearchRecords.WithAndLogger(ctx, &err, observation.Args{})
	...
	traceLog(log.Int("numRecords", numRecords))
