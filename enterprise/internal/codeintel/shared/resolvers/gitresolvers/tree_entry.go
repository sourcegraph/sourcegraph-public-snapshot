...
func (r *treeEntryResolver) Content(ctx context.Context, args *resolvers.GitTreeContentPageArgs) (string, error) {
	...
	content, err := gitserver.NewClient().ReadFile(ctx, authz.DefaultSubRepoPermsChecker, api.RepoName(r.commit.Repository().Name()), api.CommitID(r.commit.OID()), r.path, false)
	if err != nil {
		return "", err
	}
