...
func TestExecutor_LoadAuthenticator(t *testing.T) {
	...
	t.Run("owned by admin user with credential", func(t *testing.T) {
		...
		if _, err := store.UserCredentialsStore().Create(ctx, db.UserCredentialScope{
	...
	for i, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.credentials != nil {
				cred, err := store.UserCredentialsStore().Create(ctx, db.UserCredentialScope{
					Domain:              db.UserCredentialDomainCampaigns,
					UserID:              tt.user.ID,
				...
				defer func() { store.UserCredentialsStore().Delete(ctx, cred.ID) }()
			}
