...
func TestEnqueueChangesetsToPublish(t *testing.T) {
	ctx := context.Background()
	db := dbtest.NewDB(t, "")
	...
	t.Run("deadline exceeded", func(t *testing.T) {
		// This one's pretty simple: we'll provide a processing changeset that
		// never leaves processing, and EnqueueChangsetsToPublish should return
		// an error once the deadline is hit.
		batchSpec := ct.CreateBatchSpec(t, ctx, s, "test-batch-change-deadline", user.ID)
		batchChange := ct.CreateBatchChange(t, ctx, s, "test-batch-change-deadline", user.ID, batchSpec.ID)
		changesetSpec := ct.CreateChangesetSpec(t, ctx, s, ct.TestSpecOpts{
			User:      user.ID,
			Repo:      repo.ID,
	...
	for name, tc := range map[string]struct {
		draft     bool
		wantState btypes.ChangesetUiPublicationState
	}{
		"draft": {
			draft:     true,
			wantState: btypes.ChangesetUiPublicationStateDraft,
		},
		"published": {
			draft:     false,
			wantState: btypes.ChangesetUiPublicationStatePublished,
		},
	} {
		t.Run(name, func(t *testing.T) {
			batchSpec := ct.CreateBatchSpec(t, ctx, s, "test-batch-change-"+name, user.ID)
			batchChange := ct.CreateBatchChange(t, ctx, s, "test-batch-change-"+name, user.ID, batchSpec.ID)

			otherBatchSpec := ct.CreateBatchSpec(t, ctx, s, "other-test-batch-change-"+name, user.ID)
			otherBatchChange := ct.CreateBatchChange(t, ctx, s, "other-test-batch-change-"+name, user.ID, otherBatchSpec.ID)
			...
			createChangeset := func(
				...
				return ct.CreateChangeset(t, ctx, s, ct.TestChangesetOpts{
					Repo:               repo.ID,
					BatchChange:        batchChange.ID,
					CurrentSpec:        specID,
					ReconcilerState:    reconcilerState,
					UiPublicationState: uiPublicationState,
				})
			}

			createImportedChangeset := func(reconcilerState btypes.ReconcilerState) *btypes.Changeset {
				return createChangeset(reconcilerState, nil, 0)
			}

			createBranchChangeset := func(
				reconcilerState btypes.ReconcilerState,
				published interface{},
				uiPublicationState *btypes.ChangesetUiPublicationState,
			) *btypes.Changeset {
				...
				return createChangeset(reconcilerState, uiPublicationState, spec.ID)
			}

			var otherState btypes.ChangesetUiPublicationState
			if tc.wantState == btypes.ChangesetUiPublicationStateDraft {
				otherState = btypes.ChangesetUiPublicationStatePublished
			} else {
				otherState = btypes.ChangesetUiPublicationStateDraft
			}

			// Let's set up a scenario that hopefully covers off the various
			// branches we have here. We've already tested the "something is
			// locked the whole time" case above, so we can ignore that.
			//
			// We're going to attach eleven changesets to the batch change, and
			// create one more besides. Unless otherwise mentioned, these
			// changeset specs will have their spec published field omitted.
			//
			// 1.  An imported changeset, which should always be ignored, since
			//     it doesn't have a spec.
			// 2.  An errored changeset, which should be re-enqueued with the
			//     new UI publication state.
			// 3.  A failed changeset, which should be re-enqueued with the new
			//	   UI publication state.
			// 4.  A completed changeset, which should be re-enqueued with the
			//     new UI publication state.
			// 5.  A queued changeset, which should have its UI publication
			//     state set.
			// 6.  A scheduled changeset, which should have its UI publication
			//     state set.
			// 7.  A processing changeset, which should initially be ignored,
			//     and then will have its state set to completed by a goroutine
			//     and should then be re-enqueued with the new UI publication
			//     state.
			// 8.  A completed changeset that would otherwise be eligible for
			//     this call, but isn't included in the given changesets, and
			//     therefore should be left unmodified.
			// 9.  A completed changeset that has an explicit published field
			//     in its spec, and therefore should be left unmodified.
			// 10. A completed changeset that has a pre-existing (but
			// 	   different) UI publication state, which should be re-enqueued
			//	   with the new UI publication state.
			// 11. A completed changeset that has the same UI publication
			//     state, which should remain completed.
			// 12. A changeset that belongs to a different batch change
			//     altogether.
			var (
				importedChangeset         = createImportedChangeset(btypes.ReconcilerStateCompleted)
				erroredChangeset          = createBranchChangeset(btypes.ReconcilerStateErrored, nil, nil)
				failedChangeset           = createBranchChangeset(btypes.ReconcilerStateFailed, nil, nil)
				completedChangeset        = createBranchChangeset(btypes.ReconcilerStateCompleted, nil, nil)
				queuedChangeset           = createBranchChangeset(btypes.ReconcilerStateQueued, nil, nil)
				scheduledChangeset        = createBranchChangeset(btypes.ReconcilerStateScheduled, nil, nil)
				processingChangeset       = createBranchChangeset(btypes.ReconcilerStateProcessing, nil, nil)
				unincludedChangeset       = createBranchChangeset(btypes.ReconcilerStateCompleted, nil, nil)
				publishedChangeset        = createBranchChangeset(btypes.ReconcilerStateCompleted, true, nil)
				otherUiPublishedChangeset = createBranchChangeset(btypes.ReconcilerStateCompleted, nil, &otherState)
			...
			otherSpecChangeset.BatchChanges = []btypes.BatchChangeAssoc{
				{BatchChangeID: otherBatchChange.ID},
			}
			if err := s.UpdateChangeset(ctx, otherSpecChangeset); err != nil {
				t.Fatal(err)
			}

			// Set up the processing changeset update.
			go func() {
				time.Sleep(20 * time.Millisecond)
				processingChangeset.ReconcilerState = btypes.ReconcilerStateCompleted
				if err := s.UpdateChangeset(ctx, processingChangeset); err != nil {
					t.Errorf("error updating processing changeset: %v", err)
				}
			}()
			...
			// Check the end states are what we expect.
			mustGetChangesetByID := func(id int64) *btypes.Changeset {
				changeset, err := s.GetChangesetByID(ctx, id)
				if err != nil {
					t.Fatal(err)
				}
				return changeset
			...
			// These changesets should remain complete, and have no UI
			// publication state:
			for name, id := range map[string]int64{
				"imported":   importedChangeset.ID,
				"unincluded": unincludedChangeset.ID,
				"published":  publishedChangeset.ID,
				"otherSpec":  otherSpecChangeset.ID,
			} {
				changeset := mustGetChangesetByID(id)
				if have, want := changeset.ReconcilerState, btypes.ReconcilerStateCompleted; have != want {
					t.Errorf("unexpected reconciler state on %s: have=%s want=%s", name, have, want)
				}
				if have := changeset.UiPublicationState; have != nil {
					t.Errorf("unexpected non-nil UI publication state on %s: %s", name, *have)
				}
			}

			// These changesets should be enqueued with the desired UI
			// publication state:
			for name, id := range map[string]int64{
				"errored":     erroredChangeset.ID,
				"failed":      failedChangeset.ID,
				"completed":   completedChangeset.ID,
				"queued":      queuedChangeset.ID,
				"processing":  processingChangeset.ID,
				"uiPublished": otherUiPublishedChangeset.ID,
			} {
				changeset := mustGetChangesetByID(id)
				if have, want := changeset.ReconcilerState, btypes.ReconcilerStateQueued; have != want {
					t.Errorf("unexpected reconciler state on %s: have=%s want=%s", name, have, want)
				}
				if have, want := changeset.UiPublicationState, tc.wantState; have == nil || *have != want {
					t.Errorf("unexpected UI publication state on %s: have=%v want=%s", name, have, want)
				}
			}

			// This little piggy^W^Wchangeset should remain scheduled with the
			// desired UI publication state:
			changeset := mustGetChangesetByID(scheduledChangeset.ID)
			if have, want := changeset.ReconcilerState, btypes.ReconcilerStateScheduled; have != want {
				t.Errorf("unexpected reconciler state on %s: have=%s want=%s", name, have, want)
			}
			if have, want := changeset.UiPublicationState, tc.wantState; have == nil || *have != want {
				t.Errorf("unexpected UI publication state on %s: have=%v want=%s", name, have, want)
			...
			// And, finally, this changeset should remain completed and have the
			// desired UI publication state, since it already did:
			changeset = mustGetChangesetByID(sameUiPublishedChangeset.ID)
			if have, want := changeset.ReconcilerState, btypes.ReconcilerStateCompleted; have != want {
				t.Errorf("unexpected reconciler state on %s: have=%s want=%s", name, have, want)
			}
			if have, want := changeset.UiPublicationState, tc.wantState; have == nil || *have != want {
