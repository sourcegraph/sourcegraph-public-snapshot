...
func TestService_ResolveWorkspacesForBatchSpec(t *testing.T) {
	...
	simpleGitserverRepo := func(name api.RepoName, branch string, head api.CommitID, ignored bool) gitserverRepo {
		return gitserverRepo{
			Name:          name,
			BranchHeads:   map[string]api.CommitID{branch: head},
	...
	defaultGitserverRepo := func(repo *types.Repo, ignored bool) gitserverRepo {
		...
		return simpleGitserverRepo(repo.Name, branch.branch, branch.commit, ignored)
	}
