...
func TestCommentSSHKey(t *testing.T) {
	...
	expectedKey := publicKey + " Sourcegraph " + globals.ExternalURL().Host

	if have, want := sshKey, expectedKey; have != want {
		t.Errorf("found wrong ssh key: want=\"%+v\", have=\"%+v\"", publicKey, expectedKey)
	}
}
