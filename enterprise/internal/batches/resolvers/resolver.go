...
func (r *Resolver) CreateBatchSpecExecution(ctx context.Context, args *graphqlbackend.CreateBatchSpecExecutionArgs) (_ graphqlbackend.BatchSpecExecutionResolver, err error) {
	...
	batchSpec, err := btypes.NewBatchSpecFromRaw(args.Spec)
	if err != nil {
		return nil, err
	}
	act := actor.FromContext(ctx)
	batchSpec.NamespaceUserID = act.UID

	svc := service.New(r.store)
	batchChange, _, err := svc.ReconcileBatchChange(ctx, batchSpec)
	if err != nil {
		return nil, err
	}
	if batchSpec.ID == 0 {
		// create the batch change if it doesn't yet exist.
		if err := r.store.CreateBatchChange(ctx, batchChange); err != nil {
			return nil, err
		}
	}
