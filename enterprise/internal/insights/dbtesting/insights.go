...
func TimescaleDB(t testing.TB) (db *sql.DB, cleanup func()) {
	...
	initConn, closeInitConn, err := dbconn.New(dbconn.Opts{DSN: timescaleDSN})
	if err != nil {
		t.Log("")
	...
	db, closeDBConn, err := dbconn.New(dbconn.Opts{DSN: timescaleDSN})
	if err != nil {
		t.Fatal(err)
	}

	// Perform DB migrations.
	closeMigration, err := dbconn.MigrateDB(db, dbconn.CodeInsights)
	if err != nil {
