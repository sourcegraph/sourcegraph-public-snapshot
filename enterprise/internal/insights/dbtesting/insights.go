...
func TimescaleDB(t testing.TB) (db *sql.DB, cleanup func()) {
	...
	initConn, closeInitConn, err := dbconn.Connect(dbconn.Opts{DSN: timescaleDSN})
	if err != nil {
		t.Log("")
	...
	db, closeDBConn, err := dbconn.Connect(dbconn.Opts{DSN: timescaleDSN, DatabasesToMigrate: []*dbconn.Database{dbconn.CodeInsights}})
	if err != nil {
		t.Fatal(err)
	}
