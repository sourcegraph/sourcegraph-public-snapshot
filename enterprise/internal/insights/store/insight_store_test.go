...
func TestGet(t *testing.T) {
	...
	_, err = insightsDB.ExecContext(ctx, `INSERT INTO insight_view_grants (insight_view_id, global)
									VALUES (1, true),
									       (2, true)`)
	if err != nil {
		t.Fatal(err)
	...
	_, err = insightsDB.ExecContext(ctx, `INSERT INTO insight_series (series_id, query, created_at, oldest_historical_at, last_recorded_at,
	...
	_, err = insightsDB.ExecContext(ctx, `INSERT INTO insight_view_series (insight_view_id, insight_series_id, label, stroke)
...
func TestGetAll(t *testing.T) {
	...
	_, err := insightsDB.ExecContext(ctx, `INSERT INTO insight_view (id, title, description, unique_id)
	...
	_, err = insightsDB.ExecContext(ctx, `INSERT INTO insight_series (id, series_id, query, created_at, oldest_historical_at, last_recorded_at,
		next_recording_after, last_snapshot_at, next_snapshot_after, deleted_at, generation_method)
	...
	_, err = insightsDB.ExecContext(ctx, `INSERT INTO dashboard (id, title) VALUES (1, 'dashboard 1');`)
	if err != nil {
		t.Fatal(err)
	}
	_, err = insightsDB.ExecContext(ctx, `INSERT INTO dashboard_insight_view (dashboard_id, insight_view_id) VALUES (1, 5)`)
	if err != nil {
		t.Fatal(err)
	}
	_, err = insightsDB.ExecContext(ctx, `INSERT INTO dashboard_grants (dashboard_id, global) VALUES (1, true)`)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetReferenceCount(t *testing.T) {
	...
	_, err = insightsDB.ExecContext(ctx, `INSERT INTO dashboard (id, title)
...
func TestGetUnfrozenInsightCount(t *testing.T) {
	...
	t.Run("returns correct counts for unfrozen insights", func(t *testing.T) {
		...
		_, err = insightsDB.ExecContext(ctx, `INSERT INTO dashboard_grants (id, dashboard_id, user_id, org_id, global)
