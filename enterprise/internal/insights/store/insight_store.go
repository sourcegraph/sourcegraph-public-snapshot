...
type DataSeriesStore interface {
	...
	IncrementBackfillAttempts(ctx context.Context, series types.InsightSeries) (types.InsightSeries, error)
...
default_filter_search_contexts, iv.series_sort_mode, iv.series_sort_direction, iv.series_limit, i.backfill_attempts
FROM (%s) iv
