...
func (c *CaptureGroupExecutor) Execute(ctx context.Context, query string, repositories []string, interval timeseries.TimeInterval) ([]GeneratedTimeSeries, error) {
	...
	for _, repository := range repositories {
		repo, err := c.repoStore.GetByName(ctx, api.NewRepoName(repository))
		if err != nil {
	...
	for _, repository := range repositories {
		firstCommit, err := git.FirstEverCommit(ctx, c.db, api.NewRepoName(repository), authz.DefaultSubRepoPermsChecker)
		if err != nil {
		...
		for _, execution := range plan.Executions {
			...
			commits, err := git.Commits(ctx, c.db, api.NewRepoName(repository), git.CommitsOptions{N: 1, Before: execution.RecordingTime.Format(time.RFC3339), DateOrder: true}, authz.DefaultSubRepoPermsChecker)
			if err != nil {
