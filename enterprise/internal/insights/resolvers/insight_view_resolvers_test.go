...
func TestRemoveClosePoints(t *testing.T) {
	getPoint := func(month time.Month, day, hour, minute, second int) store.SeriesPoint {
		return store.SeriesPoint{
			Time:  time.Date(2021, month, day, hour, minute, second, 0, time.UTC),
			Value: 1,
		}
	}
	tests := []struct {
