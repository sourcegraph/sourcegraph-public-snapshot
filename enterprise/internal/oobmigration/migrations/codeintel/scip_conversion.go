...
// convertDocument converts the given LSIF document/result chunk set into a SCIP document.
func convertDocument(
	...
	return types.CanonicalizeDocument(&scip.Document{
...
func convertRange(
	...
	if isDefinition(resultChunks, numResultChunks, path, rangeID, r) {
...
// to the range.
func constructSymbolNames(uploadID int, document DocumentData, r RangeData) (symbolNames, implementsSymbolNames []string) {
	var (
...
// isDefinition returns true if the given range is its own definition. This is determined by checking if
// the range attached to the input range's definition result set is the same as the input range.
func isDefinition(
	resultChunks map[int]ResultChunkData,
	numResultChunks int,
	path string,
	...
	definitionResultChunk, ok := resultChunks[hashKey(r.DefinitionResultID, numResultChunks)]
	if !ok {
		return false
	}

	for _, pair := range definitionResultChunk.DocumentIDRangeIDs[r.DefinitionResultID] {
		if path == definitionResultChunk.DocumentPaths[pair.DocumentID] && pair.RangeID == rangeID {
			return true
		}
	}
...
func convertDiagnostic(diagnostic DiagnosticData) *scip.Occurrence {
...
func constructSymbolName(uploadID int, resultID ID) string {
	if resultID == "" {
		return ""
	}
...
// hashKey hashes a string identifier into the range `[0, maxIndex)`. The hash algorithm here is similar
// to the one used in Java's String.hashCode. This implementation is identical to the TypeScript version
// used before the port to Go so that we can continue to read old conversions without a migration.
func hashKey(id ID, numResultChunks int) int {
	hash := int32(0)
	...
	return int(hash % int32(numResultChunks))
}
