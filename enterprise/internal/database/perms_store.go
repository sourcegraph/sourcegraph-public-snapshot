...
func (s *permsStore) ListRepoPermissions(ctx context.Context, repoID api.RepoID, args *ListRepoPermissionsArgs) ([]*RepoPermission, error) {
	...
	if authzParams.BypassAuthzReasons.NoAuthzProvider {
		...
		isRepoUnrestricted, err := s.isRepoUnrestricted(ctx, repoID, authzParams)
		if err != nil {
			return nil, err
		}

		unrestricted = isRepoUnrestricted != nil && *isRepoUnrestricted
		...
		if unrestricted {
			...
			// TODO(milan): need to update read path for new data model
			// load user who has access via permissions syncing
			permsQueryConditions = append(permsQueryConditions, sqlf.Sprintf(`
				(
	...
	for rows.Next() {
		...
		perms = append(perms, &RepoPermission{User: &u, Reason: reason, UpdatedAt: updatedAt})
	}
...
SELECT 
	...
	(	SELECT updated_at
		...
		WHERE
			user_id = users.id
			AND object_ids_ints @> INTSET(repo.id)
	) AS permissions_updated_at
FROM users, repo
WHERE
	users.deleted_at IS NULL
	AND repo.id = %d
...
func (s *permsStore) isRepoUnrestricted(ctx context.Context, repoID api.RepoID, authzParams *database.AuthzQueryParameters) (*bool, error) {
	...
	if err := s.QueryRow(ctx, query).Scan(&unrestricted); err != nil {
		return nil, err
	}
