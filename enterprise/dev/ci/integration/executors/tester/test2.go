...
// RunTest2ConditionalStaticallySkippedSteps tests static skipping of steps.
// Cases:
// - Hard coded if: false.
// - Expression that can be evaluated statically.
// TODO: Add cases for: First step skipped, middle step skipped, last step skipped.
func RunTest2ConditionalStaticallySkippedSteps(ctx context.Context, client *gqltestutil.Client, bstore *store.Store, logger log.Logger) bool {
	batchSpec := readFileOrFatal("batch_specs/test2.yaml", logger)
	expectedDiff := []byte(readFileOrFatal("diffs/test2.diff", logger))
					...
					Description: gqltestutil.ChangesetSpecDescription{
						...
						Diffs: gqltestutil.ChangesetSpecDiffs{
							FileDiffs: gqltestutil.ChangesetSpecFileDiffs{
								RawDiff: ``, // TODO: Why is this empty?
							},
						},
					},
				...
				Nodes: []gqltestutil.BatchSpecWorkspace{
					{
						...
						Stages: gqltestutil.BatchSpecWorkspaceStages{
							...
							SrcExec: []gqltestutil.ExecutionLogEntry{
								...
								{
									Key:      "step.docker.step.1.run",
									ExitCode: 0,
								},
								{
									Key:      "step.docker.step.1.post",
									ExitCode: 0,
								},
							},
						...
						Steps: []gqltestutil.BatchSpecWorkspaceStep{
							...
							},
							{
								Number:          3,
								Run:             "IFS=$'\\n'; echo Hello World | tee -a $(find -name README.md)",
								IfCondition:     `${{ eq repository.name "github.com/sourcegraph/src-cli" }}`,
								Container:       "ubuntu:18.04",
								OutputLines:     gqltestutil.WorkspaceOutputLines{},
								ExitCode:        0,
								Environment:     []gqltestutil.WorkspaceEnvironmentVariable{},
								OutputVariables: nil,
								// No diff and diff stat for skipped steps.
								// TODO: Should that be like that?
								DiffStat: gqltestutil.DiffStat{},
								Diff: gqltestutil.ChangesetSpecDiffs{
									FileDiffs: gqltestutil.ChangesetSpecFileDiffs{},
								},
	...
	if err := RunTest(ctx, client, bstore, Test{
		...
		logger.Error("Test failed", log.Error(err))
		return false
	}
	return true
}
