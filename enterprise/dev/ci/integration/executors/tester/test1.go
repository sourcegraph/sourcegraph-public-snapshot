...
func RunTestBasicExample(ctx context.Context, client *gqltestutil.Client, bstore *store.Store, logger log.Logger) bool {
	batchSpec := readFileOrFatal("batch_specs/test1.yaml", logger)
	expectedDiff := []byte(readFileOrFatal("diffs/test1.diff", logger))
