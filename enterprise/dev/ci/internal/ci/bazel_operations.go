...
func bazelBuildEntCandidateDockerImages(apps []string, version string, tag string, rt runtype.RunType) operations.Operation {
	return func(pipeline *bk.Pipeline) {
		cmds := []bk.StepOpt{}
		...
		// key = key + ":candidate"
		cmds = append(cmds,
			bk.Key("bazel-docker-enterprise"),
			bk.DependsOn("bazel-tests"),
			bk.Env("DOCKER_BAZEL", "true"),
			bk.Env("VERSION", version),
			bk.Agent("queue", "bazel"),
		...
		pipeline.AddStep(":docker: :construction: Build Enterprise Docker images", cmds...)
	}
}
