...
func GeneratePipeline(c Config) (*bk.Pipeline, error) {
	...
	case runtype.PullRequest:
		...
		if c.Diff.Has(changed.DockerImages) {
			...
			for _, image := range images.SourcegraphDockerImages {
				testBuilds.Append(buildCandidateDockerImage(image, c.Version, c.candidateImageTag(), c.RunType))
	...
	case runtype.BackendIntegrationTests:
		ops.Append(
			buildCandidateDockerImage("server", c.Version, c.candidateImageTag(), c.RunType),
	...
	case runtype.ImagePatch:
		...
		ops = operations.NewSet(
			buildCandidateDockerImage(patchImage, c.Version, c.candidateImageTag(), c.RunType),
	...
	case runtype.CandidatesNoTest:
		for _, dockerImage := range images.SourcegraphDockerImages {
			ops.Append(
				buildCandidateDockerImage(dockerImage, c.Version, c.candidateImageTag(), c.RunType))
		}
	...
	default:
		...
		for _, dockerImage := range images.SourcegraphDockerImages {
			imageBuildOps.Append(buildCandidateDockerImage(dockerImage, c.Version, c.candidateImageTag(), c.RunType))
		}
		// Executor VM image
