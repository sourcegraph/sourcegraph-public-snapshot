...
./enterprise/dev/ci/scripts/sentry-capture.sh ./dev/ci/sg-run.sh ci logs --out="$BUILD_LOGS_LOKI_URL" --state="" --overwrite-state="failed" --build="$BUILDKITE_BUILD_NUMBER" --job="$BUILDKITE_JOB_ID"
local_exit_code=$?
