...
Resolve this issue by incrementing the 'epoch' field in the package's YAML file." |
      tee /dev/stdout | ../../../enterprise/dev/ci/scripts/annotate.sh -s "package-version-exists" -t "error"
...
contents:
  ...
  packages:
$package_usage_list
  \`\`\`" | tee /dev/stdout | ../../../enterprise/dev/ci/scripts/annotate.sh -s "custom-repo" -m -t "info"
  fi
fi
