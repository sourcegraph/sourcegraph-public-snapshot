...
func NewMockLSIFStore() *MockLSIFStore {
	return &MockLSIFStore{
		...
		WriteSymbolsFunc: &LSIFStoreWriteSymbolsFunc{
			defaultHook: func(context.Context, int, chan semantic.SymbolData) error {
				return nil
			},
		},
	}
...
type LSIFStoreWriteSymbolsFunc struct {
	defaultHook func(context.Context, int, chan semantic.SymbolData) error
	hooks       []func(context.Context, int, chan semantic.SymbolData) error
...
// the parameter and result values of this invocation.
func (m *MockLSIFStore) WriteSymbols(v0 context.Context, v1 int, v2 chan semantic.SymbolData) error {
...
// empty.
func (f *LSIFStoreWriteSymbolsFunc) SetDefaultHook(hook func(context.Context, int, chan semantic.SymbolData) error) {
	f.defaultHook = hook
}

// PushHook adds a function to the end of hook queue. Each invocation of the
// WriteSymbols method of the parent MockLSIFStore instance invokes the hook
// at the front of the queue and discards it. After the queue is empty, the
// default hook function is invoked for any future action.
func (f *LSIFStoreWriteSymbolsFunc) PushHook(hook func(context.Context, int, chan semantic.SymbolData) error) {
	f.mutex.Lock()
	f.hooks = append(f.hooks, hook)
	f.mutex.Unlock()
}
...
func (f *LSIFStoreWriteSymbolsFunc) SetDefaultReturn(r0 error) {
	f.SetDefaultHook(func(context.Context, int, chan semantic.SymbolData) error {
		return r0
	})
}
...
func (f *LSIFStoreWriteSymbolsFunc) PushReturn(r0 error) {
	f.PushHook(func(context.Context, int, chan semantic.SymbolData) error {
		return r0
	})
}

func (f *LSIFStoreWriteSymbolsFunc) nextHook() func(context.Context, int, chan semantic.SymbolData) error {
	f.mutex.Lock()
	defer f.mutex.Unlock()
