...
func (h *handler) handle(ctx context.Context, dbStore DBStore, upload store.Upload) (requeued bool, err error) {
	if requeued, err := h.resolveRevision(ctx, dbStore, upload); err != nil || requeued {
		return requeued, err
	}

	return false, withLSIFData(ctx, h.uploadStore, upload.ID, func(r io.Reader) error {
		groupedBundleData, err := correlation.Correlate(
			ctx,
			r,
			upload.ID,
			upload.Root,
			makeGetChildren(h.dbStore, h.gitserverClient, upload.RepositoryID, upload.Commit),
		)
		if err != nil {
		...
		// Start a nested transaction with Postgres savepoints. In the event that something after
		// this point fails, we want to update the upload record with an error message but do not
		// want to alter any other data in the database. Rolling back to this savepoint will allow
		// us to discard any other changes but still commit the transaction as a whole.
...
// resolveRevision ensures that the repository and revision are resolvable. If the repo does not exist, or
...
func (h *handler) resolveRevision(ctx context.Context, dbStore DBStore, upload store.Upload) (requeued bool, _ error) {
...
// TODO - document
func withLSIFData(ctx context.Context, uploadStore uploadstore.Store, id int, fn func(r io.Reader) error) (err error) {
	...
	defer func() {
		if err == nil {
			...
			if deleteErr := uploadStore.Delete(ctx, uploadFilename); deleteErr != nil {
				log15.Warn("Failed to delete upload file", "filename", uploadFilename, "err", err)
			}
		}
	}()
...
func writeData(ctx context.Context, lsifStore LSIFStore, id int, groupedBundleData *correlation.GroupedBundleData) (err error) {
	lsifTx, err := lsifStore.Transact(ctx)
	if err != nil {
		return err
	}
	defer func() { err = lsifTx.Done(err) }()

	if err := lsifTx.WriteMeta(ctx, id, groupedBundleData.Meta); err != nil {
		return errors.Wrap(err, "store.WriteMeta")
	}
	if err := lsifTx.WriteDocuments(ctx, id, groupedBundleData.Documents); err != nil {
		return errors.Wrap(err, "store.WriteDocuments")
	}
	if err := lsifTx.WriteResultChunks(ctx, id, groupedBundleData.ResultChunks); err != nil {
		return errors.Wrap(err, "writer.WriteResultChunks")
	}
	if err := lsifTx.WriteDefinitions(ctx, id, groupedBundleData.Definitions); err != nil {
		return errors.Wrap(err, "store.WriteDefinitions")
	}
	if err := lsifTx.WriteReferences(ctx, id, groupedBundleData.References); err != nil {
...
func makeGetChildren(dbStore DBStore, gitserverClient GitserverClient, repositoryID int, commit string) existence.GetChildrenFunc {
	return func(ctx context.Context, dirnames []string) (map[string][]string, error) {
		directoryChildren, err := gitserverClient.DirectoryChildren(ctx, dbStore, repositoryID, commit, dirnames)
		if err != nil {
