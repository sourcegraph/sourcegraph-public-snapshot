...
type handler struct {
	...
	fileEmbeddingJobsStore bgrepo.RepoEmbeddingJobsStore
}
...
func (h *handler) Handle(ctx context.Context, logger log.Logger, record *bgrepo.RepoEmbeddingJob) error {
	...
	reportStats := func(stats *bgrepo.EmbedRepoStats) {
		if err := h.fileEmbeddingJobsStore.UpdateRepoEmbeddingJobStats(ctx, record.ID, stats); err != nil {
	...
	if stats.IsIncremental {
		return embeddings.UpdateRepoEmbeddingIndex(ctx, h.uploadStore, indexName, previousIndex, repoEmbeddingIndex, toRemove, emptyRanks)
	} else {
...
func (h *handler) getPreviousEmbeddingIndex(ctx context.Context, logger log.Logger, repo *types.Repo) (api.CommitID, *embeddings.RepoEmbeddingIndex) {
	lastSuccessfulJob, err := h.fileEmbeddingJobsStore.GetLastCompletedRepoEmbeddingJob(ctx, repo.ID)
	if err != nil {
