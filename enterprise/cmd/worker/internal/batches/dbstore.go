...
var initBatchSpecWorkspaceExecutionWorkerStore = memo.NewMemoizedConstructor(func() (interface{}, error) {
	...
	return store.NewBatchSpecWorkspaceExecutionWorkerStore(basestore.NewHandleWithDB(db, sql.TxOptions{}), observationContext, func(ctx context.Context, tx *store.Store, batchSpecRandID string) error {
		svc := service.New(tx)
