...
func (s *IndexScheduler) Handle(ctx context.Context) (err error) {
	...
	allowGlobalPolicies := false
	if val := conf.Get().CodeIntelAutoIndexingAllowGlobalPolicies; val != nil && *val {
		allowGlobalPolicies = true
	}
	...
	// the backlog.
	repositories, err := s.dbStore.SelectRepositoriesForIndexScan(ctx, s.repositoryProcessDelay, allowGlobalPolicies, s.repositoryBatchSize)
	if err != nil {
