...
func (r *firecrackerRunner) setupFirecracker(ctx context.Context) (string, error) {
	...
	if len(r.options.FirecrackerOptions.DockerRegistryMirrorURLs) > 0 {
		var err error
		daemonConfigFile, err = newDockerDaemonConfig(r.tmpDir, r.options.FirecrackerOptions.DockerRegistryMirrorURLs)
		if err != nil {
			return "", err
		}
	}
	...
	err = os.WriteFile(cniConfigFile, []byte(cniConfig(r.options.ResourceOptions.MaxIngressBandwidth, r.options.ResourceOptions.MaxEgressBandwidth)), os.ModePerm)
	if err != nil {
		return "", err
	}
	...
	startCommand := command{
		...
		Command: flatten(
			...
			"--runtime", "docker",
			"--network-plugin", "cni",
			firecrackerResourceFlags(r.options.ResourceOptions),
			firecrackerCopyfileFlags(r.options.FirecrackerOptions.VMStartupScriptPath, daemonConfigFile, dockerConfigPath),
			firecrackerVolumeFlags(r.workspaceDevice, firecrackerContainerDir),
			"--ssh",
			"--name", r.name,
			"--kernel-image", sanitizeImage(r.options.FirecrackerOptions.KernelImage),
			"--kernel-args", config.FirecrackerKernelArgs,
			"--sandbox-image", sanitizeImage(r.options.FirecrackerOptions.SandboxImage),
			sanitizeImage(r.options.FirecrackerOptions.Image),
		),
		Operation: r.operations.SetupFirecrackerStart,
	}

	if err = runCommand(ctx, r.internalLogger, startCommand, r.commandLogger); err != nil {
		return "", errors.Wrap(err, "failed to start firecracker vm")
	}

	if r.options.FirecrackerOptions.VMStartupScriptPath != "" {
		startupScriptCommand := command{
			Key:       "setup.startup-script",
			Command:   flatten("ignite", "exec", r.name, "--", r.options.FirecrackerOptions.VMStartupScriptPath),
			Operation: r.operations.SetupStartupScript,
		}
		if err = runCommand(ctx, r.internalLogger, startupScriptCommand, r.commandLogger); err != nil {
...
func (r *firecrackerRunner) teardownFirecracker(ctx context.Context) error {
	removeCommand := command{
		...
		Command:   flatten("ignite", "rm", "-f", r.name),
		Operation: r.operations.TeardownFirecrackerRemove,
	}
	if err := runCommand(ctx, r.internalLogger, removeCommand, r.commandLogger); err != nil {
		r.internalLogger.Error("Failed to remove firecracker vm", log.String("name", r.name), log.Error(err))
	}

	if err := os.RemoveAll(r.tmpDir); err != nil {
		r.internalLogger.Error(
			"Failed to remove firecracker state tmp dir",
...
func (r *firecrackerRunner) Run(ctx context.Context, command Spec) error {
	return runCommand(ctx, r.internalLogger, formatFirecrackerCommand(command, r.name, r.options, r.dockerConfigPath), r.commandLogger)
}
