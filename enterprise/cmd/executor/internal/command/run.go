...
type command struct {
	...
	Func      func(ctx context.Context, handle *entryHandle) error
...
func runCommand(ctx context.Context, command command, logger *Logger) (err error) {
	...
	// TODO - reassess boundaries

	if command.Process != nil {
		log15.Info(fmt.Sprintf("Running subprocess: %s", strings.Join(command.Process.Command, " ")))

		handle := logger.Log(command.Key, command.Process.Command)
		defer handle.Close()

		if err := validateCommand(command.Process.Command); err != nil {
			return err
		}
		...
		log15.Info(fmt.Sprintf("Running function")) // TODO - more detail

		handle := logger.Log(command.Key, nil) // TODO - more detail
		defer handle.Close()

		if err := command.Func(ctx, handle); err != nil {
			handle.Finalize(1)
			return err
		}
...
func prepCommand(ctx context.Context, command command) (cmd *exec.Cmd, stdout, stderr io.ReadCloser, err error) {
	cmd = exec.CommandContext(ctx, command.Process.Command[0], command.Process.Command[1:]...)
	cmd.Dir = command.Process.Dir

	env := command.Process.Env
