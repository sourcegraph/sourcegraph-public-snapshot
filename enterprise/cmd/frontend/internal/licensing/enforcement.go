...
func WriteSubscriptionErrorResponseForFeature(w http.ResponseWriter, featureNameHumanReadable string) {
	WriteSubscriptionErrorResponse(
		...
		fmt.Sprintf("To use the %s feature, a site admin must upgrade the Sourcegraph license in the Sourcegraph [site configuration](https://docs.sourcegraph.com/admin/config/site_config). (The site admin may also remove the site configuration that enables this feature to dismiss this message.)", featureNameHumanReadable),
	)
}
