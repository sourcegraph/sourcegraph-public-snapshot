...
func TestGraphQL_AddRemoveThreadsToFromCampaign(t *testing.T) {
	...
	addRemoveDiagnosticsToFromThread := func(campaign int64, threads []int64) error {
	...
	tests := map[string]*func(thread int64, threads []int64) error{
		"addThreadsToCampaign":      &mocks.threadDiagnostics.Create,
		"removeThreadsFromCampaign": &mocks.threadDiagnostics.Delete,
	}
	for name, mockFn := range tests {
		t.Run(name, func(t *testing.T) {
			*mockFn = addRemoveDiagnosticsToFromThread
