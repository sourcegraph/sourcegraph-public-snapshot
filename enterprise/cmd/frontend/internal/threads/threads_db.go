...
const SelectColumns = "id, repository_id, title, is_draft, is_pending_external_creation, state, assignee_user_id, assignee_external_actor_username, assignee_external_actor_url, created_at, updated_at, base_ref, base_ref_oid, head_repository_id, head_ref, head_ref_oid, pending_patch, external_service_id, external_id, external_metadata"
...
func (dbThreads) Create(ctx context.Context, tx *sql.Tx, thread *DBThread) (*DBThread, error) {
	...
	return dbThreads{}.scanRow(dbconn.TxOrGlobal(tx).QueryRowContext(ctx, query.Query(sqlf.PostgresBindVar), query.Args()...))
}
...
func (s dbThreads) Update(ctx context.Context, id int64, update dbThreadUpdate) (*DBThread, error) {
	...
	if update.IsPendingExternalCreation != nil {
		setFields = append(setFields, sqlf.Sprintf("is_pending_external_creation=%s", *update.IsPendingExternalCreation))
	}
	if update.State != nil {
