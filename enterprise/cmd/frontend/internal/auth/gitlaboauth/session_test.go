	...
	for _, c := range cases {
		for _, ci := range c.inputs {
			...
			t.Run(ci.description, func(t *testing.T) {
				gitlab.MockListGroups = func(ctx context.Context) (*gitlab.PaginatedResult[*gitlab.Group], error) {
					return gitlab.NewMockPaginatedResult(ci.glUserGroups, ci.glUserGroupsErr)
				}
