...
func NewMockQueryResolver() *MockQueryResolver {
	return &MockQueryResolver{
		...
		SymbolsFunc: &QueryResolverSymbolsFunc{
			defaultHook: func(context.Context) ([]resolvers.AdjustedMonikerLocations, error) {
				return nil, nil
			},
		},
	}
...
// PushHook adds a function to the end of hook queue. Each invocation of the
// Symbol method of the parent MockQueryResolver instance invokes the hook
// at the front of the queue and discards it. After the queue is empty, the
// default hook function is invoked for any future action.
...
type QueryResolverSymbolsFunc struct {
	defaultHook func(context.Context) ([]resolvers.AdjustedMonikerLocations, error)
	hooks       []func(context.Context) ([]resolvers.AdjustedMonikerLocations, error)
...
// parameter and result values of this invocation.
func (m *MockQueryResolver) Symbols(v0 context.Context) ([]resolvers.AdjustedMonikerLocations, error) {
	r0, r1 := m.SymbolsFunc.nextHook()(v0)
	m.SymbolsFunc.appendCall(QueryResolverSymbolsFuncCall{v0, r0, r1})
	return r0, r1
}
...
// the parent MockQueryResolver instance is invoked and the hook queue is
// empty.
func (f *QueryResolverSymbolsFunc) SetDefaultHook(hook func(context.Context) ([]resolvers.AdjustedMonikerLocations, error)) {
	f.defaultHook = hook
}

// PushHook adds a function to the end of hook queue. Each invocation of the
// Symbols method of the parent MockQueryResolver instance invokes the hook
// at the front of the queue and discards it. After the queue is empty, the
// default hook function is invoked for any future action.
func (f *QueryResolverSymbolsFunc) PushHook(hook func(context.Context) ([]resolvers.AdjustedMonikerLocations, error)) {
	f.mutex.Lock()
	f.hooks = append(f.hooks, hook)
	f.mutex.Unlock()
}
...
// SetDefaultReturn calls SetDefaultDefaultHook with a function that returns
// the given values.
func (f *QueryResolverSymbolsFunc) SetDefaultReturn(r0 []resolvers.AdjustedMonikerLocations, r1 error) {
	f.SetDefaultHook(func(context.Context) ([]resolvers.AdjustedMonikerLocations, error) {
		return r0, r1
	})
}
...
// PushReturn calls PushDefaultHook with a function that returns the given
// values.
func (f *QueryResolverSymbolsFunc) PushReturn(r0 []resolvers.AdjustedMonikerLocations, r1 error) {
	f.PushHook(func(context.Context) ([]resolvers.AdjustedMonikerLocations, error) {
		return r0, r1
	})
}

func (f *QueryResolverSymbolsFunc) nextHook() func(context.Context) ([]resolvers.AdjustedMonikerLocations, error) {
	f.mutex.Lock()
	defer f.mutex.Unlock()
...
type QueryResolverSymbolsFuncCall struct {
	...
	// Result0 is the value of the 1st result returned from this method
	// invocation.
	Result0 []resolvers.AdjustedMonikerLocations
	// Result1 is the value of the 2nd result returned from this method
	// invocation.
	Result1 error
}
