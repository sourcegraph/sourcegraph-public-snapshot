...
func TestDeleteLSIFUpload(t *testing.T) {
	...
	if _, err := NewResolver(db, db, nil, mockResolver, nil, nil, &observation.TestContext).DeleteLSIFUpload(context.Background(), &struct{ ID graphql.ID }{id}); err != nil {
...
func TestDeleteLSIFUploadUnauthenticated(t *testing.T) {
	...
	if _, err := NewResolver(db, db, nil, mockResolver, nil, nil, &observation.TestContext).DeleteLSIFUpload(context.Background(), &struct{ ID graphql.ID }{id}); err != backend.ErrNotAuthenticated {
...
func TestDeleteLSIFIndex(t *testing.T) {
	...
	if _, err := NewResolver(db, db, nil, mockResolver, nil, nil, &observation.TestContext).DeleteLSIFIndex(context.Background(), &struct{ ID graphql.ID }{id}); err != nil {
		t.Fatalf("unexpected error: %s", err)
	}
...
func TestDeleteLSIFIndexUnauthenticated(t *testing.T) {
	...
	if _, err := NewResolver(db, db, nil, mockResolver, nil, nil, &observation.TestContext).DeleteLSIFIndex(context.Background(), &struct{ ID graphql.ID }{id}); err != backend.ErrNotAuthenticated {
		t.Errorf("unexpected error. want=%q have=%q", backend.ErrNotAuthenticated, err)
	}
}
