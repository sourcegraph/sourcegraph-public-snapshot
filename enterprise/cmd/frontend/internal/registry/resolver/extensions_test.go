...
func TestGetExtensionByExtensionID(t *testing.T) {
	...
	t.Run("non-root", func(t *testing.T) {
		...
		t.Run("2-part", func(t *testing.T) {
			mockGetRemoteRegistryExtension = func(field, value string) (*types.Extension, error) {
				...
				return &types.Extension{UUID: "u", ExtensionID: "a/b"}, nil
			}
			...
			if want := (&types.Extension{UUID: "u", ExtensionID: "a/b"}); !reflect.DeepEqual(remote, want) {
...
func TestIsWorkInProgressExtension(t *testing.T) {
	...
	for manifest, want := range tests {
		got := types.IsWorkInProgressExtension(manifest)
		if got != want {
			var label string
			if manifest == nil {
				label = "nil"
