...
func init() {
	ExtensionRegistry.ViewerPublishersFunc = extensionRegistryViewerPublishers
}
...
func (r *registryPublisher) ToOrg() (*graphqlbackend.OrgResolver, bool) {
	return r.org, r.org != nil
}

func (r *registryPublisher) toDBRegistryPublisher() store.DBPublisher {
	switch {
	case r.user != nil:
		return store.DBPublisher{UserID: r.user.DatabaseID(), NonCanonicalName: r.user.Username()}
	case r.org != nil:
		return store.DBPublisher{OrgID: r.org.OrgID(), NonCanonicalName: r.org.Name()}
	default:
		return store.DBPublisher{}
	}
}

func (r *registryPublisher) RegistryExtensionConnectionURL() (*string, error) {
	p := r.toDBRegistryPublisher()
	url := types.PublisherExtensionsURL(p.UserID != 0, p.OrgID != 0, p.NonCanonicalName)
	if url == "" {
...
func getRegistryPublisher(ctx context.Context, db dbutil.DB, publisher store.DBPublisher) (*registryPublisher, error) {
	switch {
...
func toRegistryPublisherID(extension *store.DBExtension) *registryPublisherID {
