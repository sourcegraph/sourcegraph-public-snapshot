...
func init() {
	ExtensionRegistry.PublishersFunc = extensionRegistryPublishers
}
...
func extensionRegistryPublishers(ctx context.Context, db dbutil.DB, args *graphqlutil.ConnectionArgs) (graphqlbackend.RegistryPublisherConnection, error) {
	var opt store.DBPublishersListOptions
...
type registryPublisherConnection struct {
	opt store.DBPublishersListOptions
	db  dbutil.DB

	// cache results because they are used by multiple fields
	once               sync.Once
	registryPublishers []*store.DBPublisher
	err                error
}

func (r *registryPublisherConnection) compute(ctx context.Context) ([]*store.DBPublisher, error) {
	r.once.Do(func() {
		opt2 := r.opt
		...
		r.registryPublishers, r.err = store.NewDBExtensions(r.db).ListPublishers(ctx, opt2)
	})
...
func (r *registryPublisherConnection) TotalCount(ctx context.Context) (int32, error) {
	count, err := store.NewDBExtensions(r.db).CountPublishers(ctx, r.opt)
