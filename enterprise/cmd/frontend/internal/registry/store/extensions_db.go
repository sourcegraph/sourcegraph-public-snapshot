...
// NewDBExtensions returns a new dbExtensions backed by the given database.
func NewDBExtensions(db dbutil.DB) *DBExtensions {
	return &DBExtensions{
...
const createExtensionQueryFmtstr = `
-- source:enterprise/cmd/frontend/internal/registry/extensions_db.go:Create
...
var extensionIsWIPExpr = sqlf.Sprintf(`rer.manifest IS NULL OR COALESCE((rer.manifest->>'wip')::jsonb = 'true'::jsonb, rer.manifest->>'title' SIMILAR TO %s, false)`, types.WorkInProgressExtensionTitlePostgreSQLPattern)
...
func (s *DBExtensions) listCountSQL(conds []*sqlf.Query) *sqlf.Query {
	return sqlf.Sprintf(`
...
%s`,
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&t.CreatedAt,
			&t.UpdatedAt,
			&t.NonCanonicalExtensionID,
			&t.Publisher.NonCanonicalName,
			&t.NonCanonicalIsWorkInProgress,
		); err != nil {
			return nil, err
		}
...
func (s *DBExtensions) Update(ctx context.Context, id int32, name *string) error {
	...
	q := sqlf.Sprintf(
		"UPDATE registry_extensions SET name = COALESCE(%s, name), updated_at = NOW() WHERE id = %s AND deleted_at IS NULL",
		name,
		id,
	)
...
func (s *DBExtensions) Delete(ctx context.Context, id int32) error {
	...
	q := sqlf.Sprintf("UPDATE registry_extensions SET deleted_at = NOW() WHERE id = %s AND deleted_at IS NULL", id)
