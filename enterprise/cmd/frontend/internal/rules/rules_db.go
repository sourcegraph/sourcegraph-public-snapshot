...
const selectColumns = `id, container_campaign_id, container_thread_id, name, description, template_id, template_context, definition, created_at, updated_at`
...
func (o DBRulesListOptions) sqlConditions() []*sqlf.Query {
	...
	if cond := containerCondition(o.Container); cond != nil {
		conds = append(conds, cond)
	}
	return conds
}

func containerCondition(container RuleContainer) *sqlf.Query {
	cols := []struct {
		id     int64
		column string
	}{
		{container.Campaign, "container_campaign_id"},
		{container.Thread, "container_thread_id"},
	}
	for _, col := range cols {
		if col.id != 0 {
			return sqlf.Sprintf(col.column+"=%d", col.id)
		}
	}
	return nil
}
...
// DeleteByID deletes a rule given its ID. It returns an error if no such rule exists.
//
...
// DeleteByContainer deletes all rules for a given container. It is not an error if there are no
// rules for the given container, or if there is no such container. It panics if container is the
// zero value.
//
// 🚨 SECURITY: The caller must ensure that the actor is permitted to delete the rules.
func (s DBRules) DeleteByContainer(ctx context.Context, container RuleContainer) error {
	cond := containerCondition(container)
	if cond == nil {
		panic("no container")
	}
	if err := s.delete(ctx, cond); err != nil && err != errRuleNotFound {
		return err
	}
	return nil
}
