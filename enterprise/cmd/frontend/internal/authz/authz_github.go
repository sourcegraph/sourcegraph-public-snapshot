...
func init() {
	db.ExternalServices.GitHubValidators = append(db.ExternalServices.GitHubValidators, validateGitHubProvider)
}

func validateGitHubProvider(g *schema.GitHubConnection) error {
	_, err := githubProvider(g)
	return err
}
