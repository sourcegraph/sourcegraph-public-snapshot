...
func newAuthzProvider(a *schema.GitLabAuthorization, instanceURL, token string, ps []schema.AuthProviders) (authz.Provider, error) {
	...
	case idp.Oauth != nil:
		...
		if !foundAuthProvider {
			return nil, fmt.Errorf("Did not find authentication provider matching %q. Check the [site configuration](https://docs.sourcegraph.com/admin/config/site_config) to verify an entry in [`auth.providers`](https://docs.sourcegraph.com/admin/auth) exists for %s.", instanceURL, instanceURL)
		}
	...
	case idp.External != nil:
		...
		return nil, fmt.Errorf("Did not find authentication provider matching type %s and configID %s. Check the [site configuration](https://docs.sourcegraph.com/admin/config/site_config) to verify that an entry in [`auth.providers`](https://docs.sourcegraph.com/admin/auth) matches the type and configID.", ext.AuthProviderType, ext.AuthProviderID)
	default:
