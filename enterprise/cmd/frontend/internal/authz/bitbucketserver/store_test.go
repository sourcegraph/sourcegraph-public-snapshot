...
func testStore(db *sql.DB) func(*testing.T) {
	...
	return func(t *testing.T) {
		...
		ps := &permsstore.UserPermissions{UserID: 42, Perm: authz.Read, Type: "repos"}
		load := func(s *store) (*permsstore.UserPermissions, error) {
			ps := *ps
...

		{
			...
			ch := make(chan op, 30)
			updates := make(chan *permsstore.UserPermissions)
