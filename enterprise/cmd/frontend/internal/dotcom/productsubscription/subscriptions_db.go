...
func (s dbSubscriptions) Update(ctx context.Context, id string, update dbSubscriptionUpdate) error {
	...
	if access := update.codyGatewayAccess; access != nil {
		...
		if v := access.ChatCompletionsRateLimit; v != nil {
			fieldUpdates = append(fieldUpdates, sqlf.Sprintf("cody_gateway_chat_rate_limit=%s", dbutil.NewNullInt64(*v)))
		}
		...
		if v := access.CodeCompletionsRateLimit; v != nil {
			fieldUpdates = append(fieldUpdates, sqlf.Sprintf("cody_gateway_code_rate_limit=%s", dbutil.NewNullInt64(*v)))
		}
		...
		if v := access.EmbeddingsRateLimit; v != nil {
			fieldUpdates = append(fieldUpdates, sqlf.Sprintf("cody_gateway_embeddings_api_rate_limit=%s", dbutil.NewNullInt64(*v)))
		}
