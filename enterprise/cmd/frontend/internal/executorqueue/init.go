...
import (
	...
	"github.com/sourcegraph/sourcegraph/enterprise/cmd/frontend/internal/executorqueue/resolvers"
)
...
func Init(
	...
	for _, qo := range queueOptions {
		stores[qo.Name] = qo.Store
	}
	enterpriseServices.ExecutorsResolver = resolvers.NewExecutorsResolver(conf, db, stores)
