...
func NewResolver(logger log.Logger, db database.DB, ej jobutil.EnterpriseJobs) gql.ComputeResolver {
	return &Resolver{logger: logger, db: db, enterpriseSearchJobs: ej}
}
...
func NewBatchComputeImplementer(ctx context.Context, logger log.Logger, db database.DB, args *gql.ComputeArgs, ej jobutil.EnterpriseJobs) ([]gql.ComputeResultResolver, error) {
	...
	patternType := "regexp"
	job, err := gql.NewBatchSearchImplementer(ctx, logger, db, ej, &gql.SearchArgs{Query: searchQuery, PatternType: &patternType})
	if err != nil {
		return nil, err
	}
...
func (r *Resolver) Compute(ctx context.Context, args *gql.ComputeArgs) ([]gql.ComputeResultResolver, error) {
	return NewBatchComputeImplementer(ctx, r.logger, r.db, args, r.enterpriseSearchJobs)
}
