...
func (x *rulesExecutor) planThreads(ctx context.Context) ([]graphqlbackend.ThreadPreview, error) {
	...
	planIssues := func(ctx context.Context) (issues []graphqlbackend.ThreadPreview, err error) {
		// TODO!(sqs): hack, if there are any RawChangesets, assume the whole campaign is changesets
		// only and issues arent wanted.
		if len(x.extensionData.RawChangesets) > 0 {
			return nil, nil
		}
	...
	planChangesets := func(ctx context.Context) (changesets []graphqlbackend.ThreadPreview, err error) {
		changesets = make([]graphqlbackend.ThreadPreview, len(x.extensionData.RawChangesets))
		for i, raw := range x.extensionData.RawChangesets {
			baseRepo, err := graphqlbackend.RepositoryByID(ctx, raw.BaseRepository)
			if err != nil {
				return nil, err
			}
			var headRepo *graphqlbackend.RepositoryResolver
			if raw.BaseRepository == raw.HeadRepository {
				headRepo = baseRepo
			} else {
				headRepo, err = graphqlbackend.RepositoryByID(ctx, raw.HeadRepository)
				if err != nil {
					return nil, err
				}
			}

			patch, err := diff.ParseMultiFileDiff([]byte(raw.Patch))
			if err != nil {
				return nil, err
			}
			repoComparison := &git.GQLRepositoryComparisonPreview{
				BaseRepository_: baseRepo,
				HeadRepository_: headRepo,
				FileDiffs_:      patch,
			}

			diagnostics := diagnosticsByRepo[baseRepo.DBID()]
			rawDiagnostics := toRawDiagnostics(diagnostics)
			...
			if body != "" && len(diagnosticsBodyItems) > 0 {
				body += "\n\n" + strings.Join(diagnosticsBodyItems, "\n")
			}

			changesets[i] = threads.NewGQLThreadPreview(graphqlbackend.CreateThreadInput{
				Repository:                       baseRepo.ID(),
				Title:                            raw.Title,
				Body:                             &body,
