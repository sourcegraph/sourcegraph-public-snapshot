...
DELETE FROM exp_campaigns_threads o USING delete_values d WHERE o.campaign_id=d.campaign_id AND o.thread_id=d.thread_id
`,
		campaign, pq.Array(threads),
	)
	return err
...
func (s dbCampaignsThreads) list(ctx context.Context, conds []*sqlf.Query, limitOffset *db.LimitOffset) ([]*dbCampaignThread, error) {
	q := sqlf.Sprintf(`
SELECT campaign_id, thread_id FROM exp_campaigns_threads
