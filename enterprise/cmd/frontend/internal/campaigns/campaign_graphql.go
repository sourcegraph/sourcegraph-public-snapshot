...
func (v *gqlCampaign) Author(ctx context.Context) (*graphqlbackend.Actor, error) {
	return (&actor.DBColumns{UserID: v.db.AuthorID}).GQL(ctx)
}

func (v *gqlCampaign) Body(ctx context.Context) (string, error) {
	return v.db.Description, nil
}

func (v *gqlCampaign) BodyText(ctx context.Context) (string, error) {
	return comments.ToBodyText(v.db.Description), nil
}

func (v *gqlCampaign) BodyHTML(ctx context.Context) (string, error) {
	return comments.ToBodyHTML(v.db.Description), nil
}

func (v *gqlCampaign) CreatedAt(ctx context.Context) (graphqlbackend.DateTime, error) {
	return graphqlbackend.DateTime{Time: v.db.CreatedAt}, nil
}

func (v *gqlCampaign) UpdatedAt(ctx context.Context) (graphqlbackend.DateTime, error) {
	return graphqlbackend.DateTime{Time: v.db.UpdatedAt}, nil
}

func (v *gqlCampaign) ViewerCanUpdate(ctx context.Context) (bool, error) {
	return true, nil // TODO!(sqs)
}
...
func (v *gqlCampaign) Workflow() *graphqlbackend.JSONC {
	if v.db.WorkflowJSONC == "" {
		return nil
	}
	return (*graphqlbackend.JSONC)(&v.db.WorkflowJSONC)
}
...
func (v *gqlCampaign) getEvents(ctx context.Context, beforeDate time.Time, eventTypes []events.Type) ([]graphqlbackend.ToEvent, error) {
	...
	ec, err := events.GetEventConnection(ctx,
		&graphqlbackend.EventConnectionCommonArgs{
			BeforeDate: &graphqlbackend.DateTime{Time: beforeDate},
			Types:      &eventTypes2,
		},
