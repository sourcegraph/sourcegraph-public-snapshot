...
const selectColumns = `id, author_id, namespace_user_id, namespace_org_id, name, description, is_draft, start_date, due_date, workflow_jsonc, extension_data, created_at, updated_at`
...
func (dbCampaigns) Create(ctx context.Context, campaign *dbCampaign) (*dbCampaign, error) {
	...
	args := []interface{}{
		...
		campaign.StartDate,
		campaign.DueDate,
		nnz.String(campaign.WorkflowJSONC),
