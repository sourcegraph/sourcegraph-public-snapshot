...
func (s dbCampaigns) list(ctx context.Context, conds []*sqlf.Query, limitOffset *db.LimitOffset) ([]*dbCampaign, error) {
	q := sqlf.Sprintf(`
SELECT `+selectColumns+` FROM exp_campaigns
WHERE (%s)
ORDER BY name ASC
%s`,
		sqlf.Join(conds, ") AND ("),
...
func (dbCampaigns) Count(ctx context.Context, opt dbCampaignsListOptions) (int, error) {
	...
	q := sqlf.Sprintf("SELECT COUNT(*) FROM exp_campaigns WHERE (%s)", sqlf.Join(opt.sqlConditions(), ") AND ("))
	var count int
...
func (dbCampaigns) delete(ctx context.Context, cond *sqlf.Query) error {
	...
	q := sqlf.Sprintf("DELETE FROM exp_campaigns WHERE (%s)", sqlf.Join(conds, ") AND ("))
