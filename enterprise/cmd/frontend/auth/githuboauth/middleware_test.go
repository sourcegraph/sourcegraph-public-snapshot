...
func TestMiddleware(t *testing.T) {
	...
	t.Run("unauthenticated homepage visit -> github oidc auth flow", func(t *testing.T) {
	...
	t.Run("unauthenticated subpage visit -> github oidc auth flow", func(t *testing.T) {
	...
	t.Run("login -> github auth flow (github.com)", func(t *testing.T) {
		...
		state, err := DecodeState(uredirect.Query().Get("state"))
		if err != nil {
	...
	t.Run("GitHub OAuth callback with valid state param", func(t *testing.T) {
		...
		callbackCookies := []*http.Cookie{newCookie(getStateConfig(), encodedState)}
...
func newMockProvider(t *testing.T, clientID, clientSecret, baseURL string) *mockProvider {
	var mp mockProvider
	mp.provider = newProvider(schema.AuthProviders{Github: &schema.GitHubAuthProvider{}}, baseURL, oauth2.Config{
		ClientID:     clientID,
