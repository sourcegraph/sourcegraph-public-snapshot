...
func Test_authzProvidersFromConfig(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expSeriousProblems:           []string{"Did not find authentication provider matching \"https://gitlab.mine\". Check the [site configuration](https://docs.sourcegraph.com/admin/config/site_config) to verify an entry in [`auth.providers`](https://docs.sourcegraph.com/admin/auth) exists for https://gitlab.mine."},
		},
		{
