...
func TestExternalServices_ValidateConfig(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			`,
			assert: includes("Did not find authentication provider matching \"https://gitlab.foo.bar\". Check the [site configuration](https://docs.sourcegraph.com/admin/config/site_config) to verify an entry in [`auth.providers`](https://docs.sourcegraph.com/admin/auth) exists for https://gitlab.foo.bar."),
		},
		{
			kind: "GITLAB",
			...
			assert: excludes("Did not find authentication provider matching \"https://gitlab.foo.bar\". Check the [site configuration](https://docs.sourcegraph.com/admin/config/site_config) to verify an entry in [`auth.providers`](https://docs.sourcegraph.com/admin/auth) exists for https://gitlab.foo.bar."),
		},
		{
			kind: "GITLAB",
			...
			`,
			assert: includes("Did not find authentication provider matching type bar and configID foo. Check the [site configuration](https://docs.sourcegraph.com/admin/config/site_config) to verify that an entry in [`auth.providers`](https://docs.sourcegraph.com/admin/auth) matches the type and configID."),
		},
		{
			kind: "GITLAB",
			...
			assert: excludes("Did not find authentication provider matching type bar and configID foo. Check the [site configuration](https://docs.sourcegraph.com/admin/config/site_config) to verify that an entry in [`auth.providers`](https://docs.sourcegraph.com/admin/auth) matches the type and configID."),
		},
		{
			kind: "GITLAB",
