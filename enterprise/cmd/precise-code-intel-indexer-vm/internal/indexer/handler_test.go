...
func TestHandleWithDocker(t *testing.T) {
	...
	if callCount := len(commander.RunFunc.History()); callCount != 5 {
		...
		expectedCalls := []string{
			...
			"docker run --rm --cpus 8 --memory 32G -v /tmp/testing:/data -w /data sourcegraph/lsif-go:latest lsif-go",
			"docker run --rm --cpus 8 --memory 32G -v /tmp/testing:/data -w /data -e SRC_ENDPOINT=https://indexer:hunter2@sourcegraph.test:5432 sourcegraph/src-cli:latest lsif upload -repo github.com/sourcegraph/sourcegraph -commit e2249f2173e8ca0c8c2541644847e7bf01aaef4a -upload-route /.internal-code-intel/lsif/upload",
		}
...
func TestHandleWithFirecracker(t *testing.T) {
	...
	if callCount := len(commander.RunFunc.History()); callCount != 8 {
		...
		expectedCalls := []string{
			...
			"ignite exec 97b45daf-53d1-48ad-b992-547469d8e438 -- docker run --rm --cpus 8 --memory 32G -v /repo-dir:/data -w /data sourcegraph/lsif-go:latest lsif-go",
