...
- Added support for overriding critical, site, and external service configurations via files. Specify `OVERRIDE_CRITICAL_CONFIG=critical.json`, `OVERRIDE_SITE_CONFIG=site.json`, and/or `OVERRIDE_EXTSVC_CONFIG=extsvc.json` on the `frontend` container to do this.
