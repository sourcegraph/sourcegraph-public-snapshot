...
- GitHub repository permissions will now leverage caching of team and organization permissions for user permissions syncing. Caching behaviour can be configured (or disabled entirely) via the `authorization.groupsCacheTTL` field. This can significantly reduce the amount of time it takes to perform a full permissions sync due to reduced instances of being rate limited by the code host. [#23978](https://github.com/sourcegraph/sourcegraph/pull/23978)
