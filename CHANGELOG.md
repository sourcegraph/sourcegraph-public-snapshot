...
- Repository clone progress is now always written to the database. It is written to the `repo_update_jobs` table instead of `gitserver_repos` table and the latest progress message is now preserved per repo clone job instead of per repo. `clone-progress-logging` feature flag has been removed.[]()
