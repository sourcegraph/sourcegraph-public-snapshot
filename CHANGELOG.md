...
- Endpoints for `Gitserver`, `Indexed-search`, `Searcher`, and `Symbols` are generated automatically when the correct environment variables (`GITSERVER_REPLICA_NUMBER`, `INDEXED_SEARCH_REPLICA_NUMBER`, `SEARCHER_REPLICA_NUMBER`, `SYMBOLS_REPLICA_NUMBER`) are provided in Kubernetes and Docker-Compose deployments
