...
describe("BlobContainer", () => {
	...
	it("should handle available file and unavailable definition", () => {
		...
		DefStore.directDispatch(new DefActions.DefFetched("someDef", null));
	...
	it("should handle available file and available definition", () => {
		BlobStore.directDispatch(new BlobActions.FileFetched("aRepo", "aRev", "aPath", exampleFile));
		DefStore.directDispatch(new DefActions.HighlightDef("otherDef"));
		DefStore.directDispatch(new DefActions.DefFetched("otherDef", {File: {Path: "aPath"}}));
		DefStore.directDispatch(new DefActions.RefsFetched("foo", null, [{test: "exampleData"}]));
	...
	it("should get filename from definition", () => {
		DefStore.directDispatch(new DefActions.DefFetched("someDef", {File: "somePath", Kind: "someKind"}));
	...
	it("should render def options menu", () => {
		BlobStore.directDispatch(new BlobActions.FileFetched("aRepo", "aRev", "aPath", exampleFile));
		DefStore.directDispatch(new DefActions.SelectMultipleDefs(["firstDef", "secondDef"], 10, 20));
