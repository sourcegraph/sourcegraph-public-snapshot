...
describe("withUserContext", () => {
	it("no accessToken", () => {
		...
		expect(res.context).to.eql({user: null, signedIn: false, githubToken: null});
	});
	it("with accessToken, no authInfo yet", () => {
		UserStore.activeAccessToken = "t";
		const res = renderAndGetContext(<C />);
		expect(res.actions).to.eql([new UserActions.WantAuthInfo("t")]);
		expect(res.context).to.eql({user: null, signedIn: true, githubToken: null});
	});
	...
	it("with accessToken, authInfo, and user", () => {
		...
		const res = renderAndGetContext(<C />);
		expect(res.actions).to.eql([]);
		expect(res.context).to.eql({user: sampleUser, signedIn: true, githubToken: null});
	});
	...
	it("with GitHub token", () => {
		...
		const res = renderAndGetContext(<C />);
		expect(res.actions).to.eql([]);
		expect(res.context).to.eql({user: null, signedIn: false, githubToken: sampleGitHubToken});
	});
});
