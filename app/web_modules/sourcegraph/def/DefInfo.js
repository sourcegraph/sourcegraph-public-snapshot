...
import {Dropdown, Panel} from "sourcegraph/components";
			...
			<div styleName="bg">
				...
				{def &&
					<h1 styleName="def-header">
						<Link title="View definition in code" to={defURL} className={`${colors["mid-gray"]}`}>
				...
				<div styleName="actions">
					{def && def.Repo && <Link styleName="action" to={urlToRepo(def.Repo)}>{def.Repo}</Link>}
					{def && <Link styleName="action" to={defURL}>View definition</Link>}
					<Dropdown
						styleName="translation-action"
						title="Translate"
				...
				<div styleName="examples-container">
					<Heading level="2" styleName="examples-heading">4 usage examples</Heading>
					<Panel hoverLevel="low" styleName="examples-panel">
						{def && !def.Error && (
							<ExamplesContainer
