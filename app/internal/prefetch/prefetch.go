...
func ResolveFetches(w http.ResponseWriter, req *http.Request, urls []string) error {
	...
	for _, url := range urls {
		fetch, err := http.NewRequest("GET", url, nil)
		if err != nil {
			return err
		}

		ctx := httpctx.FromRequest(req)
		httpctx.SetForRequest(fetch, ctx) // TODO maybe use another context?
		...
		fetch.Header.Set("Authorization", auth)

		out := httptest.NewRecorder()
		apihandler.ServeHTTP(out, fetch)
		resp := template.JSEscapeString(out.Body.String())
		w.Write([]byte(`
<script>window.__resolvePushPromise("` + template.JSEscapeString(url) + `","` + resp + `")</script>`))
	}
