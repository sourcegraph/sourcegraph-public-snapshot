...
declare namespace chrome.runtime {
    ...
    /** The ID of the extension/scripts. */
    ...
    /**
     * Attempts to connect to connect listeners within an extension/scripts (such as the background page), or other extensions/apps. This is useful for content scripts connecting to their extension processes, inter-app/extension communication, and web messaging. Note that this does not connect to any listeners in a content script. Extensions may connect to content scripts embedded in tabs via tabs.connect.
     * @since browser 26.
     */
    ...
    /** Retrieves the JavaScript 'window' object for the background page running inside the current extension/scripts. If the background page is an event page, the system will ensure it is loaded before calling the callback. If there is no background page, an error is set. */
    ...
    /**
     * Sends a single message to event listeners within your extension/scripts or a different extension/scripts. Similar to runtime.connect but only sends a single message, with an optional response. If sending to your extension, the runtime.onMessage event will be fired in each page, or runtime.onMessageExternal, if a different extension. Note that extensions cannot send messages to content scripts using this method. To send messages to content scripts, use tabs.sendMessage.
     * @since browser 26.
     ...
     * Sends a single message to event listeners within your extension/scripts or a different extension/scripts. Similar to runtime.connect but only sends a single message, with an optional response. If sending to your extension, the runtime.onMessage event will be fired in each page, or runtime.onMessageExternal, if a different extension. Note that extensions cannot send messages to content scripts using this method. To send messages to content scripts, use tabs.sendMessage.
     * @since browser 26.
     * @param extensionId The ID of the extension/scripts to send the message to. If omitted, the message will be sent to your own extension/scripts. Required if sending messages from a web page for web messaging.
     * @param responseCallback Optional
     * Parameter response: The JSON response object sent by the handler of the message. If an error occurs while connecting to the extension, the callback will be called with no arguments and runtime.lastError will be set to the error message.
     */
    ...
    /**
     * Fired when a message is sent from another extension/scripts. Cannot be used in a content script.
     * @since browser 26.
     */
