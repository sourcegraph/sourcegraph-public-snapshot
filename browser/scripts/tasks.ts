...
function copyExtensionAssets(toDirectory: string):
    void {
      shelljs.mkdir(
          '-p', `${toDirectory}/js`, `${toDirectory}/css`, `${toDirectory}/img`)
      ...
      shelljs.cp(
          'build/dist/css/options-style.bundle.css', `${toDirectory}/css`)
      shelljs.cp(
          'build/dist/css/options-style.bundle.css', `${toDirectory}/css`)
...
copyIntegrationAssets():
    void {
      ...
      shelljs.cp(
          'build/dist/js/phabricator.bundle.js', 'build/integration/scripts')
      shelljs.cp(
          'build/dist/js/integration.bundle.js', 'build/integration/scripts')
      shelljs.cp(
      ...
      shelljs.cp(
          'src/native-integration/extensionHostFrame.html', 'build/integration')
...
function writeSchema(
    environment: BuildEnv, browser: Browser, writeDirectory: string):
    void {
      fs.writeFileSync(
          `${writeDirectory}/schema.json`, JSON.stringify(schema, null, 4))
    }

const version = utcVersion()

function writeManifest(
    environment: BuildEnv, browser: Browser, writeDirectory: string): void {
  const manifest = {
  ...
  fs.writeFileSync(
      `${writeDirectory}/manifest.json`, JSON.stringify(manifest, null, 4))
}
