...
ALTER TABLE exp_campaigns ADD CONSTRAINT campaigns_has_1_namespace CHECK ((namespace_user_id IS NULL) != (namespace_org_id IS NULL));
CREATE INDEX exp_campaigns_namespace_user_id ON exp_campaigns(namespace_user_id);
CREATE INDEX exp_campaigns_namespace_org_id ON exp_campaigns(namespace_org_id);

CREATE TABLE exp_campaigns_threads (
	campaign_id bigint NOT NULL REFERENCES exp_campaigns(id) ON DELETE CASCADE,
...
CREATE TABLE comments (
    ...
    campaign_id bigint REFERENCES exp_campaigns(id) ON DELETE CASCADE
);
...
ALTER TABLE exp_campaigns ADD COLUMN primary_comment_id bigint NOT NULL REFERENCES comments(id) ON DELETE RESTRICT;
...
ALTER TABLE exp_campaigns ADD COLUMN is_draft boolean NOT NULL DEFAULT false;
ALTER TABLE exp_campaigns ADD COLUMN start_date timestamp with time zone;
ALTER TABLE exp_campaigns ADD COLUMN due_date timestamp with time zone;
ALTER TABLE exp_campaigns ADD COLUMN extension_data jsonb;

CREATE TABLE rules (
	id bigserial PRIMARY KEY,
	container_campaign_id bigint REFERENCES exp_campaigns(id) ON DELETE CASCADE,
