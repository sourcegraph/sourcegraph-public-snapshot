CREATE FUNCTION IF NOT EXISTS iscodyactiveevent(name text) RETURNS boolean
...
CREATE FUNCTION IF NOT EXISTS iscodyexplanationevent(name text) RETURNS boolean
    LANGUAGE plpgsql IMMUTABLE
    AS $$
BEGIN
...
CREATE FUNCTION IF NOT EXISTS iscodygenerationevent(name text) RETURNS boolean
    LANGUAGE plpgsql IMMUTABLE
    AS $$
BEGIN
  RETURN name = ANY(ARRAY[
