...
export function createController(context: PlatformContext): Controller {
    ...
    if (DEBUG) {
        ...
        if (LOG_MODEL) {
            subscriptions.add(services.editor.model.subscribe(model => log('info', 'model', model)))
        }
        ...
        subscriptions.add(services.editor.model.subscribe(v => ((window as any).sxmodel = v)))
    }
