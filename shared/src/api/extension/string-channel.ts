...
function hexEncode(buffer: ArrayBuffer): string {
    return [...new Uint8Array(buffer)].map(v => padLeft(v.toString(16), '0', 2)).join('')
}
...
function addMessageListener(target: EventTarget, listener: (ev: MessageEvent) => void): void {
    if ('on' in target) {
        return (target as any).on('message', (data: any) => listener({ data } as any))
    }
    target.addEventListener('message', listener as any)
}

enum SerializedTransferableType {
    MessagePort,
    TypedArray,
}
...
function getTypedViewConstructor(type: TypedArrayType): (v: ArrayBuffer) => ArrayBufferView | ArrayBuffer {
    switch (type) {
...
type SerializedTransferable = SerializedTransferableMessagePort | SerializedTransferableTypedArray
const messagePortMap = new Map<string, StringChannelEndpoint>()
function makeTransferable(v: unknown, ep: StringChannelEndpoint): SerializedTransferable {
...
function deserializeTransferable(transfer: SerializedTransferable, ep: StringChannelEndpoint): [any, Transferable] {
...
export function wrap(ep: StringChannelEndpoint, uid = ''): MessagePort {
    const { port1, port2 } = new MessageChannel()

    ep.addMessageListener(msg => {
        const payload: StringChannelPayload = JSON.parse(msg)
        if (payload.uid !== uid) {
            return
        }
