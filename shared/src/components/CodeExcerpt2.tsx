...
export class CodeExcerpt2 extends React.PureComponent<Props> {
    public render(): JSX.Element | null {
        const maxDigits = that.props.items.reduce((n, { line }) => {
            const digits = (line + 1).toString().length
            return digits > n ? digits : n
        }, 1)
        return (
            <pre className="file-match__code">
                {that.props.items.map(({ line, preview, highlightRanges }, i) => (
                    <code
                    ...
                    >
                        <Link
                            to={`${that.props.urlWithoutPosition}${toPositionOrRangeHash({
                                position: { line: line + 1, character: highlightRanges[0].start + 1 },
                            })}`}
                            key={i}
                            onClick={that.props.onSelect}
                        >
                            {preview}
                        </Link>
                    </code>
