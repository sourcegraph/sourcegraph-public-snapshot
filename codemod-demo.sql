...
select id from exp_campaigns order by id desc limit 1
)
delete from events where campaign_id=(select id from last_campaign) or thread_id in (select ct.thread_id from exp_campaigns_threads ct left join last_campaign on true where ct.campaign_id=last_campaign.id) and type in ('CreateThread', 'MergeThread', 'CloseThread', 'Review');
...
update0 as (
     update threads set created_at=current_timestamp - interval '11 days', updated_at=current_timestamp - interval '11 days' where id in (select thread_id from exp_campaigns_threads ct left join last_campaign on true where campaign_id=last_campaign.id)
),
update1 as (
     update comments set created_at=current_timestamp - interval '11 days', updated_at=current_timestamp - interval '11 days' where thread_id in (select thread_id from exp_campaigns_threads ct left join last_campaign on true where campaign_id=last_campaign.id)
),
update2 as (
     update events set created_at=current_timestamp - interval '11 days' where type='CreateThread' and thread_id in (select thread_id from exp_campaigns_threads ct left join last_campaign on true where campaign_id=last_campaign.id)
),
all_threads as (
     select threads.id from threads left join last_campaign on true left join exp_campaigns_threads ct on ct.campaign_id=last_campaign.id and ct.thread_id=threads.id
),
...
scatter_diagnostic_events as (
     ...
     where type='AddDiagnosticToThread' and thread_id in (select thread_id from exp_campaigns_threads ct left join last_campaign on true where campaign_id=last_campaign.id)
),
delete_most_diagnostic_events as (
     delete from events where id%9 != 1 and type='AddDiagnosticToThread' and thread_id in (select thread_id from exp_campaigns_threads ct left join last_campaign on true where campaign_id=last_campaign.id)
)
select 1;
commit;
