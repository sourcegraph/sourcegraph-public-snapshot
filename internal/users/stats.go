...
var (
	...
	WITH aggregated_stats AS (
		SELECT
			...
			users.deleted_at,
			users.site_admin,
            (SELECT COUNT(user_id) FROM user_external_accounts WHERE user_id=users.id AND service_type = 'scim') >= 1 AS scim_enabled,
...
func (s *UsersStats) ListUsers(ctx context.Context, filters *UsersStatsListUsersFilters) ([]*UserStatItem, error) {
	...
	SELECT id, username, display_name, primary_email, created_at, last_active_at, deleted_at, site_admin, scim_enabled, events_count FROM aggregated_stats WHERE %s ORDER BY %s NULLS LAST LIMIT %s OFFSET %s`, sqlf.Join(conds, "AND"), orderBy, limit, offset))
	...
	for rows.Next() {
		...
		if err := rows.Scan(&node.Id, &node.Username, &node.DisplayName, &node.PrimaryEmail, &node.CreatedAt, &node.LastActiveAt, &node.DeletedAt, &node.SiteAdmin, &node.SCIMEnabled, &node.EventsCount); err != nil {
			return nil, err
		}
