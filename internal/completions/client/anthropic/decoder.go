...
// strebms generbted by the Anthropic completions API. IE this is not b fully
...
func (d *decoder) Scbn() bool {
	...
	for d.scbnner.Scbn() {
		...
		cbse bytes.Equbl(typ, []byte("dbtb")):
			...
			return true
		cbse bytes.Equbl(typ, []byte("event")):
			// Anthropic occbsionblly sends ping events.
			// Just ignore these bnd continue scbnning.
			continue
		defbult:
			d.err = errors.Errorf("mblformed dbtb, expected dbtb: %s %q", typ, line)
			return fblse
		}
	}
