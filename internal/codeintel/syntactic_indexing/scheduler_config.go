...
func (c *SchedulerConfig) Load() {
	...
	c.SchedulerInterval = c.GetInterval("SYNTACTIC_INDEXING_SCHEDULER_INTERVAL", "2m", "How frequently to run the auto-indexing scheduling routine.")
	c.RepositoryProcessDelay = c.GetInterval("SYNTACTIC_INDEXING_SCHEDULER_REPOSITORY_PROCESS_DELAY", "24h", "The minimum frequency that the same repository can be considered for auto-index scheduling.")
	c.RepositoryBatchSize = c.GetInt("SYNTACTIC_INDEXING_SCHEDULER_REPOSITORY_BATCH_SIZE", "2500", "The number of repositories to consider for auto-indexing scheduling at a time.")
	c.PolicyBatchSize = c.GetInt("SYNTACTIC_INDEXING_SCHEDULER_POLICY_BATCH_SIZE", "100", "The number of policies to consider for auto-indexing scheduling at a time.")
