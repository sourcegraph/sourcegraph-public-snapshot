...
func TestReferences(t *testing.T) {
	...
	mockSvc.GetReferencesFunc.PushReturn(locations[:1], 1, nil)
	mockSvc.GetReferencesFunc.PushReturn(locations[1:4], 3, nil)
	mockSvc.GetReferencesFunc.PushReturn(locations[4:], 1, nil)
	...
	adjustedLocations, _, err := resolver.References(context.Background(), mockRequest)
	if err != nil {
...
func TestReferencesWithSubRepoPermissions(t *testing.T) {
	...
	adjustedLocations, _, err := resolver.References(ctx, mockRequest)
	if err != nil {
		t.Fatalf("unexpected error querying references: %s", err)
	}
	u := storeDumpToSymbolDump(uploads)
...
func TestReferencesRemote(t *testing.T) {
	...
	mockSvc.GetDumpsByIDsFunc.PushReturn(nil, nil) // empty
	...
	mockSvc.GetPackageInformationFunc.PushReturn(packageInformation3, true, nil)
	...
	mockSvc.GetReferencesFunc.PushReturn(locations[:1], 1, nil)
	mockSvc.GetReferencesFunc.PushReturn(locations[1:4], 3, nil)
	mockSvc.GetReferencesFunc.PushReturn(locations[4:5], 1, nil)
	...
	mockSvc.GetBulkMonikerLocationsFunc.PushReturn(monikerLocations[1:2], 1, nil) // refs batch 1
	mockSvc.GetBulkMonikerLocationsFunc.PushReturn(monikerLocations[2:], 3, nil)  // refs batch 2
