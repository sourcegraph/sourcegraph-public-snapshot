...
func TestGetTargetCommitPositionFromSourcePosition(t *testing.T) {
	...
	client.DiffFunc.SetDefaultHook(func(ctx context.Context, rn api.RepoName, do gitserver.DiffOptions) (*gitserver.DiffFileIterator, error) {
		return gitserver.NewDiffFileIterator(io.NopCloser(bytes.NewReader([]byte(hugoDiff)))), nil
	})
...
func TestGetTargetCommitPositionFromSourcePositionEmptyDiff(t *testing.T) {
	client := gitserver.NewMockClient()
	client.DiffFunc.SetDefaultHook(func(ctx context.Context, rn api.RepoName, do gitserver.DiffOptions) (*gitserver.DiffFileIterator, error) {
		return gitserver.NewDiffFileIterator(io.NopCloser(bytes.NewReader(nil))), nil
	})
...
func TestGetTargetCommitRangeFromSourceRangeEmptyDiff(t *testing.T) {
	client := gitserver.NewMockClient()
	client.DiffFunc.SetDefaultHook(func(ctx context.Context, rn api.RepoName, do gitserver.DiffOptions) (*gitserver.DiffFileIterator, error) {
		return gitserver.NewDiffFileIterator(io.NopCloser(bytes.NewReader([]byte(nil)))), nil
	})
	rIn := shared.Range{
		Start: shared.Position{Line: 302, Character: 15},
		End:   shared.Position{Line: 305, Character: 20},
