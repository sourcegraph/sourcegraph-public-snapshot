...
matching_symbol_names AS (
	(
		...
		JOIN codeintel_scip_symbols_lookup l5 ON l5.upload_id = ANY(%s) AND l5.segment_type = 'DESCRIPTOR_SUFFIX' AND l5.name = p.descriptor
		...
		JOIN codeintel_scip_symbols_lookup_leaves ll ON ll.upload_id = l5.upload_id AND ll.descriptor_suffix_id = l5.id
	)
)
`
