...
func TestInsertLockfileRoots(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	...
	roots := []int{nameIds["pkg-A"]}
	if err := store.InsertLockfileWithRoots(ctx, "foo", commit, roots, resolutionID); err != nil {
		t.Fatalf("inserting lockfile edges failed: %s", err)
	}
	...
	if have, want := ids[0], int32(nameIds["pkg-A"]); have != want {
		t.Fatalf("wrong ids: %#v", ids)
	}
}
