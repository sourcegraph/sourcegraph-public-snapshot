...
func testService(t *testing.T, repositoryContents map[string]string) *Service {
	...
	return newService(sandboxService, gitService, &ratelimit.InstrumentedLimiter{Limiter: rate.NewLimiter(rate.Limit(100), 1)}, 100, 1024*1024, &observation.TestContext)
}
