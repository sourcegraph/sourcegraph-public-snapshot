...
func Test_AutoIndexingDequeueOrder(t *testing.T) {
	...
	for i, test := range []struct {
		...
		{
			indexes: []shared.Index{
				{ID: 1, RepositoryID: 1, QueuedAt: clock.Now().Add(-time.Hour), State: "completed", FinishedAt: dbutil.NullTimeColumn(clock.Now())},
				{ID: 2, RepositoryID: 1, QueuedAt: clock.Now().Add(-time.Hour * 25)},
				{ID: 3, RepositoryID: 2, QueuedAt: clock.Now().Add(-time.Hour * 5)},
			},
			nextID: 2,
		},
	} {
