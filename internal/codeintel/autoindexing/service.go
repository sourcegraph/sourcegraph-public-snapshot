...
func (s *Service) GetBackgroundJobs() background.BackgroundJob { return s.backgroundJobs }
...
func (s *Service) InferIndexConfiguration(ctx context.Context, repositoryID int, commit string, bypassLimit bool) (_ *config.IndexConfiguration, hints []config.IndexJobHint, err error) {
	...
	indexJobs, err := s.backgroundJobs.InferIndexJobsFromRepositoryStructure(ctx, repositoryID, commit, bypassLimit)
	if err != nil {
		return nil, nil, err
	}

	indexJobHints, err := s.backgroundJobs.InferIndexJobHintsFromRepositoryStructure(ctx, repositoryID, commit)
	if err != nil {
		return nil, nil, err
	}
...
func (s *Service) QueueIndexes(ctx context.Context, repositoryID int, rev, configuration string, force, bypassLimit bool) (_ []types.Index, err error) {
	return s.backgroundJobs.QueueIndexes(ctx, repositoryID, rev, configuration, force, bypassLimit)
}
