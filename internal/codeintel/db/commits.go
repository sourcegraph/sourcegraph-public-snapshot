...
func (db *dbImpl) UpdateCommits(ctx context.Context, repositoryID int, commits map[string][]string) error {
	...
	// We want this to happen because many workers/servers can be inserting commits
	// for the same repository. Having them inserted in random order may cause a
	// deadlock to occur where two threads are writing the same tuples in different
	// orders: e.g. A writes t1 then t2, and B writes t2 then t1. If we always write
	// in the same order, then such a deadlock is impossible.
	var keys []string
