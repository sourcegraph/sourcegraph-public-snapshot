...
func TestParse(t *testing.T) {
	...
	autogold.Want("replace", "Command: `Replace in place: (sourcegraph) -> (smorgasboard)`, Parameters: ``").Equal(t, test("content:replace(sourcegraph -> smorgasboard)"))
	autogold.Want("replace multi arrow", "Command: `Replace in place: (a) -> (b -> c)`, Parameters: ``").Equal(t, test("content:replace(a -> b -> c)"))
	autogold.Want("replace no space", "Command: `Replace in place: (a) -> (b)`, Parameters: ``").Equal(t, test("content:replace(a->b)"))
	autogold.Want("replace no left hand side", "Command: `Replace in place: () -> (b)`, Parameters: ``").Equal(t, test("content:replace(->b)"))
}
