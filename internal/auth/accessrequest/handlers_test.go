...
func TestRequestAccess(t *testing.T) {
	...
	t.Run("existing user's email", func(t *testing.T) {
		...
		_, err = accessrequests.NewStoreWithDB(db).GetByEmail(context.Background(), newUser.Email)
		require.Error(t, err)
	...
	t.Run("existing access requests's email", func(t *testing.T) {
		...
		accessrequests.NewStoreWithDB(db).Create(context.Background(), &accessRequest)
		_, err := accessrequests.NewStoreWithDB(db).GetByEmail(context.Background(), accessRequest.Email)
		require.NoError(t, err)
	...
	t.Run("correct inputs", func(t *testing.T) {
		...
		accessRequest, err := accessrequests.NewStoreWithDB(db).GetByEmail(context.Background(), "a2@example.com")
		require.NoError(t, err)
