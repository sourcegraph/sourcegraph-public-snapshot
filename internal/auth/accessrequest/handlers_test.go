...
//
// 		_, err = accessrequests.Store.WithDB(db).GetByEmail(context.Background(), newUser.Email)
// 		require.Error(t, err)
...
// 		}
// 		accessrequests.Store.WithDB(db).Create(context.Background(), &accessRequest)
// 		_, err := accessrequests.Store.WithDB(db).GetByEmail(context.Background(), accessRequest.Email)
// 		require.NoError(t, err)
//
...
// 		assert.Equal(t, http.StatusCreated, res.Code)
//
// 		accessRequest, err := accessrequests.Store.WithDB(db).GetByEmail(context.Background(), "a2@example.com")
// 		require.NoError(t, err)
