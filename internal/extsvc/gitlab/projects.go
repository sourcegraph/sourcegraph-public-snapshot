...
// and returned.
func (c *Client) ForkProject(ctx context.Context, project *Project, namespace *string) (*Project, error) {
	if MockForkProject != nil {
		return MockForkProject(c, ctx, project, namespace)
	}
	...
	if code == http.StatusConflict {
		// 409 Conflict is returned if the fork already exists. In that case, we
		// want to ignore the error generated by doWithBaseURL, and instead get the
		// forked project and return that.
		return c.getForkedProject(ctx, project, namespace)
	} else if err != nil {
...
func (c *Client) getForkedProject(ctx context.Context, project *Project, namespace *string) (*Project, error) {
	// Inconveniently, if the namespace wasn't given, we first have to figure out
	// who this user is.
	var forkNamespace string
	if namespace != nil {
		forkNamespace = *namespace
	} else {
		...
		forkNamespace = user.Username
	}

	projectName, err := project.Name()
	if err != nil {
		return nil, errors.Wrap(err, "parsing project name")
	}

	fork, err := c.GetProject(ctx, GetProjectOp{
		PathWithNamespace: forkNamespace + "/" + projectName,
	})
	if err != nil {
		return nil, errors.Wrap(err, "getting previously forked project")
	}
