...
func TestCredentials(t *testing.T) {
	...
	presentDep, err := reposource.ParseNpmPackageVersion("left-pad@1.3.0")
	require.NoError(t, err)
	absentDep, err := reposource.ParseNpmPackageVersion("left-pad@1.3.1")
	require.NoError(t, err)
...
func TestGetDependencyInfo(t *testing.T) {
	...
	client, stop := newTestHTTPClient(t)
	defer stop()
	dep, err := reposource.ParseNpmPackageVersion("left-pad@1.3.0")
	require.NoError(t, err)
	info, err := client.GetDependencyInfo(ctx, dep)
	require.NoError(t, err)
	require.NotNil(t, info)
	dep, err = reposource.ParseNpmPackageVersion("left-pad@1.3.1")
	require.NoError(t, err)
	info, err = client.GetDependencyInfo(ctx, dep)
	require.Nil(t, info)
	require.ErrorAs(t, err, &npmError{})
...
func TestFetchSources(t *testing.T) {
	...
	client, stop := newTestHTTPClient(t)
	defer stop()
	dep, err := reposource.ParseNpmPackageVersion("is-sorted@1.0.0")
	require.Nil(t, err)
	info, err := client.GetDependencyInfo(ctx, dep)
	require.Nil(t, err)
