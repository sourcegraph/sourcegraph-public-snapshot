...
// BeforeCreateUserHook determines whether new user is allowed to be created.
func BeforeCreateUserHook(ctx context.Context, countUsers CountUsersFunc, spec *extsvc.AccountSpec) error {
...
type SetIsSiteAdminFunc func(ctx context.Context, id int32, isSiteAdmin bool) error

// AfterCreateUserHook determines whether a new user should be promoted to site
// admin based on the product license.
func AfterCreateUserHook(ctx context.Context, setIsSiteAdmin SetIsSiteAdminFunc, user *types.User) error {
	...
	if info.Plan().IsFree() {
		user.SiteAdmin = true
		if err := setIsSiteAdmin(ctx, user.ID, user.SiteAdmin); err != nil {
			return err
		}
	}
...
// BeforeSetUserIsSiteAdminHook determines whether the creation or removal of site
// admins are allowed.
func BeforeSetUserIsSiteAdminHook(ctx context.Context, isSiteAdmin bool) error {
	// Exempt user accounts that are created by the Sourcegraph Operator
	// authentication provider.
	//
	// NOTE: It is important to make sure the Sourcegraph Operator authentication
