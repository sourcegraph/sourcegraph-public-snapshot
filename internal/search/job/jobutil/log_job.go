...
func (l *LogJob) logEvent(ctx context.Context, clients job.RuntimeClients, duration time.Duration) {
	...
	if len(types) == 1 {
		...
		if a.IsAuthenticated() && !a.IsMockUser() { // Do not log in tests
			...
			err := usagestats.LogBackendEvent(ctx, clients.DB, a.UID, deviceid.FromContext(ctx), eventName, json.RawMessage(value), json.RawMessage(value), featureflag.GetEvaluatedFlagSet(ctx), nil)
			if err != nil {
			...
			if _, _, ok := isOwnershipSearch(q); ok {
				...
				//lint:ignore SA1019 existing usage of deprecated functionality. TODO: Use only the new V2 event instead.
				err := usagestats.LogBackendEvent(ctx, clients.DB, a.UID, deviceid.FromContext(ctx), "FileHasOwnerSearch", nil, nil, featureflag.GetEvaluatedFlagSet(ctx), nil)
				if err != nil {
			...
			if v, _ := q.ToParseTree().StringValue(query.FieldSelect); v != "" {
				if sp, err := filter.SelectPathFromString(v); err == nil && isSelectOwnersSearch(sp) {
					...
					//lint:ignore SA1019 existing usage of deprecated functionality. TODO: Use only the new V2 event instead.
					err := usagestats.LogBackendEvent(ctx, clients.DB, a.UID, deviceid.FromContext(ctx), "SelectFileOwnersSearch", nil, nil, featureflag.GetEvaluatedFlagSet(ctx), nil)
					if err != nil {
