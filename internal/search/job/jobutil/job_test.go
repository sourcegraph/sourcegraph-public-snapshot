  ...
  (TIMEOUT
    ...
    (LIMIT
      ...
      (PARALLEL
		...
		query:      `repohasfile:abc test`,
		protocol:   search.Streaming,
		searchType: query.SearchTypeRegex,
		want: autogold.Want("repo has file", `
(ALERT
  (query . )
  (originalQuery . )
  (patternType . literal)
    ...
    (LIMIT
      ...
      (PARALLEL
        (SEQUENTIAL
          (ensureUnique . false)
          (REPOPAGER
            (repoOpts.hasFileContent[0].path . abc)
            (PARTIALREPOS
              (ZOEKTREPOSUBSETTEXTSEARCH
                (query . substr:"test")
                (type . text))))
        ...
        (PARALLEL
          NoopJob
          (REPOSEARCH
            (repoOpts.repoFilters.0 . test)(repoOpts.hasFileContent[0].path . abc)))))))`),
	}, {
