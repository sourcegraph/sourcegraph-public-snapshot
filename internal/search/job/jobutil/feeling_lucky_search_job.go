...
func NewFeelingLuckySearchJob(log slog.Logger, initialJob job.Job, inputs *run.SearchInputs, plan query.Plan) *FeelingLuckySearchJob {
	generators := make([]next, 0, len(plan))
	for _, b := range plan {
		generators = append(generators, NewGenerator(log, inputs, b, rules))
	}
...
// apply successfully.
func NewGenerator(log slog.Logger, inputs *run.SearchInputs, seed query.Basic, rules []rule) next {
	...
	n = func(i int) next {
		...
		return func() (job.Job, next) {
			...
			child, err := NewBasicJob(log, inputs, *generated)
			if err != nil {
