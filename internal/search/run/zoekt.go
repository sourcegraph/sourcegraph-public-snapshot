...
type IndexedSearchRequest struct {
	...
	// searched.
	repos *zoektutil.IndexedRepoRevs
...
func zoektSearch(ctx context.Context, args *search.TextParameters, repos *zoektutil.IndexedRepoRevs, typ zoektutil.IndexedRequestType, since func(t time.Time) time.Duration, c streaming.Sender) error {
	if args == nil {
		return nil
	}
...
// unindexed.
func zoektIndexedRepos(indexedSet map[string]*zoekt.Repository, revs []*search.RepositoryRevisions, filter func(*zoekt.Repository) bool) (indexed *zoektutil.IndexedRepoRevs, unindexed []*search.RepositoryRevisions) {
