...
func ResolveRepoGroups(ctx context.Context, db database.DB, settings *schema.Settings) (groups map[string][]RepoGroupValue, err error) {
	...
	if mode, err := db.Users().CurrentUserAllowedExternalServices(ctx); err != nil {
		return groups, err
	} else if mode == conf.ExternalServiceModeDisabled {
		return groups, nil
	}

	repos, err := db.Repos().ListRepoNames(ctx, database.ReposListOptions{UserID: a.UID})
	if err != nil {
