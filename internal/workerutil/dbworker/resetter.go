...
loop:
	for {
		...
		for id, lastHeartbeatAge := range resetLastHeartbeatsByIDs {
			log15.Warn("Clear stalled record back to 'queued' state", "name", r.options.Name, "id", id, "timeSinceLastHeartbeat", lastHeartbeatAge)
		}
		for id, lastHeartbeatAge := range failedLastHeartbeatsByIDs {
			log15.Warn("Clear stalled record to 'failed' state", "name", r.options.Name, "id", id, "timeSinceLastHeartbeat", lastHeartbeatAge)
		}
