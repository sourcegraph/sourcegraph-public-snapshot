...
type OwnershipStore interface {
	...
	FetchExperts(ctx context.Context, label string) (map[string]int, error)
	Done(error) error
}
...
func (s *ownershipStore) FetchExperts(ctx context.Context, label string) (map[string]int, error) {
	q := `
		SELECT who, SUM(importance_indicator)
		FROM own_signals
		WHERE method = $1
		GROUP BY 1
		ORDER BY 2 DESC
		LIMIT 5;
	`
	method := fmt.Sprintf("expertise:%s", label)
	rs, err := s.Handle().QueryContext(ctx, q, method)
	if err != nil {
		return nil, err
	}
