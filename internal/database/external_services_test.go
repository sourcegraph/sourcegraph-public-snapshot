...
func TestExternalServiceStore_EnqueueSyncJobs(t *testing.T) {
	t.Parallel()
	store := NewDB(logtest.Scoped(t), dbtest.NewDB(t)).ExternalServices()
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := store.Upsert(ctx, stored...); err != nil {
			...
			jobs, err := store.GetSyncJobs(ctx, ExternalServicesGetSyncJobsOptions{})
			if err != nil {
				t.Fatal(err)
			}
...
func TestExternalServiceStore_EnqueueSingleSyncJob(t *testing.T) {
	t.Parallel()
...
func assertSyncJobCount(t *testing.T, db DB, want int) {
	t.Helper()
	var count int
	q := sqlf.Sprintf("SELECT COUNT(*) FROM external_service_sync_jobs")
	if err := db.Handle().QueryRowContext(context.Background(), q.Query(sqlf.PostgresBindVar), q.Args()...).Scan(&count); err != nil {
		t.Fatal(err)
	}
	if count != want {
...
func TestExternalServiceStore_EnqueuingSyncJobsWhileExtSvcBeingDeleted(t *testing.T) {
	...
	type testSubject func(*testing.T, context.Context, ExternalServiceStore, *types.ExternalService)

	enqueuingFuncs := map[string]testSubject{
		"EnqueueSingleSyncJob": func(t *testing.T, ctx context.Context, store ExternalServiceStore, service *types.ExternalService) {
			t.Helper()
			if err := store.EnqueueSingleSyncJob(ctx, service.ID); err != nil {
				t.Fatal(err)
			}
		},
		"EnqueueSyncJobs": func(t *testing.T, ctx context.Context, store ExternalServiceStore, _ *types.ExternalService) {
			t.Helper()
	...
	for name, enqueuingFunc := range enqueuingFuncs {
		t.Run(name, func(t *testing.T) {
			...
			// Create a new external service
			err := ExternalServicesWith(logger, store).Create(ctx, confGet, &service)
			if err != nil {
				t.Fatal(err)
			}
