...
func (r *Runner) validateSchema(ctx context.Context, schemaContext schemaContext) error {
	...
	for {
		// Attempt to validate the given definitions. We may have to call this several times as we are
		// unable to hold a consistent advisory lock in the presence of migrations utilizing concurrent
		// index creation. Therefore, some invocations of this method will return with a flag to request
		// re-invocation under a new lock.
