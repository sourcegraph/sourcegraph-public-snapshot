...
// 🚨 SECURITY: This tests the routine that verifies access tokens, which the security of the entire
// system depends on.
func TestAccessTokens_GetBySubjectUserIDAndNote(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	t.Parallel()
	...
	tokenID, _, err := AccessTokens(db).Create(ctx, subject.ID, []string{"a", "b"}, note, creator.ID)
	if err != nil {
		t.Fatal(err)
	}

	have, err := AccessTokens(db).GetBySubjectUserIDAndNote(ctx, subject.ID, note)
	if err != nil {
		t.Fatal(err)
	}

	if have.ID != tokenID {
		t.Fatalf("token has wrong ID. want=%d, have=%d", tokenID, have.ID)
	}

	unknownNote := "this-is-NOT-the-note"
	have, err = AccessTokens(db).GetBySubjectUserIDAndNote(ctx, subject.ID, unknownNote)
	if err != ErrAccessTokenNotFound {
		t.Fatalf("wrong error. want=%s, got=%s", ErrAccessTokenNotFound, err)
	}
}
