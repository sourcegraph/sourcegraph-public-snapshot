...
func TestRecentViewSignalStore_BuildAggregateFromEvents(t *testing.T) {
	...
	require.Contains(t, summaries, RecentViewSummary{UserID: 1, FilePathID: repo1PathToID["cmd/gitserver/server/lock.go"], ViewsCount: 2})
	require.Contains(t, summaries, RecentViewSummary{UserID: 1, FilePathID: repo1PathToID["cmd/gitserver/server/patch.go"], ViewsCount: 1})
	require.Contains(t, summaries, RecentViewSummary{UserID: 1, FilePathID: repo1PathToID["enterprise/cmd/frontend/main.go"], ViewsCount: 1})
	require.Contains(t, summaries, RecentViewSummary{UserID: 2, FilePathID: repo1PathToID["enterprise/cmd/frontend/main.go"], ViewsCount: 1})
	require.Contains(t, summaries, RecentViewSummary{UserID: 2, FilePathID: repo1PathToID["cmd/gitserver/server/lock.go"], ViewsCount: 1})
	require.Contains(t, summaries, RecentViewSummary{UserID: 2, FilePathID: repo2PathToID["cmd/gitserver/server/patch.go"], ViewsCount: 2})
	require.Contains(t, summaries, RecentViewSummary{UserID: 2, FilePathID: repo2PathToID["cmd/gitserver/server/lock.go"], ViewsCount: 1})
}
...
func TestRecentViewSignalStore_Insert(t *testing.T) {
	...
	t.Run("inserting initial signal", func(t *testing.T) {
		...
		require.Equal(t, 2, summaries[0].FilePathID)
		require.Equal(t, 10, summaries[0].ViewsCount)
		clearTable(ctx, db)
	})
	...
	t.Run("inserting multiple signals", func(t *testing.T) {
		...
		require.Equal(t, 2, summaries[0].FilePathID)
		require.Equal(t, 10, summaries[0].ViewsCount)
		require.Equal(t, 3, summaries[1].FilePathID)
		require.Equal(t, 20, summaries[1].ViewsCount)
		clearTable(ctx, db)
	})
	...
	t.Run("inserting conflicting entry will update it", func(t *testing.T) {
		...
		require.Len(t, summaries, 1)
		require.Equal(t, 2, summaries[0].FilePathID)
		require.Equal(t, 100, summaries[0].ViewsCount)
		clearTable(ctx, db)
	})
}
...
func TestRecentViewSignalStore_InsertPaths(t *testing.T) {
	...
	for _, summary := range summaries {
		require.Equal(t, int(math.Pow(float64(10), float64(summary.FilePathID))), summary.ViewsCount)
	}
}
