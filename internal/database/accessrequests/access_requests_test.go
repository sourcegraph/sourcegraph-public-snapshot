...
func TestAccessRequests_Update(t *testing.T) {
	...
	t.Run("non-existent access request", func(t *testing.T) {
		...
		assert.Equal(t, err, &accessrequests.ErrAccessRequestNotFound{ID: nonExistentAccessRequestID})
	})
...
func TestAccessRequests_GetByEmail(t *testing.T) {
	...
	t.Run("non-existing access request", func(t *testing.T) {
		...
		assert.Error(t, err)
		assert.Nil(t, accessRequest)
		assert.Equal(t, err, &accessrequests.ErrAccessRequestNotFound{Email: nonExistingAccessRequestEmail})
	})
	t.Run("existing access request", func(t *testing.T) {
		createdAccessRequest, err := store.Create(ctx, &types.AccessRequest{Email: "a1@example.com", Name: "a1", AdditionalInfo: "info1"})
		assert.NoError(t, err)
...
func TestAccessRequests_Count(t *testing.T) {
	...
	t.Run("all", func(t *testing.T) {
		count, err := accessRequestStore.Count(ctx, &accessrequests.AccessRequestsFilterArgs{})
		assert.NoError(t, err)
	...
	t.Run("by status", func(t *testing.T) {
		...
		count, err := accessRequestStore.Count(ctx, &accessrequests.AccessRequestsFilterArgs{Status: &pending})
		assert.NoError(t, err)
		assert.Equal(t, 1, count)

		rejected := types.AccessRequestStatusRejected
		count, err = accessRequestStore.Count(ctx, &accessrequests.AccessRequestsFilterArgs{Status: &rejected})
		assert.NoError(t, err)
		assert.Equal(t, 1, count)

		approved := types.AccessRequestStatusApproved
		count, err = accessRequestStore.Count(ctx, &accessrequests.AccessRequestsFilterArgs{Status: &approved})
		assert.NoError(t, err)
...
func TestAccessRequests_List(t *testing.T) {
	...
	t.Run("order", func(t *testing.T) {
		accessRequests, err := accessRequestStore.List(ctx, nil, &dbtypes.PaginationArgs{OrderBy: dbtypes.OrderBy{{Field: "name"}}, Ascending: true})
		assert.NoError(t, err)
		assert.Equal(t, len(accessRequests), 3)
	...
	t.Run("limit & pagination", func(t *testing.T) {
		one := 1
		accessRequests, err := accessRequestStore.List(ctx, nil, &dbtypes.PaginationArgs{First: &one})
		assert.NoError(t, err)
		...
		two := int(2)
		accessRequests, err = accessRequestStore.List(ctx, nil, &dbtypes.PaginationArgs{First: &two, After: &after, OrderBy: dbtypes.OrderBy{{Field: string(accessrequests.AccessRequestListID)}}})
		assert.NoError(t, err)
	...
	t.Run("by status", func(t *testing.T) {
		...
		// list all pending
		pending := types.AccessRequestStatusPending
		accessRequests, err := accessRequestStore.List(ctx, &accessrequests.AccessRequestsFilterArgs{Status: &pending}, nil)
		assert.NoError(t, err)
		assert.Equal(t, len(accessRequests), 1)

		// list all rejected
		rejected := types.AccessRequestStatusRejected
		accessRequests, err = accessRequestStore.List(ctx, &accessrequests.AccessRequestsFilterArgs{Status: &rejected}, nil)
		assert.NoError(t, err)
		assert.Equal(t, len(accessRequests), 1)

		// list all approved
		approved := types.AccessRequestStatusApproved
		accessRequests, err = accessRequestStore.List(ctx, &accessrequests.AccessRequestsFilterArgs{Status: &approved}, nil)
		assert.NoError(t, err)
		assert.Equal(t, len(accessRequests), 1)
	})
}
