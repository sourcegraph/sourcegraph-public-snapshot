...
func makeDateTruncExpression(unit, expr string) string {
	if unit == "week" {
		return fmt.Sprintf(`DATE_TRUNC('week', TIMEZONE('UTC', %s) + '1 day'::interval) - '1 day'::interval || 'Z'`, expr)
	}
	if unit == "rolling_month" {
		return fmt.Sprintf(`DATE_TRUNC('day', TIMEZONE('UTC', %s)) - '1 month'::interval || 'Z'`, expr)
	}

	return fmt.Sprintf(`DATE_TRUNC('%s', TIMEZONE('UTC', %s)) || 'Z'`, unit, expr)
}
