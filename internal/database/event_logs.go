...
unique_users_by_dwm AS (
  SELECT
	...
	` + makeDateTruncExpression("rolling_month", "timestamp") + ` AS rolling_month_period,
...
func (l *eventLogStore) siteUsageCurrentPeriods(ctx context.Context, now time.Time, opt *SiteUsageOptions) (summary types.SiteUsageSummary, err error) {
	...
	query := sqlf.Sprintf(siteUsageCurrentPeriodsQuery, now, now, now, now, now, sqlf.Join(conds, ") AND ("))
...
GROUP BY current_rolling_month, current_month, current_week, current_day
`
...
// by the unit parameter (e.g. day, week, month, or rolling month).
