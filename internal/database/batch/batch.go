...
// TODO - document
func WithInserter(ctx context.Context, db dbutil.DB, tableName string, columns []string, f func(inserter *Inserter) error) (err error) {
	inserter := NewInserter(ctx, db, tableName, columns...)
	defer func() {
...
// and column names. For performance and atomicity, the given handle should be a transaction.
