...
const (
	...
	accessRequestUpdateQuery = `
		...
		SET status = %s, updated_at = NOW(), decision_by = %s
		WHERE id = %s
		RETURNING %s`
)
...
func (s *accessRequestStore) Update(ctx context.Context, accessRequest *types.AccessRequest) (*types.AccessRequest, error) {
	q := sqlf.Sprintf(accessRequestUpdateQuery, accessRequest.Status, accessRequest.ID, *accessRequest.DecisionByID, sqlf.Join(accessRequestColumns, ","))
...
func scanAccessRequest(sc dbutil.Scanner) (*types.AccessRequest, error) {
	...
	if err := sc.Scan(&accessRequest.ID, &accessRequest.CreatedAt, &accessRequest.UpdatedAt, &accessRequest.Name, &accessRequest.Email, &accessRequest.Status, &accessRequest.AdditionalInfo, &accessRequest.DecisionByID); err != nil {
		return nil, err
	}
