...
func TestUsers_AutocompleteSearch(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	t.Parallel()
	...
	users, err := Users(db).AutocompleteUserSearch(ctx, "testus")
	if err != nil {
		t.Fatal(err)
	}
	...
	users2, err := Users(db).AutocompleteUserSearch(ctx, "searchable")
	if err != nil {
		t.Fatal(err)
	}
