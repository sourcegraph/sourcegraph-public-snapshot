...
func TestExternalServicesStore_ListPaginate(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("list all external services", func(t *testing.T) {
		defer func() { iterated = 0 }()

		var ess []*types.ExternalService
		err := ExternalServices.ListPaginate(ctx, ExternalServicesListOptions{
		...
		if diff := cmp.Diff(want, ess); diff != "" {
			t.Fatalf("External services mismatch (-want +got):\n%s", diff)
		}
	})

	t.Run("stop iterating in the middle", func(t *testing.T) {
		defer func() { iterated = 0 }()
		...
		err := ExternalServices.ListPaginate(ctx, ExternalServicesListOptions{
			...
			ess = append(ess, svcs...)
			return ErrStopIteratingExternalServices
		})
		if err != nil {
			t.Fatal(err)
		}
