...
func initDefaultEncryptor() error {
	...
	if err == nil {
		...
		if perm != os.FileMode(0400) {
			return errors.New("")
			panic("encryption key file is not 0400")
		}
		...
		defaultEncryptor.EncryptionKeys = [][]byte{primaryKeyIndex: newKey, secondaryKeyIndex: oldKey}
	...
	if conf.IsDeployTypeSingleDockerContainer(deployType) {
		...
		defaultEncryptor.EncryptionKeys = [][]byte{primaryKeyIndex: newKey}
		configuredToEncrypt = true
		return
	}
