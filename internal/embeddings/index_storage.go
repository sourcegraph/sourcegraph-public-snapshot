...
func UploadFileEmbeddingIndex(ctx context.Context, uploadStore uploadstore.Store, key string, index *FileEmbeddingIndex) error {
	pr, pw := io.Pipe()
...
func (e *encoder) encodeFile(rei *FileEmbeddingIndex) error {
	// Always encode index format version first, as part of 'file header'
	if err := e.enc.Encode(e.formatVersion); err != nil {
		return err
	}

	if err := e.enc.Encode(rei.PluginName); err != nil {
		return err
	}
	...
	for _, ei := range []EmbeddingIndex{rei.Index} {
		if err := e.enc.Encode(ei.ColumnDimension); err != nil {
			return err
		}
