...
func (j *FileEmbeddingJob) RecordUID() string {
	return strconv.Itoa(j.ID)
}

func (j *FileEmbeddingJob) IsRepoEmbeddingJobScheduledOrCompleted() bool {
	return j != nil && (j.State == "completed" || j.State == "processing" || j.State == "queued")
}

type EmbedFileStats struct {
...
func (e *EmbedFileStats) ToFields() []log.Field {
	return []log.Field{
