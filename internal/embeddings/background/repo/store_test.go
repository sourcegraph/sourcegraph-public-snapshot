...
func TestRepoEmbeddingJobsStore(t *testing.T) {
	...
	id1, err := store.CreateRepoEmbeddingJob(ctx, createdRepo.ID, "deadbeef", "sourcegraphProvider/model")
	require.NoError(t, err)

	id2, err := store.CreateRepoEmbeddingJob(ctx, createdRepo.ID, "coffee", "sourcegraphProvider/model")
	require.NoError(t, err)

	id3, err := store.CreateRepoEmbeddingJob(ctx, createdRepo2.ID, "tea", "sourcegraphProvider/model")
	require.NoError(t, err)
...
func TestCancelRepoEmbeddingJob(t *testing.T) {
	...
	id3, err := store.CreateRepoEmbeddingJob(ctx, createdRepo.ID, "avocado", "sourcegraphProvider/model")
	require.NoError(t, err)
...
func TestGetEmbeddableRepos(t *testing.T) {
	...
	_, err = store.CreateRepoEmbeddingJob(ctx, firstRepo.ID, "coffee", "sourcegraphProvider/model")
	require.NoError(t, err)
