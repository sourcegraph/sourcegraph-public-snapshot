...
func TestScheduleRepositoriesForEmbeddingMultiple(t *testing.T) {
	t.Parallel()
	...
	require.ErrorContains(t, err, fmt.Sprintf("Embedding job is already scheduled or completed for repo %v", string(repoNames[0])))
	count, err := store.CountRepoEmbeddingJobs(ctx, repo.ListOpts{})
	require.NoError(t, err)
	require.Equal(t, 1, count)
