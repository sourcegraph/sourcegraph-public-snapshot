...
type MockClient struct {
	...
	CommitDistanceFunc *ClientCommitDistanceFunc
	...
	return &MockClient{
		...
		CommitDistanceFunc: &ClientCommitDistanceFunc{
			defaultHook: func(context.Context, api.RepoName, api.CommitID, api.CommitID) (int, error) {
				panic("unexpected invocation of MockClient.CommitDistance")
			},
		},
		CommitExistsFunc: &ClientCommitExistsFunc{
...
// ClientCommitDistanceFunc describes the behavior when the CommitDistance
// method of the parent MockClient instance is invoked.
type ClientCommitDistanceFunc struct {
	defaultHook func(context.Context, api.RepoName, api.CommitID, api.CommitID) (int, error)
	hooks       []func(context.Context, api.RepoName, api.CommitID, api.CommitID) (int, error)
	history     []ClientCommitDistanceFuncCall
	mutex       sync.Mutex
}

// CommitDistance delegates to the next hook function in the queue and
// stores the parameter and result values of this invocation.
func (m *MockClient) CommitDistance(v0 context.Context, v1 api.RepoName, v2 api.CommitID, v3 api.CommitID) (int, error) {
	r0, r1 := m.CommitDistanceFunc.nextHook()(v0, v1, v2, v3)
	m.CommitDistanceFunc.appendCall(ClientCommitDistanceFuncCall{v0, v1, v2, v3, r0, r1})
	return r0, r1
}
...
// method of the parent MockClient instance is invoked and the hook queue is
// empty.
func (f *ClientCommitDistanceFunc) SetDefaultHook(hook func(context.Context, api.RepoName, api.CommitID, api.CommitID) (int, error)) {
	f.defaultHook = hook
}

// PushHook adds a function to the end of hook queue. Each invocation of the
// CommitDistance method of the parent MockClient instance invokes the hook
// at the front of the queue and discards it. After the queue is empty, the
// default hook function is invoked for any future action.
func (f *ClientCommitDistanceFunc) PushHook(hook func(context.Context, api.RepoName, api.CommitID, api.CommitID) (int, error)) {
	f.mutex.Lock()
	f.hooks = append(f.hooks, hook)
	f.mutex.Unlock()
}
...
// SetDefaultReturn calls SetDefaultHook with a function that returns the
// given values.
func (f *ClientCommitDistanceFunc) SetDefaultReturn(r0 int, r1 error) {
...
// PushReturn calls PushHook with a function that returns the given values.
func (f *ClientCommitDistanceFunc) PushReturn(r0 int, r1 error) {
	f.PushHook(func(context.Context, api.RepoName, api.CommitID, api.CommitID) (int, error) {
		return r0, r1
	})
}

func (f *ClientCommitDistanceFunc) nextHook() func(context.Context, api.RepoName, api.CommitID, api.CommitID) (int, error) {
	f.mutex.Lock()
	defer f.mutex.Unlock()
...
func (f *ClientCommitDistanceFunc) appendCall(r0 ClientCommitDistanceFuncCall) {
	f.mutex.Lock()
	f.history = append(f.history, r0)
	f.mutex.Unlock()
}

// History returns a sequence of ClientCommitDistanceFuncCall objects
// describing the invocations of this function.
func (f *ClientCommitDistanceFunc) History() []ClientCommitDistanceFuncCall {
	f.mutex.Lock()
	history := make([]ClientCommitDistanceFuncCall, len(f.history))
	copy(history, f.history)
	f.mutex.Unlock()
...
// ClientCommitDistanceFuncCall is an object that describes an invocation of
// method CommitDistance on an instance of MockClient.
type ClientCommitDistanceFuncCall struct {
	// Arg0 is the value of the 1st argument passed to this method
	// invocation.
	Arg0 context.Context
	// Arg1 is the value of the 2nd argument passed to this method
...
// Args returns an interface slice containing the arguments of this
// invocation.
func (c ClientCommitDistanceFuncCall) Args() []interface{} {
	return []interface{}{c.Arg0, c.Arg1, c.Arg2, c.Arg3}
}

// Results returns an interface slice containing the results of this
// invocation.
func (c ClientCommitDistanceFuncCall) Results() []interface{} {
	return []interface{}{c.Result0, c.Result1}
}
