...
func TestReadDir_SubRepoFiltering(t *testing.T) {
	...
	files, err := client.ReadDir(ctx, checker, db, repo, commitID, "", false)
	if err != nil {
...
func TestRepository_FileSystem(t *testing.T) {
	...
	for label, test := range tests {
		...
		dir1Entries, err := client.ReadDir(ctx, authz.DefaultSubRepoPermsChecker, db, test.repo, test.first, "dir1", false)
		if err != nil {
		...
		_, err = client.ReadDir(ctx, authz.DefaultSubRepoPermsChecker, db, test.repo, test.first, "dir2", false)
		...
		file1Data, err := client.ReadFile(ctx, test.repo, test.first, "dir1/file1", nil)
		if err != nil {
		...
		_, err = client.ReadFile(ctx, test.repo, test.first, "file 2", nil)
		if !os.IsNotExist(err) {
		...
		_, err = client.ReadFile(ctx, test.repo, test.second, "file 2", nil)
		if err != nil {
		...
		if _, err := client.ReadFile(ctx, test.repo, test.second, "dir1/file1", nil); err != nil {
		...
		rootEntries, err := client.ReadDir(ctx, authz.DefaultSubRepoPermsChecker, db, test.repo, test.second, ".", false)
		if err != nil {
		...
		dir1Entries, err = client.ReadDir(ctx, authz.DefaultSubRepoPermsChecker, db, test.repo, test.second, "dir1", false)
		if err != nil {
			t.Errorf("%s: fs1.ReadDir(dir1): %s", label, err)
			continue
		}
		...
		rootEntries, err = client.ReadDir(ctx, authz.DefaultSubRepoPermsChecker, db, test.repo, test.third, ".", false)
		if err != nil {
...
func TestRepository_FileSystem_quoteChars(t *testing.T) {
	...
	for label, test := range tests {
		...
		entries, err := client.ReadDir(ctx, authz.DefaultSubRepoPermsChecker, db, test.repo, commitID, ".", false)
		if err != nil {
...
func TestRepository_FileSystem_gitSubmodules(t *testing.T) {
	...
	for label, test := range tests {
		...
		_, err = client.ReadFile(ctx, test.repo, commitID, "submod", nil)
		if err != nil {
