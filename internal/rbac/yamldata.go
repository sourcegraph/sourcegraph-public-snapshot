...
func main() {
	...
	var actions = map[string]rtypes.NamespaceAction{}
...
func generateActions(output io.Writer, namespaceActions map[string]rtypes.NamespaceAction) {
	fmt.Fprintln(output, "// Code generated by internal/rbac/yamldata. DO NOT EDIT.")
	fmt.Fprintln(output, "package types")
	fmt.Fprintln(output)

	var namespaceActionConstants []string
	for actionVar, action := range namespaceActions {
		namespaceActionConstants = append(namespaceActionConstants, fmt.Sprintf("const %s NamespaceAction = \"%s\"", actionVar, action))
	}
