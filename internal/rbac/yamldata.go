...
func main() {
	...
	var actions = map[string]rtypes.PermissionNamespace{}
...
func generateActions(output io.Writer, namespaceActions map[string]rtypes.PermissionNamespace) {
	fmt.Fprintln(output, "// Code generated by internal/rbac/yamldata. DO NOT EDIT.")
	fmt.Fprintln(output, "package types")
	fmt.Fprintln(output)
	...
	for action, namespace := range namespaceActions {
		actionVarName := fmt.Sprintf("%s%sAction", sentencizeNamespace(namespace.String()), toTitleCase(action))
		namespaceActionConstants = append(namespaceActionConstants, fmt.Sprintf("const %s PermissionAction = \"%s\"", actionVarName, action))
	}
