...
func generateActions(output io.Writer, namespaceActions map[rtypes.NamespaceAction]rtypes.PermissionNamespace) {
	fmt.Fprintln(output, "// Code generated by internal/rbac/yamldata. DO NOT EDIT.")
	fmt.Fprintln(output, "package types")
	fmt.Fprintln(output)

	var namespaceActionConstants []string
	for action, namespace := range namespaceActions {
		actionVarName := fmt.Sprintf("%s%sAction", sentencizeNamespace(namespace.String()), toTitleCase(action.String()))
		namespaceActionConstants = append(namespaceActionConstants, fmt.Sprintf("const %s NamespaceAction = \"%s\"", actionVarName, action))
	}
