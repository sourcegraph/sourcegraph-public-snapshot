...
func testSyncRepo(s *repos.Store) func(*testing.T) {
	return func(t *testing.T) {
		...
		servicesPerKind := createExternalServices(t, s, func(svc *types.ExternalService) {
			svc.CloudDefault = true
		})
...
func testSyncRun(store *repos.Store) func(t *testing.T) {
	return func(t *testing.T) {
		...
		stored := types.Repos{mk("updated")}.With(types.Opt.RepoSources(svc.URN()))
		sourced := types.Repos{mk("new"), mk("updated").With(func(r *types.Repo) {
...
func testOrphanedRepo(store *repos.Store) func(t *testing.T) {
	ctx := context.Background()
	return transact(ctx, store, func(t testing.TB, store *repos.Store) {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
...
func testConflictingSyncers(store *repos.Store) func(t *testing.T) {
	return func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
...
func testSyncRepoMaintainsOtherSources(store *repos.Store) func(t *testing.T) {
	return func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
...
func testUserAddedRepos(store *repos.Store) func(t *testing.T) {
	return func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
...
func testNameConflict(store *repos.Store) func(t *testing.T) {
	return func(t *testing.T) {
		...
		// We expect repo1 to now own the name of repo2, repo2 has been deleted. Since names are also
		// unique in the code host, repo2 will have a new name, so when we sync it, it'll be re-created with
		// the new name.

		have, err := store.RepoStore.List(ctx, database.ReposListOptions{})
		if err != nil {
			t.Fatal(err)
		}

		ignore := cmpopts.IgnoreFields(types.Repo{}, "ID", "CreatedAt", "UpdatedAt", "Sources")
		if want := ([]*types.Repo{renamedRepo1}); !cmp.Equal(have, want, ignore) {
			t.Fatalf("mismatch (-have, +want):\n%s", cmp.Diff(have, want))
		}
	}
}

func testDeleteExternalService(store *repos.Store) func(t *testing.T) {
	return func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
...
func assertSourceCount(ctx context.Context, t testing.TB, store *repos.Store, want int) {
	t.Helper()
	var rowCount int
...
func assertDeletedRepoCount(ctx context.Context, t testing.TB, store *repos.Store, want int) {
	t.Helper()
	var rowCount int
