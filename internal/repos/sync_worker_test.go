...
func testSyncWorkerPlumbing(store *repos.Store) func(t *testing.T) {
	...
	return func(t *testing.T) {
		...
		// Add item to queue
		result, err := store.ExecResult(ctx, sqlf.Sprintf(`insert into external_service_sync_jobs (external_service_id) values (%s);`, testSvc.ID))
		if err != nil {
			t.Fatal(err)
		}
		...
		worker, resetter := repos.NewSyncWorker(ctx, store.Handle().DB(), h, repos.SyncWorkerOptions{
			NumHandlers:    1,
