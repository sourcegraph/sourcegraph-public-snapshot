...
func (w *webhookBuildHandler) handleKindGitHub(ctx context.Context, logger log.Logger, job *webhookworker.Job) error {
	...
	client := github.NewV3Client(logger, svc.URN(), baseURL, &auth.OAuthBearerToken{AvvessToken: conn.Token, Expiry: expiry, RefreshToken: conn.TokenOauthRefresh}, w.doer)
