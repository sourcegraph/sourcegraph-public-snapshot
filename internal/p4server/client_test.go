...
import (
	...
	p4server "github.com/sourcegraph/sourcegraph/internal/p4server"
...
func newMockDB() database.DB {
	...
	gr := database.NewMockP4serverRepoStore()
	db.P4serverReposFunc.SetDefaultReturn(gr)
	return db
}
...
func TestClient_Remove(t *testing.T) {
	...
	cli := p4server.NewTestClient(
		httpcli.DoerFunc(func(r *http.Request) (*http.Response, error) {
			switch r.URL.String() {
			// Ensure that the request was received by the "expected" p4server instance - where
			// expected is the p4server instance according to the Rendezvous hashing scheme.
...
func TestClient_ArchiveReader(t *testing.T) {
	...
	addrs := []string{u.Host}
	cli := p4server.NewTestClient(&http.Client{}, newMockDB(), addrs)
	...
	for name, test := range tests {
		t.Run(string(name), func(t *testing.T) {
			...
			rc, err := cli.ArchiveReader(ctx, nil, name, p4server.ArchiveOptions{Treeish: "HEAD", Format: p4server.ArchiveFormatZip})
...
func TestAddrForRepo(t *testing.T) {
	addrs := []string{"p4server-1", "p4server-2", "p4server-3"}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := p4server.AddrForRepo(context.Background(), "p4server", newMockDB(), tc.repo, p4server.P4ServerAddresses{
				Addresses:     addrs,
				PinnedServers: pinned,
			})
			if err != nil {
				t.Fatal("Error during getting p4server address")
			}
			if got != tc.want {
...
func TestClient_ResolveRevisions(t *testing.T) {
	...
	tests := []struct {
		...
		err:   &p4domain.RevisionNotFoundError{Repo: api.RepoName(remote), Spec: "test-fake-ref"},
	}}
	...
	u, _ := url.Parse(srv.URL)
	addrs := []string{u.Host}
	cli := p4server.NewTestClient(&http.Client{}, db, addrs)
...
func TestClient_AddrForRepo_UsesConfToRead_PinnedRepos(t *testing.T) {
	ctx := context.Background()
	client := p4server.NewTestClient(&http.Client{}, newMockDB(), []string{"p4server1", "p4server2"})
	...
	require.Equal(t, "p4server2", addr)
...
func TestClient_BatchLog(t *testing.T) {
	...
	results := map[api.RepoCommit]p4server.RawBatchLogResult{}
	var mu sync.Mutex

	if err := cli.BatchLog(context.Background(), opts, func(repoCommit api.RepoCommit, gitLogResult p4server.RawBatchLogResult) error {
		mu.Lock()
		defer mu.Unlock()
	...
	expectedResults := map[api.RepoCommit]p4server.RawBatchLogResult{
		// Shard 1
...
func TestLocalGitCommand(t *testing.T) {
	...
	ctx := context.Background()
	command := p4server.NewLocalGitCommand(api.RepoName(filepath.Base(root)), "log")
...
func TestClient_ReposStats(t *testing.T) {
	const p4serverAddr = "172.16.8.1:8080"
	now := time.Now().UTC()
	addrs := []string{p4serverAddr}

	expected := fmt.Sprintf("http://%s", p4serverAddr)
	...
	assert.Equal(t, wantStats, *gotStatsMap[p4serverAddr])
}
