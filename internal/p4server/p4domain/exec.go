...
var (
	// p4CmdAllowlist are commands and arguments that are allowed to execute and are
	// checked by IsAllowedP4Cmd
	p4CmdAllowlist = map[string][]string{
		"log":    append([]string{}, p4CommonAllowlist...),
		"show":   append([]string{}, p4CommonAllowlist...),
		"remote": {"-v"},
		"diff":   append([]string{}, p4CommonAllowlist...),
		...
		// Used in tests to simulate errors with runCommand in handleExec of p4server.
		"testcommand": {},
		"testerror":   {},
		"testecho":    {},
		"testcat":     {},
...
var p4ObjectHashRegex = regexp.MustCompile(`^[a-fA-F\d]*$`)
...
func isAllowedDiffArg(arg string) bool {
	...
	if p4ObjectHashRegex.MatchString(arg) {
		return true
	}
...
// isAllowedP4Arg checks if the arg is allowed.
func isAllowedP4Arg(allowedArgs []string, arg string) bool {
...
// IsAllowedP4Cmd checks if the cmd and arguments are allowed.
func IsAllowedP4Cmd(logger log.Logger, args []string) bool {
	if len(args) == 0 || len(p4CmdAllowlist) == 0 {
		return false
	}

	cmd := args[0]
	allowedArgs, ok := p4CmdAllowlist[cmd]
	if !ok {
		// Command not allowed
	...
	for i, arg := range args[1:] {
		if strings.HasPrefix(arg, "-") {
			...
			if !isAllowedP4Arg(allowedArgs, arg) {
				logger.Warn("IsAllowedP4Cmd.isAllowedP4Argcmd", log.String("cmd", cmd), log.String("arg", arg))
				return false
			}
		}
