...
func TestIsAllowedP4Cmd(t *testing.T) {
	allowed := [][]string{
...
func TestIsAllowedDiffP4Cmd(t *testing.T) {
	allowed := []struct {
		args []string
		pass bool
	}{
	...
	for _, cmd := range allowed {
		t.Run(fmt.Sprintf("%s returns %t", strings.Join(cmd.args, " "), cmd.pass), func(t *testing.T) {
			assert.Equal(t, cmd.pass, IsAllowedP4Cmd(logger, cmd.args))
		})
	}
}
