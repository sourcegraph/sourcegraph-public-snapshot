...
func TestProvider_FetchUserPerms(t *testing.T) {
	...
	t.Run("nil account", func(t *testing.T) {
		logger := logtest.Scoped(t)
		p := NewProvider(logger, db, "", "ssl:111.222.333.444:1666", "admin", "password", nil)
	...
	t.Run("not the code host of the account", func(t *testing.T) {
		logger := logtest.Scoped(t)
		p := NewProvider(logger, db, "", "ssl:111.222.333.444:1666", "admin", "password", []extsvc.RepoID{})
...
func NewTestProvider(logger log.Logger, urn, host, user, password string, execer p4Execer) *Provider {
	p := NewProvider(logger, database.NewMockDB(), urn, host, user, password, []extsvc.RepoID{})
	p.p4Execer = execer
	return p
}
