...
func (s *CachedAccountsClient) Delete(ctx context.Context, in *PersonSpec, opts ...grpc.CallOption) (*pbtypes.Void, error) {
	if s.Cache != nil {
		var cachedResult pbtypes.Void
		cached, err := s.Cache.Get(ctx, "Accounts.Delete", in, &cachedResult)
		if err != nil {
			return nil, err
		}
		if cached {
	...
	result, err := s.AccountsClient.Delete(ctx, in, grpc.Trailer(&trailer))
	if err != nil {
		return nil, err
	}
	if s.Cache != nil {
		if err := s.Cache.Store(ctx, "Accounts.Delete", in, result, trailer); err != nil {
			return nil, err
		}
	}
	return result, nil
