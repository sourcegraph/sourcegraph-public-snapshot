...
func Transact(dbh modl.SqlExecutor, fn func(dbh modl.SqlExecutor) error) (err error) {
	...
	if beginNewTx {
		tx, err = dbh.(*modl.DbMap).Begin() // enforce strict type here just to be cautious (what else has Begin?)
		if err != nil {
			return err
		}
		defer func() {
	...
	if err = fn(tx.(modl.SqlExecutor)); err != nil {
		return err
	}
